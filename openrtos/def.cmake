...
set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wno-unused-result -Werror")
if (DEFINED CFG_GCC_LTO)
    set(EXTRA_FLAGS "${EXTRA_FLAGS}  -fno-stack-protector")
else ()
    if (DEFINED CFG_DBG_STACK_PROTECTOR)
        set(EXTRA_FLAGS "${EXTRA_FLAGS}  -fstack-protector-all")
    endif()
endif()

if (DEFINED CFG_CPU_FA626)

    # uncomment the following cmake statement to perform a misra compliance check.
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wcomment")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wsizeof-pointer-memaccess")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Waddress")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wlogical-op")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wreturn-type")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wsequence-point")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wparentheses")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wunused-but-set-parameter")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wunused-but-set-variable")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wswitch-default")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wunused-label")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wunused-variable")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wuninitialized")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wmaybe-uninitialized")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wbool-compare")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wfloat-equal")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wshadow")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wpointer-arith")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wtype-limits")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wconversion")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wsign-compare")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wlogical-not-parentheses")
    # The following two statements are too strict and may not be enabled.
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Wformat")
    #set(EXTRA_FLAGS "${EXTRA_FLAGS} -Waggregate-return")

    set(CMAKE_C_FLAGS_DEBUG             "${EXTRA_FLAGS} -mcpu=fa626te -fno-short-enums -Wl,--no-enum-size-warning -O0 -g3 -ggdb3")
    ...
    if (GCC_VERSION VERSION_GREATER 4.9 OR GCC_VERSION VERSION_EQUAL 4.9)
        ...
        set(CMAKE_CXX_FLAGS_DEBUG           "${EXTRA_FLAGS} -mcpu=fa626te -fno-short-enums -Wl,--no-enum-size-warning -O0 -g3 -std=c++0x")
        set(CMAKE_CXX_FLAGS_RELWITHDEBINFO  "${EXTRA_FLAGS} -mcpu=fa626te -fno-short-enums -Wl,--no-enum-size-warning -Og -g3 -std=c++0x -DNDEBUG")
        set(CMAKE_CXX_FLAGS_RELEASE         "${EXTRA_FLAGS} -mcpu=fa626te -fno-short-enums -Wl,--no-enum-size-warning -O3 -std=c++0x -DNDEBUG")
        set(CMAKE_CXX_FLAGS_MINSIZEREL      "${EXTRA_FLAGS} -mcpu=fa626te -fno-short-enums -Wl,--no-enum-size-warning -Os -std=c++0x -DNDEBUG")
    endif()

    set(CMAKE_EXE_LINKER_FLAGS "${EXTRA_FLAGS} -mcpu=fa626te ${MAP_FLAGS} -Wl,--gc-sections -nostartfiles -T\"${LIBRARY_OUTPUT_PATH}/default.ld\" -L\"${LIBRARY_OUTPUT_PATH}\" ")
...
elseif (DEFINED CFG_CPU_RISCV)
    ...
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -fno-builtin-malloc -fno-builtin-calloc -fno-builtin-realloc -fno-builtin-free")
    set(EXTRA_FLAGS "${EXTRA_FLAGS} -mcpu=d25f -march=rv32imfcxv5 -g3 -ggdb3 -mcmodel=large -Wno-unused-parameter -Wno-unused-function -fno-short-enums")

    set(CMAKE_C_FLAGS_DEBUG             "${EXTRA_FLAGS} -O0 -DDEBUG")
    set(CMAKE_CXX_FLAGS_DEBUG           "${EXTRA_FLAGS} -O0 -DDEBUG -std=c++11")
    set(CMAKE_C_FLAGS_RELWITHDEBINFO    "${EXTRA_FLAGS} -Og -DNDEBUG")
    set(CMAKE_CXX_FLAGS_RELWITHDEBINFO  "${EXTRA_FLAGS} -Og -DNDEBUG -std=c++11")
    set(CMAKE_C_FLAGS_RELEASE           "${EXTRA_FLAGS} -O3 -DNDEBUG")
    set(CMAKE_CXX_FLAGS_RELEASE         "${EXTRA_FLAGS} -O3 -DNDEBUG -std=c++11")
    set(CMAKE_C_FLAGS_MINSIZEREL        "${EXTRA_FLAGS} -Os -DNDEBUG")
    set(CMAKE_CXX_FLAGS_MINSIZEREL      "${EXTRA_FLAGS} -Os -DNDEBUG -std=c++11")
    set(CMAKE_EXE_LINKER_FLAGS          "${EXTRA_FLAGS} ${MAP_FLAGS} -Wl,--gc-sections -nostartfiles -T\"${LIBRARY_OUTPUT_PATH}/default.ld\" -L\"${LIBRARY_OUTPUT_PATH}\"")
    ...
    # Define the IRQ stack size.
    #
    # If configISR_STACK_SIZE_WORDS == 1024, the actual IRQ stack size
    # is 1024 * sizeof(int) = 4KBytes.
    add_definitions(
        -DconfigISR_STACK_SIZE_WORDS=1024
    )

    # The compile flags defined in add_compile_options() will not be
    # applied to <sdk_root>/project/... folder. So USE CMAKE_<LANG>_FLAGS..
    #add_compile_options(
    #    -mcpu=d25f
    #    -g3
    #    -ggdb3
...
ITE_ADD_DEFINITIONS_IF_DEFINED(CFG_DLM_ENABLE)
