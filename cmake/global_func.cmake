...
#
# This function is used to check if the chip specific version of file
# exists. If it exists, add the chip specific version to the library.
# If it doesn't exist, add the input file path to the library.
#
# usage example:
#   If there is a chip specific version of th_interrupt_handler.c located at
#   CMAKE_SOURCE_DIR/sdk/chip/itXXXX/driver/ith/ith_interrupt_handler.c, and
#   there is a general version of ith_dma.c located at
#   CMAKE_SOURCE_DIR/sdk/driver/ith/ith_dma.c, the following statement
#   in the CMAKE_SOURCE_DIR/sdk/driver/ith/CMakeLists.txt
#
#     ITE_ADD_DRIVER_SOURCE_FILE(ith
#         ith_dma.c
#         ith_interrupt_handler.c)
#
#   is equals to
#
#     ITE_ADD_DRIVER_SOURCE_FILE(ith STATIC
#         ith_dma.c
#         CMAKE_SOURCE_DIR/sdk/chip/itXXXX/driver/ith/ith_interrupt_handler.c)
#
function(ITE_ADD_DRIVER_SOURCE_FILE lib)
    cmake_path(APPEND sdk_path ${CMAKE_SOURCE_DIR} "sdk")
    cmake_path(RELATIVE_PATH  CMAKE_CURRENT_SOURCE_DIR
               BASE_DIRECTORY ${sdk_path}
               OUTPUT_VARIABLE rel_path)
    cmake_path(APPEND chip_path ${CMAKE_SOURCE_DIR} "sdk" "chip" "it${CFG_CHIP_FAMILY}" "${rel_path}")

    foreach(in_file ${ARGN})
        cmake_path(APPEND file_path ${chip_path} ${in_file})
        if (EXISTS ${file_path})
            #message("add_library: ${lib} ${file_path}")
            add_library(${lib} STATIC ${file_path})
        else()
            #message("add_library: ${lib} ${in_file}")
            add_library(${lib} STATIC ${in_file})
        endif()
    endforeach()
endfunction()

function(ITE_REPLACE_STRING_IN_FILE file_path old_string new_string)
    # Try to find the Python interpreter
    find_package(PythonInterp REQUIRED)

    # Check if the Python interpreter is found
    if (NOT PYTHONINTERP_FOUND)
        message(FATAL_ERROR "Python interpreter not found. Please install Python.")
    else()
        execute_process(
            COMMAND ${PYTHON_EXECUTABLE} -c "import chardet"
            RESULT_VARIABLE CHARDET_NOT_FOUND
        )
        if (CHARDET_NOT_FOUND)
            message(FATAL_ERROR "Chardet module not found. Please use the 'pip install chardet' command to install it.")
        endif()
    endif()

    # Execute the Python script to replace the specified string in the file with a space
    execute_process(
        COMMAND ${PYTHON_EXECUTABLE} ${PROJECT_SOURCE_DIR}/python/replace_string_in_file.py ${file_path} ${old_string} ${new_string}
        RESULT_VARIABLE ret
    )
    ...
    #else()
    #    message(FATAL_ERROR "Error processing file.")
    #endif()
