...
static void _FileSyncFile(char *srcPartition, char *dstPartition)
{
    ...
    for (i = 0; i < gBackupFileCount; i++)
    {
        ...
        else
        {
            ...
            itcStreamRead(&srcStream.stream, pSrcDataBuf, srcStream.stream.size);
            ...
            itcStreamRead(&dstStream.stream, pDstDataBuf, dstStream.stream.size);
        ...
        if (bUpdateFile)
        {
            ...
            itcStreamWrite(&dstStream.stream, pSrcDataBuf, srcStream.stream.size);
        }
        itcFileStreamClose(&srcStream);
        itcFileStreamClose(&dstStream);
...
static int _CheckBackupStatus(void)
{
    ...
    for (i = 0; i < gBackupFileCount; i++)
    {
        ...
        itcFileStreamClose(&backupStream);
...
void BackupInit(void)
{
    ...
    switch (_CheckBackupStatus())
    {
        ...
        case NEED_RESTORE_DEFAULT:
        {
            ...
            itcStreamWrite(&specialStream.stream, "1234", sizeof("1234"));
            itcFileStreamClose(&specialStream);
