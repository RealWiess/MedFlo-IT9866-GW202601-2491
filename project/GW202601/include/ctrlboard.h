...
typedef struct
{
	...
	int       reclevel;													// /< rec volume percentage, range is 0~100
    // photo
    ...
    // dashboard;
    uint32_t odometer;
    uint32_t tripA_meter;

    // wifi
    char ssid[WIFI_SSID_MAXLEN];
    char password[WIFI_PASSWORD_MAXLEN];
    char secumode[WIFI_SECUMODE_MAXLEN];
    char ssid2[WIFI_SSID_MAXLEN];           // 備用 SSID
    char password2[WIFI_PASSWORD_MAXLEN];   // 備用密碼
    char secumode2[WIFI_SECUMODE_MAXLEN];   // 備用加密模式
    int wifi_mode;
    int wifi_on_off;
    // wifi
    char ap_ssid[WIFI_SSID_MAXLEN];  // ap mode ssid
    char ap_password[WIFI_PASSWORD_MAXLEN]; // ap mode password
...
/* ================================================================ */
/*  Gateway Status (for LCD display)                                  */
/* ================================================================ */
extern uint8_t  s_nmgw_status_flags;   /* NMGW_FLAG_xxx bitmask        */
extern int      g_ble_disc_total;      /* BLE 廣播總數                  */
extern int      g_ble_disc_passed;     /* BLE MedFlo 過濾通過數         */
extern char     g_scan_ssid[20][33];   /* scan AP SSID cache (LCD)     */
extern int      g_scan_rssi[20];       /* scan AP RSSI cache (LCD)     */
extern int      g_scan_count;          /* scan AP count (LCD)          */
