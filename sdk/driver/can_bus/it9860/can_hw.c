...
//=============================================================================
#define ALL_INT_FLAG_MASK ( CAN_AIF  | CAN_EIF  | CAN_TSIF| CAN_TPIF | CAN_RAFIF \
                          | CAN_RFIF | CAN_ROIF | CAN_RIF | CAN_BEIF | CAN_ALIF \
                          | CAN_EPIF)
//=============================================================================
...
void ithCANDisableIntrAll (CAN_HANDLE * base)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_RTIE_REG), 0x0U, interrupt_en_mask);
    ithWriteRegMaskA((base->ADDR | CAN_VER_0_REG), 0x0U, (CAN_TTIE | CAN_WTIE));
}

void ithCANClearIntrFlag (CAN_HANDLE * base, CAN_INTERRUPTS_FLAG flag)
{
    ithWriteRegMaskA((base->ADDR | CAN_RTIE_REG), (0x1U << flag), ALL_INT_FLAG_MASK);
}
...
void ithCANSetEWL (CAN_HANDLE * base, uint32_t EWL)
{
    ...
    uint32_t rtiereg = ithReadRegA((base->ADDR | CAN_RTIE_REG));
    ...
    /* avoid touching W1C flags */
    rtiereg &= ~(ALL_INT_FLAG_MASK);

    /* clear default value */
    rtiereg &= ~(CAN_EWL);

    rtiereg |= (ewl_number << 24);

    ithWriteRegA((base->ADDR | CAN_RTIE_REG), rtiereg);
}
//=============================================================================
/**
 * CAN-CTRL Reset
...
void ithCANSWReset (CAN_HANDLE * base, bool OnOff)
{
    ...
    {
        // sw reset
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_RESET,
                         (CAN_RESET | CAN_BUSOFF));
    }
    else
    {
        // sw reset stop
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), 0x0U,
                         (CAN_RESET | CAN_BUSOFF));
    }
}
...
void ithCANSetBusOFF (CAN_HANDLE * base, bool OnOff)
{
    if (OnOff)
    {
        ...
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_BUSOFF,
                         CAN_BUSOFF);
    }
    else
    {
...
void ithCANSetLoopBack (CAN_HANDLE * base, bool external, bool internal)
{
    if (external)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_LBME,
                         (CAN_LBME | CAN_BUSOFF));
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_SACK,
                         (CAN_SACK | CAN_BUSOFF));
    }
    else
    {
    ...
    if (internal)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_LBMI,
                         (CAN_LBMI | CAN_BUSOFF));
    }
    else
    {
...
void ithCANSetListenOnlyMode (CAN_HANDLE * base, bool onoff)
{
    if (onoff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_LOM,
                         (CAN_LOM | CAN_BUSOFF));
    }
    else
    {
...
void ithCANSetTransStandbyMode (CAN_HANDLE * base, bool onoff)
{
    if (onoff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_STBY,
                         (CAN_STBY | CAN_BUSOFF));
    }
    else
    {
...
void ithCANSetTransPTBSSMode (CAN_HANDLE * base, bool onoff)
{
    if (onoff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TPSS,
                         (CAN_TPSS | CAN_BUSOFF));
    }
    else
    {
...
void ithCANSetTransSTBSSMode (CAN_HANDLE * base, bool onoff)
{
    if (onoff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TSSS,
                         (CAN_TSSS | CAN_BUSOFF));
    }
    else
    {
...
void ithCANSetSTBMode (CAN_HANDLE * base, bool mode)
{
    if (mode)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TSMODE,
                         (CAN_TSMODE | CAN_BUSOFF));
    }
    else
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), 0x0U,
...
void ithCANSetFDISO (CAN_HANDLE * base)
{
    ...
    if (base->ProtocolType == protocol_FD_ISO ||
        base->ProtocolType == protocol_CAN_2_0B)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_FD_ISO,
                         (CAN_FD_ISO | CAN_BUSOFF));
    }
    else
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), 0x0U,
...
void ithCANSetBitRate (CAN_HANDLE * base, CAN_BTATTR s_bt, CAN_BTATTR f_bt)
{
    ...
    S_Seg1_DATA =
        (s_bt.Seg_1 << CAN_S_Seg_1_SHIFT) | (s_bt.Seg_2 << CAN_S_Seg_2_SHIFT) |
    ...
    F_Seg1_DATA =
        (f_bt.Seg_1 << CAN_F_Seg_1_SHIFT) | (f_bt.Seg_2 << CAN_F_Seg_2_SHIFT) |
        (f_bt.SJW << CAN_F_SJW_SHIFT) | (f_bt.Prescaler << CAN_F_PRESC_SHIFT);

    ithWriteRegA((base->ADDR | CAN_F_Seg_1_REG), F_Seg1_DATA);

    ithWriteRegMaskA((base->ADDR | CAN_EALCAP_REG),
                     f_bt.SSPOFF << CAN_SSPOFF_SHIFT, CAN_SSPOFF);
    ithWriteRegMaskA((base->ADDR | CAN_EALCAP_REG), N01_BITS_MSK << CAN_TDCEN_SHIFT,
                     CAN_TDCEN);
#endif
}
...
{
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG), 0x0U << CAN_TIMEEN_SHIFT,
                     CAN_TIMEEN);
    if (OnOff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG),
                         position << CAN_TIMEPOS_SHIFT, CAN_TIMEPOS);
        ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG),
                         N01_BITS_MSK << CAN_TIMEEN_SHIFT, CAN_TIMEEN);
    }
}
//=============================================================================
/**
...
void ithCANSetACODE (CAN_HANDLE * base, CAN_ACODE c_index, uint32_t value)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG), ~CAN_SELMASK,
                     CAN_SELMASK); // SELMASK =0;
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG), c_index,
                     CAN_ACFADR);  // Set ACFADR
                     ...
                     bool AIDEE, bool AIDE)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG), CAN_SELMASK,
                     CAN_SELMASK); // SELMASK =1;
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG), m_index,
                     CAN_ACFADR);  // Set ACFADR

    uint32_t ACF_DATA = ithReadRegA((base->ADDR | CAN_ACF_0_REG));
    ACF_DATA = (value & 0x1FFFFFFFU) |
               ((uint32_t)AIDE << 29U) |
               ((uint32_t)AIDEE << 30U);
    ithWriteRegA((base->ADDR | CAN_ACF_0_REG), ACF_DATA);
}
...
void ithCANACFEnable (CAN_HANDLE * base, CAN_ACFE index, bool OnOff)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_ACFCTRL_REG),
                     OnOff << (CAN_AE0_x_SHIFT + index),
                     N01_BITS_MSK << (CAN_AE0_x_SHIFT + index));
}
//=============================================================================
/**
...
#if 0
    (void)printf("STAT_DATA = %x", STAT_DATA);
#endif
...
uint8_t ithCANGetTBStatus (CAN_HANDLE * base, bool TBType)
{
    ...
    else
    {
        ...
        if (((STAT_DATA & CAN_TPE) >> CAN_TPE_SHIFT) == 0x0U)
        {
            TSTAT = 0x0U;
        }
        else
...
void ithCANPrimarySendCtrl (CAN_HANDLE * base, bool OnOff)
{
    ...
    if (OnOff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TPE,
                         (CAN_TPE | CAN_BUSOFF));
    }
    else
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TPA,
                         (CAN_TPA | CAN_BUSOFF));
    }
}
//=============================================================================
...
void ithCANSecondarySendCtrl (CAN_HANDLE * base, bool OnOff)
{
    ...
    if (OnOff)
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TSALL,
                         (CAN_TSALL | CAN_BUSOFF));
    }
    else
    {
        ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), CAN_TSA,
                         (CAN_TSA | CAN_BUSOFF));
    }
}
...
uint8_t ithCANGetSecondarySendIdle (CAN_HANDLE * base)
{
    ...
    if ((STAT_DATA & CAN_TSONE) == 0x0U && (STAT_DATA & CAN_TSALL) == 0x0U)
    {
        return 0x0U;
    }
    else
                     ...
                     uint16_t Trigger_Time)
{
    ...
    VER_0_DATA |= (N01_BITS_MSK << CAN_TTEN_SHIFT);
    VER_0_DATA |= (PRESCaler << CAN_T_PRESC_SHIFT);
    (void)printf("VER_0_DATA = %" PRIx32 " \n", VER_0_DATA);
    ...
    ithWriteRegMaskA((base->ADDR | CAN_CFG_STAT_REG), N01_BITS_MSK << CAN_TTTBM_SHIFT,
                     (CAN_TTTBM | CAN_BUSOFF));
}
//=============================================================================
/**
...
{
    ithWriteRegMaskA((base->ADDR | CAN_TRIG_CFG_0_REG),
                     trigtype << CAN_TTYPE_SHIFT, CAN_TTYPE);
}
//=============================================================================
/**
...
void ithTTCANSetTrigTime (CAN_HANDLE * base, uint16_t trigtime)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_TRIG_CFG_0_REG),
                     TT_TRIG_0 << CAN_TT_TRIG0_SHIFT, CAN_TT_TRIG0);
    ithWriteRegMaskA((base->ADDR | CAN_TRIG_CFG_0_REG),
                     TT_TRIG_1 << CAN_TT_TRIG1_SHIFT, CAN_TT_TRIG1);
}
//=============================================================================
/**
...
void ithTTCANSetWatchTrigTime (CAN_HANDLE * base, uint16_t watchtrigtime)
{
    ...
    ithWriteRegMaskA((base->ADDR | CAN_TT_WTRIG_0_REG),
                     TT_WTRIG_0 << CAN_TT_WTRIG0_SHIFT, CAN_TT_WTRIG0);
    ithWriteRegMaskA((base->ADDR | CAN_TT_WTRIG_0_REG),
                     TT_WTRIG_1 << CAN_TT_WTRIG1_SHIFT, CAN_TT_WTRIG1);
}
