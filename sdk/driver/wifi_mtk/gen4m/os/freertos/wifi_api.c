...
#ifdef MTK_MINISUPP_ENABLE
    status = mtk_supplicant_get_security(port, (uint8_t *)auth_mode, (uint8_t *)encrypt_type);
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Set WiFi WPA/WPA2 password with user identity-based EAP methods.
* 
* @param [IN] port          Either 0 for STA / AP Client or 1 for AP
* @param [IN] pwd           Password (8 to 256 bytes ASCII or 64 bytes Hex)
* @param [IN] pwd_len       Length of the password (8 to 256)
*
* @return Status code (>=0 means success, <0 means failure)
*/
int32_t wifi_config_set_eap_user_pwd(uint8_t port, uint8_t *pwd,
                                    uint32_t pwd_len)
{
    int32_t status = 0;
    union uint32_to_u8 converter;

    converter.num = pwd_len;

    // Perform validation checks
    if (!wifi_is_port_valid(port) || pwd == NULL || (pwd_len < 8) || (pwd_len > WIFI_LENGTH_WPA_PWD)) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }

#ifdef MTK_MINISUPP_ENABLE
    uint8_t driver_msg[WIFI_LENGTH_WPA_PWD + 5] = {0};

    driver_msg[0] = port;
    os_memcpy(&driver_msg[1], &converter.arr, sizeof(converter.arr));    // Copy converter array to driver message
    os_memcpy((driver_msg + 5), pwd, pwd_len);                           // Copy password to driver message

    if (wifi_event_notification_wait(port, WIFI_EVENT_ID_IOT_SET_WPAPWD_WAIT,
                                     (unsigned char *)&driver_msg, sizeof(driver_msg)) != WIFI_SUCC)
        status = WIFI_FAIL;
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Get WiFi WPA/WPA2 password using user identity based EAP methods.
* 
* @param [IN] port         Either 0 for STA / AP Client or 1 for AP
* @param [OUT] pwd_buf     Output buffer for password (8 to 256 bytes ASCII)
* @param [OUT] pwd_len     Output for length of the password (8 to 256)
*
* @return Status code (>=0 means success, <0 means failure)
*/
                                    ...
                                    uint32_t *pwd_len)
{
    ...
    // Perform validation checks
    if (!wifi_is_port_valid(port) || pwd_buf == NULL || pwd_len == NULL) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }

#ifdef MTK_MINISUPP_ENABLE
    status = mtk_supplicant_get_eap_user_pwd(port, pwd_buf, pwd_len);
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Set WiFi User ID.
* 
* The function sets the EAP User ID for a specific WiFi port.
*
* @param [IN] port         Port number (0 = AP, 1 = AP Client, 2 = STA)
* @param [IN] user_id      EAP User ID string
* @param [IN] user_id_len  Length of the EAP User ID string
*
* @return  status code >= 0 means success, < 0 means failure
*/
int32_t wifi_config_set_eap_user_id(uint8_t port, uint8_t *user_id, uint32_t user_id_len)
{
    int32_t status = 0;
    union uint32_to_u8 converter;

    // Processing the incoming userId length by converting it into bytes
    converter.num = user_id_len;

    // Mathematically validating if provided port is within permissible limits 
    if (!wifi_is_port_valid(port)) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }

    // Checking whether provided user ID length exceeds constant max limit
    if (user_id_len > WIFI_LENGTH_WPA_UID) {
        status =  WIFI_ERR_PARA_INVALID;
        goto final;
    }

    // Null checking for incoming userId
    if (user_id == NULL) {
        status =  WIFI_ERR_PARA_INVALID;
        goto final;
    }
...
#ifdef MTK_MINISUPP_ENABLE
    uint8_t driver_msg[WIFI_LENGTH_WPA_UID + 5] = {0};

    // Preparing the initialization message for the driver
    driver_msg[0] = port;                                // Setting the port
    os_memcpy(&driver_msg[1], &converter.arr, sizeof(converter.arr));  // Converting userId length into bytes
    os_memcpy((driver_msg + 5), user_id, user_id_len);   // Copying the incoming userID

    if (wifi_event_notification_wait(port, WIFI_EVENT_ID_IOT_SET_WPAUID_WAIT,
                                     (unsigned char *)&driver_msg, sizeof(driver_msg)) != WIFI_SUCC)
        status = WIFI_FAIL;
#endif /* #ifdef MTK_MINISUPP_ENABLE */

final:
    // Return the status value - whether was our action successful or not.
    return status;
}

/**
* @brief Get WiFi EAP User ID.
*
* The function retrieves the EAP User ID configured for a specific WiFi port.
*
* @param [IN] port          Port number (0 = AP, 1 = AP Client, 2 = STA)
* @param [OUT] user_id_buf  Buffer to store retrieved EAP User ID
* @param [OUT] user_id_len  A variable to store the length of retrieved User ID
*
* @return Status code >= 0 means success, < 0 means failure
*/
int32_t wifi_config_get_eap_user_id(uint8_t port, uint8_t *user_id_buf, uint32_t *user_id_len)
{
    int32_t status = 0;
    ...
    // Check if output parameters (buffer and length) are not NULL
    if ((user_id_buf == NULL) || (user_id_len == NULL)) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }

#ifdef MTK_MINISUPP_ENABLE
    // With MiniSupplicant enabled, retrieve User ID from it
    status = mtk_supplicant_get_eap_user_id(port, user_id_buf, user_id_len);
#endif /* #ifdef MTK_MINISUPP_ENABLE */

final:
    // Return the final status of operation
    return status;
}


/**
* @brief Set MediaTek WiFi EAP Type.
*
* This function sets the Extensible Authentication Protocol (EAP) type on 
* a given WiFi port, which determines the authorization method to be used.
*
* @param [IN] port        The port number, either 0 for STA / AP Client or 1 for AP
* @param [IN] mtk_eap_type   The MTK EAP type to be set
*
* @return Returns an integer value wherein >=0 means success, <0 means failure
*/
int32_t wifi_config_set_eap_type(uint8_t port, uint8_t mtk_eap_type)
{
    int32_t status = 0;

    // Validate the input port
    if (!wifi_is_port_valid(port)) {
        LOG_FUNC("auth port invalid");    // Logging if the port is invalid
        status = WIFI_ERR_PARA_INVALID;   // Updating the status as a parameter is invalid
        goto final;
    }
#ifdef MTK_MINISUPP_ENABLE
    uint8_t driver_msg[2] = {0};

    // Preparing the message for the driver
    driver_msg[0] = port;          // Setting the port in the message
    driver_msg[1] = mtk_eap_type;  // Setting the EAP type in the message

    // Checking if the event notification is successful or not
    if (wifi_event_notification_wait(port, WIFI_EVENT_ID_IOT_SET_EAP_WAIT,
                                     (unsigned char *)&driver_msg,
                                     sizeof(driver_msg)) != WIFI_SUCC)
        status = WIFI_FAIL;         // If event notification is not successful, set status to fail
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Get MediaTek WiFi EAP Type.
*
* This function retrieves the Extensible Authentication Protocol (EAP) type set on a 
* given WiFi port, which indicates the authentication method being used.
*
* @param [IN] port            The port number, either 0 for STA / AP Client or 1 for AP
* @param [OUT] mtk_eap_type   Pointer to hold the retrieved MTK EAP type
*
* @return Returns an integer value wherein >=0 means success, <0 means failure
*/
int32_t wifi_config_get_eap_type(uint8_t port, uint8_t *mtk_eap_type)
{
    // Status is initialized to 0, indicating success
    int32_t status = 0;

    // Check if provided port is valid
    if (!wifi_is_port_valid(port)) {
        status = WIFI_ERR_PARA_INVALID;   // Updating the status as a parameter is invalid
        goto final;
    }

    // Check if provided pointer for holding EAP type is not NULL
    if (mtk_eap_type == NULL) {
        status = WIFI_ERR_PARA_INVALID;   // Updating the status as a parameter is invalid
        goto final;
    }

#ifdef MTK_MINISUPP_ENABLE
    // Call appropriate function to get EAP type when MiniSupplicant support is enabled
    status = mtk_supplicant_get_eap_type(port, mtk_eap_type);
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Load a Certificate data from a specific address.
*
* @param [IN] port        Port number (0 for AP, 1 for AP Client, 2 for STA)
* @param [IN] cert_type   Type of certificate to be loaded
* @param [IN] start_addr  Starting address of the certificate in memory
* @param [IN] length      Length of the data at the provided address
*
* @return int32_t Status code >= 0 means success, <0 means failure
*/
int32_t wifi_config_set_load_cert(uint8_t port,
                                  uint8_t cert_type,
                                  uint32_t start_addr,
                                  uint32_t length) 
{
    ...
    // Validate if given port is valid
    if (!wifi_is_port_valid(port)) {
        LOG_FUNC("Provided port is invalid");
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }

    // Check the operational mode of the WiFi module
    if (wifi_config_get_opmode_internal(&opmode) < 0) {
        status = WIFI_FAIL;
        goto final;
    }

    // Validate that both 'start address' & 'length' are either zero or non-zero
    if (!((length == 0 && start_addr == 0) ||
          (length != 0 && start_addr != 0))) {
        LOG_FUNC("Start address and length must be either both zeros (default) or both non-zero");
        status = WIFI_FAIL;
        goto final;
    }
    ...
    switch (cert_type) {
        case WIFI_EAP_CERT_TYPE_CLIENT_CERT:
            if (start_addr == 0) {
                start_addr = DEFAULT_EAP_CERT_ADDR_CRT;
                LOG_FUNC("Use default client_cert address %p", start_addr);
            }
            if (length == 0) {
                length = DEFAULT_EAP_CERT_ADDR_CRT_LEN;
                LOG_FUNC("Use default client_cert_len address %d", length);
            }
            break;
        case WIFI_EAP_CERT_TYPE_CLIENT_KEY:
            if (start_addr == 0) {
                start_addr = DEFAULT_EAP_CERT_ADDR_KEY;
                LOG_FUNC("Use default client_key address %p", start_addr);
            }
            if (length == 0) {
                length = DEFAULT_EAP_CERT_ADDR_KEY_LEN;
                LOG_FUNC("Use default client_key_len address %d", length);
            }
            break;
        default:
            break;
    ...
    // Creating driver message to load CA.
    uint8_t driver_msg[10];
    driver_msg[0] = port;
    driver_msg[1] = (start_addr      ) & 0xFF;
    driver_msg[2] = (start_addr >>  8) & 0xFF;
    driver_msg[3] = (start_addr >> 16) & 0xFF;
    driver_msg[4] = (start_addr >> 24) & 0xFF;
    driver_msg[5] = (length          ) & 0xFF;
    driver_msg[6] = (length     >>  8) & 0xFF;
    driver_msg[7] = (length     >> 16) & 0xFF;
    driver_msg[8] = (length     >> 24) & 0xFF;
    driver_msg[9] = cert_type;

    // Send the message and wait for the event notification.
    if (wifi_event_notification_wait(port, WIFI_EVENT_ID_IOT_SET_LOAD_CERT_WAIT,
                                    (unsigned char*)&driver_msg, sizeof(driver_msg)) != WIFI_SUCC) {
        status = WIFI_FAIL;
        goto final;
    }
...
/**
* @brief Set WiFi WPA/WPA2 password for EAP-TLS client key.
*
* This function sets the password used in EAP-TLS method on a given WiFi port.
*
* @param [IN] port         Port number (0 = STA / AP Client, 1 = AP)
* @param [IN] key_pwd        Password (8 to 256 bytes ASCII or 64 bytes Hex)
* @param [IN] key_pwd_len    Length of the password (8 to 256)
*
* @return int32_t representation indicating status. >=0 means success, <0 means failure
*/
int32_t wifi_config_set_eap_key_pwd(uint8_t port, uint8_t *key_pwd, uint32_t key_pwd_len) 
{
    int32_t status = 0;
    union uint32_to_u8 converter;

    converter.num = key_pwd_len;
    ...
    // Ensuring the password is not NULL
    if (key_pwd == NULL) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }
    
    // Ensuring the length of the password is within the valid range
    if ((key_pwd_len < 8) || (key_pwd_len > WIFI_LENGTH_WPA_PKPWD)) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }
#ifdef MTK_MINISUPP_ENABLE
    uint8_t driver_msg[WIFI_LENGTH_WPA_PKPWD + 5] = {0};

    // Preparing the command message for the driver.
    driver_msg[0] = port;
    os_memcpy(&driver_msg[1], &converter.arr, sizeof(converter.arr));// Adding the length to the message
    os_memcpy((driver_msg + 5), key_pwd, key_pwd_len);               // Adding the password to the message

    // Send the event notification wait and set status based on its return value
    if (wifi_event_notification_wait(port, WIFI_EVENT_ID_IOT_SET_WPAPKPWD_WAIT,
                                    (unsigned char *)&driver_msg, sizeof(driver_msg)) != WIFI_SUCC)
        status = WIFI_FAIL;         // If event notification is not successful, set status to fail
#endif /* #ifdef MTK_MINISUPP_ENABLE */
...
/**
* @brief Get WiFi WPA/WPA2 password for user identity based EAP methods.
*
* The function retrieves the WPA/WPA2 password for a specific WiFi port used in EAP methods.
*
* @param [IN] port          Port number - 0 for STA / AP Client, 1 for AP.
* @param [OUT] key_pwd_buf    Buffer to hold retrieved password. Should be enough to hold between 8 to 256 ASCII bytes.
* @param [OUT] key_pwd_len    A variable/pointer to hold the actual length of the retrieved password.
*
* @return Returns an integer indicating status - a non-negative value means success, a negative value means failure.
*/
int32_t wifi_config_get_eap_key_pwd(uint8_t port, uint8_t *key_pwd_buf,
                                  uint32_t *key_pwd_len)
{
    int32_t status = 0; // Status initialization
    ...
    // Null checking on output parameters (buffer and length)
    if ((key_pwd_buf == NULL) || (key_pwd_len == NULL)) {
        status = WIFI_ERR_PARA_INVALID;
        goto final;
    }
    ...
    // If MiniSupplicant support is enabled, get the WPA password through it
    status = mtk_supplicant_get_eap_key_pwd(port, key_pwd_buf, key_pwd_len);

#endif /* #ifdef MTK_MINISUPP_ENABLE */

final: 
    // Return the status after completing the operations
    return status;
}
...
int32_t wifi_config_set_rx_filter(uint32_t flag)
{
    ...
    kalStrCpy((char *)rChipConfigInfo.aucCmd + kalStrLen(cCmd),
               cRxFilter);
...
#ifndef MT7921_PORTING_TMP_MASK
int32_t wifi_config_set_sap_remove_sta_disable(uint8_t *mac_addr, uint8_t disable)
{
    struct GLUE_INFO *prGlueInfo;
    struct STA_RECORD *prStaRec;
    struct BSS_INFO *prBssInfo;
    ...
    prStaRec =
        cnmGetStaRecByAddress(prGlueInfo->prAdapter,
    ...
    prStaRec->ucSapRemoveDisable = disable;
    cnmStaSendUpdateCmd(prGlueInfo->prAdapter, prStaRec, FALSE);
...
int32_t wifi_wps_connection_by_pin(uint8_t port, uint8_t *bssid, uint8_t *pin_code)
{
    ...
    LOG_FUNC("%d %s", port, pin_code);

    if (wifi_config_get_opmode(&opmode) < 0) {
        return -1;
    }
    if(WIFI_MODE_REPEATER == opmode) {
        LOG_FUNC( "Don't support WPS in Repeater mode.");
        return -1;
    }
    if(WIFI_PORT_AP < port) {
        LOG_FUNC("Port=%d, it must be 0/1:STA/AP.", port);
        return -1;
    }
    if(8 != strlen((char *)pin_code) && 4 != strlen((char *)pin_code)) {
        LOG_FUNC("Input pin_code len error.");
        return -1;
    }
...
int32_t wifi_wps_connection_by_pbc(uint8_t port, uint8_t *bssid)
{
    ...
    if(WIFI_PORT_AP < port) {
        LOG_FUNC("Port=%d, it must be AP.", port);
        return -1;
    }
...
static uint16_t config_method = 0;
static void wifi_start_ap_wps_thread_hdl( void *param)
{
    printf("config_method 0x%08x\n", config_method);
    if(config_method & WPS_CONFIG_PUSHBUTTON) {
        wifi_wps_connection_by_pbc(WIFI_PORT_AP, NULL);
    }
    printf("[%d]wifi_wps_connection_by_pbc done\n", __LINE__);
    ap_wps_task = NULL;
    vTaskDelete(NULL);
}

void wifi_start_ap_wps_thread(uint16_t config)
{
    config_method = config;
    printf("wifi_start_ap_wps_thread\n");
    if(xTaskCreate(wifi_start_ap_wps_thread_hdl, ((const char*)"ap_wps"),configMINIMAL_STACK_SIZE /*256*/, NULL, tskIDLE_PRIORITY + 3, &ap_wps_task) != pdPASS)
        printf("\n\r%s xTaskCreate(ap_wps thread) failed", __FUNCTION__);

}

uint32_t wifi_config_get_ip_address(uint8_t port)
{
    struct netif *prNetif;
    if (port == WIFI_PORT_STA) {
        prNetif = wlan_sta_netif;
    } else {
        prNetif = wlan_ap_netif;
    }

    return prNetif->ip_addr.addr;
}
