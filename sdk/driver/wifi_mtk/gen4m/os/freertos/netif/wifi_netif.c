...
static void wifi_dhcpd_domain_change(void)
{
    ...
    char buf[128] = {0};
    u32_t ip_to_add;
    const ip_addr_t *ip_unchanged = netif_ip4_addr(&ap_netif);
    const ip_addr_t *ip_netmask = netif_ip4_netmask(&ap_netif);
    ip_addr_t ip_changed;
    char *pStr;
...
void ip_ready_callback(struct netif *netif)
{
    ...
    if (netif_is_up(netif) && !ip_addr_isany_val(netif->ip_addr)) {
        if (NULL != inet_ntoa(netif->ip_addr)) {
           ...
           DBGLOG(RX, ERROR, "DHCP got Failed");
        }
    }
...
int lwip_tcpip_init(void)
{
    ...
    if (ret == 0 && strnicmp("static", (char *)mode, strlen("static")) == 0) {
        ...
        get_ip_from_nvdm("IpAddr", ip);
        IP4_ADDR(&sta_ip_info->ip, ip[0], ip[1], ip[2], ip[3]);
        
        get_ip_from_nvdm("IpNetmask", ip);
        IP4_ADDR(&sta_ip_info->netmask, ip[0], ip[1], ip[2], ip[3]);
        
        get_ip_from_nvdm("IpGateway", ip);
        IP4_ADDR(&sta_ip_info->gw, ip[0], ip[1], ip[2], ip[3]);
    } else {
        ip_addr_set_zero(&sta_ip_info->ip);
        ip_addr_set_zero(&sta_ip_info->netmask);
        ip_addr_set_zero(&sta_ip_info->gw);
    }
    ...
    rMac[4] = 0x44;
    rMac[5] = 0x55;
    printf("%s %d go to tcpip_stack_init\n", __func__, __LINE__);
...
err_t mtk_netif_input(struct netif *inp, struct pbuf *p)
{
    ...
    if ((payload[12] & 0x0F) != 0x08) {
        ...
        if ((p->len > 12) && (g_lwip_eapol_rx_socket >= 0) && (g_lwip_eapol_rx_socket_second >= 0)) {
            ...
            if (len != p->len)
                DBGLOG(RX, ERROR, "Dual interface eapol-rx relay sendto failed!\n");
...
static void dhcpd_set_ip_pool(const ip_addr_t *ap_ip_addr,
                              const ip_addr_t *ap_net_mask,
...
void wifi_dhcpd_init(dhcpd_settings_t *dhcpd_settings)
{
    ...
    pStr = ipaddr_ntoa(&dns1_ip);
    if (pStr != NULL)
        kalStrnCpy(dhcpd_settings->dhcpd_primary_dns,
                   pStr, IP4ADDR_STRLEN_MAX - 1);

    pStr = ipaddr_ntoa(&dns2_ip);
    if (pStr != NULL)
...
static int wifi_netif_init(int mode, ip_info_t *ip_info, uint8_t *mac)
{
    ...
    DBGLOG(RX, INFO, "[gen4m_netif] %s mac:%u\r\n", __func__, mac[1]);
...
{
    DBGLOG(INIT, INFO, "[%s gen4m_netif] mac:%u:%u:%u:%u:%u:%u \r\n", mode == LWIP_STA_MODE ? "STA" :"AP", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    if (tcpip_enable == false) {
        /* Don't init tcpip due to SocketInit init it */;
