...
#define __MINISUPP_DBGLOG__(level, _fmt, ...) \
	do { \
        printf("[%d]%s():(WPA DBGLOG)" _fmt, \
                __LINE__, \
...
#define __MINISUPP_WPA_MSG__(wpa_s, level, ...) \
	do { \
		minisupp_wpa_msg(wpa_s, level, __func__, __LINE__, __VA_ARGS__); \
	} while (0)
...
#define __MINISUPP_DBGLOG_ERROR__(level, ...) \
	do { \
        minisupp_wpa_msg(NULL, level, __func__, __LINE__, __VA_ARGS__); \
    } while (0)
 ...
 * Note: New line '\n' is added to the end of the text when printing to stdout.
 */
void minisupp_wpa_msg(void *ctx, int level, const char* func, int line, const char *fmt, ...) PRINTF_FORMAT(5, 6);
