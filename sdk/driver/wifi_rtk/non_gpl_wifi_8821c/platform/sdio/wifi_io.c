...
static const struct sdio_device_id sdio_ids[] = {
	{ SDIO_DEVICE(0x024C, 0xC821),.driver_data = (void *)RTL8821}
};
...
int mmpRelWifiDiverCmdMode(int mode)
{
    if (mode == RTW_MODE_P2P){
        printf("mmpRelWifiDiverCmdMode p2p mode %d \n",mode);
        gDefault_mode = mode;
    }
    return 0;
}
...
int mmpRtlWifiDriverCmdTest(void)
{
    ...
    printk("====>[mmpRtlWifiDriver][SDIO %s] We call entry function or CMD test there.... Default_Mode %d \n", (rc == 0) ? "enabled":"disabled",Default_Mode);
    ...
    if (gDefault_mode == RTW_MODE_P2P){
        Default_Mode = RTW_MODE_P2P;       
    }
...
retry_init:
    ...
    } else if (Default_Mode == RTW_MODE_P2P){

        nRet = wifi_on(RTW_MODE_P2P);
