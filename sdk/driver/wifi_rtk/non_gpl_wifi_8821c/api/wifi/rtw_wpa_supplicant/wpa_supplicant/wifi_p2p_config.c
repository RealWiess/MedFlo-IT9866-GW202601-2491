...
unsigned char g8821_p2p_state;

int gAuto_chl = 0;


extern void dhcps_init();
...
/*
	if(wifi_start_ap(ssid,RTW_SECURITY_OPEN,
						 NULL,
						 strlen(ssid),
						 strlen(passphrase),
						 channel
...
*/
	...
	// start dhcp server
	printf("wifi_start_p2p_go dhcps_init -----------------------------------  \n");
	dhcps_init();
...
#endif
    (void)printf("cmd_wifi_p2p_start %d \n",wifi_get_mode());
    if (wifi_get_mode() == RTW_MODE_P2P)
    {
        printf("cmd_wifi_p2p_start already p2p mode \n");
        
    } else {
        printf("cmd_wifi_p2p_start start to p2p mode \n");    
    	wifi_off();
    	os_sleep(0, 20000);
...
void cmd_wifi_p2p_auto_go_start(int argc, char **argv)
{
...
#ifdef P2P_5G
    u8 channel = 40;   // 36,40,44
#else
    u8 channel = 1; // 1, 6, 11
#endif
	char *ssid_in = "DIRECT-34-Ameba-12";
	char *ssid_in_test = "DIRECT-iteap";
	char *dev_name = "Ameba1234-00";	// max strlen 32
    ...
    printf("cmd_wifi_p2p_auto_go_start %d , %s(%d) , %s(%d) , %s(%d) \n",argc,argv[0],strlen(argv[0]),argv[1],strlen(argv[1]),argv[2],strlen(argv[2]));
    ...
    wifi_p2p_set_ssid( ssid_in_test);
    wifi_p2p_init_auto_go_params(res, passphrase, channel);

#else
    // argv[0] can't show 
	wifi_p2p_set_dev_name(argv[0]);	

	wifi_p2p_set_ssid(argv[1]);	
	//wifi_p2p_init_auto_go_params(res, argv[2], channel);
	wifi_p2p_init_auto_go_params(res, argv[2], channel);
#endif
...
void cmd_p2p_info(int argc, char **argv)
{
    ...
    g8821_p2p_state = p2p_state;
	printf("\r\np2p state : %d \n",g8821_p2p_state);
}
int cmd_p2p_deivce_connected(unsigned char *dev_addr)
{
    u8 dev_addr1[ETH_ALEN];

    printf("wifi_p2p_group_is_client_connected %d , wifi_p2p_group_is_intfaddr_connected %d \n",wifi_p2p_group_is_client_connected(dev_addr),wifi_p2p_group_is_intfaddr_connected(dev_addr));

    return wifi_p2p_group_is_intfaddr_connected(dev_addr);
