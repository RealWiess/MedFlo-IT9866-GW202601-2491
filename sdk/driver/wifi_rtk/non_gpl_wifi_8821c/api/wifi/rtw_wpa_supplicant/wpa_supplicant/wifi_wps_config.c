...
static void wps_config_wifi_setting(rtw_network_info_t *wifi, struct dev_credential *dev_cred)
{
	...
	strlcpy((char*)wifi->ssid.val, (char*)&dev_cred->ssid[0], sizeof(wifi->ssid.val));
	printf("\r\nwps_wifi.ssid = %s\n", wifi->ssid.val);
...
int wps_start(u16 wps_config, char *pin, u8 channel, char *ssid)
{
	...
	if(wps_config == WPS_CONFIG_DISPLAY
		|| wps_config == WPS_CONFIG_KEYPAD) {
		if(pin)
			strlcpy(wps_pin_code, pin, sizeof(wps_pin_code));
		else{
...
#if 0
    // get sdio irq handle
	task_sdio = (pthread_t) xTaskGetHandle("sdhci_thread_ir");
    param.sched_priority = 1;    
    pthread_setschedparam(task_sdio, 0, &param);
    printf("pthread_setschedparam sdio task 1 (%d)---------------------------------------------------------\n",task_sdio);
#endif
...
exit:
    ...
    pthread_setschedparam(task_sdio, 0, &param);
    printf("pthread_setschedparam sdio task 4 ---------------------------------------------------------\n");
#endif    
	wifi_unreg_event_handler(WIFI_EVENT_STA_WPS_START, wpas_wsc_sta_wps_start_hdl);
...
{
    u8 zero_mac[ETH_ALEN] = {0};    
    printf("wifi_start_ap_wps_thread %d \n",configMINIMAL_STACK_SIZE);
