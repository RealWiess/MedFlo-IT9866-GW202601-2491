...
#ifdef CONFIG_WOWLAN
uint8_t *rltk_wlan_get_ip(int idx){
#if (CONFIG_LWIP_LAYER == 1)
    uint8_t *IP =  LwIP_GetIP(&xnetif[idx]);
...
uint8_t *rltk_wlan_get_gw(int idx){
#if (CONFIG_LWIP_LAYER == 1)
    uint8_t *GW = LwIP_GetGW(&xnetif[idx]);
...
uint8_t *rltk_wlan_get_gwmask(int idx){
#if (CONFIG_LWIP_LAYER == 1)
    uint8_t *MASK = LwIP_GetMASK(&xnetif[idx]);
