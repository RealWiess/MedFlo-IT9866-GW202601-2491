...
#define GFX_USE_ERROR_LOG
        ...
        #define GFX_ERROR_MSG(format, args...)                                 \
            (void)printf("[GFX]%s(%d)[ERROR]: " format, __FUNCTION__,          \
                         __LINE__, ##args)
    #elif defined(GFX_USE_ERROR_LOG)
        #define GFX_ERROR_MSG(format, ...)                                     \
            (void)printf("[GFX]%s(%d)[ERROR]: " format, __FUNCTION__,          \
    ...
    #define GFX_LOG_MSG(format, ...)                                           \
        (void)printf("[GFX]%s(%d) " format, __FUNCTION__, __LINE__, __VA_ARGS__)

    #ifdef GFX_USE_VERBOSE_LOG
        #define GFX_VERBOSE_LOG(format, ...)                                   \
            (void)printf("[GFX]%s(%d) " format, __FUNCTION__, __LINE__,        \
                         __VA_ARGS__args)
    #else
        #define GFX_VERBOSE_LOG(format, ...)
    #endif
    ...
    #define GFX_LOG_MSG(format, args...)                                       \
        (void)printf("[GFX]%s(%d) " format, __FUNCTION__, __LINE__, ##args)
    ...
    #define GFX_HERE (void)printf("%s()[%d]\n", __FUNCTION__, __LINE__)
#else
    #define GFX_FUNC_ENTRY
    #define GFX_FUNC_LEAVE
    #define GFX_HERE
