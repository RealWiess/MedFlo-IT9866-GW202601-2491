...
#include "video/filter_nv12_jpgenc.h"
#endif
...
flip_osd()
{
    ...
    /*input */
    isp_share.addrY   = ((uint32_t)itv_rcs.curr_video->ya) + (itv_rcs.osd_window.startX) + (itv_rcs.osd_window.startY) * ((uint32_t)itv_rcs.curr_video->pitch_y);
    isp_share.addrU   = ((uint32_t)itv_rcs.curr_video->ua) + (itv_rcs.osd_window.startX) + (itv_rcs.osd_window.startY) * ((uint32_t)itv_rcs.curr_video->pitch_uv) / 2;
    isp_share.width   = (itv_rcs.osd_window.width);
    isp_share.height  = (itv_rcs.osd_window.height);
    isp_share.pitchY  = ((uint32_t)itv_rcs.curr_video->pitch_y);
    isp_share.pitchUv = ((uint32_t)itv_rcs.curr_video->pitch_uv);
    isp_share.format  = itv_rcs.curr_video->format;
    /*output */
    out_info.width    = (itv_rcs.osd_window.width);
    out_info.height   = (itv_rcs.osd_window.height);
    out_info.pitchY   = ((uint32_t)itv_rcs.curr_video->pitch_y);
    out_info.pitchUv  = ((uint32_t)itv_rcs.curr_video->pitch_uv);
    out_info.format   = MMP_ISP_OUT_NV12;
    out_info.addrY    = ((uint32_t)itv_rcs.curr_video->ya) + (itv_rcs.osd_window.startX) + (itv_rcs.osd_window.startY) * ((uint32_t)itv_rcs.curr_video->pitch_y);
    out_info.addrU    = ((uint32_t)itv_rcs.curr_video->ua) + (itv_rcs.osd_window.startX) + (itv_rcs.osd_window.startY) * ((uint32_t)itv_rcs.curr_video->pitch_uv) / 2;
...
#if defined(CFG_BUILD_FLOWER)
    NV12_pushdata((void*)itv_rcs.curr_video);
#endif
}
    ...
    void)
{
    ...
    if (itv_rcs.osd_setbase == 1)
    {
        ...
        ff->uibuf[bid]      = (uint8_t *)itv_rcs.osd_setbase_base;
        itv_rcs.osd_setbase = 0;
    }
    /*check update osd or not */
    update_osd();

    pthread_mutex_lock(&OSD_PREVIEW_MUTEX);


    if ((itv_rcs.osd_mode_isr[0] == 1) || (itv_rcs.osd_mode_isr[1] == 1))
    {
...
itv_set_osd_window (
    ...
    uint32_t    height)
{
    itv_rcs.osd_window.startX   = startX;
    itv_rcs.osd_window.startY   = startY;
    itv_rcs.osd_window.width    = width;
    itv_rcs.osd_window.height   = height;
}
