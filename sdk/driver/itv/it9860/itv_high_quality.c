...
static void
ui_position_transformation (
    MMP_ISP_OUTPUT_INFO * out_info_inner)
{
    uint8_t dsX = 0;
    uint8_t dsY = 0;
    uint8_t dw  = 0;
    ...
    dsX = out_info_inner->startX % 4;
    dsY = out_info_inner->startY % 4;

    if (dsX)
    {
        out_info_inner->startX = out_info_inner->startX + (4 - dsX);

        if (out_info_inner->width <= dsX)
        {
            (void)printf("ITV# ui_position_transformation, invalid position transformation_0\n");
        }
        else
        {
            out_info_inner->width = out_info_inner->width - dsX;
            if (!out_info_inner->width)
            {
                (void)printf("ITV# ui_position_transformation, invalid position transformation_1\n");
            }
            else
            {
                out_info_inner->width = out_info_inner->width - out_info_inner->width % 4;
                if (!out_info_inner->width)
                {
                    (void)printf("ITV# ui_position_transformation, invalid position transformation_2\n");
                }
            }
        }
    }
    ...
    if (dsY)
    {
        out_info_inner->startY = out_info_inner->startY + (4 - dsY);

        if (out_info_inner->height <= dsY)
        {
            (void)printf("ITV# ui_position_transformation, invalid position transformation_3\n");
        }
        else
        {
            out_info_inner->height = out_info_inner->height - dsY;
            if (!out_info_inner->height)
            {
                (void)printf("ITV# ui_position_transformation, invalid position transformation_4\n");
            }
            else
            {
                out_info_inner->height = out_info_inner->height - out_info_inner->height % 4;
                if (!out_info_inner->height)
                {
                    (void)printf("ITV# ui_position_transformation, invalid position transformation_5\n");
                }
            }
        }
    }

    dw  = out_info_inner->width % 4;
    dh  = out_info_inner->height % 4;

    if (dw)
    {
        out_info_inner->width = out_info_inner->width - dw;
        if (!out_info_inner->width)
        {
            (void)printf("ITV# ui_position_transformation, invalid position transformation_6\n");
        }
    }

    if (dh)
    {
        out_info_inner->height = out_info_inner->height - dh;
        if (!out_info_inner->height)
        {
            (void)printf("ITV# ui_position_transformation, invalid position transformation_7\n");
        }
    }
}
...
flip_lcd (
    int                 lcd_index,
    MMP_ISP_OUTPUT_INFO * out_info)
{
    MMP_ISP_OUTPUT_INFO out_info_inner  = {0};
    MMP_ISP_SHARE       isp_share       = {0};
    ITURotation         rot             = ITU_ROT_0;
    ...
    if (IS_ROTATE_DISPLAY())
    {
        ...
        out_info_inner.addrRGB  = (MMP_UINT32)((M2D_SURFACE *)(*_gRoateSurf))->baseScanPtr;
    }
    else
#endif
    {
        lcd_index               = 0;
        out_info_inner.addrRGB  = out_info->addrRGB;
    }

#if (CFG_CHIP_FAMILY != 970 && CFG_CHIP_FAMILY != 9860)
    if (IS_VERTICAL_DISPLAY())
    {
        out_info_inner.width    = ithLcdGetHeight();
        out_info_inner.height   = ithLcdGetWidth();
        out_info_inner.pitchRGB = out_info_inner.width * (ithLcdGetPitch() / ithLcdGetWidth());
    }
    else
#endif
    {
        ...
        switch (rot)
        {
            case ITU_ROT_90:
            case ITU_ROT_270:
                out_info_inner.width    = out_info->width;
                out_info_inner.height   = out_info->height;
                out_info_inner.pitchRGB = ithLcdGetPitch() * ithLcdGetHeight() / ithLcdGetWidth();
                break;

            default:
                out_info_inner.width    = out_info->width;
                out_info_inner.height   = out_info->height;
                out_info_inner.pitchRGB = ithLcdGetPitch();
                break;
        }
    }
    out_info_inner.startX   = out_info->startX;
    out_info_inner.startY   = out_info->startY;
    out_info_inner.format   = out_info->format;

    ui_position_transformation(&out_info_inner);

    if (isp_share.format != MMP_ISP_IN_RGB565)
    {
        mmpIspSetOutputWindow(gIspDev, &out_info_inner);
        mmpIspSetVideoWindow(gIspDev,
                0,
                0,
                out_info_inner.width,
                out_info_inner.height);
        ...
        // (void)printf("out_info_inner.addrRGB:%x, out_info_inner.width:%d, out_info_inner.height:%d, out_info_inner.startX:%d, out_info_inner.startY:%d\n",
        // out_info_inner.addrRGB, out_info_inner.width, out_info_inner.height, out_info_inner.startX, out_info_inner.startY);
    ...
    MMP_ISP_OUTPUT_INFO * out_info)
{
    ...
    if ((itv_rcs.mtal_pb_mode_isr == 1) && (itv_rcs.bVideo == true))
    {
        flip_lcd(itv_rcs.disp_buf_ridx, out_info);
    }
...
int
itv_update_uibuf_anchor (
    int             id,
    ITV_UI_PROPERTY * uiprop)
{
    ITV_STRC_FF * ff    = &(itv_rcs.ff[id]);
    const int   widx    = ff->ui_widx;
    const int   ridx    = ff->ui_ridx;
    ...
    uint32_t    height)
{
    ...
    (void)printf("ITV# itv_set_video_window sX:%d, sY:%d, w:%d, h:%d\n", startX, startY, width, height);

    if (startX % 4 || startY % 4 || width % 4 || height % 4)
    {
        (void)printf("ITV# itv_set_video_window invalid position, video layout must be set by 4 byte alignment\n");
    }
}
...
void
itv_enable_video_center_mode (
    bool enabled)
{
    itv_rcs.bCenterMode = enabled;
}

void
itv_update_video_window (
    int width,
    int height)
{
    int scal_x;
    ...
    if (itv_rcs.bCenterMode)
    {
        if ((width == 0) && (height == 0))
        {
            width   = itv_rcs.video_width;
            height  = itv_rcs.video_height;
        }
        else
        {
            itv_rcs.video_width     = width;
            itv_rcs.video_height    = height;
        }
        (void)printf("ori: display width = %d, height = %d\n", itv_rcs.vid_window.width, itv_rcs.vid_window.height);
        scal_w  = itv_rcs.vid_window.width;
        scal_h  = scal_w * height / width;

        if (scal_h > itv_rcs.vid_window.height)
        {
            // Cut left and right of the origin window
            scal_h  = itv_rcs.vid_window.height;
            scal_w  = scal_h * width / height;
        }

        scal_x                      = itv_rcs.vid_window.startX + (itv_rcs.vid_window.width - scal_w) / 2;
        scal_y                      = itv_rcs.vid_window.startY + (itv_rcs.vid_window.height - scal_h) / 2;

        itv_rcs.vid_window.startX   = scal_x;
        itv_rcs.vid_window.startY   = scal_y;
        itv_rcs.vid_window.width    = scal_w;
        itv_rcs.vid_window.height   = scal_h;
