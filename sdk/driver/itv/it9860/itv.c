...
typedef struct
{
    ...
    int                 max_vid_surf_size;

    /* VIDEO WINDOW POSITION */
    ITV_DISPLAY_WND     vid_window;
#ifdef CFG_ITV_VP_DUAL_WINDOWS
    ITV_DISPLAY_WND     vid_window_2nd;
...
next_lcd_index ()
{
    g_lcd_index_counter = (g_lcd_index_counter + 1) % itv_rcs.max_disp_buf_size;
    ...
    int lcd_index)
{
    ...
    if (isp_share.format != MMP_ISP_IN_RGB565)
    {
        ...
        {
            out_info.width  = itv_rcs.vid_window.width;
            out_info.height = itv_rcs.vid_window.height;
            ...
            out_info.startX     = itv_rcs.vid_window.startX;
            out_info.startY     = itv_rcs.vid_window.startY;
        ...
        if (itv_rcs.bDualWindowMode)
        {
            ...
            isp_share.addrY     = (uint32_t)(itv_rcs.curr_video->ya_2nd);
            isp_share.addrU     = (uint32_t)(itv_rcs.curr_video->ua_2nd);
            isp_share.addrV     = (uint32_t)(itv_rcs.curr_video->va_2nd);
            isp_share.width     = itv_rcs.curr_video->src_w_2nd;
            isp_share.height    = itv_rcs.curr_video->src_h_2nd;

            out_info.startX     = itv_rcs.vid_window_2nd.startX;
            out_info.startY     = itv_rcs.vid_window_2nd.startY;
            out_info.width      = itv_rcs.vid_window_2nd.width;
            out_info.height     = itv_rcs.vid_window_2nd.height;

            mmpIspSetOutputWindow(gIspDev, &out_info);
            mmpIspSetVideoWindow(gIspDev, 0, 0, itv_rcs.vid_window_2nd.width, itv_rcs.vid_window_2nd.height);
    ...
    void)
{
    ...
    if (itv_rcs.ff_setbase == 1)
    {
        ...
        ff->uibuf[bid]      = (uint8_t *)itv_rcs.ff_setbase_base;
    ...
    if (itv_rcs.video_update || itv_rcs.ui_update)
    {
        ...
        {
            itv_rcs.disp_buf_ridx = (itv_rcs.disp_buf_ridx + 1) % itv_rcs.max_disp_buf_size;
        }
...
#ifndef FRAME_FUNCTION_ENABLE
        itv_rcs.vid_surf_widx   = (itv_rcs.vid_surf_widx + 1) % itv_rcs.max_vid_surf_size;
#endif
    ...
    void)
{
    ...
    itv_rcs.curr_video                  = &itv_rcs.blank_yuv_frame;
    itv_rcs.max_disp_buf_size           = ITV_MAX_DISP_BUF;
    itv_rcs.max_vid_surf_size           = ITV_MAX_VSBUF;
...
itv_get_dbuf_anchor (
    void)
{
    if (QUEUE_IS_FULL(itv_rcs.disp_buf_widx, itv_rcs.disp_buf_ridx, itv_rcs.max_disp_buf_size))
    {
        return NULL;
    }
    ...
    ITV_DBUF_PROPERTY * prop)
{
    ...
    itv_rcs.disp_buf_widx = (itv_rcs.disp_buf_widx + 1) % itv_rcs.max_disp_buf_size;
    ...
    void)
{
    ...
    else
    {
        if (itv_rcs.vid_surf_ridx != itv_rcs.vid_surf_widx)
        {
            itv_rcs.vid_surf_ridx       = (itv_rcs.vid_surf_ridx + 1) % itv_rcs.max_vid_surf_size;
...
{
#ifndef FRAME_FUNCTION_ENABLE
    if (QUEUE_IS_FULL(itv_rcs.vid_surf_widx, itv_rcs.vid_surf_ridx, itv_rcs.max_vid_surf_size) && itv_rcs.check_video_surf)
    {
        return -1;
    }
#endif
...
void
itv_set_2nd_video_window (
    uint32_t    startX,
    uint32_t    startY,
    uint32_t    width,
    uint32_t    height)
{
    itv_rcs.vid_window_2nd.startX   = startX;
    itv_rcs.vid_window_2nd.startY   = startY;
    itv_rcs.vid_window_2nd.width    = width;
    itv_rcs.vid_window_2nd.height   = height;
}

void
itv_enable_video_window_dual_mode (
    bool enabled)
{
    itv_rcs.bDualWindowMode = enabled;
}
    ...
    bool            interlaced)
{
    ...
    if (interlaced)
    {
        ...
        switch (rot)
        {
            case ITU_ROT_90:
            case ITU_ROT_270:
                ispWbInfo.addrYRGB_1    = (uint32_t)itv_rcs.dbuf[1] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * ithLcdGetHeight() / ithLcdGetWidth() * itv_rcs.vid_window.startY / 2);
                ispWbInfo.addrYRGB_2    = (uint32_t)itv_rcs.dbuf[2] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * ithLcdGetHeight() / ithLcdGetWidth() * itv_rcs.vid_window.startY / 2);
                break;

            default:
                ispWbInfo.addrYRGB_1    = (uint32_t)itv_rcs.dbuf[1] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * itv_rcs.vid_window.startY / 2);
                ispWbInfo.addrYRGB_2    = (uint32_t)itv_rcs.dbuf[2] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * itv_rcs.vid_window.startY / 2);
                break;
        }
    ...
    else
    {
        ...
        switch (rot)
        {
            ...
            case ITU_ROT_270:
                ispWbInfo.addrYRGB_1    = (uint32_t)itv_rcs.dbuf[1] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * ithLcdGetHeight() / ithLcdGetWidth() * itv_rcs.vid_window.startY);
                ispWbInfo.addrYRGB_2    = (uint32_t)itv_rcs.dbuf[2] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * ithLcdGetHeight() / ithLcdGetWidth() * itv_rcs.vid_window.startY);
                break;
            ...
            default:
                ispWbInfo.addrYRGB_1    = (uint32_t)itv_rcs.dbuf[1] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * itv_rcs.vid_window.startY);
                ispWbInfo.addrYRGB_2    = (uint32_t)itv_rcs.dbuf[2] + 2 * itv_rcs.vid_window.startX +
                    (ithLcdGetPitch() * itv_rcs.vid_window.startY);
                break;
        }
    ...
    mmpIspSetWriteBufferNum(gIspDev, itv_rcs.max_vid_surf_size - 1);
...
void
itv_enable_video_lowlatency_mode (
    bool enabled)
{
    if(enabled == true)
    {
        itv_rcs.max_disp_buf_size = 2;
        itv_rcs.max_vid_surf_size = 2;
        printf("ITV# low latency mode\n");
    }
    else
    {
        itv_rcs.max_disp_buf_size = ITV_MAX_DISP_BUF;
        itv_rcs.max_vid_surf_size = ITV_MAX_VSBUF;
        printf("ITV# normal mode\n");
    }
}
#endif