...
//                              Macro Definition
//=============================================================================
#define IIC_MAGIC_ID_0      0x49494330U
...
//=============================================================================
static IIC_DEVICE IicDevices[4];
static uint32_t   pre_regData = 0;

static char *      dma_name[4] = {"dma_iic0", "dma_iic1", "dma_iic2", "dma_iic3"};
//=============================================================================
//                              Private Function Declaration
//=============================================================================
static void *     IicSlavePollingThread (void * arg);

static void       Iic_Check_Iint (IIC_PORT port);

static bool       IicInitDevice (IIC_PORT port, IIC_OP_MODE opMode);

static bool       IicInitDeviceHw (IIC_PORT port, uint32_t clock, uint32_t delay);

static void       IicTerminateDevice (IIC_PORT port);

static void       IicTerminateDeviceHw (IIC_PORT port);

static uint32_t   IicSetClock (IIC_PORT port, uint32_t clock);

static bool       IicWaitTxFifoCount (IIC_PORT port, uint32_t requestCount, int32_t timeout);

static bool       IicWaitRxFifoCount (IIC_PORT port, uint32_t requestCount, int32_t timeout);

static uint32_t   IicFiFoSendData (IIC_PORT port, uint8_t slaveAddr, uint8_t * inData, uint32_t inDataSize,
                                   IIC_DATA_MODE wStop);

static uint32_t   IicFiFoReceiveData (IIC_PORT port, uint8_t slaveAddr, uint8_t * outData, uint32_t outDataSize);

bool              IicSlaveReadWithCallback (IIC_PORT port);

void              IicSlaveFifoReadWithCallback (IIC_PORT port);

static uint32_t   IicSlaveCheckAck (IIC_PORT port, IIC_STATE state);

#ifdef IIC_USE_DMA
    #ifdef IIC_USE_DMA_INTR
static void IicDmaIntr (int ch, void * arg, uint32_t int_status);
    #endif

static bool     IicWaitDmaIdle (IIC_PORT port);

static uint32_t IicFiFoDmaSendData (IIC_PORT port, uint8_t slaveAddr, uint8_t * pbuffer, uint32_t size,
                                    IIC_DATA_MODE wStop);

static uint32_t IicFiFoDmaReceiveData (IIC_PORT port, uint8_t slaveAddr, uint8_t * pbuffer, uint32_t size);
#endif

//=============================================================================
//                              Public Function Definition
//=============================================================================
uint32_t mmpIicInitialize (IIC_PORT port, IIC_OP_MODE opMode,
                           ...
                           uint32_t delay)
{
    ...
    {
        (void)printf("Device(%d) already init!\n", port);
        return false;
    }

#if 0
    (void)printf("mmpIicInitialize\n");
#endif
    if (port >= IIC_PORT_MAX)
    {
        (void)printf("Invalid port(%d)\n", port);
        return false;
    }

    if (opMode != IIC_SLAVE_MODE && opMode != IIC_MASTER_MODE)
    {
        (void)printf("Invalid opMode(%d)\n", opMode);
        return false;
    }
    IicDevices[port].funcMutex = PTHREAD_MUTEX_INITIALIZER;
    ...
    do
    {
        if (IicDevices[port].refCount == 0)
        {
            ...
            {
                (void)printf("IicInitDevice() fail.\n");
                break;
            }
            ...
            {
                (void)printf("IicInitDeviceHw() fail.\n");
                break;
            }
        ...
        {
            (void)printf("IIC port %d already inited. Reference count is %d\n", port, IicDevices[port].refCount);
            result = 1;
        }
    } while (false);
...
uint32_t mmpIicTerminate (IIC_PORT port)
{
    uint32_t result = 0U;
...
__attribute__((used)) uint32_t mmpIicSendData (IIC_PORT port, uint8_t slaveAddr, uint8_t regAddr, uint8_t * outData,
...
uint32_t mmpIicSendDataEx (IIC_PORT port, uint8_t slaveAddr, uint8_t * outData, uint32_t outDataSize)
{
    uint32_t result = 0U;
...
uint32_t mmpIicReceiveData (IIC_PORT port, uint8_t slaveAddr, uint8_t * outData, uint32_t outDataSize, uint8_t * inData,
                            uint32_t inDataSize)
{
    uint32_t result = 0U;
...
uint32_t mmpIicReceiveData_wstop (IIC_PORT port, uint8_t slaveAddr, uint8_t * outData, uint32_t outDataSize,
                                  uint8_t * inData, uint32_t inDataSize)
{
    uint32_t result = 0U;
...
void mmpIicLockModule (IIC_PORT port)
{
    Iic_Check_Iint(port);
    pthread_mutex_lock(&IicDevices[port].funcMutex);
}

void mmpIicReleaseModule (IIC_PORT port)
{
    Iic_Check_Iint(port);
    pthread_mutex_unlock(&IicDevices[port].funcMutex);
}

uint32_t mmpIicSetClockRate (IIC_PORT port, uint32_t clock)
{
    uint32_t result = 0U;
...
uint32_t mmpIicGetClockRate (IIC_PORT port)
{
    uint32_t currClock = 0U;
...
void mmpIicSetSlaveMode (IIC_PORT port, uint32_t slaveAddr)
{
    /* Set slave address */
    ...
    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_CONTROL, REG_BIT_CONTL_TRANSFER_BYTE |
                                                                     // REG_BIT_CONTL_GC |
                                                                     REG_BIT_CONTL_I2C_ENABLE);
}

bool mmpIicSetSlaveModeCallback (IIC_PORT port, uint32_t slaveAddr, IicReceiveCallback recvCallback,
                                 IicWriteCallback writeCallback)
{
    ...
    {
        (void)printf("Device not in slave mode.\n");
        return false;
    }
...
{
    uint32_t outputBufferWriteIndex = 0U;
    uint32_t regData                = 0U;
...
bool mmpIicSlaveFiFoWrite (IIC_PORT port, uint8_t * outputBuffer, uint32_t outputBufferLength)
{
    uint32_t i             = 0U;
    ...
    // Enable IIC SCLK
    // Enable FIFO mode
    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_CONTROL, REG_BIT_CONTL_I2C_ENABLE | REG_BIT_CONTL_FIFO_EN);

    // Set endian
    ithWriteRegMaskA(IicDevices[port].regAddrBase + REG_I2C_CONTROL, ((uint32_t)1U) << 9U, ((uint32_t)1U) << 9U);
    ...
    do
    {
        ...
        mappedSysRam = (uint8_t*)ithMapVram((uint32_t)dmaWriteBuf, outputBufferLength * sizeof(uint16_t), ITH_VRAM_WRITE);
        ...
        ithDmaSetRequest(IicDevices[port].dmaChannel, ITH_DMA_NORMAL_MODE, ITH_DMA_MEM, ITH_DMA_HW_HANDSHAKE_MODE,
                         IIC0_DMA_TX_CH + port * 2);
        ithDmaStart(IicDevices[port].dmaChannel); // Fire DMA

        // Set threshold
        ithWriteRegMaskA(IicDevices[port].regAddrBase + REG_I2C_ICR, ((uint32_t)4U) << 20U, ((uint32_t)0xFU) << 20U);

        // Enable TX DMA
        ithWriteRegMaskA(IicDevices[port].regAddrBase + REG_I2C_ICR, 0x02U, 0x02U);
        ...
        // Disable TX DMA
        ithWriteRegMaskA(IicDevices[port].regAddrBase + REG_I2C_ICR, 0x00U, 0x02U);
    ...
    while (remainderSize)
    {
        uint32_t sendData = 0U;
        if (IicWaitTxFifoCount(port, 1, 1000) == false)
        {
            (void)printf("Wait TX FIFO timeout(status=0x%lx)!\n",
...
uint32_t mmpIicSlaveWrite (IIC_PORT port, uint8_t * outputBuffer, uint32_t outputBufferLength)
{
    uint32_t iicResult  = 0U;
    ...
    for (writeIndex = 0; writeIndex < outputBufferLength; writeIndex++)
    {
        ...
        /* Enable transfer bit */
        /* Write CR: Enable all interrupts and I2C enable, and disable SCL enable */
        ithWriteRegMaskA(IicDevices[port].regAddrBase + REG_I2C_CONTROL, REG_BIT_CONTL_TRANSFER_BYTE,
                         REG_BIT_CONTL_TRANSFER_BYTE);
        // ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_ICR, REG_BIT_INTR_ALL);
...
#ifdef ENABLE_INTR
static void i2c_isr(void* data)
{
    pthread_attr_t attr;
    uint32_t port = (uint32_t)data;
    uint32_t status = ithReadRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS);
    
    ithPrintf("==========intr status=0x%x(%d)==========\n", status, data);
}
#endif
...
static void Iic_Check_Iint (IIC_PORT port)
{
    if (port >= IIC_PORT_MAX)
    {
        (void)printf("Invalid device!\n");
        assert(0);
    }else
    {
        if (IicDevices[port].refCount < 1)
        {
            (void)printf("Device not init!\n");
            assert(0);
        }
    }
}
...
static bool IicInitDevice (IIC_PORT port, IIC_OP_MODE opMode)
{
    ...
    do
    {
...
#ifdef IIC_USE_DMA
    #ifdef IIC_USE_DMA_INTR
        IicDevices[port].dmaChannel =
            ithDmaRequestCh(dma_name[port], ITH_DMA_CH_PRIO_HIGH_3, IicDmaIntr, &IicDevices[port]);
    #else
        ...
        {
            (void)printf("Request semaphore fail.\n");
            break;
        }
    #endif
#endif
        ...
        if (IicDevices[port].opMode == IIC_SLAVE_MODE)
        {
            ...
            threadResult = pthread_create(&IicDevices[port].slaveThread, NULL, IicSlavePollingThread,
                                          (void *)IicDevices[port].port);
            if (threadResult)
            {
                (void)printf("Create thread for slave mode fail.\n");
                break;
            }
        }
...
static bool IicInitDeviceHw (IIC_PORT port, uint32_t clock, uint32_t delay)
{
    ...
    {
        // HDMI
        ithWriteRegMaskA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, (((uint32_t)1U) << 28U),
                         (((uint32_t)1U) << 29U) | (((uint32_t)1U) << 28U));
        ...
        if (IicDevices[port].port == IIC_PORT_0)
        {
            ...
            ithWriteRegA(ITH_GPIO_BASE + 0x238U,
                         (1 << 15) | (1UL << 31) | (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
            // reset IIC0
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (((uint32_t)1U) << 18U), (((uint32_t)1U) << 18U));
            (void)usleep(5);
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (((uint32_t)0U) << 18U), (((uint32_t)1U) << 18U));
#ifdef ENABLE_INTR
            if (IicDevices[port].opMode == IIC_MASTER_MODE)
            {
                // Initialize I2C IRQ
            	ithIntrDisableIrq(ITH_INTR_I2C0);
            	ithIntrClearIrq(ITH_INTR_I2C0);

            	// register I2C Handler to IRQ
            	ithIntrRegisterHandlerIrq(ITH_INTR_I2C0, i2c_isr, (void*)port);

            	// Enable I2C IRQ
            	ithIntrEnableIrq(ITH_INTR_I2C0);
            }
#endif            
        }
    #endif
        ...
        if (IicDevices[port].port == IIC_PORT_1)
        {
            ...
            ithWriteRegA(ITH_GPIO_BASE + 0x23CU, (((uint32_t)1U) << 15U) | (((uint32_t)1U) << 31U) |
                                                     (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
            // reset IIC1
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (((uint32_t)1U) << ((uint32_t)19U)),
                             (((uint32_t)1U) << ((uint32_t)19U)));
            (void)usleep(5);
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (0U << 19U), (((uint32_t)1U) << ((uint32_t)19U)));
#ifdef ENABLE_INTR
            if (IicDevices[port].opMode == IIC_MASTER_MODE)
            {
                // Initialize I2C IRQ
            	ithIntrDisableIrq(ITH_INTR_I2C1);
            	ithIntrClearIrq(ITH_INTR_I2C1);

            	// register I2C Handler to IRQ
            	ithIntrRegisterHandlerIrq(ITH_INTR_I2C1, i2c_isr, (void*)port);

            	// Enable I2C IRQ
            	ithIntrEnableIrq(ITH_INTR_I2C1);
            }
#endif            
        }
    #endif
        ...
        if (IicDevices[port].port == IIC_PORT_2)
        {
            ...
            ithWriteRegA(ITH_GPIO_BASE + 0x240U, (((uint32_t)1U) << 15U) | (((uint32_t)1U) << 31U) |
                                                     (IicDevices[port].ioSCLK << 16U) | (IicDevices[port].ioSDA));
            // reset IIC2
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (((uint32_t)1U) << 20U), (((uint32_t)1U) << 20U));
            (void)usleep(5);
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (0U << 20U), (((uint32_t)1U) << 20U));
#ifdef ENABLE_INTR
            if (IicDevices[port].opMode == IIC_MASTER_MODE)
            {
                // Initialize I2C IRQ
            	ithIntrDisableIrq(ITH_INTR_I2C2);
            	ithIntrClearIrq(ITH_INTR_I2C2);

            	// register I2C Handler to IRQ
            	ithIntrRegisterHandlerIrq(ITH_INTR_I2C2, i2c_isr, (void*)port);

            	// Enable I2C IRQ
            	ithIntrEnableIrq(ITH_INTR_I2C2);
            }
#endif            
        }
    #endif
        ...
        if (IicDevices[port].port == IIC_PORT_3)
        {
            ...
            ithWriteRegA(ITH_GPIO_BASE + 0x244U, (((uint32_t)1U) << 15U) | (((uint32_t)1U) << 31U) |
                                                     (IicDevices[port].ioSCLK << 16U) | (IicDevices[port].ioSDA));
            // reset IIC3
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (((uint32_t)1U) << ((uint32_t)21U)),
                             (((uint32_t)1U) << ((uint32_t)21U)));
            (void)usleep(5);
            ithWriteRegMaskA(ITH_HOST_BASE + 0x1CU, (0U << 21U), (((uint32_t)1U) << ((uint32_t)21U)));
#ifdef ENABLE_INTR
            if (IicDevices[port].opMode == IIC_MASTER_MODE)
            {
                // Initialize I2C IRQ
            	ithIntrDisableIrq(ITH_INTR_I2C3);
            	ithIntrClearIrq(ITH_INTR_I2C3);

            	// register I2C Handler to IRQ
            	ithIntrRegisterHandlerIrq(ITH_INTR_I2C3, i2c_isr, (void*)port);

            	// Enable I2C IRQ
            	ithIntrEnableIrq(ITH_INTR_I2C3);
            }
#endif            
        }
    #endif
        ...
        // Set clock rate
        IicDevices[port].clockRate = IicSetClock(port, clock);
        (void)printf(">>>>>>>>>IIC clock=%ld\n", IicDevices[port].clockRate);

        ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_CONTROL,
                     REG_BIT_CONTL_I2C_ENABLE | REG_BIT_CONTL_CLK_ENABLE | REG_BIT_CONTL_FIFO_EN);

#ifdef ENABLE_INTR
        if (IicDevices[port].opMode == IIC_MASTER_MODE)
        {
            /* Write CR: Enable NACK interrupts */
            ithWriteRegA(
...
static void IicTerminateDeviceHw (IIC_PORT port)
{
    ...
    if (IicDevices[port].port == IIC_PORT_0)
    {
        ithWriteRegA(ITH_GPIO_BASE + 0x238U, (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
    }
    #endif
    #ifdef CFG_I2C1_ENABLE
    if (IicDevices[port].port == IIC_PORT_1)
    {
        ithWriteRegA(ITH_GPIO_BASE + 0x23CU, (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
    }
    #endif
    #ifdef CFG_I2C2_ENABLE
    if (IicDevices[port].port == IIC_PORT_2)
    {
        ithWriteRegA(ITH_GPIO_BASE + 0x240U, (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
    }
    #endif
    #ifdef CFG_I2C3_ENABLE
    if (IicDevices[port].port == IIC_PORT_3)
    {
        ithWriteRegA(ITH_GPIO_BASE + 0x244U, (IicDevices[port].ioSCLK << 16) | (IicDevices[port].ioSDA));
    }
    #endif
#endif
    return;
                                 ...
                                 IIC_DATA_MODE wStop)
{
    ...
    {
        // reset HW
        (void)printf("(%s)reset IIC when TX FIFO still full\n", __func__);
        if (IicInitDeviceHw(port, IicDevices[port].clockRate, IicDevices[port].delay) == false)
        {
            (void)printf("IicInitDeviceHw() reset fail.\n");
        }
        result = 1;
        return result;
    }
    ...
    while(1)
    {
        status = ithReadRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS);
        //ithPrintf("==========iic send status=0x%lx(%d)==========\n", status, port);
        if((status & (((uint32_t)1U) << 6U))) //check if BERR intr
        {
            printf("iic NACK(status=0x%lx)!\n", status);

            //slave NACK, send stop and clear FIFO.
            ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_CONTROL,
                REG_BIT_CONTL_I2C_ENABLE |
                (((uint32_t)1U) << 12U) |
                REG_BIT_CONTL_STOP |
...
#if 0            
            //try use gpio mode to generate stop bit
            ithGpioClear(IicDevices[port].ioSDA);
    	    ithGpioSetOut(IicDevices[port].ioSDA);
    	    ithGpioSetMode(IicDevices[port].ioSDA, ITH_GPIO_MODE0);
            ithGpioClear(IicDevices[port].ioSCLK);
    	    ithGpioSetOut(IicDevices[port].ioSCLK);
    	    ithGpioSetMode(IicDevices[port].ioSCLK, ITH_GPIO_MODE0);
            IicTerminateDeviceHw(port);
            usleep(10);
            ithGpioSet(IicDevices[port].ioSCLK);
            usleep(10);
            ithGpioSet(IicDevices[port].ioSDA);
#endif            
            if (IicInitDeviceHw(port, IicDevices[port].clockRate, IicDevices[port].delay) == false)
            {
                (void)printf("IicInitDeviceHw() reset fail.\n");
            ...
            result = 1;
            break;
        }else if((status & (((uint32_t)1U) << 13U))) //check TX FIFO empty
        {
            break;
        }else
        {
            if (--timeout <= 0)
            {
                printf("Wait TX FIFO empty timeout(status=0x%lx)!\n", status);
                if (IicInitDeviceHw(port, IicDevices[port].clockRate, IicDevices[port].delay) == false)
                {
                    (void)printf("IicInitDeviceHw() reset fail.\n");
                }
...
// Enable FIFO mode
    ...
    while (remainderSize)
    {
        ...
        {
            (void)printf("Wait RX FIFO timeout(status=0x%lx)!\n",
                         ithReadRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS));
            // (void)printf("read data(status=0x%x)!\n", ithReadRegA(IicDevices[port].regAddrBase +
            // REG_I2C_DATA_RCV_STATE));
            result = 1;
            break;
        }
...
void IicSlaveFifoReadWithCallback (IIC_PORT port)
{
    static uint32_t slaveBufferReadIndex = 0U;
    ...
    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_CONTROL,
                 REG_BIT_CONTL_CHECK_DOWN_EN | REG_BIT_CONTL_I2C_ENABLE | REG_BIT_CONTL_FIFO_EN);
    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_ICR, REG_BIT_INTR_ALL);
    ...
    {
        if (IIC_REG_BIT(regData, 0) == 0) /* W or read data not end */
        {
            (void)printf("Slave in READ\n");
#ifdef IIC_USE_DMA
            ...
            if (recvDataSize)
            {
                ...
                ithDmaSetRequest(IicDevices[port].dmaChannel, ITH_DMA_HW_HANDSHAKE_MODE, IIC0_DMA_TX_CH + port * 2 + 1,
                                 ITH_DMA_NORMAL_MODE, ITH_DMA_MEM);
                ithDmaStart(IicDevices[port].dmaChannel); // Fire DMA
...

                {
                    ...
                    mappedSysRam = (uint8_t*)ithMapVram((uint32_t)dmaReadBuf, recvDataSize, ITH_VRAM_READ);
                ...
                if (IIC_REG_BIT(regData, 29) == 1)
                {
                    ithWriteRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS,
                                 REG_BIT_STATUS_DATA_END_CLEAR | REG_BIT_STATUS_CHECK_CLEAR);
                }
            }
#else
            ...
            if (recvDataSize > 8)
            {
                (void)printf("size=%d, should less than 8!\n", recvDataSize);
            }
    ...
    {
        // (void)printf("Slave Address Not Match!\n");
    }
}
...
static bool IicWaitDmaIdle (IIC_PORT port)
{
    ...
    {
        // (void)printf("IicWaitDmaIdle IIC + 0x18 = 0x%X\n", ithReadRegA(IicDevices[port].regAddrBase +
        // REG_I2C_BUS_MONITOR));
        (void)usleep(1000);
    }
                                    ...
                                    IIC_DATA_MODE wStop)
{
    ...
    do
    {
        ...
        mappedSysRam = (uint8_t*)ithMapVram((uint32_t)dmaWriteBuf, (size + 1) * sizeof(uint16_t), ITH_VRAM_WRITE);
        ...
        // (void)printf("IicFiFoDmaSendData\n");
        // (void)usleep(1000*10);
        if (IicWaitDmaIdle(port) == false)
        {
            (void)printf("ERROR at %s[%d]\n", __FUNCTION__, __LINE__);
...
static uint32_t IicFiFoDmaReceiveData (IIC_PORT port, uint8_t slaveAddr, uint8_t * pbuffer, uint32_t size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (IicWaitTxFifoCount(port, 1, 1000) == false)
        {
            (void)printf("Wait TX FIFO timeout!\n");
            result = 1;
            break;
        }
    ...
    #if 0
    // (void)printf("IicFiFoDmaReceiveData\n");
    // (void)usleep(1000*10);
    #endif
    if (IicWaitDmaIdle(port) == false)
    {
...

    {
        ...
        mappedSysRam = (uint8_t*)ithMapVram((uint32_t)dmaReadBuf, size, ITH_VRAM_READ);
