...
//
////////////////////////////////////////////////////////////
typedef MMP_UINT8 (*BLOCK_CHECK_METHOD) (
    MMP_UINT32 pba);
...
const MMP_UINT8 g_ScramblerTable[16] = {
    0x3f, 0xf6, 0x90, 0xfa, 0x2d, 0xcc, 0x75, 0xfb,
...
CorrectBitIn8kBDataBuffer (
    ...
    MMP_UINT8   ErrorBit);
static void
CorrectAllErrorBitsIn8kBDataBuffer (
    MMP_UINT8   * data,
    MMP_UINT8   * spare);
...
GenErrBit (
    ...
    MMP_UINT32  Address);
static void
GenErrBitsInWhilePage (
    MMP_UINT8   * pDataPtr,
    MMP_UINT8   TotalSizeInkB,
    MMP_UINT32  ErrBitNumOf1KB);
static void
BCH_ECC_TEST (
    void);
    ...
    void * data)
{
    ...
    {
        (void)printf("Nf.ISR: something wrong!!\n");
    }
    SYS_SetEventFromIsr(NandIsrEvent);
    // (void)printf("Nf.isr.out\n");
}
        #endif
    ...
    NF_CMD_TYPE type)
{
    ...
    {
        // (void)printf("WaitNfReady.in,type=%x\n",type);
        EventRst = SYS_WaitEvent(NandIsrEvent, 20);
        // (void)printf("WaitNfReady.event.got!!rst=%x\n",EventRst);
        if (result)
        {
            (void)printf("[Nf ERR]:INTR time out\n");
            result = LL_ERROR;
            goto end;
        }
    }
    ...
    do
    {
        ...
        if (timeOut-- == 0)
        {
            ...
            (void)printf("\tLB4K_HWECC_WaitCmdReady wait idle time out[1]!!!,0X131A=%X.\n", nfStatus);
            AHB_ReadRegister(NF_REG_CLOCK_DIVIDE, &nfStatus);
            (void)printf("cLOCK1=%X.\r\n", nfStatus);
            // AHB_ReadRegister(NF_REG_STORAGE_ADDR3, &nfStatus);
            // (void)printf("SprSize=%x.\r\n",nfStatus);
            result = LL_ERROR;
            goto end;
        }
    ...
    {
        (void)printf("NAND_CMD_2ND_READ70_STATUS fail\n");
        result = LL_ERROR;
        goto end;
    }
    ...
    MMP_BOOL    EnableEcc)
{
    ...
    else
    {
        ...
        // (void)printf("2.ECC=[%08x,%08x]",Reg704C,BchStatus);
        ...
        {
            // (void)printf("6.ECC, it can correction[%08x,%08x]x\n",Reg704C,Reg7050);
        ...
        {
            (void)printf("5.ECC, it can not correction[%08x,%08x]x\n", Reg704C, Reg7050);
            result = NF_OVER4ERR;
        }
    }
    return result;
    ...
    unsigned long   * pPageSize)
{
    ...
    (void)printf("0x701C = %x\n", Reg32);
    AHB_ReadRegister(0xD070003C, &Reg32);
    (void)printf("NF_REG_STATUS = %x\n", Reg32);
    ...
    (void)printf("\t-------------------- attribute Nand Flash Initial --------------------\n");
    (void)printf("\tInit %s\n", g_NfAttrib.bInit ? "OK" : "FAIL");
    (void)printf("\t%s\n", g_NfAttrib.bMultiDie ? "Multi-Die" : "Single-Die");
    (void)printf("\tNumber Of Blocks = %u\n", g_NfAttrib.numOfBlocks);
    (void)printf("\tPages Per Block  = %u\n", g_NfAttrib.pagePerBlock);
    (void)printf("\tPage Size        = %u Bytes\n", g_NfAttrib.pageSize);
    (void)printf("\tTotal Size       = %u MB\n", g_NfAttrib.pagePerBlock * g_NfAttrib.pageSize * (g_NfAttrib.numOfBlocks / 1024 ) / 1024);
    (void)printf("\tID1              = 0x%04X\n", g_NfAttrib.ID1);
    (void)printf("\tID2              = 0x%04X\n", g_NfAttrib.ID2);
    (void)printf("\tNand Maker       = %u\n", g_NfAttrib.nandMaker);
    (void)printf("\tCheck Method     = %u\n", g_NfAttrib.checkMethod);
    (void)printf("\tModel Name       = %s\n", g_NfAttrib.modelName);
    (void)printf("\tClock01(0x7024)  = 0x%08X\n", g_NfAttrib.clockValue);
    (void)printf("\t0x701C           = 0x%08X\n", g_NfAttrib.register701C);
    ...
    {
        (void)printf("Unknown nandflash ID\n");
        result = LL_ERROR;
        goto end;
    }
    ...
    if (!NF_InitDma())
    {
        (void)printf("LB4K_HWECC_Initialize: Initial DMA fail.\n");
        result = LL_ERROR;
        goto end;
    }
        #endif
...
/*
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB4K_HWECC_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB4K_HWECC_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB4K_HWECC_Read: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB4K_HWECC_Read: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    MMP_UINT8   * pBuffer)
{
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB4K_HWECC_LBA_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB4K_HWECC_LBA_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB4K_HWECC_LBA_Read: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB4K_HWECC_LBA_Read: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    MMP_UINT8   index)
{
    ...
    switch (index)
    {
        ...
        default:
            (void)printf("LB4K_HWECC_ReadPart: Unknown ECC value. eccResult = %d\n", eccResult);
            result = LL_ERROR;
            break;
    }
        #endif
    ...
    if ((index == LL_RP_DATA) && (((MMP_UINT32)pBuffer & 0x03) == 0))
    {
        ...
        // (void)printf("R2.2:set pBuffer=%x\n",pBuffer);
        AHB_WriteRegister(NF_REG_READ_DATA_BASE_ADDR, (MMP_UINT32)pBuffer);
    }
    else
    {
        // (void)printf("R2.2:set g_pNFdataBuf=%x\n",g_pNFdataBuf);
        AHB_WriteRegister(NF_REG_READ_DATA_BASE_ADDR, (MMP_UINT32)g_pNFdataBuf);
    }
    // --- Set destination spare address
    AHB_WriteRegister(NF_REG_READ_SPARE_BASE_ADDR, (MMP_UINT32)g_pNFspareBuf);
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB4K_HWECC_ReadPart: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB4K_HWECC_ReadPart: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB4K_HWECC_ReadPart: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;
    ...
    MMP_UINT32 pba)
{
    ...
    {
        (void)printf("\tLB4K_HWECC_IsBadBlock: ERROR! pba(%u) >= g_NfAttrib.numOfBlocks(%u)\n", pba, g_NfAttrib.numOfBlocks);
        return LL_ERROR;
    }
    ...
    // 2. Read first page
    {
        ...
        // (void)printf("Spr->dummy1=%08x,[%08x]\r\n",pSpare->dummy1,WRITE_SYMBOL);
        ...
        #endif
        {
            ...
            if (g_NfAttrib.pagePerBlock == 0x80)
            {
                ...
                if (IsEnHwScrambler)
                {
                    ...
                    {
                        (void)printf("this NAND flash does NOT match our NAND DATA structure[1]\r\n");
                        IfDataEqual00FF = LL_ERROR;
                    }
                    if (IsErasedBlock((MMP_UINT8 *)dataBuffer, (MMP_UINT8 *)pSpare) == MMP_FALSE)
                    {
                        (void)printf("this NAND flash does NOT match our NAND DATA structure[2]\r\n");
                        IfDataEqual00FF = LL_ERROR;
                    }
                }
                else
                {
                    for (i = 0; i < DATA_SIZE / 4; i++)
                    {
                        ...
                        {
                            (void)printf("this NAND flash does NOT match our NAND DATA structure[3][%x,%x]\n", i, PtCmprData0[i]);
                            {
                                MMP_UINT32 k;
                                for (k = 0; k < 1024; k++)
                                {
                                    (void)printf("%02x ", dataBuffer[k]);
                                    if ((k & 0x1FF) == 0x1FF)
                                    {
                                        (void)printf("\n[%x]", k + 1);
                                    }
            ...
            if (IfDataEqual00FF == LL_OK)
            {
                ...
                {
                    (void)printf("LB4K_HWECC_IsBadBlock: ERROR! g_NfAttrib.pfCheckMethod == MMP_NULL\n");
        ...
        #endif
        {
            ...
            if (g_NfAttrib.pagePerBlock == 0x80)
            {
                ...
                for (i = 0; i < DATA_SIZE / 4; i++)
                {
                    if ((PtCmprData0[i]) && (PtCmprData0[i] != 0xFFFFFFFF))
                    {
                        (void)printf("this NAND flash does NOT match our NAND DATA structure[0]\r\n");
                        IfDataEqual00FF = LL_ERROR;
                        break;
                    }
                }
    ...
    MMP_UINT8   * pByte)
{
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB4K_HWECC_Read 1B: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB4K_HWECC_Read 1B: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB4K_HWECC_Read 1B: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB4K_HWECC_Read 1B: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    NF_ATTRIBUTE    * pNfAttrib)
{
    ...
    if (!NfChipSelect(chipIndex))
    {
        return LL_ERROR;
    }

    if (NAND_CMD_2ND_READ70_STATUS(&NfTmpStatus) == LL_ERROR)
    {
        return LL_ERROR;
    }
    ...
    if (!result)
    {
        ...
        (void)printf("GetNandId: ID1 = 0x%04X.\n\r", regData1);
        (void)printf("GetNandId: ID2 = 0x%04X.\n\r", regData2);
        ...
        switch (Byte1)
        {
            case 0xAD: // Hynix
                ...
                {
                    (void)printf("GetNandId: Get Hynix Nandflash ID Failed. ID = 0x%02X 0x%02X 0x%02X 0x%02X\n",
            ...
            case 0x98: // Toshiba
                ...
                {
                    (void)printf("GetNandId: Get Toshiba Nandflash ID Failed. ID = 0x%02X 0x%02X 0x%02X 0x%02X\n",
                    Byte1, Byte2, Byte3, Byte4);
                    result = LL_ERROR;
                }
                break;
            ...
            case 0xEC: // Samsung
                ...
                {
                    (void)printf("GetNandId: Get Samsung Nandflash ID Failed. ID = 0x%02X 0x%02X 0x%02X 0x%02X\n",
                    Byte1, Byte2, Byte3, Byte4);
                    result = LL_ERROR;
                }
                break;
            ...
            case 0x20: // Numonyx
                ...
                {
                    (void)printf("GetNandId: Get Numonyx Nandflash ID Failed. ID = 0x%02X 0x%02X 0x%02X 0x%02X\n",
                    Byte1, Byte2, Byte3, Byte4);
                    result = LL_ERROR;
                }
                break;
    ...
    MMP_UINT32 chipIndex)
{
    ...
    switch (chipIndex)
    {
        case 0:  regValue   = 0x00;
            chipSelect      = 0x00000008;
            break;

        case 1:  regValue   = 0x08;
            chipSelect      = 0x00010008;
            break;

        case 2:  regValue   = 0x0C;
            chipSelect      = 0x00030008;
            break;

        case 3:  regValue   = 0x04;
            chipSelect      = 0x00030008;
            break;

        default: result     = MMP_FALSE;
            break;
    }
            #else
    switch (chipIndex)
    ...
    if (chipIndex == 1)
    {
        (void)printf("NfChipSelect: Use 2nd die\n");
    }
    else if (chipIndex > 1)
    {
        (void)printf("NfChipSelect: Use unknown die, FATAL ERROR!\n");
    }
    ...
    MMP_UINT8   * pSpare)
{
    ...
    MMP_UINT8   SprSbr[SPARE_SIZE]  = {
        0x43, 0x07, 0x02, 0x98, 0xc0, 0xd7, 0x8f, 0x22,
        0xe6, 0x84, 0xf9, 0x34, 0xf4, 0x94, 0x56, 0xc0
    };
        #else
    MMP_UINT8   SprSbr[SPARE_SIZE] = {
        0x32, 0xaf, 0x49, 0x07, 0xd0, 0xc2, 0xe7, 0x40,
        0xbb, 0xe4, 0x4c, 0x26, 0x7f, 0x61, 0x63, 0xc3,
        0xda, 0x79, 0xc2, 0xae, 0x40, 0x5d, 0xdd, 0x1f
    };          // BCH 24-bit
        #endif
    ...
    if (pAlignSpareBuf == pSpare)
    {
        ...
        #if 0
    {
        (void)printf("---------- SPARE DATA ----------\n");
        for ( i = 0; i < SPARE_SIZE; i += 8 )
        {
            (void)printf("0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockToshibaMethod01: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockToshibaMethod02: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockToshibaMethod03: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockSamsungMethod01: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockSamsungMethod02: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockHynixMethod01: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockHynixMethod02: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockNumonyxMethod01: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockNumonyxMethod02: \n");
    ...
    MMP_UINT32 pba)
{
    ...
    // (void)printf(" --> _IsBadBlockPowerFlashMethod01: \n");
    ...
    MMP_UINT32 * NfStatus)
{
    ...
    *NfStatus = 0;
    ...
    do
    {
        ...
        if (timeOut-- == 0)
        {
            (void)printf("\tLB4K_HWECC_WaitCmdReady wait idle time out[2]!!!\n");
            result = LL_ERROR;
            goto end;
        }
    } while ((*NfStatus & 0xc000) != NFIDLE);
    ...
    MMP_UINT32 PageLoop)
{
    ...
    // (void)printf("GCNTS:PageLoop=%x.\r\n",PageLoop);
    // (void)printf("GCNTS:g_pNFeccBuf=%x.\r\n",BCHpt);
    ...
    MMP_UINT8   BitLoop)
{
    ...
    // (void)printf("GEA:PageLoop=%x,BitLoop=%x.\r\n",PageLoop,BitLoop);
    ...
    MMP_UINT8   ErrorBit)
{
    ...
    if ((Last1kB_Index & g_pNFeccCurrRdMask) == Last1kB_Index)
    {
        ...
        // (void)printf("1.Last1kB_Index=%x\n",Last1kB_Index);
    }
    else
    {
        Last1kB_Index = g_TotalNumOfMapBit;
        // (void)printf("2.Last1kB_Index=%x\n",Last1kB_Index);
    }
    ...
    if (ErrorPage >= (Last1kB_Index - 1))
    {
        ...
        {
            (void)printf("Error Address >= 0x418, means Error bit is in ECC code!!\r\n");
            return;
        }

        if (ErrorAddr >= (ECC_PAGE_SIZE / 2))
        {
            (void)printf("Error Address >= 0x400, means Error bit is in spare area!!\r\n");
    ...
    else
    {
        if (ErrorAddr >= (ECC_PAGE_SIZE / 2))
        {
            (void)printf("Error Address >= 0x400, means Error bit is in ECC code!!\r\n");
            return;
        }
    }
    ...
    MMP_UINT8   * spare)
{
    ...
    (void)printf("TotalErrorPageNum=%x.\r\n", TotalErrorPageNum);
        #ifdef WIN32
    ...
    ithInvalidateDCacheRange(g_pNFeccBuf, (8 + (BCH_MODE * 2)) * TotalErrorPageNum);
        #endif
/*
    (void)printf("g_pNFeccBuf=[");
    for(i=0;i<32;i++)
    {
        (void)printf("%02x,",g_pNFeccBuf[i] );
        if( (i&0x0F)==0x0F) (void)printf("\r\n             ");
    }
    (void)printf("]\n");
*/
/*
    (void)printf("g_dataBuf=\n");
    for(j=0;j<1024;j++)
    {
        (void)printf("%02x,",data[j] );
        if( (j&0x0F)==0x0F) (void)printf("\r\n");
    }
    (void)printf("\n");
*/
    for (PageLoop = 0; PageLoop < TotalErrorPageNum; PageLoop++)
    {
        ...
        for (BitLoop = 0; BitLoop < ErrorNum; BitLoop++)
        {
            ...
            (void)printf("PgLp=%x,BtLp=%x,[", PageLoop, BitLoop);
            (void)printf("ErrPg=%x,ErrAddr=%x,ErrBit=%x.]\r\n", ErrorPage, ErrorAddr, ErrorBit);
    ...
    MMP_UINT32  CprSize)
{
    ...
    for (i = 0; i < CprSize; i++)
    {
        if (pBuf1[i] != pBuf2[i])
        {
            (void)printf("Compare error, i=%x,pf1=%x,pf2=%x,Ori=[%x],Dis[%x]\n", i, pBuf1, pBuf2, pBuf1[i], pBuf2[i]);
