...
_IsBadBlockToshibaMethod01 (
    MMP_UINT32 pba);                                            // Check byte 0 in data & 1st bytes in spare of 1st & 2nd page
static MMP_UINT8
_IsBadBlockToshibaMethod02 (
    MMP_UINT32 pba);                                            // Check byte 0 in data & 1st bytes in spare of last page
static MMP_UINT8
_IsBadBlockToshibaMethod03 (
    MMP_UINT32 pba);                                            // Check byte 0 in data & 1st bytes in spare of 1st page & last page
static MMP_UINT8
_IsBadBlockSamsungMethod01 (
    MMP_UINT32 pba);                                            // Check 1st bytes in spare of 1st & 2nd page
static MMP_UINT8
_IsBadBlockSamsungMethod02 (
    MMP_UINT32 pba);                                            // Check 1st bytes in spare of last page
static MMP_UINT8
_IsBadBlockHynixMethod01 (
    MMP_UINT32 pba);                                            // Check 1st bytes in spare of 1st & 2nd page
static MMP_UINT8
_IsBadBlockHynixMethod02 (
    MMP_UINT32 pba);                                            // Check 1st bytes in spare of last page & (last-2) page
static MMP_UINT8
_IsBadBlockNumonyxMethod01 (
    MMP_UINT32 pba);                                            // Check 1st & 6th byte in spare of 1st page
static MMP_UINT8
_IsBadBlockNumonyxMethod02 (
    MMP_UINT32 pba);                                            // Check 1st byte in spare of last page
static MMP_UINT8
    ...
    MMP_UINT32 chipIndex)
{
    ...
    switch (chipIndex)
    {
        ...
        #else
    (void)printf("NF chip select!!\n");
    switch (chipIndex)
    {
        case 0:  chipSelect = 0x00000000;
            break;

        case 1:  chipSelect = 0x00000001;
            break;

        case 2:  chipSelect = 0x00000002;
            break;

        case 3:  chipSelect = 0x00000003;
            break;

        default: result     = MMP_FALSE;
            break;
    }
    (void)printf("NF chip select[2]\n");
    // ithWriteRegMaskA(ITH_GPIO_BASE + 0xD4, chipSelect , 0x00000003); //set NAND Chip Select (GPIO:0x7CD4)
        #endif

    (void)printf("CpSl,R=%x,%x\n", result, MMP_TRUE);
    ...
    NF_ATTRIBUTE    * pNfAttrib)
{
    ...
    if (NfChipSelect(chipIndex) != MMP_TRUE)
    {
        return LL_ERROR;
    }
    ...
    unsigned long blk)
{
    ...
    (void)printf("ll.set_rev_blk(%x,%x)\n", blk, g_ReservedBlockNum);
    ...
    unsigned long   * pPageSize)
{
    ...
    ithWriteRegMaskA (
        NF_REG_RD_SCRAMB_CHECK,
        0x00001818,
        0x00001F1F);                                            // set nand patch
    ...
    */
    ...
    if (!NF_InitDma())
    {
        (void)printf("LB_HWECC_Initialize: Initial DMA fail.\n");
        result = LL_ERROR;
        goto end;
    }
    #endif
    ...
    MMP_UINT8   * pBuffer)
{
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB_HWECC_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB_HWECC_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB_HWECC_Read: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB_HWECC_Read: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    MMP_UINT8   * pBuffer)
{
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB_HWECC_LBA_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB_HWECC_LBA_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB_HWECC_LBA_Read: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB_HWECC_LBA_Read: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    MMP_UINT8   * pByte)
{
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB_HWECC_Read 1B: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB_HWECC_Read 1B: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB_HWECC_Read 1B: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                (void)printf("LB_HWECC_Read 1B: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    MMP_UINT8   index)
{
    ...
    switch (index)
    {
        ...
        default:
            (void)printf("LB_HWECC_ReadPart: Unknown ECC value. eccResult = %d\n", eccResult);
            result = LL_ERROR;
            break;
    }
    #endif
    ...
    if (result == LL_OK)
    {
        ...
        switch (eccResult)
        {
            ...
            case NF_ECCCORPASS:
                (void)printf("LB_HWECC_ReadPart: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;

            case NF_PARITYADDRERR:
                (void)printf("LB_HWECC_ReadPart: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                (void)printf("LB_HWECC_ReadPart: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;
    ...
    MMP_UINT32 pba)
{
    ...
    {
        (void)printf("IBB[%x] is BAD block for reserved block,(%x,%x)\n", pba, g_ReservedBlockNum);
        return LL_ERROR;
    }
    ...
    // 2. Read first page
    {
    ...
    #endif
        {
            ...
            if (IfDataEqual00FF == LL_OK)
            {
                ...
                {
                    (void)printf("LB_HWECC_IsBadBlock: ERROR! g_NfAttrib.pfCheckMethod == MMP_NULL\n");
