...
//feature B0 register
#define    NF_REG_OTP_EN                 (0x40)
#define    NF_REG_OTP_PRT                (0x80)
/***************************
 *
 **************************/
...
typedef struct SPI_NF_INFO_TAG	//NF_INFO_TAG
{
    uint32_t  Init;
    uint32_t  fromCfg;
    uint32_t  CurBlk;
...
typedef struct NF_ECC_TAG
{
    uint8_t  RegA0;
	uint8_t  RegB0;
	uint8_t  RegC0;
	uint8_t  Reg7C;
	uint8_t  enableEccBit;		
    uint8_t  EccErrBit;	
	uint8_t  EccErrThreshold;	
	uint8_t  RegDummy[2];
	uint32_t  block;
	uint32_t  page;
} NF_ECC_INFO;
 ...
 * global variables
 **************************/
extern uint32_t        gExtRawSprSize;

extern uint8_t         gForcePioMode;
extern uint8_t         gNeedRawSpare;
extern uint8_t         gForceEccEnable;
extern uint8_t         gDualDieNand;

extern NF_ECC_INFO         gEccInf;
extern uint8_t    gSupportEccErrBit;
extern uint8_t    gEccErrThreshold;

extern uint32_t        gCfgRsvBlkSize;
extern uint8_t         gAxiInfoBuffer[192];
...
uint8_t spiNf_PageReadEcc(uint32_t blk, uint32_t ppo, uint8_t *dBuf, uint8_t *ecc);
...
uint8_t spiNf_SetWtProtect(uint32_t blk, uint32_t bNum, uint8_t mod);
void spiNf_SwitchToDie0InISR(void);
uint8_t SpiNf_SwitchToDie0(void);
void spiNf_Reset(void);
uint8_t spiNf_PageReadRaw (uint32_t blk, uint32_t ppo, uint8_t * dBuf);
uint8_t spiNf_setQuadIoOperation (unsigned char value);
uint8_t spiNf_SetOtpEnableBit(uint8_t mode);
uint8_t spiNf_GetOtpProtectStatus(void);
