...
#if !defined(CFG_DEVICE_IPC)
#if FHOST_RX_REORDER
void rx_reorder_callback(void const *param);
...
void rx_reorder_callback(void const *param)
{
    if(0 == co_list_cnt(&fhost_rx_env.rx_reorder_list)) {
        return;
    }
    // Move window
    rtos_mutex_lock(fhost_rx_env.rx_reorder_lock, -1);
    fhost_rx_env.next_rx_seq_no = SEQNO_ADD(fhost_rx_env.next_rx_seq_no, 1);
    //aic_dbg("Callback Next %d, %d\r\n", fhost_rx_env.next_rx_seq_no, sys_now());

    // Check next one if exits in list
    struct fhost_rx_buf_tag *rx_buf = (struct fhost_rx_buf_tag *)co_list_pick(&fhost_rx_env.rx_reorder_list);
    uint8_t *frame = (uint8_t *)rx_buf->payload;
    struct mac_hdr *machdr_ptr = (struct mac_hdr *)frame;
    uint16_t rx_seq_no = (machdr_ptr->seq >> 4);
    while(rx_seq_no == fhost_rx_env.next_rx_seq_no) {
        // pop it
        rx_buf = (struct fhost_rx_buf_tag *)co_list_pop_front(&fhost_rx_env.rx_reorder_list);
        // Forward the buffer to the networking stack
        fhost_rx_buf_forward(rx_buf);
        fhost_rx_env.next_rx_seq_no = SEQNO_ADD(fhost_rx_env.next_rx_seq_no, 1);
        //rtos_timer_restart(fhost_rx_env.rx_reorder_timer, 0, false);
        //aic_dbg("R3 %d, %d\r\n", sys_now(), ret);

        //pick one
        rx_buf = (struct fhost_rx_buf_tag *)co_list_pick(&fhost_rx_env.rx_reorder_list);
        if(!rx_buf) {
            break;
        }
        //aic_dbg("pick list 1\r\n");
        frame = (uint8_t *)rx_buf->payload;
...
bool rx_buf_seq_cmp(struct co_list_hdr const *elementA,
                                struct co_list_hdr const *elementB)
{
    struct fhost_rx_buf_tag *rx_buf_A = (struct fhost_rx_buf_tag *)elementA;;
    uint8_t *frame_A = (uint8_t *)rx_buf_A->payload;
    struct mac_hdr *machdr_ptr_A = (struct mac_hdr *)frame_A;
    uint16_t seq_no_A = (machdr_ptr_A->seq >> 4);
    struct fhost_rx_buf_tag *rx_buf_B = (struct fhost_rx_buf_tag *)elementB;;
    uint8_t *frame_B = (uint8_t *)rx_buf_B->payload;
    struct mac_hdr *machdr_ptr_B = (struct mac_hdr *)frame_B;
    uint16_t seq_no_B = (machdr_ptr_B->seq >> 4);

    //aic_dbg("CMP %d %d\r\n", seq_no_A, seq_no_B);
    if(seqno_lt(seq_no_A, seq_no_B)) {
        return 1;
    } else {
        return 0;
    }
...
void insert_rx_buf_reorder_list(void *buf)
{
    //aic_dbg("Insert List\r\n");
    co_list_insert(&fhost_rx_env.rx_reorder_list, buf, rx_buf_seq_cmp);
}

void flush_rx_buf_reorder_list(void)
{
    //aic_dbg("flush_rx_buf_reorder_list\r\n");

    struct fhost_rx_buf_tag *buf;
    struct mac_hdr *machdr_ptr;
    uint16_t rx_seq_no;

    buf = (struct fhost_rx_buf_tag *)co_list_pick(&fhost_rx_env.rx_reorder_list);
    while(buf) {
        buf = (struct fhost_rx_buf_tag *)co_list_pop_front(&fhost_rx_env.rx_reorder_list);
        // Forward the buffer to the networking stack
        fhost_rx_buf_forward(buf);
        machdr_ptr = (struct mac_hdr *)buf->payload;
        rx_seq_no = (machdr_ptr->seq >> 4);
...
void clear_rx_buf_reorder_list(void)
{
    //aic_dbg("clear_rx_buf_reorder_list\r\n");
    struct fhost_rx_buf_tag *buf;
...
uint8_t rwnx_rxdataind(void *pthis, void *hostid)
{
    ...
    #if PLF_RSSI_DATAPKT
    rssi_saved = (int8_t)((info->vect.recvec1b >> 8) & 0xFF);
    ...
    #endif /* NX_BEACONING */
    //aic_dbg("R %d, %x %x\r\n", (machdr_ptr->seq >> 4), IS_QOS_DATA(machdr_ptr->fctl), (info->flags & RX_FLAGS_NEED_TO_REORD_BIT));
...
AGAIN:
    ...
    //dump(data, 64);
    uint16_t prot_type = 0, prot_type_offset = machdr_len_get(machdr_ptr->fctl);
    uint8_t protocol = 0,   protocol_offset = machdr_len_get(machdr_ptr->fctl);
    uint16_t port_num = 0,  port_num_offset = machdr_len_get(machdr_ptr->fctl);
    uint8_t not_filtered = 1; // handle by MCU
    ...
    switch (rx_buf->info.vect.statinfo & RX_HD_DECRSTATUS)
    {
        ...
        case RX_HD_DECR_TKIP:
            ...
            port_num_offset     += MAC_IV_LEN + MAC_EIV_LEN;
            protocol_offset     += MAC_IV_LEN + MAC_EIV_LEN;
            break;
        case RX_HD_DECR_WEP:
            prot_type_offset    += MAC_IV_LEN;
            port_num_offset     += MAC_IV_LEN;
            protocol_offset     += MAC_IV_LEN;
            break;
        default:
            break;
    }
    ...
    memcpy(&port_num, &data[port_num_offset], 2);
    protocol = data[protocol_offset];

    //aic_dbg("port %d %02x %02x\r\n", co_ntohs(port_num), co_ntohs(port_num), protocol);
    if (co_ntohs(prot_type) == LLC_ETHERTYPE_IP) {
        uint8_t nb = 0;
        ip_packet_filter_t *filter = get_hostif_user_filter();

        if (68 == co_ntohs(port_num)) { //dhcp
            not_filtered = 0;
        }

        while (filter && (0xEA == filter->used)) {
            //aic_dbg("F %02X %02X\r\n", filter->destination_port, filter->protocol);
            if ((filter->protocol == protocol) && (filter->destination_port == co_ntohs(port_num))) {
                not_filtered = 0;
                break;
            }
            nb ++;
    ...
    if ((co_ntohs(prot_type) != LLC_ETHERTYPE_EAP_T) && (HOSTIF_ST_AWAKE == get_hostif_wlan_status()) && (not_filtered)) {
       // aic_dbg("%s %d\r\n", __func__, (sizeof(struct rx_info) + info->vect.frmlen));
        //dump((uint8_t *)info, 64);
        e2a_data_send((uint8_t *)info, (sizeof(struct rx_info) + info->vect.frmlen));
        fhost_rx_buf_push(buf);
        return 0;
    }
 ...
 */
static uint8_t rwnx_msgackind(void *pthis, void *hostid)
{
...
void rwnx_timer_task(void *arg)
{
    ...
    while (1) {
        ...
        do {
            timer_hdl_exec = (rwnx_timer_handle)co_list_pick(&timer_hdl_list);
            if(timer_hdl_exec) {
                if (timer_hdl_exec->expired_ms <= cur_time_ms) {
                    timer_hdl_exec = co_list_pop_front(&timer_hdl_list);
                    (timer_hdl_exec->cb)(timer_hdl_exec->arg);
                }
            }
...
static struct recv_msdu *reorder_frame_in_process = NULL;
static bool reorder_frame_to_async = false;
#ifdef CONFIG_REORD_FORWARD_LIST
static struct co_list reord_forward_list;
static rtos_mutex reord_forward_lock;
#else
static rtos_mutex reord_frame_ind_lock;
#endif
#endif
...
int reord_single_frame_ind(struct recv_msdu *prframe)
{
    ...
    #else
    rtos_mutex_lock(reord_frame_ind_lock, -1);
    #endif
    if (prframe->forward) {
        #ifdef CONFIG_REORD_FORWARD_LIST
        rtos_mutex_lock(reord_forward_lock, -1);
        co_list_push_back(&reord_forward_list, &prframe->hdr);
        rtos_mutex_unlock(reord_forward_lock);
        #else
        reorder_frame_in_process = prframe;
        reorder_frame_to_async = false;
        #endif
        fhost_rx_buf_forward(prframe->rx_buf);
        #ifdef CONFIG_REORD_FORWARD_LIST
        rtos_mutex_lock(reord_forward_lock, -1);
        if (co_list_find(&reord_forward_list, &prframe->hdr)) {
            co_list_extract(&reord_forward_list, &prframe->hdr);
            need_free = true;
        }
        rtos_mutex_unlock(reord_forward_lock);
        #endif
    ...
    #else
    if (!(reorder_frame_in_process && reorder_frame_to_async)) {
        reord_rxframe_free(prframe);
    }
    reorder_frame_in_process = NULL;
    reorder_frame_to_async = false;
    rtos_mutex_unlock(reord_frame_ind_lock);
    #endif
    return 0;
}
#else
...
void reord_timeout_handler(void *arg)
{
    ...
    DBG_REORD("reord TO\n");
    if (reord_rxframes_process(preorder_ctrl, true) == true) {
        int ret;
        #if (AICWF_RWNX_TIMER_EN)
        if (preorder_ctrl->reord_timer) {
...
int rwnx_rxdataind_aicwf(struct fhost_rx_buf_tag *buf)
{
    ...
    struct mac_hdr *machdr_ptr = (struct mac_hdr *)frame;
    struct rx_info *info =&buf->info;
    uint8_t mac_hdr_len = MAC_SHORT_MAC_HDR_LEN + MAC_HDR_QOS_CTRL_LEN;
...
void rwnx_reord_init(void)
{
    ...
    #ifdef CONFIG_FHOST_RX_ASYNC
    #ifdef CONFIG_REORD_FORWARD_LIST
    ret = rtos_mutex_create(&reord_forward_lock, "reord_forward_lock");
    if (ret < 0) {
        WRN_REORD("reord forward mutex create fail\n");
    }
    co_list_init(&reord_forward_list);
    #else
    ret = rtos_mutex_create(&reord_frame_ind_lock, "reord_frame_ind_lock");
    if (ret < 0) {
        WRN_REORD("reord frame ind lock create fail\n");
    }
    #endif
    #endif
    #if (AICWF_RWNX_TIMER_EN)
...
void rwnx_reord_deinit(void)
{
    ...
    #ifdef CONFIG_FHOST_RX_ASYNC
    #ifdef CONFIG_REORD_FORWARD_LIST
    rtos_mutex_delete(reord_forward_lock);
    #else
    rtos_mutex_delete(reord_frame_ind_lock);
    #endif
    #endif
    #if (AICWF_RWNX_TIMER_EN)
    rwnx_timer_deinit();
...
{
    return (void *)reorder_frame_in_process;
}

#ifdef CONFIG_REORD_FORWARD_LIST
void *reorder_frame_match(struct fhost_rx_buf_tag *buf)
{
    struct recv_msdu *frame = NULL;

    rtos_mutex_lock(reord_forward_lock, -1);
    frame = (struct recv_msdu *)co_list_pick(&reord_forward_list);
    while (frame) {
        if (frame->rx_buf == buf) {
            co_list_extract(&reord_forward_list, &frame->hdr);
            break;
        }
        frame = (struct recv_msdu *)co_list_next(&frame->hdr);
    }
    rtos_mutex_unlock(reord_forward_lock);
...
bool reorder_frame_match(struct fhost_rx_buf_tag *buf, struct fhost_rx_async_desc_tag *desc)
{
    ...
    if (reorder_frame_in_process) {
        struct fhost_rx_buf_tag *buf_in_process = (struct fhost_rx_buf_tag *)reorder_frame_in_process->rx_buf;
        if (buf_in_process == buf) {
            match = true;
            reorder_frame_to_async = true;
            desc->frame_ptr = (void *)reorder_frame_in_process;
            desc->frame_type = RX_ASYNC_REORDER_FRAME;
        }
    }
