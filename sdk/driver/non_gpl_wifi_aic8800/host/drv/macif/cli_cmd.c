...
int Cli_RunCmd(char *CmdBuffer)
{
    ...
    if (cli_cmd == NULL) {
        AIC_LOG_PRINTF("cli_cmd malloc fail\n");
        return -1;
    }
    ...
    AIC_LOG_PRINTF("put cli cmd to list\n");
...
void cli_cmd_task(void *argv)
{
    ...
    while (1) {
        ...
        AIC_LOG_PRINTF("cli cmd sema wait: ret=%d\n", ret);
        rtos_mutex_lock(cli_cmd_mutex, -1);
        cli_cmd_t *cli_cmd = (cli_cmd_t *)co_list_pop_front(&cli_cmd_list);
        rtos_mutex_unlock(cli_cmd_mutex);
        if (cli_cmd == NULL) {
            AIC_LOG_PRINTF("cli cmd list empty\n");
            continue;
        }
...
bool aic_cli_cmd_init(struct rwnx_hw *rwnx_hw)
{
    ...
    if (cli_cmd_mutex == NULL) {
        AIC_LOG_PRINTF("cli_cmd_mutex create fail\n");
        return FALSE;
    }
