...
#include "usb_port.h"
#include "../../platform/ite_usb/porting.h"
#elif CONFIG_SDIO_SUPPORT
#include "sdio_port.h"
#include "sdio_def.h"
#include "../../platform/ite_sdio/porting.h"
...
static rtos_semaphore fhost_tx_task_sem = NULL;
static rtos_semaphore fhost_tx_task_exit_sem = NULL;
static bool fhost_tx_task_exit_flag = false;
#endif
        ...
        while (1)
        {
            ...
            while (!co_list_is_empty(&txq->ready)) {
                if (sdiodev->fw_avail_bufcnt <= DATA_FLOW_CTRL_THRESH) {
                    ...
                    net_buf_tx_t *net_buf = txdesc->host.packet_addr;

                    #if 0
                    if (net_buf->data_len == txdesc->host.packet_len) { //802.11
                        tx_len = (txdesc->host.packet_len + offsetof(struct hostdesc, cfm_cb));
                        ...
                        memcpy(tx_buf, &txdesc->host, offsetof(struct hostdesc, cfm_cb));
                        *(uint32_t *)tx_buf = txdesc->host.tid;
                        memcpy((tx_buf + offsetof(struct hostdesc, cfm_cb)), net_buf->data_ptr, txdesc->host.packet_len);
                    ...
                    #endif
                    {
                        ...
                        #endif
                        tx_buf = net_buf->data_ptr - (offsetof(struct hostdesc, cfm_cb));
                        tx_len = (txdesc->host.packet_len + offsetof(struct hostdesc, cfm_cb));
...
static void fhost_tx_task_func(void *param)
{
    ...
    for (;;) {
        #if defined PLATFORM_ITE_FREERTOS
        ret = rtos_semaphore_wait(fhost_tx_task_sem, -1);

        if (fhost_tx_task_exit_flag) {
            break;
        }
        #endif

        #if FHOST_TX_MSG_USE_QUEUE
        rtos_queue_read(fhost_tx_task_queue, &msg, -1, false);
        #else
        rtos_signal_recv(fhost_tx_task, &msg, 4);
        #endif
...
exit:
    ...
    #if 0 //defined PLATFORM_ITE_FREERTOS
    rtos_semaphore_signal(fhost_tx_task_exit_sem, false);
    #else
    if (fhost_tx_task) {
...
#else
static void fhost_tx_task_func(void)
{
    struct fhost_vif_tag *fhost_vif;
    net_buf_tx_t *net_buf;
    uint32_t msg;
                         ...
                         cb_fhost_tx cfm_cb, void *cfm_cb_arg)
{
    ...
    if (type == IEEE802_3)
    {
        ...
        if (co_ntohs(host->ethertype) == LLC_ETHERTYPE_EAP_T) {
            aic_dbg("%s eapol, len %d(%d)\r\n", __func__, net_buf->data_len, rtos_now(0));
    ...
    txdesc->host.packet_addr = net_buf;
    ...
    FHOST_TXQ_UNLOCK();
#ifdef CONFIG_SDIO_SUPPORT
    if ((txq_ready_cnt == 1) || (0 == rtos_queue_cnt(fhost_tx_task_queue)))
#else
...
#endif
    {
        ...
        if (0 == ret) {
            #if defined PLATFORM_ITE_FREERTOS
            rtos_semaphore_signal(fhost_tx_task_sem, false);
            #endif
        } else {
...
void fhost_tx_desc_netbuf_free(struct fhost_tx_desc_tag *desc)
{
    ...
    fhost_tx_release_buf(net_buf, p_hd->cfm.status,
                         p_hd->cfm_cb, p_hd->cfm_cb_arg);
...
void fhost_tx_flush_txq(uint8_t sta_id)
{
    ...
    while (desc = (struct fhost_tx_queue_tag *)fhost_tx_dequeue_cfm_list()) {
        fhost_tx_desc_netbuf_free(desc);
    }
}
...
void fhost_tx_do_vif_disable(uint8_t vif_idx)
{
    ...
    while (desc)
    {
        fhost_tx_release_buf((net_buf_tx_t *)desc->txdesc.host.packet_addr, 0, desc->txdesc.host.cfm_cb,
                             desc->txdesc.host.cfm_cb_arg);
        desc = fhost_tx_dequeue(txq);
    }
...
int fhost_tx_task_init(void)
{
    ...
    #if defined PLATFORM_ITE_FREERTOS
    if (rtos_semaphore_create(&fhost_tx_task_sem, "fhost_tx_task_sem", 0x7FFFFFFF, 0)) {
        aic_dbg("fhost_tx_task_sem create fail\n");
        return 3;
    }

    //if (rtos_semaphore_create(&fhost_tx_task_exit_sem, "fhost_tx_task_exit_sem", 0x7FFFFFFF, 0)) {
    //    aic_dbg("fhost_tx_task_exit_sem create fail\n");
    //    return 4;
    //}

    //fhost_tx_task_exit_flag = false;
    #endif

    if (rtos_task_create(fhost_tx_task_func, "Fhost TX task", FHOST_TX_TASK,
                         fhost_tx_stack_size, NULL, fhost_tx_priority, &fhost_tx_task)) {
...
void fhost_tx_task_deinit(void)
{
    ...
    if (fhost_tx_task) {
        #if defined PLATFORM_ITE_FREERTOS
        //fhost_tx_task_exit_flag = true;
        //AIC_LOG_PRINTF("signal fhost_tx_task_sem\n");     
        //rtos_semaphore_signal(fhost_tx_task_sem, false);
        //AIC_LOG_PRINTF("wait fhost_tx_task_exit_sem\n");    
        //rtos_semaphore_wait(fhost_tx_task_exit_sem, 1000);
        //fhost_tx_task_exit_flag = false;
        #endif
        rtos_task_delete(fhost_tx_task);
        fhost_tx_task = NULL;
    ...
    #ifdef CONFIG_FHOST_TX_AC_SCHEDULE
    fhost_tx_ac_flush();
    if (fhost_tx_task_sem) {
        rtos_semaphore_delete(fhost_tx_task_sem);
        fhost_tx_task_sem = NULL;
    }
    #else
    fhost_tx_list_flush();
    #endif
    ...
    #if defined PLATFORM_ITE_FREERTOS
    if (fhost_tx_task_sem != NULL) {
        rtos_semaphore_delete(fhost_tx_task_sem);
        fhost_tx_task_sem = NULL;
    }
    
    if (fhost_tx_task_exit_sem != NULL) {
        rtos_semaphore_delete(fhost_tx_task_exit_sem);
        fhost_tx_task_exit_sem = NULL;
    }
    #endif
}
