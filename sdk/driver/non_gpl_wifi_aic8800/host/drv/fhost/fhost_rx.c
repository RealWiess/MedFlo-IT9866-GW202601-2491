...
void fhost_rx_buf_forward(struct fhost_rx_buf_tag *buf)
{
    ...
    else
    {
        ...
        #if 0//NX_BEACONING
        uint8_t f2others = fhost_frame2others(buf, &fhost_vif->mac_addr);
        if((VIF_AP == fhost_vif->mac_vif->type) && f2others) {
            send_to_tx_task((uint8_t *)buf->payload + payl_offset, info->vect.frmlen - payl_offset);
        }
        #endif /* NX_BEACONING */
        ...
        if (co_ntohs(ethhdr->type) == LLC_ETHERTYPE_EAP_T) {
            aic_dbg("%s eapol, len %d(%d)\r\n", __func__, (info->vect.frmlen - payl_offset), rtos_now(0));
            net_eth_receive((uint8_t *)buf->payload + payl_offset, (info->vect.frmlen - payl_offset), net_if);
        } else {
            // Forward to the networking stack
            #ifdef CONFIG_FHOST_RX_ASYNC
            struct fhost_rx_buf_tag *buf_in_process = NULL;
            struct fhost_rx_async_desc_tag *desc = NULL;
            desc = fhost_rx_async_desc_alloc();
            if (desc == NULL) {
                aic_dbg("Err: async desc alloc fail\n");
                return;
            }
            desc->data = (uint8_t *)buf->payload + payl_offset;
            desc->len = info->vect.frmlen - payl_offset;

            #ifdef CONFIG_REORD_FORWARD_LIST
            void *frame = NULL;
            do {
                frame = fhost_rx_frame_match(buf);
                if (frame) {
                    desc->frame_ptr = frame;
                ...
                aic_dbg("Err: no frame to match buf:%p\n", buf);
                fhost_rx_async_desc_free(desc);
                return;
            } while (0);
            #else
            if (fhost_rx_frame_match(buf, desc)) {
            }
            #if (AICWF_RX_REORDER)
            else if (reorder_frame_match(buf, desc)) {
            }
            #endif
            else {
                aic_dbg("Err: no frame to match buf:%p\n", buf);
                aic_dbg("rx_frame_in_process:%p\n", fhost_rx_frame_in_process_get());
                #if (AICWF_RX_REORDER)
                aic_dbg("reorder_frame_in_process:%p\n", reorder_frame_in_process_get());
                #endif
                fhost_rx_async_desc_free(desc);
                return;
            }
            ...
            fhost_rx_async_post_enqueue(desc);
            #endif
            rx_eth_data_process((uint8_t *)buf->payload + payl_offset, info->vect.frmlen - payl_offset);
        }
    }
}
 ...
 ****************************************************************************************
 */
static void fhost_rx_buf_len_update(struct fhost_rx_buf_tag *buf, struct rxdesc_tag *status)
{
    struct rx_info *info =&buf->info;
...
static RTOS_TASK_FCT(fhost_rx_task)
{
    ...
    for (;;)
    {
        ...
        if (msg.desc.status & RX_STAT_UF)
        {
            ...
            if (fhost_rx_env.monitor_cb != NULL)
                fhost_rx_env.monitor_cb((struct fhost_rx_buf_tag *) uf_buf, fhost_rx_uf_buf_push, true);
            else
                fhost_rx_uf_buf_push(uf_buf);
        }
        #endif // NX_UF_EN
        if (msg.desc.status & RX_STAT_MONITOR)
        {
            // Call the callback function
            if (fhost_rx_env.monitor_cb != NULL)
                fhost_rx_env.monitor_cb(buf, fhost_rx_buf_push, false);
            else
                fhost_rx_buf_push(buf);
        }
