...
extern void clear_rx_buf_reorder_list(void);
/**
 ****************************************************************************************
...
static void fhost_cntrl_cfgrwnx_scan(void *msg)
{
    ...
    req.add_ies = (uint32_t)cmd->extra_ies;
...
static void fhost_cntrl_cfgrwnx_ctrl_port(void *msg)
{
    ...
    if (!mac_vif || ((mac_vif->type != VIF_STA) && (mac_vif->type != VIF_AP)))
        goto send_resp;
...
static void fhost_cntrl_cfgrwnx_roc(void *msg)
{
	printk("%s in\n", __func__);
	int ret;
...
static void fhost_cntrl_cfgrwnx_cancel_roc(void *msg)
{
	...
	return rwnx_send_cancel_roc(cntrl_rwnx_hw, cmd->fhost_vif_idx);
}
#endif
...
struct fhost_cntrl_link *fhost_cntrl_cfgrwnx_link_open(void)
{
    ...
    if (bind(sock, (struct sockaddr_in *)&recv_addr, sizeof(recv_addr)) < 0) {
    ...
    if(getsockname(sock, (struct sockaddr_in *)&recv_addr, &len) < 0) {
    ...
    if (connect(cfgrwnx_link[i].sock_send, (struct sockaddr_in *)&recv_addr, len) < 0) {
        printf("sock_connect failed\n");
        goto err;
    }

    if ((cfgrwnx_link[i].sock_recv = accept(sock, (struct sockaddr_in *)&recv_addr, &len)) < 0) {
...
int fhost_cntrl_cfgrwnx_event_send(struct cfgrwnx_msg_hdr *msg_hdr, int sock)
{
    ...
    //TRACE_FHOST("fhost_cntrl_cfgrwnx_event_send %d (%d)\r\n", msg_hdr->id, msg_hdr->len);
...
retry_recv:
    times ++;
    ret = recv(link->sock_recv, msg_hdr, (sizeof *msg_hdr), MSG_PEEK);
    if (ret < 0) {
        TRACE_FHOST("Peek errno %d\r\n", ret);
#ifdef CONFIG_AIC_SOCKET_WRAPPER
        rtos_task_suspend(10);
        if (times > 5) {
            return -1;
        }
        //#define EAGAIN       35        /* Resource temporarily unavailable */
        if (35 == get_sock_error(link->sock_recv))
            goto retry_recv;
#endif
        return -1;
    }
