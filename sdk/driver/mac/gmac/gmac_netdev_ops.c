...
static int ftgmac030_xmit_frame(void* packet, uint32_t length, bool is_vlan, uint16_t vlan_tci, struct eth_device *netdev)
{
    ...
    skb = __netdev_alloc_skb(netdev, 0);
    skb->data = packet;
    skb->len = length;
    if (is_vlan)
    {
    ...
    */
    count = (int)DIV_ROUND_UP(len, ctrl->tx_fifo_limit);
    ...
    if (count) 
    {
        ...
        */
        ...
        tx_ring->next_to_use += (u16)count;
        if (tx_ring->next_to_use >= tx_ring->count)
        {
            tx_ring->next_to_use -= (u16)tx_ring->count;
        }
...
static int ftgmac030_change_mtu(struct eth_device *netdev, int new_mtu)
{
	...
	if ((new_mtu < ETH_ZLEN + ETH_FCS_LEN + VLAN_HLEN) ||
	    (max_frame > (int)ctrl->max_hw_frame_size)) 
    {
	...
	ctrl->max_frame_size = (u32)max_frame;
	e_info(drv, "changing MTU from %d to %d\n", netdev->mtu, new_mtu);
	netdev->mtu = (uint32_t)new_mtu;
...
#endif		
	default:
		return generic_mii_ioctl(&ctrl->mii, data, cmd);
	}
}
