...
//#define     WEIGHT_NUM 9
        ...
        #else    
            MMP_FLOAT ScaleRatio[WEIGHT_NUM] = {(1088.0f / CFG_LCD_WIDTH), (1920.0f / CFG_LCD_HEIGHT), (1280.0f / CFG_LCD_HEIGHT), (576.0f / CFG_LCD_WIDTH), (480.0f / CFG_LCD_WIDTH), (720.0f / CFG_LCD_HEIGHT), (640.0f / CFG_LCD_HEIGHT), (288.0f / CFG_LCD_WIDTH), (352.0f / CFG_LCD_HEIGHT)};
        #endif
    #else
...
MMP_FLOAT ScaleRatioWeiBal[2] = {(4.0f / 1), 1.0f};
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->OutInfo.EnableCCFun || (pISPctxt->WorkFlags & ISP_FLAGS_ENABLE_BLUR))
    {
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE &&
        pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
    {
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE &&
        pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_NORMAL)
    {
    ...
    MMP_FLOAT  weightMatrix[][ISP_SCALE_TAP])
{
    ...
    {
        if (pISPctxt->EnableWeightBalance)
        {
            if (scale == ScaleRatioWeiBal[0])
            {
                for (i = 0; i < tapSize; i++)
                {
                    weightMatrix[i][0] = 0.33;
                    weightMatrix[i][1] = 0.33;
                    weightMatrix[i][2] = 0.33;
                    weightMatrix[i][3] = 0;
                }
            }
            else
    ...
    MMP_BOOL weightBal)
{
    ...
    pISPctxt->EnableWeightBalance = weightBal;
    ...
    {
        _ISP_CreateWeighting(pISPctxt, ScaleRatioWeiBal[0], ISP_SCALE_TAP, ISP_SCALE_TAP_SIZE, WeightMat);

        for (j = 0; j < ISP_SCALE_TAP_SIZE; j++)
        {
            for (i = 0; i < ISP_SCALE_TAP; i++)
            {
                pISPctxt->WeightMatInt[0][j][i] = (MMP_UINT8)ISP_FloatToFix(WeightMat[j][i], 1, 6);
            }
        }

        _ISP_CreateWeighting(pISPctxt, ScaleRatioWeiBal[1], ISP_SCALE_TAP, ISP_SCALE_TAP_SIZE, WeightMat);

        for (j = 0; j < ISP_SCALE_TAP_SIZE; j++)
        {
            for (i = 0; i < ISP_SCALE_TAP; i++)
            {
                pISPctxt->WeightMatInt[1][j][i] = (MMP_UINT8)ISP_FloatToFix(WeightMat[j][i], 1, 6);
            }
        }
    }
    ...
    MMP_ISP_INFORMAT format)
{
    ...
    if (pISPctxt->EnableYUVProcess || pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO_BYPASS ||
        pISPctxt->ispMode == MMP_ISP_MODE_SHOW_IMAGE_BYPASS || pISPctxt->ispMode == MMP_ISP_MODE_TRANSFORM_BYPASS)
    {
        pISPctxt->OutInfo.EnableCSFun = MMP_FALSE;
    }
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->OutInfo.Width < (pISPctxt->ScaleFun.DstPosX + pISPctxt->ScaleFun.DstWidth))
    {
        ...
        {
            pISPctxt->ScaleFun.DstPosX = pISPctxt->ScaleFun.DstWidth = pISPctxt->ScaleFun.DstPosY =
                pISPctxt->ScaleFun.DstHeight                         = 0;
        }
        else
        {
    ...
    if (pISPctxt->PVOutInfo.Width < (pISPctxt->PVScaleFun.DstPosX + pISPctxt->PVScaleFun.DstWidth))
    {
        ...
        {
            pISPctxt->PVScaleFun.DstPosX = pISPctxt->PVScaleFun.DstWidth = pISPctxt->PVScaleFun.DstPosY =
                pISPctxt->PVScaleFun.DstHeight                           = 0;
        }
        else
        {
        ...
        (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_InputBuf))
    {
        ...
        if (pISPctxt->ispCore == MMP_ISP_CORE_0 ||
            (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_FALSE))
        {
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN);
        }
        else if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE &&
                 pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
        {
        ...
        if ((pISPctxt->ispCore == MMP_ISP_CORE_0 ||
             (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_FALSE)) &&
            pISPctxt->EnableWeightBalance == MMP_TRUE && !(pISPctxt->WorkFlags & ISP_FLAGS_ENABLE_BLUR))
        {
            pISPctxt->PreScaleFun.DstWidth = _MIN((pISPctxt->InInfo.PanelWidth / 4), ISP_SCALE_MAX_LINE_BUFF_LEN);
        }
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PreScaleMatrix)
    {
        ...
        {
            if (pISPctxt->WorkFlags & ISP_FLAGS_ENABLE_BLUR)
                ISP_SetIntPreScaleMatrix_Reg(pISPctxt, pISPctxt->WeightMatInt[1]);
            else
                ISP_SetIntPreScaleMatrix_Reg(pISPctxt, pISPctxt->WeightMatInt[0]);
        }
        //}
        //else
        //{
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleMatrixH)
    {
        ...
        {
            ISP_SetIntScaleMatrixH_Reg(pISPctxt, pISPctxt->WeightMatInt[0]);
        }
        //}
        //else
        //{
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleMatrixV)
    {
        ...
        {
            ISP_SetIntScaleMatrixV_Reg(pISPctxt, pISPctxt->WeightMatInt[0]);
        }
        //}
        //else
        //{
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1)
    {
        ...
        if (pISPctxt->EnPreview == MMP_TRUE)
        {
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixH)
            {
                ...
                // else
                //{
                //    _ISP_CreateWeighting(pISPctxt->ScaleFun.HCI, ISP_SCALE_TAP, ISP_SCALE_TAP_SIZE,
                //    pISPctxt->ScaleFun.WeightMatX); ISP_SetScaleMatrixH_Reg(pISPctxt, &pISPctxt->ScaleFun);
                //}
            }
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixV)
            {
                ...
                // else
                //{
                //    _ISP_CreateWeighting(pISPctxt->ScaleFun.VCI, ISP_SCALE_TAP, ISP_SCALE_TAP_SIZE,
                //    pISPctxt->ScaleFun.WeightMatY); ISP_SetScaleMatrixV_Reg(pISPctxt, &pISPctxt->ScaleFun);
                //}
            }
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutAddress ||
