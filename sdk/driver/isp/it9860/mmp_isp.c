...
static ISP_DEVICE      g_weibal_IspDev      = NULL;
static sem_t           g_weibal_ISPEvent    = {0};

//=============================================================================
//                Private Function Definition
//=============================================================================
static void weibal_isp0ISR(void *arg)
{
    sem_post(&g_weibal_ISPEvent);
    mmpIspClearInterrupt(g_weibal_IspDev);
}
...
{
    ISP_RESULT  result    = ISP_SUCCESS;
    ISP_CONTEXT *pISPctxt;

    if ((ptDev == MMP_NULL) || (*ptDev == MMP_NULL))
    {
        return result;
    }
    pISPctxt = (ISP_CONTEXT *)*ptDev;
    ...
    MMP_ISP_CORE_INFO *coreInfo)
{
    ...
    ISP_ContextInitialize(pISPctxt, coreInfo, MMP_FALSE);
...
//=============================================================================
ISP_RESULT
mmpIspSetColorCtrl(ISP_DEVICE ptDev, MMP_ISP_COLOR_CTRL colorCtrl, MMP_INT value)
{
    ISP_RESULT    result            = ISP_SUCCESS;
    ISP_CONTEXT * pISPctxt          = (ISP_CONTEXT *)ptDev;
    ...
    {
        ISP_SetColorCorrMatrix((void *)&pISPctxt->CCFun, pISPctxt->hue, pISPctxt->saturation, pISPctxt->contrast,
                               pISPctxt->midPoint, pISPctxt->colorEffect, pISPctxt->brightness, 0);
    }
    else
    {
        isp_msg_ex(ISP_MSG_TYPE_ERR, " err 0x%x !\n", result);
    ...
    const MMP_ISP_SHARE *data)
{
    ...
    if (pISPctxt->OutInfo.EnableLcdOnFly && !pISPctxt->OutInfo.EnableFieldScale &&
    ...
    if (data->EnableKeepLastField && pISPctxt->OutInfo.EnableFieldScale && (!data->isProgressive))
    {
    ...
    MMP_ISP_SUBTITLE_FORMAT format)
{
    ...
    {
        InvHCI =
            1.0 / (MMP_FLOAT)(((MMP_INT)(16384.0f * SubRefWidth / (MMP_FLOAT)pISPctxt->InInfo.SrcWidth)) / 16384.0f);
    }

    if (PreSrcHeight != pISPctxt->InInfo.SrcHeight)
    {
        InvVCI =
            1.0 / (MMP_FLOAT)(((MMP_INT)(16384.0f * SubRefHeight / (MMP_FLOAT)pISPctxt->InInfo.SrcHeight)) / 16384.0f);
    }
    ...
    MMP_ISP_CORE_INFO *coreInfo)
{
    ...
    ISP_ContextInitialize(pISPctxt, coreInfo, MMP_TRUE);
    ...
    MMP_UINT32 srcAddr)
{
    ...
    mmpIspInitialize(&g_weibal_IspDev, MMP_ISP_CORE_0);
    mmpIspSetCoreWeiBal(g_weibal_IspDev, &ISPCOREINFO);
    mmpIspSetMode(g_weibal_IspDev, MMP_ISP_MODE_TRANSFORM);
    ISPSHARE.width = ithLcdGetWidth(); //Block_W * W_num;//
    ISPSHARE.height = ithLcdGetHeight(); //Block_H * H_num;//
    ISPSHARE.pitchY = ithLcdGetPitch(); //Block_W * W_num * 2;//
    ISPSHARE.format = MMP_ISP_IN_RGB565;
    ISPSHARE.addrY = srcAddr;
    outInfo.width = ithLcdGetWidth() / 16; // 50
    outInfo.height = ithLcdGetHeight() / 4; // 120;
    outInfo.pitchRGB = ithLcdGetWidth() / 16 * 2; //50 * 2
    outInfo.format = MMP_ISP_OUT_RGB565;
    outInfo.addrRGB = dstAddr;
    mmpIspSetOutputWindow(g_weibal_IspDev, &outInfo);
    mmpIspSetVideoWindow(g_weibal_IspDev, 0, 0, outInfo.width, outInfo.height);
    mmpIspEnableCMDQ(g_weibal_IspDev);
    mmpISPRegisterIRQ(g_weibal_IspDev, weibal_isp0ISR);
    mmpIspEnableInterrupt(g_weibal_IspDev);
    sem_init(&g_weibal_ISPEvent, 0, 1);
    sem_wait(&g_weibal_ISPEvent);
    mmpIspPlayImageProcess(g_weibal_IspDev, &ISPSHARE);
    sem_wait(&g_weibal_ISPEvent);
    sem_destroy(&g_weibal_ISPEvent);
    memset(&g_weibal_ISPEvent, 0, sizeof(g_weibal_ISPEvent));
    mmpIspTerminate(&g_weibal_IspDev);
    ...
    ISP_DEVICE ptDev)
{
    ...
    pISPctxt->WorkFlags |= ISP_FLAGS_ENABLE_BLUR;
    ...
    MMP_UINT32 height)
{
    ...
    mmpIspInitialize(&g_weibal_IspDev, MMP_ISP_CORE_0);
    mmpIspSetCoreWeiBal(g_weibal_IspDev, &ISPCOREINFO);
    mmpIspSetMode(g_weibal_IspDev, MMP_ISP_MODE_TRANSFORM_BYPASS);
    ISPSHARE.width = width;
    ISPSHARE.height = height;
    ISPSHARE.pitchY = width;
	ISPSHARE.pitchUv = width;
    ISPSHARE.format = MMP_ISP_IN_YUV444;
    ISPSHARE.addrY = srcAddr;
	ISPSHARE.addrU = srcAddr + width * height;
	ISPSHARE.addrV = srcAddr + width * height * 2;
    outInfo.width = width;
    outInfo.height = height;
    outInfo.pitchY = width;
	outInfo.pitchUv = width;
    outInfo.format = MMP_ISP_OUT_YUV444;
    outInfo.addrY = dstAddr;
	outInfo.addrU = dstAddr + width * height;
	outInfo.addrV = dstAddr + width * height * 2;
    mmpIspSetOutputWindow(g_weibal_IspDev, &outInfo);
    mmpIspSetVideoWindow(g_weibal_IspDev, 0, 0, outInfo.width, outInfo.height);
    mmpIspEnableBlur(g_weibal_IspDev);
    mmpISPRegisterIRQ(g_weibal_IspDev, weibal_isp0ISR);
    mmpIspEnableInterrupt(g_weibal_IspDev);
    sem_init(&g_weibal_ISPEvent, 0, 1);
    sem_wait(&g_weibal_ISPEvent);
