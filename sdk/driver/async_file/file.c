...
#define REG_BIT_XD_CARD_DETECT          (1U << 17U)

#define REG_BIT_MMC_CARD_DETECT         (1U << 19U)
#define REG_BIT_MMC_CARD_POWER          (1U << 21U)

#define REG_BIT_MS_CARD_DETECT          (1U << 20U)
#define REG_BIT_MS_CARD_POWER           (1U << 21U)
    ...
    void)
{
    ...
    if (!mutex)
    {
        (void)printf("CREATE FILE MUTEX FAIL\r\n");
        result = 1;
        goto end;
    }
    ...
    if (!queue)
    {
        (void)printf("CREATE FILE MSGQ FAIL\r\n");
...
#else
    {
        ...
        if (MMP_SUCCESS != pthread_create(&thread,
                       ...
                       MMP_NULL))
        {
            (void)printf("CREATE FILE THREAD FAIL\r\n");
    ...
    void)
{
    ...
    if (result != 0)
    {
        (void)printf("WRITE EXIT MESSAGE FAIL\r\n");
        goto end;
    }
    ...
    if (result != 0)
    {
        (void)printf("DESTROY FILE MSGQ FAIL\r\n");
        goto end;
    }
    ...
    if (result != 0)
    {
        (void)printf("DESTROY FILE MUTEX FAIL\r\n");
        goto end;
    }
    ...
    void                * callback_arg)
{
    ...
    if (result != 0)
    {
        (void)printf("PalFileOpenWriteClose write fail !!\n");
    }
    return Wsize;
}
    ...
    const MMP_CHAR * filename)
{
    ...
    {
        MMP_INT32   dwFileERRCode   = 0;
        MMP_UINT32  dwFileSize      = 0U;
        FILE        * file;
