    ...
    const MMP_CHAR *string2)
{
    ...
    /* 1. Handle NULL pointers by setting result (no early return) */
    if (string1 == NULL || string2 == NULL)
    {
        ...
        {
            result = 1; // only string2 is NULL
        }
        else
        {
            result = -1; // only string1 is NULL
        }
    }
    else
    {
        /* 2. Default path: perform character comparison loop */
        for (;;)
        {
            /* 2.1 Convert next character to lowercase */
            f = _f_tolower((unsigned char)*string1);
            l = _f_tolower((unsigned char)*string2);

            /* 2.2 If characters differ, store difference in result and break */
            if (f != l)
            {
                result = f - l;
                break;
            ...
            /* 2.3 If both reached end of string, result stays 0 and break */
            ...
            /* 2.4 Characters equal and not at end, advance to next */
            string1++;
            string2++;
        }
    }
    ...
    const MMP_CHAR *string)
{
    ...
    memory = (MMP_CHAR *)calloc(size, sizeof(MMP_CHAR));
    if (NULL != memory)
    {
    ...
    const MMP_WCHAR *string)
{
    ...
    memory = (MMP_WCHAR *)calloc(size, sizeof(MMP_WCHAR));
    if (NULL != memory)
    {
