...
void spiSetClockOutBypass (SPI_HW_PORT port, SPI_OUT_CLK_BYPASS bypass)
{
    ...
    switch (bypass)
    {
        case SPI_OUT_CLK_BYPASS_ENABLE:
            ithWriteRegMaskA(port + REG_SPI_MISC, REG_CLKOUT_BYPASS_ENABLE << REG_CLKOUT_BYPASS_OFFSET,
                             REG_CLKOUT_BYPASS_MASK);
            break;
        case SPI_OUT_CLK_BYPASS_DISABLE:
            ithWriteRegMaskA(port + REG_SPI_MISC, REG_CLKOUT_BYPASS_DISABLE << REG_CLKOUT_BYPASS_OFFSET,
                             REG_CLKOUT_BYPASS_MASK);
            break;
        default:
...
void spiTransmitDataOutput (SPI_HW_PORT port, bool enable)
{
    ...
    ithWriteRegMaskA(port + REG_SPI_CONTROL_2, (enable == true) ? REG_CR2_TXDOE_ENABLE : REG_CR2_TXDOE_DISABLE,
                     REG_CR2_TXDOE_MASK);
}
...
void spiSetGarbageFilterLength (SPI_HW_PORT port, uint32_t filterLength)
{
    ...
    ithWriteRegMaskA(port + REG_SPI_FREERUN, filterLength << REG_TXFR_FRUN_FILTER_LENG_OFFSET,
                     REG_TXFR_FRUN_FILTER_LENG_MASK);
}
