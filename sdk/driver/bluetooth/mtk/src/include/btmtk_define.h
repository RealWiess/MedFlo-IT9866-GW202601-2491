...
#define BTMTK_LOG_LVL_DEF	BTMTK_LOG_LVL_ERR /* default setting */
...
#define BTMTK_INFO_RAW(p, l, fmt, ...)						\
	do {	\
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_INFO) {	\
			...
			PRINTF_INFO("[btmtk_info] "fmt, ##__VA_ARGS__); \
			for (cnt_ = 0; cnt_ < len_; ++cnt_)	\
				PRINTF_INFO("0x%02X ",ptr[cnt_]); \
			PRINTF_INFO("\n");  \
		}	\
	} while (0)
