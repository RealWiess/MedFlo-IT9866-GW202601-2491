...
#define BT_MCU_MINIMUM_INTERFACE_NUM	1
...
*/
	EventGroupHandle_t compare_event;
...
typedef int (*cif_open_ptr)(struct btmtk_dev *hdev);
typedef int (*cif_close_ptr)(struct btmtk_dev *hdev);
...
typedef int (*cif_send_cmd_ptr)(struct btmtk_dev *bdev, sk_buff *skb,
...
typedef int (*cif_send_and_recv_ptr)(struct btmtk_dev *bdev,
		...
		int delay, int retry, int pkt_type, bool flag);
typedef int (*cif_event_filter_ptr)(struct btmtk_dev *bdev, sk_buff *skb);
...
typedef ssize_t (*cif_log_read_to_user_ptr)(char *buf, size_t count);
...
struct hif_hook_ptr {
	...
	cif_mutex_lock_ptr		handle_mutex_lock;
	cif_mutex_unlock_ptr		handle_mutex_unlock;
	cif_flush_ptr				flush;
...
struct h4_recv_pkt {
	...
	int (*recv)(struct btmtk_dev *bdev, sk_buff *skb);
};
...
//static inline sk_buff *mtk_add_stp(struct btmtk_dev *bdev, sk_buff *skb);
...
int btmtk_recv_event(struct hci_dev *hdev, sk_buff *skb);
int btmtk_recv_acl(struct hci_dev *hdev, sk_buff *skb);
int btmtk_recv_iso(struct hci_dev *hdev, sk_buff *skb);
...
void btmtk_save_filter_vendor_cmd(sk_buff *skb,
