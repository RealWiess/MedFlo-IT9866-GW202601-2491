...
// static const uint32_t shareGpioPin[3]={ CFG_GPIO_SHARE_SPI_FOR_NOR,
//                                          CFG_GPIO_SHARE_SPI_FOR_SLAVE,
//                                          CFG_GPIO_SHARE_SPI_FOR_NAND};
// #else

#ifdef CFG_NOR_SHARE_SPI_NAND
static const uint32_t shareGpioPin[3] = {19U, 0U, 24U};
#else
static const uint32_t shareGpioPin[3] = {50U, 51U, 0U};
#endif
...
static int SpiRead (int file, char * ptr, int len, void * info)
{
    ...
    if (NULL != spiObj)
    {
        ...
        for (readIndex = 0; readIndex < len; readIndex++)
        {
            ...
            if (spiInfo && spiInfo->cmdBuffer && spiInfo->dataBuffer)
            {
                ...
                switch (spiInfo->readWriteFunc)
                {
                    case ITP_SPI_PIO_READ:
                        spiResult = mmpSpiPioRead(spiObj->port, spiInfo->cmdBuffer, spiInfo->cmdBufferSize,
                                                  spiInfo->dataBuffer, spiInfo->dataBufferSize, spiInfo->dataLength);
                        break;

                    case ITP_SPI_DMA_READ:
                        spiResult = mmpSpiDmaRead(spiObj->port, spiInfo->cmdBuffer, spiInfo->cmdBufferSize,
                                                  spiInfo->dataBuffer, spiInfo->dataBufferSize, spiInfo->dataLength);
                        break;
            ...
            if (spiInfo && spiInfo->cmdBuffer && spiInfo->dataBuffer)
            {
                ...
                switch (spiInfo->readWriteFunc)
                {
                    case ITP_SPI_DMA_WRITE:
                        spiResult = mmpSpiDmaWrite(spiObj->port, spiInfo->cmdBuffer, spiInfo->cmdBufferSize,
                                                   spiInfo->dataBuffer, spiInfo->dataBufferSize, spiInfo->dataLength);
                        break;

                    case ITP_SPI_PIO_WRITE:
                        spiResult = mmpSpiPioWrite(spiObj->port, spiInfo->cmdBuffer, spiInfo->cmdBufferSize,
                                                   spiInfo->dataBuffer, spiInfo->dataBufferSize, spiInfo->dataLength);
                        break;
                ...
                {
                    (void)printf("itp spi write fail\n");
                }
            }
        }
