...
static ITPKeypadEvent kpLastEvent2;
#endif
...
#ifdef CFG_KEYPAD_MULTIPLE
static void Keypad2Probe(void)
{
    ITPKeypadEvent ev;

    ev.code = itpKeypad2Probe();
    gettimeofday(&ev.time, NULL);

    if (ev.code == -1)
    {
        if (kpLastEvent2.code == -1)
        {
            return;
        }
        else
        {
            ev.code = kpLastEvent2.code;
            ev.flags = ITP_KEYPAD_UP;
            kpLastEvent2.code = -1;
            kpLastEvent2.time = ev.time;
        }
    }
    else
    {
        long msec = itpTimevalDiff(&kpLastEvent2.time, &ev.time);

        if ((kpLastEvent2.code == ev.code) && (kpLastEvent2.flags & ITP_KEYPAD_DOWN))
        {
            if (msec < CFG_KEYPAD_PRESS_INTERVAL)
                return;
        ...
        {
            ev.flags = ITP_KEYPAD_DOWN;
            kpLastEvent2.code = ev.code;
        }
    }
    kpLastEvent2.flags = ev.flags;
    kpLastEvent2.time = ev.time;
