...
vcd_dump(void)
{
    ...
    if (!g_fname)
    {
        (void)printf("[RTOS][DUMP] No dump name is specified!!\n");
        goto end;
    }

    ITP_LOG_DBG("dump to %s\n", g_fname);
    if ((fp = fopen(g_fname, "wb")) == NULL)
    {
        (void)printf("[RTOS][DUMP] Can not create dump file!!\n");
        goto end;
    }

    if (!g_buf_idx && !g_buf_full)
    {
        (void)printf("[RTOS][DUMP] No data to dump!!\n");
        goto end;
    }

    // vTaskSuspendAll();
    (void)printf("[RTOS][DUMP] Starting dump....\n");

    if (!(buf = malloc(BUFSIZE + 80)))
    {
        (void)printf("[RTOS][DUMP] Out of memory!!\n");
        goto end;
    }
    ...
    if ((g_max_id + 1) + (g_max_tag + 1 ) >= MAX_SIG_RANGE)
    {
        ...
        (void)printf("[RTOS][DUMP] Out of signal to dump!!\n");
    }
    ...
    while (n++ < event_size)
    {
        ...
        {
            (void)printf("[RTOS][DUMP] unknown task stat 0x%08x.\n", g_buf_ptr[i]);
        }
        else
        {
            ...
            if (total_time < t)
            {
                (void)printf("[RTOS][DUMP] Error: time stamp overflow!!\n");
                return;
            }
            t = total_time;
        }
    ...
    (void)printf("[RTOS][DUMP] Totally %u us is recorded.\n", total_time);
...
next:
    ...
    // xTaskResumeAll();
    (void)printf("[RTOS][DUMP] Dump complete %d event....\n", event_size);
    ...
    #else
    (void)printf("[RTOS][DUMP] Warnning: do not support vcd dump to file.\n");
    ...
    int task_id)
{
    ...
    if (g_max_id >= MAX_TASK_RECORD)
    {
        ...
        (void)printf("[RTOS][DUMP] Out of task to record\n");
    }
    ...
    int         count)
{
    ...
    {
        (void)printf("[RTOS][DUMP] Can not creat trace buffer\n");
    }
    else
    {
        (void)printf("[RTOS][DUMP] Open VCD %s file.\n", filename);
    }
    ...
    int tag)
{
    ...
    if (id > 15)
    {
        (void)printf("[RTOS][DUMP] Out of tag id number.\n");
        goto end;
    }

    if (tag & 0xff000000)
    {
        (void)printf("[RTOS][DUMP] tag value can not exceed 24-bits.\n");
    }
