/** @file
 * PAL ALT_CPU functions.
 *
 * @author Steven Hsiao
 * @version 1.0
 * @date 2017/10/16
 * @copyright ITE Tech. Inc. All Rights Reserved.
 */
#include <errno.h>
#include <sys/socket.h>
#include "openrtos/FreeRTOS.h"
#include "alt_cpu/alt_cpu_device.h"


#ifdef CFG_RSL_MASTER
extern ITPDevice itpDeviceRslMaster;
#endif
#ifdef CFG_RSL_SLAVE
extern ITPDevice itpDeviceRslSlave;
#endif
#ifdef CFG_SW_PWM
extern ITPDevice itpDeviceSwPwm;
#endif
#ifdef CFG_PATTERN_GEN
extern ITPDevice itpDevicePatternGen;
#endif
#ifdef CFG_SW_UART
extern ITPDevice itpDeviceSwUart;
#endif
#ifdef CFG_SW_SERIAL_PORT
extern ITPDevice itpDeviceSwSerialPort;
#endif

#ifdef CFG_ALT_CPU_CUSTOM_DEVICE
extern ITPDevice itpDeviceAltCpuCustom;
#endif

#ifdef CFG_OLED_CTRL
extern ITPDevice itpDeviceOledCtrl;
#endif

#ifdef CFG_FREQ_DETECT
extern ITPDevice itpDeviceFreqDetect;
#endif

#ifdef CFG_WATCHDOG_MONITOR
extern ITPDevice itpDeviceWatchdogMonitor;
#endif

#ifdef CFG_SW_UART_LIN
extern ITPDevice itpDeviceSwUartLIN;
#endif


typedef struct
{
    int         engineMode;
    ITPDevice*  ptDevice;
} ALT_CPU_ENGINE;

static pthread_mutex_t gAltCpuMutex  = PTHREAD_MUTEX_INITIALIZER;
static ALT_CPU_ENGINE gtCurDevice = { 0 };

static ALT_CPU_ENGINE gptAltCpuEngineArray[] =
{
#ifdef CFG_RSL_MASTER
    {ALT_CPU_RSL_MASTER, &itpDeviceRslMaster},
#endif
#ifdef CFG_RSL_SLAVE
    {ALT_CPU_RSL_SLAVE, &itpDeviceRslSlave},
#endif
#ifdef CFG_SW_PWM
    {ALT_CPU_SW_PWM, &itpDeviceSwPwm},
#endif
#ifdef CFG_PATTERN_GEN
    {ALT_CPU_PATTERN_GEN, &itpDevicePatternGen},
#endif
#ifdef CFG_SW_UART
    {ALT_CPU_SW_UART, &itpDeviceSwUart},
#endif
#ifdef CFG_SW_SERIAL_PORT
    {ALT_CPU_SW_SERIAL_PORT, &itpDeviceSwSerialPort},
#endif
#ifdef CFG_ALT_CPU_CUSTOM_DEVICE
    {ALT_CPU_CUSTOM_DEVICE, &itpDeviceAltCpuCustom},
#endif
#ifdef CFG_OLED_CTRL
    {ALT_CPU_OLED_CTRL, &itpDeviceOledCtrl},
#endif
#ifdef CFG_FREQ_DETECT
    {ALT_CPU_FREQ_DETECT, &itpDeviceFreqDetect},
#endif
#ifdef CFG_WATCHDOG_MONITOR
    {ALT_CPU_WATCHDOG_MONITOR, &itpDeviceWatchdogMonitor},
#endif
#ifdef CFG_SW_UART_LIN
    {ALT_CPU_SW_UART_LIN, &itpDeviceSwUartLIN},
#endif
};

static int altCpuIoctl(int file, unsigned long request, void *ptr, void *info)
{
    uint32_t i = 0;
    int newEngineMode = 0;
    ITPDevice *pNewDevice = NULL;
    int ret = 0;
    
    (void)pthread_mutex_lock(&gAltCpuMutex);
    switch (request)
    {
    	case ITP_IOCTL_ALT_CPU_SWITCH_ENG:
    	    //Engine is running, therefore, need to reload new engine image.
    	    newEngineMode = *(int*)ptr;
            (void)printf("newEngine Mode: %d\n", newEngineMode);
            if ((gtCurDevice.engineMode == ALT_CPU_UNKNOWN_DEVICE) || (gtCurDevice.engineMode != newEngineMode))
            {
                (void)printf("device mode: %d\n", newEngineMode);
                (void)usleep(100*1000);
                iteRiscResetCpu(ALT_CPU);
                for (i = 0; i < (sizeof(gptAltCpuEngineArray) / sizeof(ALT_CPU_ENGINE)); i++)
                {
                    if (newEngineMode == gptAltCpuEngineArray[i].engineMode)
                    {
                        pNewDevice = gptAltCpuEngineArray[i].ptDevice;
                        gtCurDevice.ptDevice = pNewDevice;
                        break;
                    }
                }

                if (i == (sizeof(gptAltCpuEngineArray) / sizeof(ALT_CPU_ENGINE)))
                {
                    (void)printf("itp_alt_cpu.c(%d), requested ALT CPU engine is not exited\n", __LINE__);
                }
            }
            (void)printf("device change is done....\n");
            (void)usleep(100*1000);
    		break;
        default:
            if (gtCurDevice.ptDevice != NULL)
            {
                ret = gtCurDevice.ptDevice->ioctl(file, request, ptr, info);
            }
            break;
    }    
    (void)pthread_mutex_unlock(&gAltCpuMutex);
    return ret;
}

static int altCpuRead(int file, char *ptr, int len, void* info)
{
    int ret = 0;
    (void)pthread_mutex_lock(&gAltCpuMutex);
    if (gtCurDevice.ptDevice != NULL)
    {
        ret =  gtCurDevice.ptDevice->read(file, ptr, len, info);
    }
    (void)pthread_mutex_unlock(&gAltCpuMutex);
    return ret;
}

static int altCpuWrite(int file, char *ptr, int len, void* info)
{
    int ret = 0;
    (void)pthread_mutex_lock(&gAltCpuMutex);
    if (gtCurDevice.ptDevice != NULL)
    {
        ret =  gtCurDevice.ptDevice->write(file, ptr, len, info);
    }
    (void)pthread_mutex_unlock(&gAltCpuMutex);
    return ret;
}

static int altCpuSeek(int file, int ptr, int dir, void *info)
{
    int ret = 0;
    (void)pthread_mutex_lock(&gAltCpuMutex);
    if (gtCurDevice.ptDevice != NULL)
    {
        ret =  gtCurDevice.ptDevice->lseek(file, ptr, dir, info);
    }
    (void)pthread_mutex_unlock(&gAltCpuMutex);
    return ret;
}

const ITPDevice itpDeviceAltCpu =
{
    ":altcpu",
    itpOpenDefault,
    itpCloseDefault,
    altCpuRead,
    altCpuWrite,
    altCpuSeek,
    altCpuIoctl,
    NULL
};
