 ...
 ********************************************/
    ...
    #define CFG_EXTRTC_USE_TIMER
        ...
        #define RTC_TIMER       ITH_TIMER9      // CFG_RTC_TIMER
        #define RTC_TIMER_INT   ITH_INTR_TIMER9 // CFG_RTC_INT_TIMER
    #endif
...
_timerInit (void);
static long
_timerGetTime (long * us);
static void
_timerSetTime (long                         sec,
                                    long    us);
...
#ifdef   CFG_EXTRTC_USE_INTERNAL_RTC
static void
_internalRtcInit (void);
static long
_internalRtcGetTime (long * usec);
static void
_internalRtcSetTime (long                       sec,
                                          long  usec);
    ...
    void)
{
    ...
    while (1)
    {
        ...
        if (cnt > 2000)
        {
            ITP_LOG_INFO("cnt>2000, counterA not work, c=%d\n", cnt);
            break;
        }
        ITP_LOG_DBG("adj:3, %d\n", sec2);
        ...
        ITP_LOG_DBG("adj:4, %06d,%06d,(%d)\n", us1, us2, cnt);
        if (cnt > 1000000)
        {
            ITP_LOG_INFO("cnt>1000000, counter not work, c=%d\n", cnt);
            break;
        }
        ...
        if ((us1 % 1000) > 500)
        {
            adjRatio++;                    // aound up if >500us
        }
        ITP_LOG_DBG("adjFcr1:%d(%d)\n", adjRatio, us1);
        ...
        if (adjCnt++ > 10)
        {
            ITP_LOG_INFO("adjust times has over %d\n", adjCnt);
            break;
        }
    }
    ...
    if ((adjRatio < 950) || (adjRatio > 1050))
    {
        ITP_LOG_DBG("adjust us ratio from %d to 1000\n", adjRatio);
        adjRatio = 1000;
    }

    ITP_LOG_INFO("internal Rtc micro-second adjust ratio:%d(%d)\n", adjRatio, us1);
    return adjRatio;
}
    #endif
    ...
    void)
{
    ...
    {
        ITP_LOG_DBG("iRTC GOT BUS clock: %d\n", ithGetBusClock());
