...
static int UsbInit(void)
{
    ...
    if (res)
        ITP_LOG_ERR(" iteUasDriverRegister() res 0x%08X \n", res );
#endif

#if defined(CFG_MSC_ENABLE)
    res = iteMscDriverRegister();
    if (res)
        ITP_LOG_ERR(" iteMscDriverRegister() res 0x%08X \n", res );
#endif

#if defined(CFG_USB_HID)
    extern int iteUsbHidRegister(void);
    res = iteUsbHidRegister();
    if (res)
        ITP_LOG_ERR(" iteUsbHidRegister() res 0x%08X \n", res );
#else

    #if defined(CFG_USB_MOUSE)
    res = iteUsbMouseRegister();
    if (res)
        ITP_LOG_ERR(" iteUsbMouseRegister() res 0x%08X \n", res );
    #endif

    #if defined(CFG_USB_KBD)
    res = iteUsbKbdRegister();
    if (res)
        ITP_LOG_ERR(" iteUsbKbdRegister() res 0x%08X \n", res );
    #endif
#endif
    ...
    extern int iteUvcDriverRegister(void);
    res = iteUvcDriverRegister();
    if (res)
        ITP_LOG_ERR(" iteUvcDriverRegister() res 0x%08X \n", res );
#endif
    ...
    if (res)
        ITP_LOG_ERR(" iteUsbSerialRegister() res 0x%08X \n", res );

    res = iteUsbOptionRegister();
    if (res)
        ITP_LOG_ERR(" iteUsbOptionRegister() res 0x%08X \n", res );
#endif
    ...
    if (res) {
      ITP_LOG_ERR(" usblp_register_driver()=0x%08X\n", res );
    }
#endif
    ...
    /* create usb host tasklet thread */
    {
        ...
        if (res)
            ITP_LOG_ERR(" create usb host thread fail! 0x%08X \n", res );
    }
    ...
    /* detect usb device insert into our usb host or not */
    {
        ...
        if (res)
            ITP_LOG_ERR(" create usb host detect thread fail! 0x%08X \n", res );
    }
