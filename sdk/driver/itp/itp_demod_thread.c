...
// =============================================================================
#if 0
    #define _trace_enter()  (void)printf("enter: %s()[#%d]\n", __FUNCTION__, __LINE__)
    #define _trace_leave()  (void)printf("leave: %s()[#%d]\n", __FUNCTION__, __LINE__)
#else
    #define _trace_enter()
    #define _trace_leave()
#endif
    ...
    void * args)
{
    ...
    {
        (void)printf(" tsi(%d-th) L2 buf alloc fail !! %s", tsi_L2_idx, __FUNCTION__);
    }
    ...
    while (_TsiThreadGetThreadStatus(tsi_L2_idx))
    {
        ...
        if (elapsedTime > TSI_TIMER_GAP)
        {
            ...
            while (_TsiThreadGetCacheStatus(tsi_L2_idx))
            {
                ...
                if (timeOut > 50)
                {
                    (void)printf(" %d-th tsi get sample timeout at %s()\n", tsi_L2_idx, __FUNCTION__);
                    continue;
                }
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (elapsedTime > TSI_READ_BUF_TIMEOUT)
            {
                ...
                (void)printf("\t\tvalidSize = %d, L2 read time out (%f sec)\n", validSize, TSI_READ_BUF_TIMEOUT / 1000.0);
                ret = 1;
                break;
            }
            // usleep(1000);
