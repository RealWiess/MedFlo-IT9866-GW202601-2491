...
int close(int file)
{
    ...
    if ((((uint32_t)file) & ITP_DEVICE_MASK) == 0)
        dev = itpDeviceTable[ITP_DEVICE_SOCKET >> ITP_DEVICE_BIT];
    else
        dev = itpDeviceTable[(((uint32_t)file) & ITP_DEVICE_MASK) >> ITP_DEVICE_BIT];
...
{
    const ITPDevice *dev = itpDeviceTable[(((uint32_t)file) & ITP_DEVICE_MASK) >> ITP_DEVICE_BIT];
    if (dev)
    {
        /*  Pass call on to device driver.  Return result.        */
...
{
    int               file = (int)fp;
    const ITPFSDevice *dev = (ITPFSDevice *)itpDeviceTable[(((uint32_t)file) & ITP_DEVICE_MASK) >> ITP_DEVICE_BIT];
