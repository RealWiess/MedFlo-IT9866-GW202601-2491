    ...
    if(!res)
    {
        if(USB_DEVICE_CONNECT(usb_state))
        {
            ...
            for (i = 0; i < USB_MAX_INTERFACE; i++)
            {
                ...
                if (USB_DEVICE_MOUSE(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb mouse is interted!! \n", itpusb->index );
                ...
                if (USB_DEVICE_KBD(itpusb->intf[i].type))
                    ITP_LOG_INFO(" USB%d: usb keyboard is interted!! \n", itpusb->index );
                #endif

                #if defined(CFG_USB_OPTION)
                if (USB_DEVICE_OPTION(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb OPTION is interted!! \n", itpusb->index );
                ...
                if (USB_DEVICE_ECM(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb ECM is interted!! \n", itpusb->index );
                ...
                if (USB_DEVICE_ECM(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb ECM_ex is interted!! \n", itpusb->index );
        ...
        if(USB_DEVICE_DISCONNECT(usb_state))
        {
            for (i = 0; i < USB_MAX_INTERFACE; i++)
            {
                ...
                #if defined(CFG_USB_MOUSE)
                if (USB_DEVICE_MOUSE(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb mouse is disconnected!! \n", itpusb->index );
                ...
                #if defined(CFG_USB_KBD)
                if (USB_DEVICE_KBD(itpusb->intf[i].type))
                    ITP_LOG_INFO(" USB%d: usb keyboard is disconnected!! \n", itpusb->index );
                #endif

                #if defined(CFG_USB_OPTION)
                if (USB_DEVICE_OPTION(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb OPTION is disconnected!! \n", itpusb->index );
                ...
                #if defined(CFG_USB_ECM)
                if (USB_DEVICE_ECM(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb ECM is disconnected!! \n", itpusb->index );
                ...
                #if defined(CFG_USB_ECM_EX)
                if (USB_DEVICE_ECM(itpusb->intf[i].type)) {
                    ITP_LOG_INFO(" USB%d: usb ECM_ex is disconnected!! \n", itpusb->index );
...
static void* UsbHostDetectHandler(void* arg)
{
    while(1)
    {
        ...
        if(_CheckUsbState(&itpUsb[USB0]))
            ITP_LOG_ERR(" itp usb0 check state fail! \n" );
    #endif

    #if defined(CFG_USB1_ENABLE)
        if(_CheckUsbState(&itpUsb[USB1]))
            ITP_LOG_ERR(" itp usb1 check state fail! \n" );
    #endif
