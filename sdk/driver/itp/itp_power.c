...
#define GET_GPIO_MASK(g_pin)   ((g_pin >= 0 && g_pin <= 127) ? (1U << (g_pin % 32)) : 0)
    ...
    void)
{
    ...
    // RTC Source = 32k, check if first booting
    if (((value & ITH_RTC_DIV_EN_MASK) == 0) || ((value & ITH_RTC_DIV_CYCLE_MASK) != 0x8000))
    {
        // Reset RTC
        ithRtcCtrlEnable(ITH_RTC_RESET);
        ithRtcCtrlDisable(ITH_RTC_RESET);

        ithWriteRegA(ITH_RTC_BASE + ITH_RTC_DIV_REG, ITH_RTC_DIV_EN_MASK | ITH_RTC_DIV_SRC_MASK | 0x8000);
        ...
        // Set RTC Power mode
        ithRtcCtrlEnable(ITH_RTC_PWREN_MODE);
        ithRtcCtrlEnable(ITH_RTC_EXTPWRUP_EN);
        // 0:level-triggered (H) 1:level-triggered (L)2:rising edge-triggered 3:falling edge-triggered
        ithRtcSetExtPowerUpMode(0x3);

        // RTC power off
        (void)printf("RTC power ready, system power down\n");
        ithRtcCtrlEnable(ITH_RTC_SW_POWEROFF);
        for (;;)
        {
        }
...
/*#ifdef CFG_JPEG_ENABLE
    ...
    (void)usleep(50000);                  // to avoid white flicker
    ...
    void)
{
    ...
    ITP_LOG_DBG("orgional cpu/bus clock ratio=%d/%d, change to %d/%d\n",
    powerCpuRatio, powerBusRatio, CFG_POWER_STANDBY_CPU_FREQ_RATIO, CFG_POWER_STANDBY_BUS_FREQ_RATIO);
...
SleepFunc (
    register uint32_t   * inParam,
    register uint32_t   * outParam,
    register uint32_t   * var)
{
        #if defined(CFG_POWER_WAKEUP_UART)
    ...
    for (;;)
    {
        ...
        if (RET & GET_GPIO_MASK(TP_GPIO_PIN))
            #else
        if (!(RET & GET_GPIO_MASK(TP_GPIO_PIN)))
            #endif
        {
    ...
    for (;;)
    {
        ...
        if (RET & GET_GPIO_MASK(WAKEUP_GPIO))
            #else
        if (!(RET & GET_GPIO_MASK(WAKEUP_GPIO)))
            #endif
        {
...
PowerSleepForDoubleClick (
    void)
{
    (void)printf("PowerSleepContinue\n");
    ...
    void)
{
    ...
    #if defined(CFG_NET_WIFI_SDIO_VND_MHD) || defined(CFG_NET_WIFI_SDIO_NGPL_ATBM6031)
    // wowl
...
//    ithCmdQWaitEmpty(   ITH_CMDQ1_OFFSET);
    ...
    #ifdef CFG_PMIC_WD_ENABLE
    ioctl(ITP_DEVICE_PMIC, ITP_IOCTL_DISABLE, NULL);
    #endif
...
//    }
    ...
    // wowl
            #if defined(CFG_NET_WIFI_SDIO_MHD_WOWL)
    ithGpioClear(CFG_POWER_WAKEUP_GPIO_PIN);
    ithGpioSetOut(CFG_POWER_WAKEUP_GPIO_PIN);
...
void
EmptyIrq (void) __attribute__((naked)) __attribute__((__optimize__ ("-fno-stack-protector")));
    ...
    void)
{
    ...
    {
        // must small than SRAM size and 32-bit align
        ITP_LOG_DBG("power suspend function size: %d\n", suspendFuncSize);
