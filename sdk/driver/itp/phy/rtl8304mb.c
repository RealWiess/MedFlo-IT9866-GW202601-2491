...
static rtk_port_t rtl8304mb_port[4] = { RTL8304MB_PORT0, RTL8304MB_PORT1, RTL8304MB_PORT2, RTL8304MB_PORT3 };
...
void PhyInit(int ethMode)
{
    ...
    ret = rtk_cpu_enable_set(ENABLED);
    if (ret)
    {
        printf("[ERR] rtk_cpu_enable_set() ret=0x%X \n", ret);
        goto end;
    }
    ret = rtk_cpu_tagPort_set(RTL8304MB_PORT3, DISABLED);
    if (ret)
    {
    ...
    ret = rtk_cpu_tagPort_get(&port, &enTag);
    if (ret);
    {
...
int rtl8304mb_mib_dump(int port)
{
    ...
    printf("port %d(%" PRIu32 "): \n", port, (uint32_t)rtl8304mb_port[port]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_TXBYTECNT, cntValue);
    if (ret) goto end;
    printf("MIB_TXBYTECNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXBYTECNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXBYTECNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_TXPKTCNT, cntValue);
    if (ret) goto end;
    printf("MIB_TXPKTCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXPKTCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXPKTCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXDPCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXDPCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXCRCCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXCRCCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXFRAGCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXFRAGCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_TXBRDCNT, cntValue);
    if (ret) goto end;
    printf("MIB_TXBRDCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXBRDCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXBRDCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_TXMULTCNT, cntValue);
    if (ret) goto end;
    printf("MIB_TXMULTCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXMULTCNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXMULTCNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_TXUNICNT, cntValue);
    if (ret) goto end;
    printf("MIB_TXUNICNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXUNICNT, cntValue);
    if (ret) goto end;
    printf("MIB_RXUNICNT %X %X \n", cntValue[1], cntValue[0]);
    ret = rtk_mib_get(rtl8304mb_port[port], MIB_RXSYMBLCNT, cntValue);
    if (ret) goto end;
