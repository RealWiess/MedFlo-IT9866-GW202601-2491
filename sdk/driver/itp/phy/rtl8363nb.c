...
void PhyInit(int ethMode)
{
    ...
    ret = rtk_cpu_tagPort_set(EXT_PORT0, CPU_INSERT_TO_NONE);
    if (ret) {
    ...
    ret = rtk_port_macForceLinkExt_set(EXT_PORT0, MODE_EXT_RMII_PHY, &mac_cfg);
    if (ret) {
...
void rtl8363nb_mib_dump(int port)
{
    ...
    printf(" etherStatsOctets: %" PRId64 " \n", stats.etherStatsOctets);
