...
static int StcOpen(const char* name, int flags, int mode, void* info)
{
	...
	ithTimerReset(ITH_TIMER2);//reset timer
    ithTimerCtrlEnable(ITH_TIMER2, ITH_TIMER_UPCOUNT);//up count the counter
    ithTimerIsrEnable(ITH_TIMER2);
    ithTimerSetCounter(ITH_TIMER2, 0x0);//init counter to 0
    ithTimerSetLoad(ITH_TIMER2,0x0);//set reload
    ithTimerEnable(ITH_TIMER2);//enabel time
