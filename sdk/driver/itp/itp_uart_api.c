    ...
    int Baudrate)
{
    ...
    if (ComNum == 0)
    {
        (void)printf("ComPort should start from 1.\n");
        return -1;
    }
    ...
    (void)printf("ComInit start, ComName: %s\n", ComName);
    ...
    *hCom = CreateFileA(ComName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (!SetupComm(*hCom, UART_BUFFER_SIZE, UART_BUFFER_SIZE))
    {
        ...
        (void)printf("Cannot set in/out buffer size\n");
        CloseHandle(*hCom);
        return -1;
    }
    ...
    {
        (void)printf("Cannot set timeout\n");
        CloseHandle(*hCom);
        return -1;
    }
    ...
    (void)printf("ComInit OK, ComNum=%d Baudrate=%d\n", ComNum, Baudrate);
    ...
    int     len)
{
    ...
    {
        (void)printf("Cannot find ComPort\n");
        return 0;
    }
