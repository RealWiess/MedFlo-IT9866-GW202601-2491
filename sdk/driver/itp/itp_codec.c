...
static int CodecRead(int file, char *ptr, int len, void* info)
{
    ...
    strlcpy(filepath, CODEC_PATH, sizeof(filepath));
    strlcat(filepath, &codecTable[file][0], sizeof(filepath));

    strlcat(filepath, ".codecs", sizeof(filepath));
    f = fopen(filepath, "rb");
    if (!f)
    {
        errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
        ITP_LOG_ERR("cannot find codec %s\n", filepath );
        return 0;
    }
    ...
    {
        errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
        ITP_LOG_ERR("cannot seek codec %s\n", filepath );
        goto end;
    }
    ...
    {
        errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
        ITP_LOG_ERR("cannot read codec %s\n", filepath );
        goto end;
    }
    ...
    {
        errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
        ITP_LOG_ERR("decompress size mismatch: %" PRIu32 " != %d\n", imagesize, len );
        goto end;
    }
    ...
    if (strncmp(buf2, "SMAZ", 4) == 0)
    {
        ...
        {
            errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
            ITP_LOG_ERR("out of memory: %" PRIu32 "\n", imagesize );
            goto end;
        }
        ...
        {
            errno = (ITP_DEVICE_CODEC << ITP_DEVICE_ERRNO_BIT) | __LINE__;
            ITP_LOG_ERR("cannot read codec %s %" PRIu32 " %d\n", filepath,contentsize,nResult );
            goto end;
        }
