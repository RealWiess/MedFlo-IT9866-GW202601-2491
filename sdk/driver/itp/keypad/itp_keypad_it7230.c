...
#ifdef LED_FUNCTION
void
IT7230_LEDStatus (WORD wLedStatus);
#endif

// void IT7230_CheckPage(BYTE page);
WORD
IT7230_CapS_Read_Reg (BYTE                  page,
                                     BYTE   addr_byte);
void
IT7230_CapS_Write_Reg (BYTE                 page,
                                      BYTE  addr_byte,
                                      WORD  data_word);
void
IT7230_Init_CapS_Table (void);
WORD
IT7230_GetKeyStatus (void);
void
IT7230_Restort_Reg (void);
WORD
IT7230_Check_ESD (void);
BYTE
IT7230_Init_Status (void);
WORD
IT7230_GetLowContactStatus (void);
WORD
IT7230_LowContactCalibration (void);
    ...
    void)
{
    ...
    (void)printf("[Keypad error] IT7235BR hangup, and reset keypad device.\n");
#endif
...
#ifdef  ENABLE_KEYPAD_DBG_MODE
    (void)printf(" end of keypad reset flow\n");
#endif
}
...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, IT7230_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}
    ...
    uint32_t    cnt)
{
    ...
    if (cnt > 2)
    {
        ...
        (void)printf("incorrect value:: cnt(%d) > 2\n", cnt);
    }
    else
    {
        cBuf[0] = cmd;
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, IT7230_IIC_ADDR, cmd, buf, cnt);
#endif
}
    ...
    BYTE page)
{
...
#ifdef  ENABLE_KEYPAD_DBG_MODE
    (void)printf("_CheckPage:page=%x,gPage=%x\n", page, gRegPage);
#endif
    ...
    if (page != gRegPage)
    {
        for (i = 0; i < Retry; i++)
        {
            ...
            {
                // (void)printf("   ##1 disable all intr(%x) ##\n",page);
            ...
            if (_i2c_write(CAPS_PSR, wPage, 2) == 0)
            {
                ...
                if ((IT7230_Init_Status() == FINISH_RESTORT) || (IT7230_Init_Status() == PRE_RESTORT))
                {
                    ...
                    // (void)printf("   ## restore all intr(%x) ##\n",page);
                }
#endif
    ...
    BYTE    addr_byte)
{
    ...
    for (i = 0; i < Retry; i++)
    {
        ...
        if ((IT7230_Init_Status() == FINISH_RESTORT) || (IT7230_Init_Status() == PRE_RESTORT))
        {
            // (void)printf("   ##3 disable all intr(%x,%x) ##\n",page,addr_byte);
            portSAVEDISABLE_INTERRUPTS();
        }
#endif
        ...
        if (_i2c_read(wBuf, 1, buf, 2) == 0)
        {
            ...
            {
                portRESTORE_INTERRUPTS();
                // (void)printf("   ## restore all intr(%x,%x) ##\n",page,addr_byte);
            }
#endif
    ...
    WORD    data_word)
{
    ...
    for (i = 0; i < Retry; i++)
    {
        ...
        if ((IT7230_Init_Status() == FINISH_RESTORT) || (IT7230_Init_Status() == PRE_RESTORT))
        {
            // (void)printf("   ##2 disable all intr(%x,%x,%x) ##\n",page,addr_byte,data_word);
            portSAVEDISABLE_INTERRUPTS();
        }
#endif
        ...
        if (_i2c_write(addr_byte, wPage, 2) == 0)
        {
            ...
            {
                portRESTORE_INTERRUPTS();
                // (void)printf("   ## restore all intr(%x,%x,%x) ##\n",page,addr_byte,data_word);
            }
#endif
            break;
        }
...
#ifdef  ENABLE_KEYPAD_DBG_MODE
        (void)printf("wCAPS_S2OHCR_CHECK:%04X\n", wCAPS_S2OHCR_CHECK);
#endif
    ...
    void)
{
    ...
    if (IT7230_Init_Status() == FINISH_RESTORT)
    {
        ...
        {
#ifdef  ENABLE_KEYPAD_DBG_MODE
            (void)printf("wCAPS_S2OHCR_CHECK:%04X/CAPS_S2HTR:%04X\n", wCAPS_S2OHCR_CHECK, IT7230_CapS_Read_Reg(PAGE_0, CAPS_S2HTR));
#endif
            return IT7230_Init();
        }
    }
    ...
    if (IT7230_Init_Status() == FINISH_RESTORT)
    {
        ...
        if (!(INTPin & TK_GPIO_MASK))
        {
            ...
            if ((IT7230_CapS_Read_Reg(PAGE_1, CAPS_PCR) & 0x3006) != 0x3006)
            {
                ...
                (void)printf("[IT7230 WARNING]: PCR!=0x3006,(%x)\n", IT7230_CapS_Read_Reg(PAGE_1, CAPS_PCR));
                SleepTime = IT7230_Init();
                (void)printf("[IT7230 WARNING]: reinit time = %d\n", SleepTime);
                if (SleepTime != 0)
                {
                    (void)printf("sleep for restoring register\n");
                    usleep(SleepTime * 1000);
                    IT7230_Restort_Reg();
                    SleepTime = 0;
#ifdef  ENABLE_KEYPAD_DBG_MODE
                    (void)printf("ReInitial_IT7230_success\n");
#endif
                }
                (void)printf("[IT7230 WARNING]: reinit success\n");
            }
            // (void)printf("int,read = %x,%x\n",PAGE_0,CAPS_SXCHSR);
            wTKeyStatus = IT7230_CapS_Read_Reg(PAGE_0, CAPS_SXCHSR);

            if (wLastTKeyStatus != wTKeyStatus)
            {
                wLastTKeyStatus = wTKeyStatus;
            }
        }
        else
        {
    ...
    void)
{
...
#ifdef  ENABLE_KEYPAD_DBG_MODE
    (void)printf("itpKeyPadProbe_IT7230\n");
#endif
    ...
    if (ChkSum > 1)
    {
        ...
        {
            (void)printf("[KEYPAD]warning:: multi-key, skip it(tk_value=%x, chk=%d)\n", tk_value, ChkSum);
...
#ifdef  ENABLE_KEYPAD_DBG_MODE
    (void)printf("[KEYPAD]:got key=%04x\n", tk_value);
#endif
    ...
    void)
{
    ...
    if (gKpFd != -1)
    {
        (void)printf("open keypad file device OK, gKpFd = %d!!\n", gKpFd);
    }
    else
    {
        (void)printf("open keypad fd FAIL, gKpFd = %d!!\n", gKpFd);
    }
#endif
    ...
    if (SleepTime != 0)
    {
        ...
        SleepTime = 0;
#ifdef  ENABLE_KEYPAD_DBG_MODE
        (void)printf("Initial_IT7230_success\n");
#endif
    }
    pthread_mutex_unlock(&keypad_mutex);
}
