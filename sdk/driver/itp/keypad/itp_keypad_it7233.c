...
#define I2CReadByteData                 itpI2cReadByte_
#define I2CWriteByteData                itpI2cWriteByte_
    ...
    uint8_t dLen)
{
    ...
    (void)printf("	RdIcReg,fd=%x, [%x,", gTkDev, regAddr);
#endif
    ...
    uint32_t    len)
{
...
#ifdef  ENABLE_TK_IIC_DBG_MSG
    (void)printf("	WtTkIcReg(fd=%x,[%x,%x,%x]\n", gTkDev, regAddr, *regData, len);
#endif
    ...
    if (i2cret < 0)
    {
        (void)printf("%s[%d]I2C write fail \n", __FILE__, __LINE__);
        return -1;
    }
    ...
    uint8_t reg)
{
    ...
    i2cret = readChipReg_(reg, &buf[0], 1);
    ...
    uint8_t byte)
{
    ...
    i2cret = writeChipReg_(addr, &tmpB, 1);
}
...
{
    #ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("TK init initial::in\n");
    #endif

    ithEnterCritical();

    ithGpioClearIntr(TK_GPIO_PIN);
    ithGpioRegisterIntrHandler(TK_GPIO_PIN, tk_isr_, NULL);
    ...
    #ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("TK init initial::out\n");
    #endif
}
...
_tanslateTouchValue (
    ...
    uint8_t totalKeyNum)
{
    uint8_t     ChkSum  = checkSum_(value);
    uint8_t     i       = 0;
    uint16_t    flag;

#ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("parseTchKey(%x,%x), chkSum=%x\n", value, totalKeyNum, ChkSum);
#endif
        ...
        switch (totalKeyNum)
        {
            case 5:
                switch (value)
                {
                    ...
                    default:
#ifdef  ENABLE_KEYPAD_DBG_MSG
                        (void)printf("touch keypad error: totalKeyNum incorrect2, keyNum=%d, V=%x\n", totalKeyNum, value);
#endif
                        return 0xFF;
                }
            ...
            case 16:
            default:
                (void)printf("touch keypad error: totalKeyNum incorrect, keyNum=%d, V=%x\n", totalKeyNum, value);
                return 0xFF;
        }
        return 0xFF;
    }
    ...
    switch (totalKeyNum)
    {
        ...
        case 16:
        {
            ...
            if (ChkSum > 2)
            {
                (void)printf("[KEYPAD]warning:: multi-key, skip it(value=%x, chk=%d)\n", value, ChkSum);
                return -1;
            }
            switch (value & 0xC3)
            {
                ...
                default:
                    (void)printf("[KEYPAD]warning:: incorrect X, skip it(value=%x, chk=%d)\n", value, value & 0xC3);
                    return -1;
            }
            ...
            switch (value & 0x3C)
            {
                ...
                default:
                    (void)printf("[KEYPAD]warning:: incorrect Y, skip it(value=%x, chk=%d)\n", value, value & 0x3C);
                    return -1;
            }
            return (BtnY * 4 + BtnX);
        }
        ...
        default:
            (void)printf("touch keypad error: totalKeyNum incorrect, keyNum=%d\n", totalKeyNum);
            break;
    }
    return (uint8_t)0xFF;
}
    ...
    uint8_t page)
{
    ...
    if (gRegPage != page)
    {
        ...
        (void)printf("write page=0x%X to regAddr 0x%X\n", page, 0xF0);
#endif
    ...
    FW_Info * fwInfo)
{
    ...
    (void)printf("get FW version:fwInfo=%x, size=%d:\n", fwInfo, gFwVerCycle);

    _checkPage(page);

    // write 0xF1 0x80
    cbuf[0] = 0x80;
    i2cret  = writeChipReg_(0xF1, cbuf, 1);
    ...
    cbuf[0] = 0x01;
    cbuf[1] = 0x01;
    i2cret  = writeChipReg_(0x40, cbuf, 2);
    if (i2cret)
    {
        (void)printf("Err3\n");
    }
    ...
    // wait INTr pull-down
    // (void)printf("Before INTr check\n");
    // while(ithGpioGet(TK_GPIO_PIN));
    (void)printf("after INTr check\n");
    ...
    // read 0x40 16BYTES
    i2cret = readChipReg_(0x40, &buf[0], 21);
    if (i2cret)
    {
        (void)printf("Err7\n");
    }
    ...
    {
        memcpy((uint8_t *)fwInfo, (uint8_t *)buf, gFwVerCycle);
    }

#ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("FW info buffer::\n");
    ...
    cbuf[0] = 0x00;
    i2cret  = writeChipReg_(0xF3, cbuf, 1);
    if (i2cret)
    {
        (void)printf("Err8\n");
    }
    ...
    void * arg)
{
    ...
    for (;;)
    {
        if (gTkInitFinished)
        {
            if (getTkIntr_() == true)
            {
                temp = getTouchKeyStatus_();

                pthread_mutex_lock(&keypad_mutex);
                if (temp != gCurrKeyStatus)
                {
                    gCurrKeyStatus = temp;
                }
...
#ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("itpKeyPadProbe, cnt=%d\n", gCnt++);
#endif

    index = _getTkIndex();

#ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("[KEYPAD] send_key, index=%d\n", index);
#endif
    ...
    void)
{
    ...
    {
        (void)printf("itpKeypadInit...SUCCESS!!\n");
    }
    else
    {
        (void)printf("%s[%d] get file device fail\n", __FILE__, __LINE__);
        return;
    }
    ...
    for (;;)
    {
        ...
        if (cnt++ > 100)
        {
            (void)printf("%s[%d] load IT7233 Config fail\n", __FILE__, __LINE__);
            return;
        }
    }
    ...
    // [5] get IT7233 Config
    {
        ...
        (void)printf("The %d-key FW version of IT7236:(%02X.%02X.%02X.%02X)\n", gTotalTouchKeyNum, b[0], b[1], b[2], b[3]);
    }

    // [6] Create a pthread for probing keypad event
#ifdef  ENABLE_KEYPAD_DBG_MSG
    (void)printf("Create touch keypad pthread~~\n");
#endif
    pthread_attr_init(&attr);
    res = pthread_create(&task, &attr, tkProbeHandler_, NULL);
    if (res)
    {
        (void)printf("%s[%d] Create thread fail, res=%x \n", __FILE__, __LINE__, res);
        return;
    }
