...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, CY20336_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}
    ...
    uint32_t    cnt)
{
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, CY20336_IIC_ADDR, cmd, buf, cnt);
#endif
}
    ...
    void)
{
    ...
    result  = i2c_read_(&buf_reg, 1, &buf_data, 1);
    if (result)
    {
        (void)printf("Err1\n");
        return 0;
    ...
    // (void)printf("[xx=] %x, key=%x ",buf_data,KeyValue);
...
//  result = mmpIicSendData(IIC_MASTER_MODE, CY20336_IIC_ADDR, CY_LIGHT_REG_ADRRESS, &light_status, 1);
    ...
    result = i2c_write_(CY_LIGHT_REG_ADRRESS, &light_status, 1);

    pthread_mutex_unlock(&keypad_mutex);
}

void
itp_cy_keypad_light_on_all ()
{
    unsigned int    result          = 0;
...
//      (void)printf("[LED2]-status %x\n",light_status);
    buf_reg = CY_LIGHT_REG_ADRRESS;
    result  = i2c_write_(buf_reg, &light_status, 1);
    ...
    unsigned int    interval_ms)
{
    ...
    if (i == LED_CMD_SIZE)
    {
        (void)printf("-W- itp_cy_keypad_light_flash, cmd full!");
        return -1;
    }
    else
    {
...
void
itp_cy_keypad_indoor_led_flash ()
{
...
void
itp_cy_keypad_sgate_card_led_flash ()
{
    ...
    unsigned long start_ticks)
{
    ...
    // (void)printf(" [LIGHT  Elapsed]%d ,start_ticks %d,portTICK_RATE_MS %d\n", ret,start_ticks,portTICK_RATE_MS);
    return ret;
}
    ...
    void)
{
    ...
    if (!kpIntr)
    {
//      (void)printf("[KEYPAD %d,%d]-INT",CFG_GPIO_KEYPAD,TK_GPIO_PIN);
        value = getTouchKey_cy_();
        if (!value)
        {
            return -1;// invalid
        }
...
itpKeypadInit (
    void)
{
    (void)printf("[KEYPAD]-itpKeypadInit");
