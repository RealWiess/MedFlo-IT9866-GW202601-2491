...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, IT7235_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}
    ...
    uint32_t    cnt)
{
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, IT7235_IIC_ADDR, cmd, buf, cnt);
#endif
}
    ...
    uint8_t page)
{
    ...
    if (gRegPage != page)
    {
        // result = mmpIicSendData(g_kpI2cPort, IT7235_IIC_ADDR, 0xF0, &page, 1);
        result = i2c_write_(0xF0, &page, 1);
        if (result)
        {
            ...
            {
                (void)printf("[KEYPAD]warning: IIC hang up(1), reset keypad device, result=%x.\n", result);
                resetDevice_();
            }

#ifdef  ENABLE_KEYPAD_DBG_MODE
            (void)printf("[KEYPAD]check iic status(busy/ready).01\n");
            for (;;)
            {
                ...
                {
                    (void)printf(" iic busy.01,cnt=%X,reg=%x\n", cnt, reg32);
                }
                if (++cnt > 0xFFFFF)
                {
                    (void)printf("STOP!!\n");
            ...
            (void)printf("[KEYPAD]warning:: checkPage_() try again!!\n");
#endif
            ...
            if (result)
            {
                ...
                if (result == I2C_WAIT_TRANSMIT_TIME_OUT)
                {
                    (void)printf("[KEYPAD]warning: IIC hang up(2), reset keypad device, result=%x.\n", result);
                    resetDevice_();
                }

                (void)printf("%s[%d]ERROR = %x \n", __FILE__, __LINE__, result);

#ifdef  ENABLE_KEYPAD_DBG_MODE
                (void)printf("[KEYPAD]check iic status(busy/ready).02\n");
                for (;;)
                {
                    reg32 = ithReadRegA((ITH_IIC_BASE + 0x04));
                    if (!(reg32 & 0x0C))
                    ...
                    if ((cnt & 0xFFFF) == 0xFFFF)
                    {
                        (void)printf(" iic busy.02,cnt=%X,reg=%x\n", cnt, reg32);
                    }
                    if (++cnt > 0xFFFFF)
                    {
                        (void)printf("STOP!!\n");
                ...
                (void)printf("[KEYPAD]warning::for checking IIC busy status.\n");
#endif
    ...
    void)
{
    ...
    // result = mmpIicReceiveData(g_kpI2cPort, IT7235_IIC_ADDR, buf, 1, buf, 2);
    result  = i2c_read_(buf, 1, buf, 2);
    if (result)
    {
        // mmpIicStop(g_kpI2cPort);
        usleep(1000);
        ...
        if (result == I2C_WAIT_TRANSMIT_TIME_OUT)
        {
            (void)printf("[KEYPAD]warning: IIC hang up(3), reset keypad device, result=%x.\n", result);
            resetDevice_();
            return 0;
        }

#ifdef  ENABLE_KEYPAD_DBG_MODE
        (void)printf("[KEYPAD]warning:: retry getTouchKey_(), result=%x,buf=[%x,%x]\n", result, buf[0], buf[1]);
        (void)printf("[KEYPAD]check iic status(busy/ready).03\n");
        for (;;)
        {
            reg32 = ithReadRegA((ITH_IIC_BASE + 0x04));
            if (!(reg32 & 0x0C))
            ...
            if ((cnt & 0xFFFF) == 0xFFFF)
            {
                (void)printf(" iic busy.03,cnt=%X,reg=%x\n", cnt, reg32);
            }
            if (++cnt > 0xFFFFF)
            {
                (void)printf("STOP!!\n");
        ...
        if (result)
        {
            ...
            if (result == I2C_WAIT_TRANSMIT_TIME_OUT)
            {
                (void)printf("[KEYPAD]warning: IIC hang up(4), reset keypad device, result=%x.\n", result);
                resetDevice_();
            }

            (void)printf("[KEYPAD][%x,%x]%s[%d]ERROR = %x \n", buf[0], buf[1], __FILE__, __LINE__, result);
            gRegPage = 0xFE;

#ifdef  ENABLE_KEYPAD_DBG_MODE
            (void)printf("[KEYPAD]check iic status(busy/ready).04\n");
            for (;;)
            {
                reg32 = ithReadRegA((ITH_IIC_BASE + 0x04));
                if (!(reg32 & 0x0C))
                ...
                if ((cnt & 0xFFF) == 0xFFF)
                {
                    (void)printf(" iic busy.04,cnt=%X,reg=%x\n", cnt, reg32);
                }
                if (++cnt > 0xFFFF)
                {
                    (void)printf("STOP!!\n");
