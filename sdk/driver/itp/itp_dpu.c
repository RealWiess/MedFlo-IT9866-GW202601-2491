...
// #define  ENABLE_PATCH_DMA_ISSUE
    ...
    #define ITP_DPU_BIG_ENDIAN      0x00300000  // 0x00300000 sha mode

    #define ITP_SHA_LITTLE_ENDIAN   0x00000000
    #define ITP_SHA_BIG_ENDIAN      0x00000004
#else
    #define ITP_DPU_LITTLE_ENDIAN   0x00000004
    #define ITP_DPU_BIG_ENDIAN      0x00300004
#endif

#ifdef  ENABLE_DPU_MUTEX
    ...
    void)
{
    ...
    // (void)printf("DpuIsrSemaphor=%x\n",DpuIsrSemaphor);

    // (void)printf("Enable DPU IRQ~~leave\n");
}
    ...
    switch (Info->dpuMode)
    {
        ...
        case ITP_DPU_DES_MODE:
            switch (Info->cipher)
            {
                ...
                default:
                    (void)printf("[DPU ERROR] incorrect cipher mode of DES\n");
                    break;
            }
            break;
        ...
        case ITP_DPU_DES3_MODE:
            switch (Info->cipher)
            {
                ...
                default:
                    (void)printf("[DPU ERROR] incorrect cipher mode of DES3\n");
                    break;
            }
            break;
    ...
    ITP_DPU_INFO * Info)
{
    ...
    {
        (void)printf("itp dpu driver out of memory\n");
        goto end;
    }
    if (Info->dpuMode == ITP_DPU_AES_MODE)
    {
        cipher_len = 16;
    }
    ...
    #if (CFG_CHIP_FAMILY == 9830)
    ithFlushDCacheRange((void *)srcBuff, Info->dpuLen);
    memset(tem_sbuf, 1, Info->dpuLen);
    #endif

    memcpy(tem_sbuf, srcBuff, Info->dpuLen);

    #if (CFG_CHIP_FAMILY == 9830)
    // need double check this statement if real chip
    ithFlushDCacheRange((void *)tem_sbuf, Info->dpuLen);
    #endif

    #ifdef  ENABLE_ITP_DPU_DBG_MEASUREMENT
    gettimeofday(&endT, NULL);
    (void)printf("mcp:Dur=%d,%d\n", itpTimevalDiff(&startT, &endT), Info->dpuLen);
    #endif
    ...
    if (Info->dpuMode == ITP_DPU_CRC_MODE)
    {
        ...
        else
        {
            ...
            if ((uint32_t)ptr32 & 0x03)
            {
                ...
                (void)printf("crcS.byte alignment,ptr1=%x.ptr8=%x\n", ptr_1, ptr8);
    ...
    else
    {
...

        {
            ...
            if (Info->keyLen)
            {
                ...
                if (buf_addr & 0x03)
                {
                    align_buf = (uint8_t *)((buf_addr & ~0x03) + 4);
                }
#endif
...
#if (CFG_CHIP_FAMILY == 9830)
        if (Info->dpuMode == ITP_DPU_SHA_MODE)
        {
            // do check Src Sizem of SHA mode (only 512-bit alignment)
            if (Info->dpuLen & 0x3F)
            {
                (void)printf("[ITP_DPU] SHA error, source data must 64-byte alignment!! SrcSize= %x\n", (Info->dpuLen));
                while (1)
                {
                }
            }
        ...
        if (Info->dpuMode == ITP_DPU_SHA_MODE)
        {
            ithDpuSetDpuEndian(ITP_SHA_BIG_ENDIAN);
        }
        else
        {
        ...
        if (Info->dpuMode == ITP_DPU_SHA_MODE)
        {
            uint8_t     * ptr_1 = (uint8_t *)malloc(64);
            uint32_t    * ptr32 = (uint32_t *)(((uint32_t)ptr_1 + 3) & ~3);

            ithDpuGetSha256Value((unsigned int *)ptr32);
            memcpy((uint8_t *)tem_dbuf, (uint8_t *)ptr32, 32);
            ithFlushDCacheRange((void *)tem_dbuf, 0x20);
            free(ptr_1);

            // ithPrintVram8(tem_dbuf, 0x20);
        }
#endif
...
#if (CFG_CHIP_FAMILY == 9830)
            else if (g_DpuInfo.dpuMode == ITP_DPU_SHA_MODE)
            {
