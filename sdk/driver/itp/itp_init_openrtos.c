...
extern void ithWIFICardPowerProcess(void);
#endif
...
static void MemDbgHandler(void *arg)
{
    ...
    if (ithMemDbgGetFlag(ITH_MEMDBG1, &id0, &id1, &id2, &id3) >= 0)
    {
        ...
        #if defined(CFG_DBG_STACK_OVERFLOW)
...
static void MemDbgHandler(void *arg)
{
    ...
    if (dbgFlag0 >= 0)
        ithPrintf("[ERROR] Memory access detect on region 0 by %s. (%d)\n", dbgFlag[dbgFlag0], dbgFlag0);

    if (dbgFlag1 >= 0)
        ithPrintf("[ERROR] Memory access detect on region 1 by %s. (%d)\n", dbgFlag[dbgFlag1], dbgFlag1);
...
void LoadDLModule(void)
{
	...
	NorRead(SPI_0, NOR_CSN, CFG_UPGRADE_DLM_PACKAGE_POS, header, 20);
	...
	NorRead(SPI_0, NOR_CSN, CFG_UPGRADE_DLM_PACKAGE_POS + header_size, content_buf, content_size);

	// hardware decompress
	if (strncmp(content_buf, "SMAZ", 4) == 0)
	{
		uint8_t dcpsMode = ITP_DCPS_SEAL_MODE;
...
void LoadDLModule(void)
{
    ...
    {
        ITP_LOG_ERR("get gap size error:\n" );
        goto end;
    }

	// read rom header size
    pos = CFG_UPGRADE_DLM_PACKAGE_POS / (blocksize + gapsize);
    blockcount = CFG_UPGRADE_DLM_PACKAGE_POS % (blocksize + gapsize);
    if (blockcount > 0)
    {
        ITP_LOG_WARN("rom position 0x%X and block size 0x%X are not aligned; align to 0x%X\n", CFG_UPGRADE_IMAGE_POS, blocksize, (pos * blocksize) );
    }

    ITP_LOG_DBG("blocksize=%d pos=0x%X align=%d\n", blocksize, pos, blockcount );

    if (lseek(fd, pos, SEEK_SET) != pos)
    {
        ITP_LOG_ERR("seek to rom position %d error\n", pos );
        goto end;
    }
...
void itpInit(void)
{
    ...
    // init power device
#if defined(CFG_BATTERY_ENABLE) || defined(CFG_EXTERNAL_POWER_ON) || defined(CFG_POWER_STANDBY) || defined(CFG_POWER_SLEEP) || defined(CFG_POWER_HIBERNATION) || defined(CFG_POWER_SUSPEND)
    ...
    if (cpu_support_pmp())
    {
        ...
        uint32_t                   cache_linesz = CACHE_LINE_SIZE(DCACHE);
        const cpu_pmp_tor_config_t config_params_0 = {
            cpu_pmp_0,
            (void *)ITH_ALIGN_DOWN(0x1000 - 12, cache_linesz),
            cpu_pmp_r_on,
            cpu_pmp_w_off,
            cpu_pmp_x_on,
            cpu_pmp_l_on
        };
        cpu_pmp_tor_config(CPU_PMP, &config_params_0);

        const cpu_pmp_tor_config_t config_params_1 = {
            cpu_pmp_1,
            (void *)ITH_ALIGN_DOWN(0x1000, cache_linesz),
            cpu_pmp_r_on,
            cpu_pmp_w_on,
            cpu_pmp_x_on,
            cpu_pmp_l_off
        };
        cpu_pmp_tor_config(CPU_PMP, &config_params_1);

        const cpu_pmp_tor_config_t config_params_2 = {
            cpu_pmp_2,
            (void *)ITH_ALIGN_DOWN((uint32_t)(&__etext), cache_linesz),
            cpu_pmp_r_on,
            cpu_pmp_w_off,
            cpu_pmp_x_on,
            cpu_pmp_l_on
        };
        cpu_pmp_tor_config(CPU_PMP, &config_params_2);

        const cpu_pmp_tor_config_t config_params_4 = {
            cpu_pmp_1,
            (void *)0xFFFFFFFF,
            cpu_pmp_r_on,
            cpu_pmp_w_on,
        ...
        cpu_pmp_tor_config(CPU_PMP, &config_params_4);
    }
#endif
    ...
    // init riscv cpu 2
#ifdef CFG_RISCV2_ENABLE
    itpRegisterDevice(ITP_DEVICE_RISCV2, &itpDeviceRiscv2);
#endif
    ...
    ioctl((int)ITP_DEVICE_WATCHDOG, ITP_IOCTL_INIT,   NULL);
    ioctl((int)ITP_DEVICE_WATCHDOG, ITP_IOCTL_ENABLE, NULL);
...
#ifdef CFG_PMIC_WD_ENABLE
	itpRegisterDevice(ITP_DEVICE_PMIC, &itpDevicePmicWD);
	ioctl(ITP_DEVICE_PMIC, ITP_IOCTL_INIT, NULL);
	ioctl(ITP_DEVICE_PMIC, ITP_IOCTL_ENABLE, NULL);    
#endif
 ...
 */
    ...
    itpRegisterDevice(ITP_DEVICE_IR3, &itpDeviceIr3);
    ioctl(ITP_DEVICE_IR3, ITP_IOCTL_INIT, NULL);
#endif
    ...
    itpRegisterDevice(ITP_DEVICE_NORAES, &itpDeviceNorAES);
    ioctl(ITP_DEVICE_NORAES, ITP_IOCTL_INIT, NULL);
#endif
#endif

	// init Yaffs2
#ifdef CFG_FS_YAFFS2
	itpRegisterDevice(ITP_DEVICE_YAFFS2, &itpFSDeviceYaffs2.dev);
	ioctl(ITP_DEVICE_YAFFS2, ITP_IOCTL_INIT, NULL);
#endif
...
#if defined(CFG_DLM_ENABLE) && !defined(CFG_NET_WIFI_SDIO_POWER_ON_OFF_USER_DEFINED)
	LoadDLModule();
#endif
    ...
    itpRegisterDevice(ITP_DEVICE_USBHRAW, &itpDeviceUsbhRaw);
#endif
    ...
    // init socket device
#if defined(CFG_NET_ENABLE) && !defined(CFG_NET_WIFI_SDIO_VND_MHD)
    ...
    itpRegisterDevice(ITP_DEVICE_USBDRAW, &itpDeviceUsbdRaw);
    ioctl(ITP_DEVICE_USBDRAW, ITP_IOCTL_INIT, NULL);
#endif

    // init usb device mode as rs232 device
#if defined(CFG_USBD_ACM) || defined(CFG_USBD_CD_CDCACM) || defined(HAVE_USBD_CD_CDCACM)
    itpRegisterDevice(ITP_DEVICE_USBDACM, &itpDeviceUsbdAcm);
    ioctl(ITP_DEVICE_USBDACM, ITP_IOCTL_INIT, NULL);
#endif

#if defined(CFG_USBD_ECM) || defined(CFG_USBD_CD_CDCECM)
    itpRegisterDevice(ITP_DEVICE_USBDECM, &itpDeviceUsbdEcm);
    ioctl(ITP_DEVICE_USBDECM, ITP_IOCTL_INIT, NULL);
#endif

#if defined(CFG_USBD_NCM) || defined(CFG_USBD_CD_CDCNCM)
    ...
    itpRegisterDevice(ITP_DEVICE_TP, &itpDeviceTp);
    ioctl(ITP_DEVICE_TP, ITP_IOCTL_INIT, NULL);
#endif
	...
	// init mem bist test device.
#ifdef CFG_MEM_BIST_ENABLE
	itpRegisterDevice(ITP_DEVICE_MEMBIST, &itpDeviceMemBist);
	ioctl(ITP_DEVICE_MEMBIST, ITP_IOCTL_INIT, NULL);
	ioctl(ITP_DEVICE_MEMBIST, ITP_IOCTL_INIT_TASK, NULL);
#endif
