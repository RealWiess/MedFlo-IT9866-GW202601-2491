...
#define ALC5628_I2CADR (0x30U >> 1U)
...
static uint32_t curr_out1_volume = DEFAULT_DB_OUT1_VOLUME;               /* 0 dB */
...
static int32_t         _alc5628_DA_running   = 0;
static int32_t         _alc5628_AD_running   = 0;
static pthread_mutex_t ALC5628_MUTEX         = PTHREAD_MUTEX_INITIALIZER;

static int32_t         _alc5628_linein       = 0;
static int32_t         _alc5628_i2sin        = 1;
static int32_t         _alc5628_spkout       = 0;
static int32_t         _alc5628_hpout        = 0;

static int32_t         _alc5628_cold_start   = 1; /* alc5628q should enable power only once */

static int32_t         i2s_sample_rate       = 48000;
...
/* ************************************************************************** */
static void I2C_common_write_word (uint8_t RegAddr, uint16_t d)
{
    int32_t  success = 0;
    int32_t  flag;
    int32_t  retry = 0;
    ...
    {
        if (0 == (flag = mmpIicSendData(IIC_PORT, ALC5628_I2CADR, RegAddr, &tmp, 2)))
        {
            success = 1;
#ifdef ALC5628_DEBUG_I2C_COMMON_WRITE
            (void)printf("ALC5628_I2CADR# IIC WriteOK!\n");
#endif
            break;
        }
    }
    if (success == 0)
    {
        (void)printf("ALC5628# IIC Write Fail!\n");
        // for (;;) { (void)usleep(500000); }
    }
    // ithWriteRegMaskA(ITH_GPIO_BASE | 0xD0, (0 << 28), (0x3 << 28)); /* IIC: external APB IIC */
}

static uint16_t I2C_common_read_word (uint8_t RegAddr)
{
    ...
    for (retry = 0; retry < 50; retry++)
    {
        if (0 == (flag = mmpIicReceiveData(IIC_PORT, ALC5628_I2CADR, &RegAddr, 1, &d, 2)))
        {
            success = 1;
#ifdef ALC5628_DEBUG_I2C_COMMON_WRITE
            (void)printf("ALC5628_I2CADR# IIC Read OK!\n");
#endif
            break;
        }
    }
...
static void alc5628_write_reg (uint8_t reg_addr, uint16_t value)
{

#ifdef ALC5628_DEBUG_PROGRAM_REGVALUE
    (void)printf("ALC5628# write reg[0x%02x] = 0x%04x\n", reg_addr, value);
#endif
    I2C_common_write_word(reg_addr, value);
}

static uint16_t alc5628_read_reg (uint8_t reg_addr)
{
...
static void _output_path_slect (OutPath enRL)
{
    ...
    if (_alc5628_spkout)
    {
        ...
        alc5628_writeRegMask(0x02, curr_out1_volume << 8 | curr_out1_volume << 0,
                             0x1F << 8 | 0x1F << 0);                        // DAC analog(R|L) SPK vol
        alc5628_writeRegMask(0x0C, 0 << 14 | 0 << 6, 0x1 << 14 | 0x1 << 6); // DAC SPK mix
    }
    if (_alc5628_hpout)
    {
        alc5628_writeRegMask(0x04, 0 << 15 | 0 << 7, 0x1 << 15 | 0x1 << 7); // HP out on
        alc5628_writeRegMask(0x04, curr_out1_volume << 8 | curr_out1_volume << 0,
                             0x1F << 8 | 0x1F << 0);                        // DAC analog(R|L) HP vol
        alc5628_writeRegMask(0x0C, 0 << 15 | 0 << 7, 0x1 << 15 | 0x1 << 7); // DAC HP mixer
    }
    (void)usleep(50000); /* Delay (50 ms) to allow HP amps to settle */
    (void)printf("_alc5628_spkout=%d _alc5628_hpout=%d\n", _alc5628_spkout, _alc5628_hpout);
}
...
void itp_codec_playback_set_direct_vol (uint32_t target_vol)
{
    ...
    if (_alc5628_DA_running)
    {
        ...
        if (_alc5628_linein)
        {
            alc5628_writeRegMask(0x0A, curr_out1_volume << 8 | curr_out1_volume << 0,
                                 0x1F << 8 | 0x1F << 0); // Line in volume
        }
...
void itp_codec_playback_get_currvolperc (uint32_t * currvolperc)
{
    ...
    for (i = 0; i < 99; i++)
    {
        if ((alc5628_perc_to_reg_table[i] >= curr_out1_volume) && (curr_out1_volume > alc5628_perc_to_reg_table[i + 1]))
        {
            *currvolperc = i;
            return;
        }
...
void itp_codec_playback_mute (void)
{
    ...
    if (_alc5628_DA_running)
    {
        ...
        if (_alc5628_linein)
        {
            alc5628_writeRegMask(0x0A, 3 << 14 | 3 << 6, 0x3 << 14 | 0x3 << 6); // 0xC8C8
        }
        if (_alc5628_i2sin)
        {
            alc5628_writeRegMask(0x0C, 3 << 14 | 3 << 6, 0x3 << 14 | 0x3 << 6);
        }
        if (_alc5628_spkout)
        {
            alc5628_writeRegMask(0x3E, 0 << 12, 0x1 << 12);
        }
    }
    pthread_mutex_unlock(&ALC5628_MUTEX);
}
...
void itp_codec_playback_unmute (void)
{
    ...
    if (_alc5628_DA_running)
    {
        ...
        if (_alc5628_linein)
        {
            alc5628_writeRegMask(0x0A, 0 << 14 | 0 << 6, 0x3 << 14 | 0x3 << 6);
        }
        if (_alc5628_i2sin)
        {
            alc5628_writeRegMask(0x0C, 0 << 14 | 0 << 6, 0x3 << 14 | 0x3 << 6);
        }
        if (_alc5628_spkout)
        {
            alc5628_writeRegMask(0x3E, 1 << 12, 0x1 << 12);
        }
    }
...
void itp_codec_rec_init (uint32_t input_source)
{
    ...
    (void)printf("alc5628 AD invalid no use\n");
    init_alc5628_common();
    _alc5628_AD_running = 1;
}
...
void itp_codec_set_linein_enable (int32_t yesno)
{
    _alc5628_linein = yesno;
    if (_alc5628_linein)
    {
        uint16_t adj = curr_out1_volume;
        adj >>= 1;
        adj = (adj << 8) | adj;
...
void itp_codec_set_i2sin_enable (int32_t yesno)
{
