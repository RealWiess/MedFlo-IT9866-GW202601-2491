...
#define ALC5616_I2CADR (0x36U >> 1U)
...
static uint32_t curr_out1_volume = DEFAULT_DB_OUT1_VOLUME;               /* -11.25 dB */
...
static int32_t         _alc5616_DA_running   = 0;
static int32_t         _alc5616_AD_running   = 0;
static int32_t         _alc5616_micin        = 1;
static int32_t         _alc5616_linein       = 0;
static int32_t         _alc5616_lineout      = 0;
static int32_t         _alc5616_spkout       = 1;
static int32_t         _alc5616_hpout        = 1;
static int32_t         _alc5616_channel      = 2; // note set 2 can get louder

static int32_t         _alc5616_cold_start   = 1; /* alc5616q should enable power only once */
static int32_t         _alc5616_mute         = 0;
...
static void            I2C_common_write_word (uint8_t RegAddr, uint16_t d)
{
    ...
    {
        if (0 == (flag = mmpIicSendData(IIC_PORT, ALC5616_I2CADR, RegAddr, (uint8_t *)&tmp, 2)))
        {
            success = 1;
            break;
        }
    ...
    if (success == 0)
    {
        (void)printf("ALC5616# IIC Write Fail!\n");
        while (1)
        {
            (void)usleep(500000);
        }
...
static uint16_t I2C_common_read_word (uint8_t RegAddr)
{
    ...
    for (retry = 0; retry < 50; retry++)
    {
        if (0 == (flag = mmpIicReceiveData(IIC_PORT, ALC5616_I2CADR, &RegAddr, 1, (uint8_t *)&d, 2)))
        {
            success = 1;
            break;
        }
...
static void alc5616_write_reg (uint8_t reg_addr, uint16_t value)
{
#ifdef ALC5616_DEBUG_PROGRAM_REGVALUE
    (void)printf("ALC5616# write reg[0x%02x] = 0x%04x\n", reg_addr, value);
#endif
    I2C_common_write_word(reg_addr, value);
}

static uint16_t alc5616_read_reg (uint8_t reg_addr)
{
    uint16_t value = I2C_common_read_word(reg_addr);
#ifdef ALC5616_DEBUG_PROGRAM_REGVALUE
    (void)printf("ALC5616# read reg[0x%02x] = 0x%04x\n", reg_addr, value);
#endif
    return value;
}
...
static void init_alc5616_common (void)
{
...

    {
        ...
        (void)usleep(10000); /* Delay (10 ms) to allow HP amps to settle */

        if (_alc5616_spkout)
        {
            alc5616_write_reg(0x03,
                              0x2727); /* unmute SPKL/SPKR  & Gain -46.5dB:0x2727 0dB:0x0808 +12dB:0x0000 (0xC8C8)*/
        ...
        if (_alc5616_hpout)
        {
            alc5616_write_reg(0x02,
                              0x2727); /* unmute HPOL/HPOR & Gain -46.5dB:0x2727 0dB:0x0808 +12dB:0x0000 (0xC8C8)*/
...
void itp_codec_playback_mute (void)
{
    ...
    alc5616_write_reg(0x2A, 0x6363); /*Stereo DAC digital mixer control ,note:if comment, farend have no voice when
                                        communicating press mute */
    alc5616_writeRegMask(0x65, (0 << 14) | (0 << 15),
                         (1 << 14) | (1 << 15)); /*power down OUTMIXL/R note:prevent switch pop sound*/
...
void itp_codec_playback_unmute (void)
{
    ...
    alc5616_write_reg(0x2A, 0x0202); /*Stereo DAC digital mixer control ,note:if comment, farend have no voice when
                                        communicating press mute */
    alc5616_writeRegMask(0x65, (1 << 14) | (1 << 15),
                         (1 << 14) | (1 << 15)); /*power on OUTMIXL/R note:prevent switch pop sound*/
...
{
    uint32_t inpvol = MIN_INPUT_PGA_VOLUME;
...
{
    uint32_t inpvol = curr_input_pga_volume;
