...
static int32_t         _wm8960_micin         = 1;
static int32_t         _wm8960_linein        = 0;
static int32_t         _wm8960_spkout        = 1;
static int32_t         _wm8960_hpout         = 1;

static int32_t         _wm8960_cold_start    = 1; /* WM8960 should enable power only once */
...
void mic_input_control (void)
{
    ...
    data16 = ((0x1 & 0x1) << 8) | // differential inputs
             ((0x0 & 0x1) << 7) | ((0x1 & 0x1) << 6) | ((0x1 & 0x3) << 4) | ((0x1 & 0x1) << 3);
...
static void init_wm8960_common (void)
{
    ...
    if (0) /* restore previous playback volume */
    {
        ...
        if (curr_out1_volume > MAX_OUT1_VOLUME)
        {
            curr_out1_volume = MAX_OUT1_VOLUME;
        }
        else if (curr_out1_volume < MIN_OUT1_VOLUME)
        {
            curr_out1_volume = MIN_OUT1_VOLUME;
        }
    ...
    /* programming WM8960 */
    {
        ...
        if (_wm8960_spkout)
        {
            data16 = 0x78;            /* Enable SPK_L and SPK_R */
        }
        ...
        wm8960_write_reg(28, 0x010); /* Disable POBCTRL and SOFT_ST. BUFDCOPEN remain enabled  (Delay from R25 = 080 to
                                        Disable POBCTRL and SOFT_ST >100mS) */
        //-
        data16 = 0x1E0;
        if (_wm8960_spkout)
        {
            data16 = 0x1F8; /* Enable LOUT1 and ROUT1 */
        }
        ...
        wm8960_write_reg(3, 0x100);  /* ROUT1VOL (HP) = Analogue Mute, Enable OUT1VU, load volume settings to both left
                                        and right channels */
        wm8960_write_reg(40, 0x100); /* SPK_LN & SPK_LP (SPK) = Analogue Mute */
        wm8960_write_reg(41, 0x100); /* SPK_RN & SPK_RP = Analogue Mute, Enable OUT1VU, load volume settings to both
        ...
        if (_wm8960_spkout)
        {
            ...
            wm8960_write_reg(
                49,
                0x077); /* Enable Class D Speaker Outputs , 0x77: Left speaker only, 0xF7: Left&Right speaker enable*/
        }
        else
        {
    ...
    //		wm8960_write_reg(37, 0x080); //Enable RINPUT3 to right output mixer (RI2RO), RINPUT3 to Right mixer vol =
    // 0dB 		wm8960_write_reg(2, 0x179); //LOUT1 Vol = 0dB, volume update enabled 		wm8960_write_reg(3, 0x179); //ROUT1
    // Vol = 0dB, volume update enabled
    //	}
    pthread_mutex_unlock(&WM8960_MUTEX);
}
...
void itp_codec_playback_amp_volume_down (void)
{
    ...
    switch (curr_out1_volume)
    {
        case 0x7D ... 0x7F:
            {
                itp_codec_playback_set_direct_vol(0x7C);
                break;
            } /* +3 dB */
        case 0x7A ... 0x7C:
            {
                itp_codec_playback_set_direct_vol(0x79);
                break;
            } /*  0 dB */
        case 0x77 ... 0x79:
            {
                itp_codec_playback_set_direct_vol(0x76);
                break;
            } /* -3 dB */
        case 0x74 ... 0x76:
            {
                itp_codec_playback_set_direct_vol(0x73);
                break;
            } /* -6 dB */
        case 0x70 ... 0x73:
            {
                itp_codec_playback_set_direct_vol(0x6F);
                break;
            } /* -10dB */
        case 0x5C ... 0x6F:
            {
                itp_codec_playback_set_direct_vol(0x5B);
                break;
            } /* -30dB */
        case 0x31 ... 0x5B:
            {
                itp_codec_playback_set_direct_vol(0x30);
                break;
            } /* -73dB */
    }
}
...
void itp_codec_playback_amp_volume_up (void)
{
    ...
    switch (curr_out1_volume)
    {
        case 0x7C ... 0x7E:
            {
                itp_codec_playback_set_direct_vol(0x7F);
                break;
            } /* +6 dB */
        case 0x79 ... 0x7B:
            {
...
void itp_codec_rec_set_direct_vol (uint32_t target_vol)
{
    ...
    else if (target_vol > curr_input_pga_volume)
    {
        direction = 1;
    } /* + */
    else
