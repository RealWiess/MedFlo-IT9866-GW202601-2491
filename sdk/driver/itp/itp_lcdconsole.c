...
static unsigned char *lcd_base_a, *lcd_base_b;
#ifdef DRAW_VIDEO_BUF
static unsigned char *lcd_base_c;
#endif
...
static void lcd_drawchars(unsigned short x, unsigned short y,
                          ...
                          unsigned char *lcd_console_address) {
  unsigned char *dest;
  unsigned short row;

  dest = (unsigned char *)(lcd_console_address + y * lcd_line_length +
                           x * lcd_bpp);
...
static void console_scrollup(void) {
  ...
  memset(lcd_base_a + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg,
         CONSOLE_ROW_SIZE);
  memset(lcd_base_b + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg,
         CONSOLE_ROW_SIZE);

#ifdef DRAW_VIDEO_BUF
  memcpy(lcd_base_c, lcd_base_c + CONSOLE_ROW_SIZE, CONSOLE_SCROLL_SIZE);
  memset(lcd_base_c + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg,
         CONSOLE_ROW_SIZE);
#endif
}
...
static inline void console_back(void) {
  ...
  lcd_putc_xy(console_col * ITP_FONT_WIDTH, console_row * ITP_FONT_HEIGHT, ' ');
}
...
static int LcdConsolePutchar(int c) {
  ...
  if (c == '\n') {
    ithFlushDCacheRange((void *)lcd_base_a,
                        lcd_line_length * ithLcdGetHeight());
    ithFlushDCacheRange((void *)lcd_base_b,
                        lcd_line_length * ithLcdGetHeight());
#ifdef DRAW_VIDEO_BUF
    ithFlushDCacheRange((void *)lcd_base_c,
                        lcd_line_length * ithLcdGetHeight());
#endif
  }
  return c;
...
void LcdConsoleInit(void) {
  ...
  lcd_base_a = (unsigned char *)ithMapVram(ithLcdGetBaseAddrA(),
                                           lcd_line_length * ithLcdGetHeight(),
                                           ITH_VRAM_WRITE);
  lcd_base_b = (unsigned char *)ithMapVram(ithLcdGetBaseAddrB(),
                                           lcd_line_length * ithLcdGetHeight(),
                                           ITH_VRAM_WRITE);
#ifdef DRAW_VIDEO_BUF
  lcd_base_c = (unsigned char *)ithMapVram(ithLcdGetBaseAddrC(),
                                           lcd_line_length * ithLcdGetHeight(),
                                           ITH_VRAM_WRITE);
#endif
...
static int LcdConsoleWrite(int file, char *ptr, int len, void *info) {
  ...
  for (i = 0; i < len && ptr[i] != 0; i++) lcd_putc(ptr[i]);
...
static int LcdConsoleIoctl(int file, unsigned long request, void *ptr,
                           void *info) {
  switch (request) {
...
const ITPDevice itpDeviceLcdConsole = {
    ":lcdconsole",   itpOpenDefault,  itpCloseDefault, itpReadDefault,
    LcdConsoleWrite, itpLseekDefault, LcdConsoleIoctl, NULL};
