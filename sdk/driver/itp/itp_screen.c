...
static void ScreenTvOut (ITPTvOut mode)
{
    ...
    switch (mode)
    {
        ...
        default:
            ITP_LOG_ERR("unknown tvout mode: %d\n", mode);
            return;
    }
...
static void ScreenResume (void)
{
    ...
    switch (screenLastMode)
    {
        ...
        default:
            ITP_LOG_ERR("unknown screen mode: %d\n", screenLastMode);
            return;
    }
...
static int ScreenIoctl (int file, unsigned long request, void * ptr, void * info)
{
    ...
    switch (request)
    {
        case ITP_IOCTL_RESET:
            ...
            ITP_LOG_INFO("LCD width:%d height:%d pitch:%d baseA:0x%X baseB:0x%X baseC:0x%X\n", ithLcdGetWidth(),
                         ithLcdGetHeight(), ithLcdGetPitch(), lcdBaseA, lcdBaseB, lcdBaseC);
        #else
            ITP_LOG_INFO("LCD width:%d height:%d pitch:%d baseA:0x%X baseB:0x%X\n", ithLcdGetWidth(), ithLcdGetHeight(),
                         ithLcdGetPitch(), lcdBaseA, lcdBaseB);
        #endif
