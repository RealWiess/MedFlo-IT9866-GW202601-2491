...
// =============================================================================
    ...
    #define ZONE_ERROR  0x1
        ...
        #define itp_msg(type, string, ...)                                     \
            ((void)((type & LOG_ZONES) ? (void)printf(string, __VA_ARGS__) : 0))
        ...
        #define itp_msg(type, string, args...)                                 \
            ((void)((type & LOG_ZONES) ? (void)printf(string, ##args) : 0))
    ...
    void    * info)
{
    ...
    do
    {
        ...
        if (1)
        {
            ...
            if (!liveT.tv_usec)
            {
                tsd_dbg_get_clock(&liveT);
            }

            if (tsd_dbg_get_duration(&startT) > 60000)
            {
                tsd_dbg_get_clock(&startT);
                (void)printf("\tliveTime: %d sec\n", tsd_dbg_get_duration(&liveT));
            }
        }
#endif
        ...
        if (read_size != len)
        {
            (void)printf(" warning: DemodRead() get wrong size (%d, %d)\n", read_size, len);
        }
    } while (false);
    ...
    void)
{
    ...
    if (g_demodTsCtxt.refCnt == 0)
    {
        ...
        {
            (void)printf("create tsd handle fail (demodId=%d, tsiId=%d) !!\n", presetInfo.demod_id, presetInfo.tsi_id);
        }
        else
        {
            demod_count++;
        ...
        (void)printf(" g_demodTsCtxt.demod_total_cnt=%d\n", g_demodTsCtxt.demod_total_cnt);
        for (i = 0; i < g_demodTsCtxt.demod_total_cnt; i++)
        {
            ...
            if (i < ITP_MAX_DEMOD_SUPPORT_CNT)
            {
                ...
                for (j = ITP_TS_COUNTRY_AUSTRALIA; j < ITP_TS_COUNTRY_CNT; j++)
                {
                    switch (j)
                    {
                        case ITP_TS_COUNTRY_AUSTRALIA:      countryId = TSD_COUNTRY_AUSTRALIA;
                            break;

                        case ITP_TS_COUNTRY_AUSTRIA:        countryId = TSD_COUNTRY_AUSTRIA;
                            break;

                        case ITP_TS_COUNTRY_CHINA:          countryId = TSD_COUNTRY_CHINA;
                            break;

                        case ITP_TS_COUNTRY_FRANCE:         countryId = TSD_COUNTRY_FRANCE;
                            break;

                        case ITP_TS_COUNTRY_GERMANY:        countryId = TSD_COUNTRY_GERMANY;
                            break;

                        case ITP_TS_COUNTRY_GREECE:         countryId = TSD_COUNTRY_GREECE;
                            break;

                        case ITP_TS_COUNTRY_HUNGARY:        countryId = TSD_COUNTRY_HUNGARY;
                            break;

                        case ITP_TS_COUNTRY_ITALY:          countryId = TSD_COUNTRY_ITALY;
                            break;

                        case ITP_TS_COUNTRY_NETHERLANDS:    countryId = TSD_COUNTRY_NETHERLANDS;
                            break;

                        case ITP_TS_COUNTRY_POLAND:         countryId = TSD_COUNTRY_POLAND;
                            break;

                        case ITP_TS_COUNTRY_PORTUGAL:       countryId = TSD_COUNTRY_PORTUGAL;
                            break;

                        case ITP_TS_COUNTRY_RUSSIAN:        countryId = TSD_COUNTRY_RUSSIAN;
                            break;

                        case ITP_TS_COUNTRY_SPAIN:          countryId = TSD_COUNTRY_SPAIN;
                            break;

                        case ITP_TS_COUNTRY_TAIWAN:         countryId = TSD_COUNTRY_TAIWAN;
                            break;

                        case ITP_TS_COUNTRY_UK:             countryId = TSD_COUNTRY_UK;
                            break;

                        case ITP_TS_COUNTRY_DENMARK:        countryId = TSD_COUNTRY_DENMARK;
                            break;

                        case ITP_TS_COUNTRY_SWEDEN:         countryId = TSD_COUNTRY_SWEDEN;
                            break;

                        case ITP_TS_COUNTRY_FINLAND:        countryId = TSD_COUNTRY_FINLAND;
                            break;

                        default:
                        case ITP_TS_COUNTRY_NONE:           countryId = TSD_COUNTRY_UNKNOW;
                            break;
                    }
    ...
    switch (request)
    {
        ...
        case ITP_IOCTL_GET_INFO:
        {
            ...
            for (j = 0; j < demodInfo.ts_service_cnt; j++)
            {
                ...
                if (!pSrvcUserInfo)
                {
                    (void)printf("pSrvcUserInfo = 0x%x\n", pSrvcUserInfo);
                    continue;
                }
