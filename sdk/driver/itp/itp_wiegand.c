...
static Risc1OpenEngine_(void)
{
    ...
    (void)printf("pWiegandCtxt: 0x%X\n", pWiegandCtxt);
    (void)printf("cmd Buffer: 0x%X\n", pWiegandCtxt->cmdBuffer);
    (void)printf("cmd Buffer Size: %d\n", pWiegandCtxt->cmdBufferSize);
    (void)printf("wiegand Buffer: 0x%X\n", pWiegandCtxt->wg1wiegandBuffer);
    (void)printf("wiegand Buffer Size: %d\n", pWiegandCtxt->wg1wiegandBufferSize);
    ...
    int parameter2)
{
    ...
    // (void)printf("oriCommandBuffer = 0x%x\n", oriCommandBuffer);
    ...
    void    * info)
{
    ...
    *(int *)ptr = NULL;
    ...
    if ((bit_count == ctxt->bit_count) && value)
    {
        ...
        (void)printf("sw card_id: %s, bit_count: %d\n", ctxt->card_id, bit_count);
    ...
    void            * info)
{
    ...
    switch (request)
    {
        ...
        case ITP_IOCTL_ENABLE:
            if (ithCodecCommand_(CMD_WIEGAND_ENABLE, ctxt->index, ctxt->gpio_d0, ctxt->gpio_d1) == false)
            {
                (void)printf("WiegandIoctl risc codec is not running");
            }
            break;
