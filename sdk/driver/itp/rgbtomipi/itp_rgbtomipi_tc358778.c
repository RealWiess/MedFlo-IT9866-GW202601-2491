    ...
    uint16_t RegAddr)
{
    ...
    *pdbuf++    = (uint8_t)(RegAddr & 0x00ff);
    ...
    if (0 != (flag = mmpIicReceiveData(IIC_PORT_0, TC358778_IICADDR, dbuf, 2, pdbuf, 2)))
    {
        (void)printf("TC358778_IICADDR I2C Read error, reg = %02x\n", RegAddr);
    ...
    uint16_t    data)
{
    ...
    *pdbuf      = (uint8_t)(data & 0x00ff);
    ...
    if (0 != (flag = mmpIicSendDataEx(IIC_PORT_0, TC358778_IICADDR, dbuf, 4)))
    {
        (void)printf("TC358778_IICADDR I2c write error, reg = %02x val =%02x\n", RegAddr, data);
        // mmpIicGenStop(IIC_PORT_0);
    }
    return flag;
}
