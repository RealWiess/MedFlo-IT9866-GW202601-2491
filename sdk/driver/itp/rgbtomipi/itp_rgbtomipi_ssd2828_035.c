    ...
    void)
{
    ...
    // if(result == 0)
    // {
    //    //ITP_LOG_ERR("MIPI SPI port initial FAIL!! \n", );
    //    return;
    // }
    ...
    unsigned int * data)
{
    ...
    memcpy(data, dBuf, 2);
    // (void)printf(" RdData = %04x, %02x, %02x\n",*data, dBuf[0], dBuf[1] );
}
...
ErrEnd:
    ...
    (void)printf("GOT error, tmp=%x\n", tmp);
    return 0;
}

static void
ssd2828_write_to_lcd (
    unsigned short  * buf,
    int             size)
{
    unsigned int    i   = 0;
    ...
    unsigned char               force_update)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        else if (table[i].cmd == REGFLAG_END_OF_TABLE)
        {
            break;
        }
        else
        {
            ...
            {
                if (j >= sizeof(table[i].para_list))
                {
                    break;
                }

                dcs[1 + j] = table[i].para_list[j];
                // (void)printf("dcs=[%02x, %02x],",dcs[1+j],table[i].para_list[j]);
            }
    ...
    // (void)printf("LCM_setting_table count is:%d,init count is %d\n",count,i);
}
    ...
    void)
{
    ...
    (void)printf("ssd2828_gpio_init::\n");
    ssd2828_gpio_init();

    (void)printf("ssd2828_reset::\n");
    ssd2828_reset();

    (void)printf("ssd2828_mipi_init::\n");
    ssd2828_init();

    for (i = 0; i < 63; i++)
    {
        temp++;
        (void)printf("read %02x=%04x\r\n", temp, ssd_read_register(temp));
    }
}
// =============================================================================
