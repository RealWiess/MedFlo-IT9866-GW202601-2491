...
static int card_init(uint8_t lun)
{
	...
	if(rc)
		ITP_LOG_ERR(" %s(%d): rc=0x%X(%d) \n", __FUNCTION__, lun, rc, rc );
	return rc;
}
...
static int pfsg_read_sector(uint8_t lun, uint32_t blockId, uint32_t sizeInSector, uint8_t *buf)
{
	...
	ITP_LOG_DBG(" %s(%d,%d,%d) rc=0x%X(%d) \n", __FUNCTION__, lun, blockId, sizeInSector, rc, rc );
	return rc;
}
...
static int UsbdFsgGetDeviceOffset(void)
{
	...
	{
		if (strcmp(str, "A") == 0)
			norPartitionOffset = 0x200;
		else if (strcmp(str, "B") == 0)
			norPartitionOffset = 0x200 + disk.size[0]/512;
		else if (strcmp(str, "C") == 0)
			norPartitionOffset = 0x200 + (disk.size[0]+ disk.size[1])/512;
		else if (strcmp(str, "D") == 0)
			norPartitionOffset = 0x400 +(disk.size[0]+ disk.size[1]+ disk.size[2])/512;
		else
		{
