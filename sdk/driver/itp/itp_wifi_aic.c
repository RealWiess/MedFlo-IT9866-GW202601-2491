    ...
    ITPWifiInfo * info)
{
    ...
    if (1)
    {
        ...
        {
            info->netmask = aic_sta_ipv4_netmask();
        }
        else
        {
        ...
        if (WifiCurrentState == STASTATE)
        {
            info->address = aic_sta_ipv4_ipaddr();
        }
        else
        {
    ...
    // memcpy(&info->hardwareAddress, get_mac_address(), 6);
    ...
    void            * info)
{
    ...
    switch (request)
    {
        ...
        case ITP_IOCTL_INIT:
            (void)printf("WifiIoctl ITP_IOCTL_INIT AP62xx\n");
            // sleep to wake up
            wifi_on();
        ...
        case ITP_IOCTL_EXIT:
            (void)printf("WifiIoctl ITP_IOCTL_EXIT \n");
