...
static uint8_t         sample_width        = 0;
static bool            gb_i2s_is_external_ = false;
...
static bool            b_dacRPwait       = false;
    int32_t output = ithReadRegA(I2S_REG_OUT_CTRL) & 0x1; // check i2s enable
    b_dacRPwait = wait;

    if (b_dacRPwait && output)
    {
        int32_t nSampleRate = 0;
        int32_t bytes       = 0;
        int32_t nCh         = ((ithReadRegA(I2S_REG_OUT_CTRL) >> 4) & 0x1) + 1;
        int32_t i           = 0;
...
static void i2s_power_off_ (Resetmode mode)
{
    uint32_t data32;
    data32 = ithReadRegA(MMP_AUDIO_CLOCK_REG_40);
    if (mode == Reset)
    {
        data32 &= ~(((uint32_t)1U) << ((uint32_t)17U)); /* disable AMCLK */
    }
    ...
    if (mode == Reset)
    {
        data32 &= ~((((uint32_t)1U) << ((uint32_t)19U)) | (((uint32_t)1U) << ((uint32_t)21U)) |
                    (((uint32_t)1U) << ((uint32_t)23U))); /* disable ZCLK  (IIS DAC clock)                 */
    }
    if (mode == DAreset)
    {
...
static void i2s_reset_ (Resetmode mode)
{
    uint32_t data32;
    ...
    if (mode == DAreset)
    {
        data32 &= ~(((uint32_t)0xAU) << ((uint32_t)28U)); // normal option DA engine
    }
    if (mode == ADreset)
    {
        data32 &= ~(((uint32_t)0x1U) << ((uint32_t)28U)); // normal option AD engine
    }
    if (mode == Reset)
    {
...
static void i2s_aws_sync_ ()
{
    (void)printf("i2s_aws_sync_ gb_i2s_is_external_ %d\n", gb_i2s_is_external_);
    int32_t tmp = 0;
    if (gb_i2s_is_external_)
    {
        tmp = 0;
    }
    else
    ...
    ithWriteRegA(I2S_REG_CODEC_SET,
                 (((uint32_t)1U) << ((uint32_t)31U)) // ADCWS/DACWS syn
                     ...
                     | (tmp << 2U)                   // select External/Internal DAC
                     | (I2S_SLAVE_MODE << 1U)        // IIS Output Slave/Master Mode
                     | (I2S_SLAVE_MODE << 0U)        // IIS Input Slave/Master Mode
    );
    (void)usleep(10);

    ithWriteRegMaskA(I2S_REG_CODEC_SET, 0, (uint32_t)1 << 31);
}

static void i2s_set_BTclock_ (uint32_t demanded_sample_rate, uint8_t * demanded_sample_width, Resetmode mode)
{
    uint8_t data8 = 0x0;

    (void)printf("I2S# %s, demanded_sample_rate: %u gb_i2s_is_external_ %d\n", __func__, demanded_sample_rate,
                 gb_i2s_is_external_);
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_40, 0x0002B801); // AMCLK PLL2_OUT2 /2 ,[9:0]:amclk_ratio (1)

    if (gb_i2s_is_external_ == 1)
    {
        if (demanded_sample_rate == 48000 || demanded_sample_rate == 8000)
        { //
            // 12.288 MHz
    ...
    else
    {
        ...
        else if (demanded_sample_rate == 8000)
        {
            // 12MHz
    ...
    if (mode == DAreset)
    {
        ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, (0x22A88800U | data8)); // reset DA
    }
    if (mode == ADreset)
    {
        ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, (0x12A88800U | data8)); // reset AD
    }
    if (mode == Reset)
    {
        ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, (0xF2A88800U | data8)); // reset all
    }
...
{
    uint32_t GpioTable[] = {22, 23, 24, 25, 26};
...
void i2s_init_DAC (STRC_I2S_SPEC * i2s_spec)
{
    ...
    (void)printf("I2S# %s Start  %d %d\n", __func__, i2s_spec->sample_rate, i2s_spec->channels);
    ...
    i2s_set_BTclock_(i2s_spec->sample_rate, &sample_width, DAreset);
    ...
    data32 = (resolution_type << ((uint32_t)28U)) /* ADC resolution bits, 00:16bits; 01:24bits; 10:32bits; 11:8bits */
...
void i2s_init_ADC (STRC_I2S_SPEC * i2s_spec)
{
    ...
    i2s_set_BTclock_(i2s_spec->sample_rate, &sample_width, ADreset);
...
{
    gb_i2s_is_external_ = is_external;
}

bool i2s_is_externalCODEC ()
{
    return gb_i2s_is_external_;
}
