 ...
 * @return              TS_DEMUX_QUEUE_ERROR_CODE whether the creation is
 * success
 *                      or not.
 */
    ...
    PID_INFO    * ptPidInfo)
{
     ...
     && (NULL != ptPidInfo))
    {
        ...
        pQ->pOutPesBuffer   = GET32(ptPidInfo->pOutPesBuffer);          // add
                                                                        // memory
                                                                        // offset?
 ...
 * @return          TS_DEMUX_QUEUE_ERROR_CODE wheter the deletion is successed
 * or
 *                  not.
 */
...
TS_DEMUX_QUEUE_ERROR_CODE
TsDemuxQueue_DestroyQueueByIdx_ (
    uint    channelId,
    uint    pesIndex)
{
    QUEUE_DATA_BLOCK * ptQueue;
    ...
    return TsDemuxQueue_DestroyQueue_(ptQueue);
}
    ...
    uint32  * pSampleSize)
{
    ...
    // (void)printf("c(%d)p(%d) w(%d)r(%d)\n", channelId, pesIndex,
    // ptQueue->pesBufferWriteIdx, ptQueue->pesBufferReadIdx);
    ...
    void)
{
    ...
    for (; 0 < i; --i)
    {
        result = TsDemux_GetReturnCode_();
        (void)printf("result(%X), %s(%d)\n", result, __FUNCTION__, __LINE__);
    ...
    uint channelId)
{
    if ((0 <= channelId) && (channelId < MAX_CHANNEL_COUNT))
    {
        ...
        TsDemux_SetCommand_(XCPU_COMMAND_CFG_CHANGE);
        TsDemux_WaitCommandDone_();
        ...
        {
            (void)printf("%s port(%d), service(%d), &(ptChanInfo->tPidInfo[j])(0x%X)\n", __FUNCTION__,
                channelId, i, &(ptChanInfo->tPidInfo[i]));
            TsDemuxQueue_CreateQueue_(channelId, i, &(ptChanInfo->tPidInfo[i]));
        }
    ...
    uint channelId)
{
    if ((0 <= channelId) && (channelId < MAX_CHANNEL_COUNT))
    {
        ...
        for (i = 0; i < validPidCount; ++i)
        {
            TsDemuxQueue_DestroyQueueByIdx_(channelId, i);
        }
        return 0;
    }
    return -1;
...
iteTsDemux_GetNextFrame (
    ...
    uint32  * pFrameSize)
{
    return !TsDemuxQueue_GetReady_(channelId, pesIndex, (void **)ppFrameStartAddr, pFrameSize);
}
...
{
    TsDemuxQueue_SetFree_(channelId, pesIndex, (void **)&pFrameStartAddr);
}
    ...
    void)
{
    ...
    for (i = 0; i < MAX_CHANNEL_COUNT; i++)
    {
        ...
        for (j = 0; j < gtSpsPpsInfoArray.tChannelSpsPpsArray[i].validServiceCount; j++)
        {
            ...
            {
                (void)printf("err ! %d-th channel, %d-th server => spsSize=%d, ppsSize=%d\n", i, j, pVideoSpsPpsInfo->spsSize, pVideoSpsPpsInfo->ppsSize);
            }
        }
