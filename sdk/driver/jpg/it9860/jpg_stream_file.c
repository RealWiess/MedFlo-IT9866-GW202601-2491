    ...
    void                * extraData)
{
    ...
    FILE *  fp     = NULL; // Use a local variable for clarity
    ...
    if (pHJStream)
    {
        ...
        switch (pHJStream->jStreamInfo.streamIOType)
        {
            case JPG_STREAM_IO_READ:
                fp = fopen((char *)pHJStream->jStreamInfo.jstream.path, "rb");
                if (fp == NULL)
                {
                    jpg_msg_ex(JPG_MSG_TYPE_ERR, " open read jpg fail (%s) !!",
                               (char *)pHJStream->jStreamInfo.jstream.path);
                    result = JPG_ERR_OPEN_FAIL; // Indicate open failure
                }
                break;

            case JPG_STREAM_IO_WRITE:
#if 1 // async mode mark it.
                fp = fopen((char *)pHJStream->jStreamInfo.jstream.path, "wb");
                if (fp == NULL)
                {
                    jpg_msg_ex(JPG_MSG_TYPE_ERR, " open write jpg fail (%s) !!",
                               (char *)pHJStream->jStreamInfo.jstream.path);
                    result = JPG_ERR_OPEN_FAIL; // Indicate open failure
                }
#endif
            ...
            default:
                result = JPG_ERR_INVALID_PARAMETER; // Handle unknown type
                break;
        }
        pHJStream->privData[0] = (void *)fp; // Store the file pointer

        // If file opened successfully, proceed
        if ((result == JPG_ERR_OK) && (fp != NULL))
        {
            jpg_msg_ex(0, " pHJStream->privData[0] = %p\n",
            ...
            if (pHJStream->jStreamInfo.streamIOType == JPG_STREAM_IO_READ)
            {
                ...
                {
                    int fseek_result;
                    fseek_result = fseek((FILE *)pHJStream->privData[0], 0, SEEK_END);
                    if (fseek_result != 0)
                    {
                        result = JPG_ERR_FILE_OPER_FAIL;
                        break;
                    }

                    pHJStream->streamSize =
                        ftell((FILE *)pHJStream->privData[0]);
                    fseek_result = fseek((FILE *)pHJStream->privData[0], 0, SEEK_SET);
                    if (fseek_result != 0)
                    {
                        result = JPG_ERR_FILE_OPER_FAIL;
                        break;
        ...
        // If any error occurred after opening, close the file
        if ((result != JPG_ERR_OK) && (fp != NULL))
        {
            fclose(fp);
            pHJStream->privData[0] = NULL;
        }
    }
...
_file_write_callback (
    ...
    void        * arg)
{
    // (void)printf("_file_write_callback ,file = 0x%x , result = %lu ,arg = 0x%x\n",file , result,arg);
    if (arg) // free destBuf
    {
        free(arg);
        // malloc_stats();
    ...
    if (pHJStream)
    {
        ...
        switch (heapType)
        {
            ...
            case JPG_HEAP_BS_BUF:
                ...
                (void)printf("g_vramBsBuf[0] = 0x%x, size= %u\n", g_vramBsBuf[0], requestSize);
    #endif
                break;
            ...
            case JPG_HEAP_LINE_BUF:
                ...
                (void)printf("g_vramLineBuf = 0x%x, size= %u\n", g_vramLineBuf, requestSize);
    #endif
                break;
            ...
            case JPG_HEAP_ENC_BS_RING_BUF:
                ...
                (void)printf("g_vramEncRingBuf = 0x%x, size= %u\n", g_vramEncRingBuf, requestSize);
    #endif
                break;
