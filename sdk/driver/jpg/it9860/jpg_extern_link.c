    ...
    JCOMM_HANDLE * pHJComm)
{
    ...
    jpg_msg(0, "set to Isp out(x, y, w, h, pitch, outAddr, format, rotType) = (%d, %d, %ld, %ld, %ld, %p, %d, %d)\n",
    ...
    {
        case JPG_ROT_TYPE_0:
            (void)mmpIspSetDisplayRotateType(gIspDev, MMP_ISP_ROTATE_0);
            break;

        case JPG_ROT_TYPE_90:
            (void)mmpIspSetDisplayRotateType(gIspDev, MMP_ISP_ROTATE_90);
            break;

        case JPG_ROT_TYPE_180:
            (void)mmpIspSetDisplayRotateType(gIspDev, MMP_ISP_ROTATE_180);
            break;

        case JPG_ROT_TYPE_270:
            (void)mmpIspSetDisplayRotateType(gIspDev, MMP_ISP_ROTATE_270);
            break;
		default:
			; /*Do nothing*/
            break;
    ...
    do
    {
        ...
        if (pJOutBufInfo_0->pBufAddr != NULL)
        {
            ...
            (void)memset((void *)&outInfo, 0, sizeof(MMP_ISP_OUTPUT_INFO));
            outInfo.startX  = (MMP_INT)pJDispInfo->dstX;
            outInfo.startY  = (MMP_INT)pJDispInfo->dstY;
            ...
            if (MjpegFullScreen != 0) // for Mjpeg handshake Decoder.
            {
                outInfo.startX  = 0;
                outInfo.startY  = 0;
                outInfo.width   = ithLcdGetWidth();
            ...
            switch (pJDispInfo->outColorSpace)
            {
                ...
                case JPG_COLOR_SPACE_RGB565:
                    ...
                    outInfo.pitchRGB    = (uint16_t)pJOutBufInfo_0->pitch;
                    ...
                    (void)mmpIspSetOutputWindow(gIspDev, &outInfo);
                    (void)mmpIspSetVideoWindow(gIspDev, 0, 0, outInfo.width, outInfo.height);
                    (void)mmpIspSetOutputFormat(gIspDev, MMP_ISP_OUT_DITHER565A);
                    break;
                ...
                case JPG_COLOR_SPACE_YUV420:
                    ...
                    (void)printf(" isp output: y=0x%lx, u=0x%lx, v=0x%lx, pitchY=%d, pitchUv=%d\n",
        ...
        else
        {
            ispRst = mmpIspSetDisplayWindow(
                gIspDev,
                (MMP_INT)pJDispInfo->dstX,
                (MMP_INT)pJDispInfo->dstY,
                pJDispInfo->dstW,
                pJDispInfo->dstH);
            if (ispRst != ISP_SUCCESS)
            {
        ...
        {
            if ( ((uint32_t)pJColorCtrl->ctrlFlag & (uint32_t)COLOR_COMP_HUE) != 0u )        // Hue
            {
            ...
            if ( ((uint32_t)pJColorCtrl->ctrlFlag & (uint32_t)COLOR_COMP_CONTRAST) != 0u )        // Contrast
            {
            ...
            if ( ((uint32_t)pJColorCtrl->ctrlFlag & (uint32_t)COLOR_COMP_SATURATION) != 0u )        // Saturation
            {
            ...
            if ( ((uint32_t)pJColorCtrl->ctrlFlag & (uint32_t)COLOR_COMP_BRIGHTNESS) != 0u )        // Brightness
            {
    ...
    void            * extraData)
{
    ...
    switch (cmd)
    {
        case JEL_CTRL_ISP_INIT:
            ...
            switch ((JEL_SET_ISP_MODE)value)
            {
                ...
                case JEL_SET_ISP_SHOW_IMAGE:
                    ...
                    // end of for VP1.
                    ispRst                  = mmpIspSetCore(gIspDev, &ISPCOREINFO);
                    if (ispRst != ISP_SUCCESS)
                    {
        ...
        case JEL_CTRL_ISP_HW_RESET:
            ispResetHwEngine((ITHIspCore)MMP_ISP_CORE_0);
            ispResetHwReg((ITHIspCore)MMP_ISP_CORE_0);
            break;
    ...
    #if defined(CFG_LCD_PQ_TUNING) || defined(CFG_JPG_INTERACTIVE_ENABLE)
            (void)pthread_mutex_unlock(&ISP_CORE_0_MUTEX);
    #endif
            break;
        ...
        case JEL_CTRL_ISP_IMG_PROC:
            if (extraData != NULL)
            {
                ...
                {
                    case JPG_COLOR_SPACE_RGB565:    ispShare.format = MMP_ISP_IN_RGB565;
                        break;

                    case JPG_COLOR_SPACE_ARGB8888:  ispShare.format = MMP_ISP_IN_RGB888;
                        break;

                    case JPG_COLOR_SPACE_YUV444:    ispShare.format = MMP_ISP_IN_YUV444;
                        break;

                    case JPG_COLOR_SPACE_YUV422:    ispShare.format = MMP_ISP_IN_YUV422;
                        break;

                    case JPG_COLOR_SPACE_YUV420:    ispShare.format = MMP_ISP_IN_YUV420;
                        break;

                    case JPG_COLOR_SPACE_YUV422R:   ispShare.format = MMP_ISP_IN_YUV422R;
                        break;

                    case JPG_COLOR_SPACE_YUV411:    ispShare.format = MMP_ISP_IN_YUV422;
                        break;
                ...
                {
    #if defined(CFG_LCD_PQ_TUNING) || defined(CFG_JPG_INTERACTIVE_ENABLE)
                    (void)pthread_mutex_lock(&ISP_CORE_0_MUTEX);
    #endif

                    (void)mmpIspPlayImageProcess(gIspDev, &ispShare);
                }
            }
            break;
...
#ifndef CFG_JPEG_DMA_ENABLE
                jpgReadVram(info->dstAddr, info->srcAddr, info->sizeByByte);
#else
				ithDmaMemcpy(info->dstAddr, info->srcAddr, info->sizeByByte);
#endif
            }
            break;
