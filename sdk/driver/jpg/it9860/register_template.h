...
#define DEFINE_REGISTER_TEMPLATE(descType, id_enum_type)     \
    static descType *first_##descType##_item = NULL; \
    static void                                   \
    ...
    descType##_find(                              \
        id_enum_type   id){                       \
        descType *dev = NULL;                        \
        while((dev = descType##_next(dev)) != NULL)  \
...
#define REGISTER_ITEM(type, X, x) { \
    ...
    if((bool)CONFIG_##type##_##X##_DESC)  { _register_##type##_item(&type##_##x##_desc);} }
