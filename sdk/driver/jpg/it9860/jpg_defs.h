 ...
 **/
    ...
    #define jpg_msg(type, string, ...)                                         \
        ((void)((type & jpgMsgOnFlag) ? (void)printf(string, __VA_ARGS__) : 0))
    #define jpg_msg_ex(type, string, ...)                                      \
        do                                                                     \
        {                                                                      \
            if (type & jpgMsgOnFlag)                                           \
            {                                                                  \
                (void)printf(string, __VA_ARGS__);                             \
                (void)printf("  %s [#%d]\n", __FILE__, __LINE__);              \
            }                                                                  \
    ...
    #define jpg_msg(type, string, args...)                                     \
        do                                                                     \
        {                                                                      \
            if ((((uint32_t)type) & jpgMsgOnFlag) != 0u)                       \
            {                                                                  \
                (void)printf(string, ##args);                                  \
            }                                                                  \
        } while (false)
    ...
    #define jpg_msg_ex(type, string, args...)                                  \
        ...
        #define _jpg_trace_enter(type, string, args...)                        \
            do                                                                 \
            {                                                                  \
                jpg_msg(type, "  enter: %s()[#%d], ", __FUNCTION__, __LINE__); \
 ...
 **/
    ...
    #define _jpg_thread_join(pThread, attr)                                    \
        do                                                                     \
        {                                                                      \
            if (pThread)                                                       \
                pthread_join((*pThread), attr);                                \
        } while (false)
    ...
    #define _jpg_mutex_init(type, mutex)                                       \
        ...
        {                                                                      \
            ...
            {                                                                  \
                ...
                jpg_msg(type, " mutex_init: %s, 0x%lx\n", #mutex, mutex);      \
            }                                                                  \
        } while (false)
