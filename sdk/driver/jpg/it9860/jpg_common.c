...
#define SET_BYTE_VALUE(ptr, value)      do {(*(ptr)) = (value); ((ptr)++); } while (false)

#define SET_WORD_VALUE(ptr, wValue)     do { (*(ptr)) = (uint8_t)((wValue) >> 8); ((ptr)++);  \
                                             (*(ptr)) = (uint8_t)(wValue); ((ptr)++); \
} while (false)

#define SET_LOW_BYTE_VALUE(ptr, value)  (*(ptr)) |= (uint8_t)((value) & 0x0fu)
#define SET_HIGH_BYTE_VALUE(ptr, value) (*(ptr)) |= (uint8_t)(((value) << 4) & 0xf0u)
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    do
    {
        ...
        {
            (void)pJStreamDesc->jControl(pJCommDev->hJCodec.pHInJStream,
        ...
        if (pJStreamDesc->jHeap_mem != NULL)
        {
            ...
            if (pJLineBufInfo->addrAlloc == NULL)
            {
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    JPG_ERR         result              = JPG_ERR_OK;
    JPG_STREAM_DESC * pJStreamDesc      = &pJCommDev->hJCodec.pHInJStream->jStreamDesc;
    JPG_BUF_INFO    *pJOutBufInfo_0 = &pJCommDev->hJCodec.pHOutJStream->jStreamInfo.jstream.mem[0];
    JPG_BUF_INFO    *pJOutBufInfo_1 = &pJCommDev->hJCodec.pHOutJStream->jStreamInfo.jstream.mem[1];
    JPG_BUF_INFO    *pJOutBufInfo_2 = &pJCommDev->hJCodec.pHOutJStream->jStreamInfo.jstream.mem[2];
    ...
    do
    {
        ...
        switch (pJDispInfo->outColorSpace)
        {
            ...
            case JPG_COLOR_SPACE_RGB565:
                ...
                if (pJOutBufInfo_0->pBufAddr == NULL)
                    {
                        jpg_msg_ex(JPG_MSG_TYPE_ERR, " %s() Allocate Line buf fail !! ", __FUNCTION__);
                        result = JPG_ERR_ALLOCATE_FAIL;
                        break;
            ...
            if (pJStreamDesc->jHeap_mem != NULL)
                ...
                if (pJOutBufInfo_1->pBufAddr == NULL)
                    {
                        jpg_msg_ex(JPG_MSG_TYPE_ERR, " %s() Allocate Line buf fail !! ", __FUNCTION__);
                        result = JPG_ERR_ALLOCATE_FAIL;
                        break;
                ...
                if (pJOutBufInfo_2->pBufAddr == NULL)
                    {
                        jpg_msg_ex(JPG_MSG_TYPE_ERR, " %s() Allocate Line buf fail !! ", __FUNCTION__);
                        result = JPG_ERR_ALLOCATE_FAIL;
                        break;
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    do
    {
        ...
        if (pJStreamDesc->jHeap_mem != NULL)
        {
            ...
            hdrSize               = ( (6u * 2u) + JPG_ENC_DQT_LENGTH + JPG_ENC_DRI_LENGTH + JPG_ENC_SOF_LENGTH + JPG_ENC_DHT_LENGTH + JPG_ENC_SOS_LENGTH);
            pJHwBsCtrl->pJHdrInfo   = pJStreamDesc->jHeap_mem(pJCommDev->hJCodec.pHInJStream,
                                                            JPG_HEAP_DEF, hdrSize, &pJHwBsCtrl->jHdrInfoSize);
            if ( (pJHwBsCtrl->pJHdrInfo == NULL) ||
    ...
    JPG_COMM_DEV * pJCommDev)
{
	...
	#if 0
    jpg_msg(0, "Image Sample Frequency = (%d %d %d %d %d %d %d %d)  \n",
    ...
    pJFrmComp->realWidth = (uint16_t)((pJFrmComp->imgWidth + ((uint32_t)pJFrmComp->widthUnit - 1u)) & ~((uint32_t)pJFrmComp->widthUnit - 1u));
    pJFrmComp->realHeight = (uint16_t)((pJFrmComp->imgHeight + ((uint32_t)pJFrmComp->heightUnit - 1u)) & ~((uint32_t)pJFrmComp->heightUnit - 1u));
    ...
    do
    {
            ...
            (pJFrmComp->jFrmInfo[0].verticalSamp == 1u))
        {
            ...
            pJCommDev->hJCodec.ctrlFlag |= (uint32_t)JPG_FLAGS_DEC_UV_HOR_DOWNSAMPLE;
            pJFrmComp->decColorSpace    = JPG_COLOR_SPACE_YUV422;
        }
                 ...
                 (pJFrmComp->jFrmInfo[0].verticalSamp == 2u))
        {
            ...
            {
                // Just for test.
                pJCommDev->hJCodec.ctrlFlag |= (uint32_t)JPG_FLAGS_DEC_UV_VER_DUPLICATE;
                pJFrmComp->decColorSpace    = JPG_COLOR_SPACE_YUV422;
            }
        }
                 ...
                 (pJFrmComp->jFrmInfo[0].verticalSamp == 1u))
        {
            ...
            pJCommDev->hJCodec.ctrlFlag |= (uint32_t)JPG_FLAGS_DEC_UV_HOR_DUPLICATE;
            pJFrmComp->decColorSpace    = JPG_COLOR_SPACE_YUV422;
        }
        else
        {
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    do
    {
        ...
        pJFrmSizeInfo->startX        = (uint16_t)pJDispInfo->srcX;
        pJFrmSizeInfo->startY        = (uint16_t)pJDispInfo->srcY;
        pJFrmSizeInfo->dispWidth     = (uint16_t)pJDispInfo->srcW;
        pJFrmSizeInfo->dispHeight    = (uint16_t)pJDispInfo->srcH;

        // verify range
        if ((pJFrmSizeInfo->startX + pJFrmSizeInfo->dispWidth) > pJFrmSizeInfo->realWidth)
        {
            jpg_msg_ex(JPG_MSG_TYPE_ERR, "X = %d, dispWidth = %d, realWidth = %d ! ", pJFrmSizeInfo->startX, pJFrmSizeInfo->dispWidth, pJFrmSizeInfo->realWidth);
        ...
        {
            jpg_msg_ex(JPG_MSG_TYPE_ERR, "X = %d, dispWidth = %d, realWidth = %d ! ", pJFrmSizeInfo->startY, pJFrmSizeInfo->dispHeight, pJFrmSizeInfo->realHeight);
            result = JPG_ERR_INVALID_PARAMETER;
            break;
        }
        ...
        else
        {
            ...
            pJFrmSizeInfo->dispWidth     = (uint16_t)pJFrmCompInfo->imgWidth;
            pJFrmSizeInfo->dispHeight    = (uint16_t)pJFrmCompInfo->imgHeight;
        }
        ...
        {
            jpg_msg_ex(JPG_MSG_TYPE_ERR, "err, mcuDispRight=%d > mcuRealWidth=%d or mcuDispDown=%d > mcuRealHeight=%d !\n", pJFrmSizeInfo->mcuDispRight, pJFrmSizeInfo->mcuRealWidth, pJFrmSizeInfo->mcuDispDown, pJFrmSizeInfo->mcuRealHeight);
            result = JPG_ERR_INVALID_PARAMETER;
            break;
        }
    } while (false);
    ...
    JPG_FRM_COMP    * pJFrmComp)
{
    ...
    SET_WORD_VALUE(ptCur, 0xFFDBu); // FF DB => marker
    SET_WORD_VALUE(ptCur, segLeng);
    *ptCur = 0x00;
    SET_HIGH_BYTE_VALUE(ptCur, 0u);
    SET_LOW_BYTE_VALUE(ptCur, 0u);
    // skip the precision & destination field identifier of Q table
    ptCur++;

    (void)memcpy(ptCur, &pJFrmComp->qTable.table[0][0], 64); // 64 = JPG_Q_TABLE_ELEMENT_NUM
    ptCur   += 64;

    *ptCur  = 0x00;
    SET_HIGH_BYTE_VALUE(ptCur, 0u);
    SET_LOW_BYTE_VALUE(ptCur, 1u);
    // skip the precision & destination field identifier of Q table
    ptCur++;
    (void)memcpy(ptCur, &pJFrmComp->qTable.table[1][0], 64); // 64 = JPG_Q_TABLE_ELEMENT_NUM
    ptCur += 64;
    ...
    uint16_t    restartInterval)
{
    ...
    // to do :
    SET_WORD_VALUE(ptCur, 0xFFDDu); // FF DD => marker
    SET_WORD_VALUE(ptCur, segLeng);
    ...
    JPG_FRM_COMP    * pJFrmComp)
{
    ...
    SET_WORD_VALUE(ptCur, 0xFFC0u); // FF C0 => marker
    SET_WORD_VALUE(ptCur, segLeng);

    // set "8" for the sample precision field of scan segment
    SET_BYTE_VALUE(ptCur, 8);

    SET_WORD_VALUE(ptCur, pJFrmComp->header_height);
    SET_WORD_VALUE(ptCur, pJFrmComp->header_width);
    ...
    JPG_FRM_COMP    * pJFrmComp)
{
    ...
    SET_WORD_VALUE(ptCur, 0xFFC4u); // FF C4 => marker
    SET_WORD_VALUE(ptCur, segLeng);

    for (HTabSel = 0; HTabSel < 2u; HTabSel++)
    {
        // DC Huffman table
        *ptCur = 0x00;
        SET_HIGH_BYTE_VALUE(ptCur, 0u);
        ...
        (void)memcpy(ptCur, &Def_DCHuffTable[HTabSel][0], 28);
        ...
        // AC Huffman table
        *ptCur                                      = 0x00;
        SET_HIGH_BYTE_VALUE(ptCur, 1u);
        SET_LOW_BYTE_VALUE(ptCur, HTabSel);

        ptCur++;

        // copy "Number of Huffman codes of length i"
        (void)memcpy(ptCur, &Def_ACHuffTable[HTabSel][0], 178);
    ...
    JPG_FRM_COMP    * pJFrmComp)
{
    ...
    SET_WORD_VALUE(ptCur, 0xFFDAu); // FF DA => marker
    SET_WORD_VALUE(ptCur, segLeng);
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    do
    {
        ...
        SET_WORD_VALUE(pCur, 0xFFD8u); // FF D8 => marker
    ...
    JPG_FRM_COMP * pJFrmComp)
{
    ...
    do
    {
        ...
        pJFrmComp->jFrmInfo[1].horizonSamp  = 1u;
        pJFrmComp->jFrmInfo[1].verticalSamp = 1u;

        pJFrmComp->jFrmInfo[2].horizonSamp  = 1u;
        pJFrmComp->jFrmInfo[2].verticalSamp = 1u;

        switch (pJFrmComp->encColorSpace)
        {
            case JPG_COLOR_SPACE_YUV422:
            pJFrmComp->jFrmInfo[0].horizonSamp  = 2u;
            pJFrmComp->jFrmInfo[0].verticalSamp = 1u;
                break;
        ...
        pJFrmComp->jFrmInfo[0].qTableSel      = 0u;
        pJFrmComp->jFrmInfo[0].dcHuffTableSel = 0u;
        pJFrmComp->jFrmInfo[0].acHuffTableSel = 0u;

        pJFrmComp->jFrmInfo[1].qTableSel      = 1u;
        pJFrmComp->jFrmInfo[1].dcHuffTableSel = 1u;
        pJFrmComp->jFrmInfo[1].acHuffTableSel = 1u;

        pJFrmComp->jFrmInfo[2].qTableSel      = 1u;
        pJFrmComp->jFrmInfo[2].dcHuffTableSel = 1u;
        pJFrmComp->jFrmInfo[2].acHuffTableSel = 1u;
    } while (false);
...
_jpg_enc_set_q_table (
    ...
    uint32_t        quality)
{
    uint8_t *pQTable_Y = NULL;
    uint8_t *pQTable_UV = NULL;
    uint32_t     i;
    ...
    {
        quality = (quality < 50u) ? (5000u / quality) : (200u - (quality << 1));

        // adjust Q table
        for (i = 0; i < JPG_Q_TABLE_SIZE; i++)
        {
            // luminance
            pQTable_Y[i]  = (uint8_t)(((Def_QTable_Y[i] * quality) + 50u) / 100u);
            pQTable_Y[i]  = (pQTable_Y[i] != 0u) ? pQTable_Y[i] : 1u;

            // chrominance
            pQTable_UV[i] = (uint8_t)(((Def_QTable_UV[i] * quality) + 50u) / 100u);
            pQTable_UV[i] = (pQTable_UV[i] != 0u) ? pQTable_UV[i] : 1u;
        }
    }
    ...
    JPG_COMM_DEV * pJCommDev)
{
    ...
    do
    {
        ...
        pJFrmCompInfo->header_width  = (pJCommDev->jCommInitParam.header_width != 0u) ? (pJCommDev->jCommInitParam.header_width) : (pJCommDev->jCommInitParam.encWidth);
        pJFrmCompInfo->header_height = (pJCommDev->jCommInitParam.header_height != 0u) ? (pJCommDev->jCommInitParam.header_height) : (pJCommDev->jCommInitParam.encHeight);
        pJFrmCompInfo->exColorSpace     = pJCommDev->jCommInitParam.exColorSpace;

        // set encode image component info
        result = _jpg_enc_set_comp_select(pJFrmCompInfo);
        ...
        pJFrmCompInfo->realWidth       = (uint16_t)((pJFrmCompInfo->imgWidth + ((uint32_t)pJFrmCompInfo->widthUnit - 1u)) & ~((uint32_t)pJFrmCompInfo->widthUnit - 1u));
        pJFrmCompInfo->realHeight      = (uint16_t)((pJFrmCompInfo->imgHeight + ((uint32_t)pJFrmCompInfo->heightUnit - 1u)) & ~((uint32_t)pJFrmCompInfo->heightUnit - 1u));
        pJFrmCompInfo->compNum         = 3u;
        ...
        pJHwCtrl->dcHuffW2talCodeLenCnt[0] = 12u;
        pJHwCtrl->acHuffW2talCodeLenCnt[0] = 162u;
        pJHwCtrl->dcHuffW2talCodeLenCnt[1] = 12u;
        pJHwCtrl->acHuffW2talCodeLenCnt[1] = 162u;

        // set Q table
        (void)_jpg_enc_set_q_table(pJFrmCompInfo, pJCommDev->jCommInitParam.encQuality);

        (void)_jpg_enc_gen_header(pJCommDev);
    } while (false);
    ...
    void            * extraData)
{
    ...
    do
    {
        ...
        // ------------------------
        // craete dev info
        pJCommDev = malloc(sizeof(JPG_COMM_DEV));
        if ( pJCommDev == NULL )
        {
            jpg_msg_ex(JPG_MSG_TYPE_ERR, " err, allocate fail !!");
            result = JPG_ERR_ALLOCATE_FAIL;
        ...
        (void)memset(pJCommDev, 0x0, sizeof(JPG_COMM_DEV));
    ...
    void                * extraData)
{
    ...
    pJCommDev = DOWN_CAST(JPG_COMM_DEV, pHJComm, hJComm);
    if ( (pJCommDev != NULL) && (pJCommInitParam != NULL) )
    {
        JPG_CODEC_DESC *pJCodecDesc = NULL;

        do
        {
            JPG_CODEC_HANDLE * pHJCodec = &pJCommDev->hJCodec;

            (void)memcpy(&pJCommDev->jCommInitParam, pJCommInitParam, sizeof(JCOMM_INIT_PARAM));
            ...
            if (pJCommDev->hJCodec.bInitialed == false)
            {
                ...
                // ----------------------------
                // init func
                if ( pJCommDev->jCodecDesc.init != NULL )
                {
    ...
    void            * extraData)
{
    ...
    pJCommDev = DOWN_CAST(JPG_COMM_DEV, pHJComm, hJComm);
    if ( (pJCommDev != NULL) && (pJCommDev->hJCodec.bInitialed == true) )
    {
        JPG_CODEC_HANDLE    * pHJCodec      = &pJCommDev->hJCodec;
        JPG_STREAM_DESC     * pJStreamDesc  = &pJCommDev->hJCodec.pHInJStream->jStreamDesc;
        JPG_HW_BS_CTRL      * pJHwBsCtrl    = &pJCommDev->hJCodec.jHwBsCtrl;
        ...
        if ( pJCommDev->jCodecDesc.deInit != NULL )
        ...
        if (pJStreamDesc->jFree_mem != NULL)
        {
            switch (pJCommDev->jCommInitParam.codecType)
            {
                ...
                case JPG_CODEC_DEC_MJPG:
                    ...
                    if (pJLineBufInfo->addrAlloc != NULL)
                    {
                        pJStreamDesc->jFree_mem(pHJCodec->pHInJStream,
                                                JPG_HEAP_LINE_BUF,
                                                pJLineBufInfo->addrAlloc);
                        pJLineBufInfo->addrAlloc = NULL;
                    }

                    // free display buffer
                    result = _jpg_free_disp_buf(pJCommDev);
                    break;
                ...
                case JPG_CODEC_ENC_JPG:
                    ...
                    if (pJHwBsCtrl->pJHdrInfo != NULL)
                    {
                        pJStreamDesc->jFree_mem(pJCommDev->hJCodec.pHInJStream,
                                                JPG_HEAP_DEF,
                                                pJHwBsCtrl->pJHdrInfo);
                        pJHwBsCtrl->pJHdrInfo = NULL;
                    }
    ...
    void            * extraData)
{
    ...
    pJCommDev = DOWN_CAST(JPG_COMM_DEV, pHJComm, hJComm);
    if ( (pJCommDev != NULL) && (pJCommDev->hJComm.pJFrmComp != NULL) )
    {
        do
        {
            JPG_MULTI_SECT_INFO * pJMultiSectInfo = &pJCommDev->hJCodec.jMultiSectInfo;

            (void)memcpy(&pJCommDev->hJCodec.jFrmCompInfo, pJCommDev->hJComm.pJFrmComp, sizeof(JPG_FRM_COMP));

            if ( (pJCommDev->pre_setting != NULL) && (pJCommDev->hJCodec.bSkipPreSetting == false) )
            {
            ...
            if ( pJCommDev->jCodecDesc.setup != NULL )
            {
                ...
                // case JPG_CODEC_DEC_JPROG:
                    ...
                    case JPG_CODEC_ENC_MJPG:
                    {
                         ...
                         **/
                        pJShare2Isp->width             = (uint16_t)pJCommDev->jCommInitParam.encWidth;
                        pJShare2Isp->height            = (uint16_t)pJCommDev->jCommInitParam.encHeight;
                        ...
                        pJShare2Isp->pitchY            = (uint16_t)pJInBufInfo[0].pitch;
                        pJShare2Isp->pitchUv           = (uint16_t)pJInBufInfo[1].pitch;
                        pJShare2Isp->colorSpace         = pJCommDev->jCommInitParam.encColorSpace;
                        pJShare2Isp->sliceCount        = (uint16_t)(pJMultiSectInfo->section_hight >> 3);
                    }
                    break;
                    default:
					; /*Do nothing*/
        ...
        do
        {
            ...
            switch (pJCommDev->jCommInitParam.codecType)
            {
                case JPG_CODEC_DEC_JPG:
                    ...
                    shiftByteNum                    = ((uint32_t)pTmpBsAddr & 0x3u);
                    pJCommDev->hJCodec.sysBsBufSize = (((pJInBufInfo[pHJComm->actIOBuf_idx].bufLength + shiftByteNum) + 0x3u) & ~0x3u);
                        pJCommDev->hJCodec.pSysBsBuf    = pJInBufInfo[pHJComm->actIOBuf_idx].pBufAddr;
                    }
                    else
                    {
                ...
                case JPG_CODEC_DEC_MJPG:
                {
                    ...
                    pJHwBsCtrl->shiftByteNum        = ((uint32_t)pTmpBsAddr & 0x3u);
                    pJCommDev->hJCodec.sysBsBufSize = (((pJInBufInfo[pHJComm->actIOBuf_idx].bufLength + pJHwBsCtrl->shiftByteNum) + 0x3u) & ~0x3u);
                    ...
                    if (pJCommInit->pJDispInfo->outColorSpace != JPG_COLOR_SPACE_RGB565) //  mjpeg handshake mode will pass it here  , Benson
                    {
                        ...
                        pJLineBufInfo->comp1Pitch  = (uint16_t)pJOutBufInfo[0].pitch;
                        pJLineBufInfo->comp2Addr    = pJOutBufInfo[1].pBufAddr;
                        pJLineBufInfo->comp23Pitch = (uint16_t)pJOutBufInfo[1].pitch;
                ...
                case JPG_CODEC_ENC_JPG:
                    ...
                    pJLineBufInfo->comp1Pitch  = (uint16_t)pJInBufInfo[0].pitch;
                        pJLineBufInfo->comp2Addr    = pJInBufInfo[1].pBufAddr;
                    pJLineBufInfo->comp23Pitch = (uint16_t)pJInBufInfo[1].pitch;
                        pJLineBufInfo->comp3Addr    = pJInBufInfo[2].pBufAddr;
                    pJLineBufInfo->sliceNum    = (uint16_t)(pJMultiSectInfo->section_hight >> 3);
                    }
                    break;
                default:
				; /*Do nothing*/
            ...
            if ( pJCommDev->jCodecDesc.fire != NULL )
            {
    ...
    void            * extraData)
{
    ...
    if (pJCommDev != NULL)
    {
        if (pJCommDev->jCodecDesc.control != NULL)
        {
            result = pJCommDev->jCodecDesc.control(&pJCommDev->hJCodec, cmd, value, extraData);
        }
    }

    if ( (result != JPG_ERR_OK) && (result != JPG_ERR_NO_IMPLEMENT) )
    {
        jpg_msg_ex(JPG_MSG_TYPE_ERR, " %s() err 0x%x !! ", __FUNCTION__, result);
    }
    _jpg_trace_leave(JPG_MSG_TYPE_TRACE_JCOMM);
