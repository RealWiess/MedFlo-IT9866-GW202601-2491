...
#ifdef CFG_JPEG_FAST_ENCODE_ENABLE
#define DEF_BS_RING_BUF_LENGTH      (2048 << 10)
#else
#define DEF_BS_RING_BUF_LENGTH      (64 << 10)
#endif
#define DEF_ENC_SYS_BS_BUF_LENGTH   (4096 << 10)    // 256 << 10)
...
#else
				ithDmaMemcpy((uint32_t)pHJStream->privData[0] + pHJStream->curBsPos, (uint32_t)destBuf, length);
#endif
...
#if (_MSC_VER)
            case JPG_STREAM_CMD_GET_VRAM_LINE_BUF:      if (value)
                {
                    *value = g_vramLineBuf;
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_BS_BUF_A:      if (value)
                {
                    *value = g_vramBsBuf[0];
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_BS_BUF_B:      if (value)
                {
                    *value = g_vramBsBuf[1];
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_ENC_RING_BUF:  if (value)
                {
                    *value = g_vramEncRingBuf;
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_ENC_YUV_BUF:   if (value)
                {
                    *value = g_vramYuvBuf;
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_DISP_BUF_0:    if (value)
                {
                    *value = g_vramDispBuf[0];
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_DISP_BUF_1:    if (value)
                {
                    *value = g_vramDispBuf[1];
                }
                break;

            case JPG_STREAM_CMD_GET_VRAM_DISP_BUF_2:    if (value)
                {
    ...
    if (pHJStream)
    {
        ...
        switch (heapType)
        {
            ...
            case JPG_HEAP_BS_BUF:
                ...
                (void)printf("g_vramBsBuf[0] = 0x%x, size= %d\n", g_vramBsBuf[0], pHJStream->streamSize);
    #endif
                break;
            ...
            case JPG_HEAP_ENC_YUV_BUF:
                ...
                (void)printf("g_vramYuvBuf = 0x%x, size= %u\n", g_vramYuvBuf, requestSize);
    #endif
                break;
