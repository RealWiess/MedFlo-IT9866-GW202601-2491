...
_MJPG_Enc_HW_Wait_Idle(
    ...
    JPG_REG       *bufStatus)
{
#define JPG_ENC_STOP_STATUS     ((JPG_REG)JPG_STATUS_BITSTREAM_BUF_FULL | (JPG_REG)JPG_STATUS_ENCODE_COMPLETE | (JPG_REG)JPG_STATUS_LINE_BUF_EMPTY)
    ...
    bool                *bEncIdle)
{
    ...
    do{
        ...
        uint32_t        sysRamRemainSize = 0;

        *bEncIdle = false;

        //----------------------------------------
        // check system bs buffer full or not
        sysRamRemainSize = (uint32_t)pHJCodec->pSysBsBuf + pHJCodec->sysBsBufSize - (uint32_t)pMjEncoder->pSysBsBuf_Cur;
        if( sysRamRemainSize < 2u ) // reserve last 2 bytes
        {
            // sys bs buffer full
        ...
        if( bsBufValidSize > sysRamRemainSize )
        {
            // sys bs buffer full
            result = JPG_ERR_ENC_OVER_BS_BUF;
            jpg_msg_ex(JPG_MSG_TYPE_ERR, " Jpg err 0x%x (Need to extend AP bit stream buffer) !", result);
    ...
    void                *extraData)
{
    ...
    do{
        ...
        if( pJStreamDesc->jHeap_mem != NULL )
        {
            ...
            (void)memset(pHJCodec->privData, 0x0, sizeof(MJPG_ENCODER));
    ...
    void                *extraData)
{
    ...
    do{
        ...
        if( (pHJCodec->ctrlFlag & (uint32_t)JPG_FLAGS_MJPG_FIRST_FRAME) != 0u )
        {
            ...
            // set HW register
            (void)_MJPG_Enc_HW_Update(pHJCodec);
        }
        else
        {
            JPG_LINE_BUF_INFO   *pJLineBufInfo = &pHJCodec->jLineBufInfo;
            ...
            JPG_SetBitstreamBufRwSizeReg(ringBsSize & ~0x3u);
			#if 0
    ...
    void                *extraData)
{
    ...
    do{
        ...
        // --------------------------------
        // Set slice number
        procSliceNum = (uint32_t)pJLineBufInfo->sliceNum;
        ...
        JPG_SetLineBufCtrlReg(((JPG_REG)JPG_MSK_LINE_BUF_WRITE_END | (JPG_REG)JPG_MSK_LAST_ENCODE_DATA));
        ...
        pHJCodec->sysValidBsBufSize = ((uint32_t)pMjEncoder->pSysBsBuf_Cur - (uint32_t)pHJCodec->pSysBsBuf);
    }while(false);
