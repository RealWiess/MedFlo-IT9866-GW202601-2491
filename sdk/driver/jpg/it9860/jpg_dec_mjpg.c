    ...
    bool                * bEnd)
{
    ...
    do
    {
        ...
        *bEnd       = false;
        ...
        result = _MJPG_Wait_Bs_Buf_Empty(JPG_DEC_TIMEOUT_COUNT);
        if (result != JPG_ERR_OK)
        {
    ...
    JPG_CODEC_HANDLE * pHJCodec)
{
    ...
    do
    {
        ...
        JPG_FRM_SIZE_INFO   * pJFrmSizeInfo = &pHJCodec->jFrmSizeInfo;
        JPG_FRM_COMP        * pJFrmComp     = &pHJCodec->jFrmCompInfo;
        uint16_t            real_w          = pJFrmSizeInfo->realWidth;
        ...
        {
            pJLineBufInfo->comp1Pitch = (uint16_t)(((uint32_t)real_w + 0x7u) & ~0x7u);
            if (pJLineBufInfo->comp1Pitch < 2048u)
            {
                pJLineBufInfo->comp1Pitch = 2048u;
            }
    ...
    do
    {
        ...
        if (pJStreamDesc->jHeap_mem != NULL)
        {
            ...
            pHJCodec->privData = pJStreamDesc->jHeap_mem(
                ...
                sizeof(MJPG_DEOCDER), NULL);
            if (pHJCodec->privData != NULL)
            {
                (void)memset(pHJCodec->privData, 0x0, sizeof(MJPG_DEOCDER));
            }
        }
    } while (false);
    ...
    void                * extraData)
{
    ...
    do
    {
        ...
        if (pHJComm->pJDispInfo->outColorSpace == JPG_COLOR_SPACE_RGB565)
        {
            ...
            if (result != JPG_ERR_OK)
            {
                (void)printf("jpg_dec_mjpeg something wrong!\n");
            }
        }
        else
        {
        ...
        if ( (pHJCodec->ctrlFlag & (uint32_t)JPG_FLAGS_MJPG_FIRST_FRAME) != 0u )
        {
            ...
            {
                pJHwCtrl->codecCtrl = ((uint16_t)JPG_OP_DECODE | (uint16_t)pMjDecoder->triggerMode);
            }
    ...
    void                * extraData)
{
    ...
    do
    {
        ...
        else
        {
            ...
            // (void)printf("actBsSize = %d\n",actBsSize);  This is important.

            while ( (actBsSize > 0u) && (bEnd == false) )
            {
                (void)usleep(1000);
            ...
            if (pHJCodec->bLastSection == true)
            {
                result = _MJPG_HW_Wait_Idle(JPG_DEC_TIMEOUT_COUNT);
                if (result != JPG_ERR_OK)
                {
                    jpg_msg_ex(JPG_MSG_TYPE_ERR, " Jpg Err !!");
                }
            ...
            if (result != JPG_ERR_OK)
            {
                jpg_msg_ex(JPG_MSG_TYPE_ERR, " Isp err, err 0x%x !! ", result);
                JPG_LogReg(true);
            }
        }
    } while (false);
