    ...
    JPG_DISP_INFO   * pJDispInfo)
{
    if ((bExifOrientation == true) && (decType == JPG_DEC_PRIMARY))
    {
        switch (exifOrientationType)
        {
            ...
            case 1: pJDispInfo->rotType = JPG_ROT_TYPE_0;
                break;                                              // (row, column) = (T, L)

            case 2:             break;                              // (row, column) = (T, R) => MIRROR

            case 3: pJDispInfo->rotType = JPG_ROT_TYPE_180;
                break;                                              // (row, column) = (B, R)

            case 4:             break;                              // (row, column) = (B, L) => FLIP;

            case 5:             break;                              // (row, column) = (L, T)

            case 6: pJDispInfo->rotType = JPG_ROT_TYPE_90;
                break;                                              // (row, column) = (R, T)

            case 7:             break;                              // (row, column) = (R, B)

            case 8: pJDispInfo->rotType = JPG_ROT_TYPE_270;
                break;                                              // (row, column) = (L, B)
        }
    ...
    JPG_DEV * pJpgDev)
{
    ...
    if (pJpgDev->initParam.encQuality > 90)
    {
        ratio = 4;
    }
    else if (pJpgDev->initParam.encQuality > 85)
    {
        ratio = 10;
    }
    else
    ...
    uint32_t        * pJpgSize)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (AsyncSaveAllAddr)
            {
                (void)printf("AsyncSaveAllAddr has memory leak issue , addr= 0x%x\n", AsyncSaveAllAddr);
            }
            ...
            if (pJpgDev->pExifInfo)
            {
                ...
                {
                    (void)printf("open AsyncSaveAllAddr fail\n");
                }
...
#define SET_OUTPUT_INFO 0xCC
        ...
        while (curState != SET_UNKNOW_INFO)
        {
            ...
            if (bSkip == false)
            {
                ...
                {
                    case JPG_STREAM_FILE:   pHJtStream->jStreamDesc = jpg_stream_file_desc;
                        break;

                    case JPG_STREAM_MEM:    pHJtStream->jStreamDesc = jpg_stream_mem_desc;
                        break;
    ...
    JPG_EXTRA_INFO)
{
    ...
    if (pJpgDev && (pJpgDev->status != JPG_STATUS_FAIL))
    {
        switch (pJpgDev->initParam.codecType)
        {
            case JPG_CODEC_DEC_JPG_CMD: result  = _jpg_dec_cmd_setup(pJpgDev);
                break;

            case JPG_CODEC_DEC_JPG:     result  = _jpg_dec_setup(pJpgDev);
                break;

#if (CONFIG_JPG_CODEC_DESC_DEC_MJPG_DESC)
            case JPG_CODEC_DEC_MJPG:    result = _jpg_dec_mjpg_setup(pJpgDev);
                break;
#endif

#if (CONFIG_JPG_CODEC_DESC_ENCODER_DESC)
            case JPG_CODEC_ENC_JPG:     result = _jpg_enc_setup(pJpgDev);
                break;
#endif

#if (CONFIG_JPG_CODEC_DESC_ENC_MJPG_DESC)
            case JPG_CODEC_ENC_MJPG:    result = _jpg_enc_mjpg_setup(pJpgDev);
                break;
#endif
    ...
    void            * extraData)
{
    ...
    if (pJpgDev && pJpgUserInfo)
    {
        ...
        if (pJpgDev->status != JPG_STATUS_FAIL)
        {
            ...
            // (void)printf("ite_jpg.c pJpgUserInfo->colorFormate = %d\n",pJpgUserInfo->colorFormate);
        }
        else
        {
