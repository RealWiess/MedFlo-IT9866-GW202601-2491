...
#define RTK_API_LOCK()     do { if (!ithCpuInInterrupt()) pthread_mutex_lock(&api_mutex);  } while (false)
#define RTK_API_UNLOCK()   do { if (!ithCpuInInterrupt()) pthread_mutex_unlock(&api_mutex);  } while (false)
#else
#define RTK_API_LOCK()
