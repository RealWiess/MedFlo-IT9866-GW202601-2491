...
WaitTxVidStable ();
static void
SetInputMode (BYTE                              InputMode,
                                        BYTE    bInputSignalType);
static void
SetCSCScale (BYTE                               bInputMode,
                                        BYTE    bOutputMode);
// static void SetupAFE(BYTE ucFreqInMHz);
static void
SetupAFE (VIDEOPCLKLEVEL PCLKLevel);
static void
FireAFE ();
...
HDCP_GetVr (BYTE * pVr);
static SYS_STATUS
HDCP_GetBCaps (PBYTE                                        pBCaps,
                                              unsigned int  * pBStatus);
static SYS_STATUS
...
static SYS_STATUS
HDCP_GetKSVList (BYTE                                   * pKSVList,
                                                 BYTE   cDownStream);
static SYS_STATUS
HDCP_CheckSHA (BYTE                                     M0[],
...
_CODE RegSetEntry   HDMITX_DefaultVideo_Table[] = {
    ...
    ////////////////////////////////////////////////////
    {0x72, 0xff, 0x00},
    {0x70, 0xff, 0x00},// input  PCLK delay 0x0~0x3
...
//////////////////////////////////////////////////////////////////////
BOOL
SetupVideoInputSignal_66121 (
    BYTE inputSignalType)
{
...
//    HDMITX_WriteI2C_Byte_66121(REG_TX_SW_RST,          B_AUD_RST_HDMITX|B_AREF_RST|B_HDCP_RST_HDMITX);
    ...
    HDMITX_DEBUG_PRINTF(("=====Fire IT66121 AFE=====\n"));
    FireAFE();
    return TRUE;
}
#if 0
    ...
    BYTE bEnable)
{
    ...
    if (bEnable)
    {
        ...
        {
            HDMITX_DEBUG_PRINTF(("hdmitx_drv.c(%d), 66121 autenticate is failed\n", __LINE__));
        ...
        {
            HDMITX_DEBUG_PRINTF(("hdmitx_drv.c(%d), 66121 autenticate is success\n", __LINE__));
        }
    }
    else
    {
...
BOOL
EnableAudioInfoFrame_66121 (
    BYTE    bEnable,
    BYTE    * pAudioInfoFrame)
{
    if (!bEnable)
    {
        DISABLE_AVI_INFOFRM_PKT();
...
void
SetOutputColorDepthPhase_66121 (
    BYTE    ColorDepth,
    BYTE    bPhase)
{
    BYTE    uc;
...
//    // HDES = 190, HDEE = 2110, VDES = 19, VDEE = 559, VDS2 = 581, VDE2 = 1121
    ...
    {VESA_1366X768p60,   0x07, 0x62, 0xB8, 0x61, 0x1A, 0x1A, 0x30, 0xFF, 0xFF, 0xFF},   // *
    // HDES = 354, HDEE = 1720, VDES = 26, VDEE = 794
    ...
    {VESA_1400x1050p60,  0x05, 0x76, 0xEE, 0x61, 0x23, 0x3D, 0x40, 0xFF, 0xFF, 0xFF},   // *
    // HDES = 374, HDEE = 1774, VDES = 35, VDEE = 1085
...
static _CODE SYNCGEN_DEGEN_Setting  YPBPR_DeGen_Table[] =
{
    //                          0x90,    0x91,  0x92,  0x93,   0x94,   0x95, 0x96,    0x97, 0x98 ,  0x99  , 0x9A, 0x9B,   0x9C,   0x9D,  0x9E,   0x9F,   0xA0, 0xA1,   0xA2,  0xA3  ,0xA4 , 0xA5   0xA6
    {CEA_720x480i60,   0xb9,  0x1a, 0x87, 0x57, 0x30, 0x0F, 0x4D, 0x00, 0x0d, 0x02, 0x0E, 0xFE, 0x00, 0x15, 0x05, 0x21, 0x0A, 0xf0, 0x10, 0x51, 0xf0, 0x00, 0x10},
    {CEA_720x576i50,   0xe9,  0x1a, 0x8D, 0x5D, 0x30, 0x08, 0x47, 0x00, 0x71, 0x02, 0x12, 0x32, 0x10, 0x4B, 0x6B, 0x21, 0x0A, 0xF0, 0x43, 0x81, 0xf0, 0x00, 0x40},
    {CEA_1920x1080i50, 0xaf,  0x44, 0xe2, 0x62, 0x80, 0x02, 0x2e, 0x00, 0x60, 0x04, 0x13, 0x2f, 0x20, 0x46, 0x62, 0x42, 0x0a, 0xf0, 0x3d, 0x22, 0x30, 0x00, 0x40},
    {CEA_1920x1080i60, 0xaf,  0x44, 0xe2, 0x62, 0x80, 0x02, 0x2e, 0x00, 0x64, 0x04, 0x13, 0x2f, 0x20, 0x46, 0x62, 0x42, 0x0a, 0xf0, 0x3d, 0x22, 0x30, 0x00, 0x40},
    {CEA_720x480p60,   0x99,  0x35, 0x78, 0x48, 0x30, 0x58, 0x3c, 0x03, 0x00, 0x00, 0x23, 0x03, 0x20, 0xff, 0xff, 0xff, 0x0c, 0x52, 0xff, 0x5f, 0xf0, 0x00, 0x00},
    {CEA_720x576p50,   0xf9,  0x35, 0x84, 0x54, 0x30, 0x00, 0x40, 0x00, 0x00, 0x00, 0x2a, 0x6a, 0x20, 0xff, 0xff, 0xff, 0x70, 0x42, 0xff, 0x3f, 0xf0, 0x00, 0x00},
    {CEA_1280x720p50,  0x7f,  0x16, 0x36, 0x36, 0x61, 0x02, 0x2a, 0x00, 0xd0, 0x02, 0x18, 0xe8, 0x20, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x4f, 0xf0, 0x00, 0x00},
    {CEA_1280x720p60,  0x7f,  0x16, 0x36, 0x36, 0x61, 0x02, 0x2a, 0x00, 0xd0, 0x02, 0x18, 0xe8, 0x20, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00},
    {CEA_1920x1080p60, 0xff,  0xff, 0xe0, 0x60, 0x80, 0x00, 0x2c, 0x00, 0x60, 0x04, 0x2a, 0x62, 0x40, 0xff, 0xff, 0x00, 0x01, 0x60, 0xff, 0xf0, 0xf0, 0x00, 0x00},
    {CEA_1920x1080p50, 0x9f,  0x78, 0xda, 0x5a, 0x80, 0x20, 0x4c, 0x00, 0x30, 0x04, 0x27, 0x5f, 0x40, 0xff, 0xff, 0xff, 0x64, 0x34, 0xff, 0x3f, 0x00, 0x00, 0x00},
    {CEA_1920x1080p24, 0xdf,  0xAB, 0xeB, 0x6B, 0x80, 0x2b, 0x57, 0x00, 0x60, 0x04, 0x27, 0x5f, 0x40, 0xff, 0xff, 0xff, 0x64, 0x34, 0xff, 0x3f, 0x00, 0x00, 0x00},
};
static _CODE SYNCGEN_DEGEN_Setting  CVBS_DeGen_Table[] =
{
    {CEA_720x480i60, 0xb9, 0x1a, 0x87, 0x57, 0x30, 0x0F, 0x4D, 0x00, 0x0d, 0x02, 0x0E, 0xFE, 0x00, 0x15, 0x05, 0x21, 0x0A, 0xf0, 0x10, 0x51, 0xf0, 0x00, 0x10},
    ...
    if ((bInputSignalType & (T_MODE_DEGEN | T_MODE_SYNCGEN | T_MODE_SYNCEMB)) == (T_MODE_DEGEN))
    {
        switch (inputDevice)
        {
            ...
            case TXIN_CVBS:
                ...
                HDMITX_WriteI2C_Byte_66121(0x98, CVBS_DeGen_Table[i].Reg98);
                HDMITX_WriteI2C_Byte_66121(0x99, CVBS_DeGen_Table[i].Reg99);
                ...
                HDMITX_WriteI2C_Byte_66121(0xA6, CVBS_DeGen_Table[i].RegA6);
                HDMITX_WriteI2C_Byte_66121(0xB1, 0x0);
                HDMITX_WriteI2C_Byte_66121(0xB2, 0x0);
                return TRUE;
            ...
            case TXIN_YPBPR:
                ...
                HDMITX_WriteI2C_Byte_66121(0x98, YPBPR_DeGen_Table[i].Reg98);
                HDMITX_WriteI2C_Byte_66121(0x99, YPBPR_DeGen_Table[i].Reg99);
                ...
                HDMITX_WriteI2C_Byte_66121(0xA6, YPBPR_DeGen_Table[i].RegA6);
                HDMITX_WriteI2C_Byte_66121(0xB1, 0x0);
                HDMITX_WriteI2C_Byte_66121(0xB2, 0x0);
                return TRUE;
...
BOOL
ProgramSyncEmbeddedByVIC_66121 (
    BYTE    VIC,
    BYTE    bInputSignalType)
{
    int i;
...
BOOL
ProgramSyncEmbeddedByTiming_66121 (
    VIDEO_Timing    * pVTiming,
    BYTE            bInputSignalType)
{
    ...
    BYTE    Fs)
{
    ...
    if (aVIC)
    {
        switch (Fs)
        {
            case AUDFS_32KHz: n = 4096;
                break;

            case AUDFS_44p1KHz: n = 6272;
                break;

            case AUDFS_48KHz: n = 6144;
                break;

            case AUDFS_88p2KHz: n = 12544;
                break;

            case AUDFS_96KHz: n = 12288;
                break;

            case AUDFS_176p4KHz: n = 25088;
                break;

            case AUDFS_192KHz: n = 24576;
                break;
    ...
    else
    {
        switch (Fs)
        {
            case AUDFS_32KHz: SampleFreq = 32000L;
                break;

            case AUDFS_44p1KHz: SampleFreq = 44100L;
                break;

            case AUDFS_48KHz: SampleFreq = 48000L;
                break;

            case AUDFS_88p2KHz: SampleFreq = 88200L;
                break;

            case AUDFS_96KHz: SampleFreq = 96000L;
                break;

            case AUDFS_176p4KHz: SampleFreq = 176000L;
                break;

            case AUDFS_192KHz: SampleFreq = 192000L;
                break;
...
static SYS_STATUS
SetAudioInfoFrame (
    Audio_InfoFrame * pAudioInfoFrame)
{
    BYTE uc;
...
void
setHDMITX_UpdateChStatFs_66121 (
    ULONG Fs)
{
    BYTE uc;
...
void
EnableHDMIAudio_66121 (
    BYTE    AudioType,
    BOOL    bSPDIF,
    ULONG   SampleFreq,
    BYTE    ChNum,
    BYTE    * pIEC60958ChStat,
    ULONG   TMDSClock)
{
    ...
    if (AudioType != T_AUDIO_DSD)
    {
        ...
        switch (SampleFreq)
        {
            case  44100L: Fs = AUDFS_44p1KHz;
                break;

            case  88200L: Fs = AUDFS_88p2KHz;
                break;

            case 176400L: Fs = AUDFS_176p4KHz;
                break;

            case  32000L: Fs = AUDFS_32KHz;
                break;

            case  48000L: Fs = AUDFS_48KHz;
                break;

            case  96000L: Fs = AUDFS_96KHz;
                break;

            case 192000L: Fs = AUDFS_192KHz;
                break;

            case 768000L: Fs = AUDFS_768KHz;
                break;
...
BOOL
EnableVendorSpecificInfoFrame_66121 (
    BYTE    bEnable,
    BYTE    * pInfoFrame)
{
    BYTE    checksum;
...
BOOL
getHDMITX_AuthenticationDone ()    // 2015.3.24, my.wei add
{
