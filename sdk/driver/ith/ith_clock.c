...
static uint32_t     ClkApb2Val = 0U, ClkAhbVal = 0U, ClkAxiVal = 0U, ClkMem2Val = 0U, ClkRiscVal = 0U;
static uint32_t     ClkCapVal = 0U, ClkDPVal = 0U;
...
#define MEMWRAPCLK_REG             (ITH_HOST_BASE + 0x0014)
#define MEMWRAPCLK_SRC_SEL_OFFSET  12
#define MEMWRAPCLK_SRC_SEL_MASK    0x7
#define MEMWRAPCLK_RATIO_OFFSET    0
#define MEMWRAPCLK_RATIO_MASK      0x3FF
...
void ithSetCpuClockRatioMultiple(unsigned int ratio , unsigned int multiple)
{
...
void ithSetMemClockRatioMultiple(unsigned int ratio , unsigned int multiple)
{
    unsigned int value = 0;
...
void ithSetBusClockRatioMultiple(unsigned int ratio , unsigned int multiple)
{
    unsigned int value = 0;
...
unsigned int ithGetCpuClock(void)
{
    ...
    if (clkCpu == 0)
    {
        unsigned int src = (ithReadRegA(AXICLK_REG) >> AXICLK_SRC_SEL_OFFSET) & AXICLK_SRC_SEL_MASK;
        unsigned int div = (ithReadRegA(AXICLK_REG) >> AXICLK_RATIO_OFFSET) & AXICLK_RATIO_MASK;

        unsigned int clk = GetSrcClk(src);
        div    = div != 0 ? div : 1;
        clkCpu = clk / div;
        (void)printf("clk=%d clkCpu=%u\n", clk, clkCpu);
        div    = (ithReadRegA(ITH_HOST_BASE + 0x8) >> 0) & 0x3;
        div    = div != 0 ? div : 1;
        clkCpu = clkCpu / div;
        (void)printf("clkCpu=%u\n", clkCpu);
    }
    #endif
    return clkCpu;
    #endif // CFG_ITH_FPGA
...
#else
    uint32_t div = (ithReadRegA((ITH_HOST_BASE + 0x0008)) & 0x3);

    if (div == 0)
    {
        div = 1;
    }
    return GetArmCpuClock() / div;
#endif
}
 ...
 * Get APB2 CLK in Hz
 */
unsigned int ithGetAPB2clk(void)
{
#if (CFG_CHIP_FAMILY == 9860)
		uint32_t src = ((ithReadRegA(APB2CLK_REG) >> APB2CLK_SRC_SEL_OFFSET) & APB2CLK_SRC_SEL_MASK);
		uint32_t div = ((ithReadRegA(APB2CLK_REG) >> APB2CLK_RATIO_OFFSET) & APB2CLK_RATIO_MASK);
 ...
 * Get MCLK in Hz
 */
unsigned int ithGetMclk(void)
{
#if (CFG_CHIP_FAMILY == 9860)
    uint32_t src = ((ithReadRegA(MEMWRAPCLK_REG) >> MEMWRAPCLK_SRC_SEL_OFFSET) & MEMWRAPCLK_SRC_SEL_MASK);
    uint32_t div = ((ithReadRegA(MEMWRAPCLK_REG) >> MEMWRAPCLK_RATIO_OFFSET) & MEMWRAPCLK_RATIO_MASK);
...
#define DIV_ROUND(a, b) (((a) + ((b) / 2)) / (b))
    ...
    if (!SDM_Sel)
    {
            ITH_LOG_ERR("it dose not support spread spectrum on non-SDM PLL; "
                        "reg 0x%X=0x%X\n",
    ...
    switch (sdm_dv)
    {
    case 0: sdm += (int)(16 * 1024);
        break;
    case 1: sdm += (int)(17 * 1024);
        break;
    case 2: sdm += (int)(18 * 1024);
        break;
    case 3: sdm += (int)(16.5f * 2048);
        break;
    default:
        /* Do nothing */
        break;
    ...
    case 2: sp_n = 4;
        break;                   // center-spread
    default:
        ITH_LOG_ERR("unknown mode of spread spectrum\n");
        return;
    }
    ...
    for (sp_stepy = 1; sp_stepy <= 63; sp_stepy++)
    {
        ...
        if (sp_level < 2 || sp_level > 511)
            continue;

        sp_stepx = DIV_ROUND(n2, sp_level) - 1;
        if (sp_stepx < 0 || sp_stepx > 63)
            continue;
    ...
    if (sp_stepy >= 63)
    {
        ITH_LOG_ERR("setting spread spectrum fails\n");
        return;
    }
    ...
    ITH_LOG_INFO("PLL %d spread spectrum: modulation width=%4.2f%%, modulation frequency=%d\n", pll, (float)sp_stepy * sp_level * 100.0f / sdm, CFG_OSC_CLK / (sp_n * sp_level * (sp_stepx + 1)));
}
