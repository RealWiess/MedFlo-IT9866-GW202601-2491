...
{
    void * ptr = (void *)ithMapVram(src, size, ITH_VRAM_READ);
    (void)memcpy(dest, ptr, size);
    ithUnmapVram(ptr, size);
}

void ithWriteVram (uint32_t dest, const void * src, uint32_t size)
{
    void * ptr = (void *)ithMapVram(dest, size, ITH_VRAM_WRITE);
    (void)memcpy(ptr, src, size);
    ithUnmapVram(ptr, size);
}

void ithCopyVram (uint32_t dest, uint32_t src, uint32_t size)
{
    void * destPtr = (void *)ithMapVram(dest, size, ITH_VRAM_WRITE);
    void * srcPtr  = (void *)ithMapVram(src, size, ITH_VRAM_READ);

    (void)memcpy(destPtr, srcPtr, size);
