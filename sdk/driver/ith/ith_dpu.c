...
//=============================================================================
    ...
    #define ITH_DPU_LOG_INFO(fmt, ...)    LOG_INFO(fmt, ##__VA_ARGS__)
    #define ITH_DPU_LOG_WARNING(fmt, ...) LOG_INFO(fmt, ##__VA_ARGS__)
    #define ITH_DPU_LOG_DEBUG(fmt, ...)   LOG_INFO(fmt, ##__VA_ARGS__)
#else
    #define ITH_DPU_LOG_INFO(fmt, ...)    LOG_INFO(fmt, ##__VA_ARGS__)
    #define ITH_DPU_LOG_WARNING(fmt, ...) LOG_WARN(fmt, ##__VA_ARGS__)
    #define ITH_DPU_LOG_DEBUG(fmt, ...)   LOG_DBG(fmt, ##__VA_ARGS__)
#endif

#define ITH_DPU_LOG_ERROR(fmt, ...) LOG_ERR(fmt, ##__VA_ARGS__)
...
/*
	...
	(void)printf("DPU fire ~~~~~~~~~\n");
	for(i=0; i<0x40; i+=4)
	{
		(void)printf("%08x ",ithReadRegA( ITH_DPU_REG_DCR + i));
		if( (i&0xC)==0x0C )	(void)printf("\n");
	}
*/
...
{
	unsigned int    StatusReg32,TomeOutCnt=0x2000;
	unsigned char	Result = 1;

	//(void)ithPrintf("DCR:6\n");
	...
	do
	{
		...
		if(TomeOutCnt--==0)
		{
			(void)printf("ith DPU timeOut-970, dsr_base=%" PRIx32 ", %" PRIx32 "!!\n",ITH_DPU_REG_DSR, ithReadRegA(ITH_DPU_REG_DSR) );
			Result = 0;
			break;
		}
		(void)usleep(1000);
		//(void)printf("ith DPU-970, dsr=%x, %x!!\n",ITH_DPU_REG_DSR, ithReadRegA(ITH_DPU_REG_DSR) );
	} while((StatusReg32&0x00000001) == 0x00000000);

	//(void)ithPrintf("DCR5:0x%08x\n",ithReadRegA(ITH_DPU_BASE+0x00));
