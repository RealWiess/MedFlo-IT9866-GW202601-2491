...
uint64_t        timerCounter[ITH_TIMER_COUNT];
#endif
...
void ithTimerCtrlEnable(ITHTimer timer, ITHTimerCtrl ctrl)
{
    ...
    case ITH_TIMER_EN:
		...
		ithSetRegBitA(ITH_TIMER_BASE + ((timer >> 3) & 0x1) * ITH_TIMER_BASE_1TO2_GAP + ITH_TIMER1_CR_REG + (timer & 0x7) * 0x4, ITH_TIMER_RELOAD_UPDATE_BIT);
        break;
    ...
    case ITH_TIMER_EN64:
        ...
        default:
            ITH_LOG_ERR("Unsupport 64-bit timer: %d\r\n", timer );
            break;
        }
        break;
...
static void timer_isr(void *arg)
{
    ...
    if (intr & 0x4)
    {
        ...
        timerTime[timer] += (time * (uint64_t)1000000 / (uint64_t)ithGetBusClock());
        timerCounter[timer] += time;
    }
