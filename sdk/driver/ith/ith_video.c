...
static uint32_t VLD_BUFFER_HEIGHT, COL_BUFFER_HEIGHT, CMD_DATA_BUFFER_COUNT, CMD_DATA_BUFFER_HEIGHT;
    ...
    void)
{
    uint32_t i;
    ithVideoDisableClock();
    for (i = 0; i < 100u; i++) {asm ("");}
	while ((ithReadRegA(0xD8000090) & 0x20000000) != 0) usleep(1000);
    ...
    void)
{
    ...
    if ( (value & VIDEO_MASK_VLD_ERROR) != 0u)
    {
        return true;
    }
    return false;
    ...
    void)
{
    ...
    if(CFG_LCD_WIDTH >= CFG_LCD_HEIGHT)
    {
		...
		TILING_BLOCK            = (8u);
        FRAME_BUFFER_COUNT      = (4u);
        VIDEO_MAX_WIDTH         = (1280u);
        VIDEO_MAX_HEIGHT        = (720u);
        VIDEO_BUFFER_PITCH      = (VIDEO_MAX_WIDTH);
        Y_BUFFER_OFFSET         = (VIDEO_BUFFER_PITCH * VIDEO_MAX_HEIGHT * 3u) / 2u;
        U_BUFFER_OFFSET         = (VIDEO_BUFFER_PITCH * VIDEO_MAX_HEIGHT);
        V_BUFFER_OFFSET         = (VIDEO_BUFFER_PITCH * VIDEO_MAX_HEIGHT);
        VIDEO_BUFFER_HEIGHT     = (VIDEO_MAX_HEIGHT * 3u / 2u) * FRAME_BUFFER_COUNT;
        BLANK_BUFFER_HEIGHT     = (16u);
        BLANK_U_BUFFER_OFFSET   = (16u*16u);
        BLANK_V_BUFFER_OFFSET   = (16u*16u+8u*8u);
        DB_BUFFER_HEIGHT        = (8u);                              // (frame_pitch/16) * 2 * 8 * 8
        MV_BUFFER_HEIGHT        = (180u);                            // (pitch x height)/2
        TC_BUFFER_HEIGHT        = (4u);                              // frame_width/2 * 8
        VLD_BUFFER_HEIGHT       = (32u);                             // frame_pitch * 4 * 8 * 2
        COL_BUFFER_HEIGHT       = (0u);
        CMD_DATA_BUFFER_COUNT   = (3u);
        CMD_DATA_BUFFER_HEIGHT  = (500u);
#else
	    TILING_BLOCK            = (8u);
		if(CFG_RAM_SIZE == 0x8000000) // 1080p
		{
	        FRAME_BUFFER_COUNT      = (4u);
#ifdef CFG_DYNAMIC_ROTATE_ENABLE
	        VIDEO_MAX_WIDTH         = (1920u);
	        VIDEO_MAX_HEIGHT        = (1920u);
#else
			VIDEO_MAX_WIDTH         = (1920u);
	        VIDEO_MAX_HEIGHT        = (1088u);
#endif
		}
		else if(CFG_RAM_SIZE == 0x1000000) // 480x272
		{
	        FRAME_BUFFER_COUNT      = (2u);
#ifdef CFG_DYNAMIC_ROTATE_ENABLE
			VIDEO_MAX_WIDTH         = (480u);
	        VIDEO_MAX_HEIGHT        = (480u);
#else
	        VIDEO_MAX_WIDTH         = (480u);
	        VIDEO_MAX_HEIGHT        = (272u);
#endif
		}
	    else if(CFG_RAM_SIZE == 0x4000000) //720p
	    {
	        FRAME_BUFFER_COUNT      = (4u);
#ifdef CFG_DYNAMIC_ROTATE_ENABLE
	        VIDEO_MAX_WIDTH         = (1280u);
	        VIDEO_MAX_HEIGHT        = (1280u);
#else
            VIDEO_MAX_WIDTH         = (1280u);
	        VIDEO_MAX_HEIGHT        = (720u);
#endif
    ...
    VIDEO_DECODER_DESC *p_video_desc)
{
    ...
    // [9850] 640x480  : 2723840 Byte
        ...
        totalMemSize = (VIDEO_BUFFER_HEIGHT +
                    ...
                    1u ) * VIDEO_BUFFER_PITCH;

    (void)printf("totalMemSize = %d\n", totalMemSize);
    (void)ithVideoBufAlloc(totalMemSize);
    ...
    AVC_DECODER *ptDecoder)
{
    ...
    if ( (ptDecoder == NULL) || (ptDecoder->pAVCDecoderBuf == NULL) )
    {
    ...
    for (i = 0; i < frame_buf_cnt; i++)
    {
        ...
        //(void)ithPrintf("VIDEO(%d) %x %x %x\n", i, py, pu, pv);
    }

    // DB Buffer
    {
        uint8_t *addr = pbuf + ((VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT) * VIDEO_BUFFER_PITCH);
    ...
    // MV Buffer
    {
        uint8_t *addr = pbuf + ((VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT + DB_Buf_Height) * VIDEO_BUFFER_PITCH);
    ...
    // TC Buffer
    {
        uint8_t *addr = pbuf + ((VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT + DB_Buf_Height + MV_BUFFER_HEIGHT) * VIDEO_BUFFER_PITCH);
    ...
    // VLD Buffer
    {
        uint8_t *addr = pbuf + ((VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT + DB_Buf_Height + MV_BUFFER_HEIGHT + TC_BUFFER_HEIGHT) * VIDEO_BUFFER_PITCH);
        _HARDWARE_SetVLDBufAddr(addr);
        //(void)ithPrintf("VLD %x\n", addr);
    }

    // COL Buffer
    {
        uint8_t *addr = pbuf + ((VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT + DB_Buf_Height + MV_BUFFER_HEIGHT + TC_BUFFER_HEIGHT + VLD_BUFFER_HEIGHT) * VIDEO_BUFFER_PITCH);
        ...
        //(void)ithPrintf("COL %x\n", addr);
    }

    // CMD Buffer
    for (i = 0; i < CMD_DATA_BUFFER_COUNT; i++)
    {
        uint8_t *addr = pbuf + ((CMD_DATA_BUFFER_HEIGHT * i + VIDEO_BUFFER_HEIGHT + BLANK_BUFFER_HEIGHT + DB_Buf_Height + MV_BUFFER_HEIGHT + TC_BUFFER_HEIGHT + VLD_BUFFER_HEIGHT + COL_BUFFER_HEIGHT) * VIDEO_BUFFER_PITCH);

        ptDecoder->ppCmdDataBufAddr[i] = addr;
        //(void)ithPrintf("CMD(%d) %x\n", i, addr);
    }
#ifdef WIN32
    (void)memset(ptDecoder->ppCmdDataBufAddr[0], 0, CMD_DATA_BUFFER_COUNT * CMD_DATA_BUFFER_HEIGHT * VIDEO_BUFFER_PITCH);
#endif
    ...
    AVC_DECODER *ptDecoder)
{
    ...
    if ((cmdDataBufSelect > 2u) || (ptDecoder->decodeBufSelect > (ptDecoder->frameBufCount - 1u)) )
    {
    ...
    uint32_t    timeout)
{
    ...
    uint16_t                xMacroBlock_Last;
    uint16_t                yMacroBlock_Last;
    ...
    for (; (hwStatus == VIDEO_HARDWARE_STATUS_BUSY) && (finalCountDown > 0u); finalCountDown--)
    {
#if 0
    ...
    if (hwStatus == VIDEO_HARDWARE_STATUS_BUSY)
    {
        ...
        //(void)printf("firecount0: %u, fail hwStatus: 0x%X, x_last: %u, y_last: %u, addr: 0xF88, cabErr: 0x%X\n",
    ...
    else
    {
        ...
        yMacroBlock_Last    = ithReadRegA(REG_VIDEO_TC_DEBUG_INFO_4_80H) & VIDEO_MASK_MACROBLOCK_Y_COORDINATE_T;
        cabErr              = ithReadRegA(REG_VIDEO_CABAC_DEBUG_INFO_0_88H) & VIDEO_MSK_CABAC_DEBUG_INFO_0_88H;
        if ((cabErr & 0x8000u) != 0u)
        {
            //(void)printf("firecount1: %u, ok hwStatus: 0x%X, x_last: %u, y_last: %u, addr: 0xF88, cabErr: 0x%04X\n",
            //    gFireCount, hwStatus, xMacroBlock_Last, yMacroBlock_Last, cabErr);
    ...
    AVC_DECODER *ptDecoder)
{
    ...
    {
        (void)ithPrintf("AVC Close : wait avc idle timeout....1\n");
        ithVideoHostReset();
        (void)ithPrintf("AVC Close : wait avc idle timeout....2\n");
        ithVideoReset(ptDecoder);
        (void)ithPrintf("AVC Close : wait avc idle timeout....3\n");
    }
    ...
    uint8_t     *v_sys_addr)
{
    ...
    (void)printf( "video decode buffer Y[%lu] [SYS](0x%lX) [VRAM](0x%lX)\n", buf_index,  (uint32_t)y_sys_addr, y_vram_addr);
    (void)printf( "video decode buffer U[%lu] [SYS](0x%lX) [VRAM](0x%lX)\n", buf_index,  (uint32_t)u_sys_addr, u_vram_addr);
    (void)printf( "video decode buffer V[%lu] [SYS](0x%lX) [VRAM](0x%lX)\n", buf_index,  (uint32_t)v_sys_addr, v_vram_addr);
    y_vram_addr >>= 3;
    u_vram_addr >>= 3;
    v_vram_addr >>= 3;
...
uint32_t _HARDWARE_GetDecodedVideoBufVramAddr(
    ...
    uint32_t    yuv_index)
{
    uint16_t    offset  = buf_index * 4u * 3u + ((buf_index >= 8u) ? 16u : 0u) + yuv_index * 4u;
    uint32_t    ret     = 0;
    ...
    uint8_t *mv_buf_sys_addr)
{
    ...
    //(void)printf( "video mv buffer [SYS](0x%X) [VRAM](0x%X)\n", mv_buf_sys_addr, mv_buf_vram_addr);
    mv_buf_vram_addr >>= 3;
    ...
    uint8_t *tc_buf_sys_addr)
{
    ...
    //(void)printf( "video tc buffer [SYS](0x%X) [VRAM](0x%X)\n", tc_buf_sys_addr, tc_buf_vram_addr);
    tc_buf_vram_addr >>= 3;
    ...
    uint8_t *vld_buf_sys_addr)
{
    ...
    //(void)printf( "video vld buffer [SYS](0x%X) [VRAM](0x%X)\n", vld_buf_sys_addr, vld_buf_vram_addr);
    vld_buf_vram_addr >>= 3;
    ...
    uint8_t *db_buf_sys_addr)
{
    ...
    //(void)printf( "video db buffer [SYS](0x%X) [VRAM](0x%X)\n", db_buf_sys_addr, db_buf_vram_addr);
    db_buf_vram_addr >>= 3;
    ...
    uint8_t *cmd_data_buf_sys_addr)
{
	...
	uint32_t cmd_data_buf_vram_addr_t = cmd_data_buf_vram_addr;

    //(void)printf( "video cmd data buffer [SYS](0x%X) [VRAM](0x%X)\n", cmd_data_buf_sys_addr, cmd_data_buf_vram_addr );
	cmd_data_buf_vram_addr_t >>= 3;
	cmd_data_buf_vram_addr_t >>= 23;
	cmd_data_buf_vram_addr_t <<= 17;
    cmd_data_buf_vram_addr_t |= (0xf180);
    ithWriteRegA(REG_VIDEO_SPECIAL_FUNC_SETTING_F0H, (uint32_t)cmd_data_buf_vram_addr_t);
    ...
    uint8_t *pv)
{
    ...
    if (TILING_MODE_OFF)
    {
        for (i = 0; i < 16; i++)
        {
            (void)memset((void *)(py + i * VIDEO_BUFFER_PITCH), 0x10, 16);
        }
        for (i = 0; i < 8; i++)
        {
            (void)memset((void *)(pu + i * VIDEO_BUFFER_PITCH), 0x80, 8);
        }
        for (i = 0; i < 8; i++)
        {
            (void)memset((void *)(pv + i * VIDEO_BUFFER_PITCH), 0x80, 8);
        }
    }
}
