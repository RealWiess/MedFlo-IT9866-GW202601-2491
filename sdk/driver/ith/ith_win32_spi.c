...
int SpiOpen(DWORD dwClockRate)
{
    ...
    {
        ///(void)printf("unable to open ftdi device: %d (%s)\n", ret, ftdi_get_error_string(ftdi));
        ftdi_free(ftdi);
        return EXIT_FAILURE;
    }
    ...
    {
        unsigned int chipid;
        (void)printf("ftdi_read_chipid: %d\n", ftdi_read_chipid(ftdi, &chipid));
        //(void)printf("FTDI chipid: %X\n",      chipid);
    }
...
static ULONG SpiWrite(DWORD ctrlLen, PWriteControlByteBuffer pCtrlBuf, DWORD dataLen, PWriteDataByteBuffer pDataBuf)
{
    ...
    if (data_size > 0)
    {
        ...
        {
            (void)memcpy(&data[data_size], pCtrlBuf, ctrlLen);
            data_size += ctrlLen;
        }
        if (dataLen > 0 && pDataBuf != NULL)
        {
            (void)memcpy(&data[data_size], pDataBuf, dataLen);
            data_size += dataLen;
        }
    ...
    do
    {
        ...
        if (prereadstaus != 0)
        {
            if (rdLen >= prereadstaus)
            {
                ...
                (void)memcpy((void *)&pdest[i], (void *)rdBuf, prereadstaus);
            ...
            {
                error = SpiRead(wrLen, &wrBuf, rdLen, &rdBuf);
                (void)memcpy((void *)&pdest[i], (void *)rdBuf, rdLen);
                i    += prereadstaus;
            }
        }
...
static void SecWriteMemory(uint16_t SecNum, uint16_t SecOffset, uint8_t *pDataBuffer, uint32_t DataLen)
{
    ...
    if (DataLen > 0x10000)
    {
        (void)printf("Top Half: Data length is too large\n");
        return;
    }
    ...
    if (ulRemainLen)
    {
        ...
        if (usDataLen > 0x100)
        {
            (void)printf("Buttom Half: Data Length is too large\n");
        }
...
void *ithMapVram(uint32_t vram_addr, uint32_t size, uint32_t flags)
{
    ...
    (void)memcpy(&vramBufferCache[vram_addr], &vramBuffer[vram_addr], size);
...
void ithUnmapVram(void *sys_addr, uint32_t size)
{
    ...
    {
        (void)memcpy(&vramBuffer[vram_addr], &vramBufferCache[vram_addr], size);
