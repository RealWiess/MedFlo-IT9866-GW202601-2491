...
static void RunScript(const uint32_t *cmd, unsigned int cmd_len)
{
    ...
    while (idx < cmd_len)
    {
        ...
        case DATA_WAIT0_CMD: // DATA_WAIT0(addr, mask)
            {
                ...
                ITH_LOG_DBG("DATA_WAIT0(0x%04x, 0x%04x);\n", addr, mask );
            }
            idx += 3;
            break;
        ...
        case DATA_WAIT1_CMD: // DATA_WAIT1(addr, mask)
            {
                ...
                ITH_LOG_DBG("DATA_WAIT1(0x%04x, 0x%04x);\n", addr, mask );
            }
            idx += 3;
            break;
        ...
        case CALL_CMD: // CALL(addr)
            {
                int addr = cmd[idx + 1];
                ITH_LOG_DBG("Do not support command CALL(0x%08x);\n", addr );
            }
            idx += 2;
            break;
        ...
        case WRITE_MASK_CMD: // WRITE_MASK(addr, data, mask)
            {
                ...
                ITH_LOG_DBG("WRITE_MASK(0x%04x, 0x%04x, 0x%04x);\n", addr, val, mask );
            }
            idx += 4;
            break;
        ...
        case GOTO_CMD: // GOTO(addr)
            {
                int addr = cmd[idx + 1];
                ITH_LOG_ERR("Do not support command GOTO(0x%08x);\n", addr );
            }
            idx += 2;
            break;
        ...
        case READ_MASK_CMD: // READ_MASK(addr, mask)
            {
                ...
                ITH_LOG_DBG("READ_MASK(0x%04x, 0x%04x);\n", addr, mask );
            }
            idx += 3;
            break;
        ...
        case BGTE_CMD: // BGTE(val, offset)
            {
                ...
                int offset = cmd[idx + 2];
                if ((unsigned)rval >= (unsigned)val) idx += offset;
                ITH_LOG_DBG("BGTE(0x%04x, 0x%04x);\n", val, offset );
            }
            idx += 3;
            break;
        ...
        case BLTE_CMD: // BLTE(val, offset)
            {
                ...
                int offset = cmd[idx + 2];
                if ((unsigned)rval <= (unsigned)val) idx += offset;
                ITH_LOG_DBG("BLTE(0x%04x, 0x%04x);\n", val, offset );
            }
            idx += 3;
            break;
        ...
        default: // WRITE(addr, data)
            {
                ...
                ITH_LOG_DBG("WRITE(0x%04x, 0x%04x);\n", addr, data );
            }
            idx += 2;
            break;
        }
