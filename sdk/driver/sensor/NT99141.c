    ...
    uint16_t RegAddr)
{
    ...
    *pdbuf++            = (uint8_t)(RegAddr & 0x00ff);
    ...
    evt.slaveAddress    = NOVATEK_IICADDR;
    evt.cmdBuffer       = dbuf;
    evt.cmdBufferSize   = 2;
    ...
    if (result != 0)
    {
        ithPrintf("NOVATEK_ReadI2C_8Bit read address 0x%02x Error!\n", RegAddr);
    }
    value = (dbuf[2] & 0xFF);
    ...
    uint8_t     data)
{
    ...
    *pdbuf              = (uint8_t)(data);
    ...
    {
        (void)printf("NOVATEK_WriteI2C_8Bit I2c Write Error, reg=%04x val=%02x\n", RegAddr, data);
    }
