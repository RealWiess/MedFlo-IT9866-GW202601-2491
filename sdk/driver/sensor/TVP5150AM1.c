    ...
    uint8_t data)
{
    ...
    *pdbuf              = (uint8_t)(data);
    ...
    evt.slaveAddress    = TVP5150_IICWRADDR;
    evt.cmdBuffer       = dbuf;
    evt.cmdBufferSize   = 2;

    if (0 != (result = write(gMasterDev, (char *)&evt, 1)))
    {
        (void)printf("_TVP5150_WriteI2c_Byte Write Error, reg=%02x val=%02x\n", RegAddr, data);
    }
    return result;
}
...
void TVP5150SetProperty (
    MODULE_SETPROPERTY  Property,
    uint16_t            Value)
{}

void TVP5150PowerDown (
    uint8_t enable)
{}
