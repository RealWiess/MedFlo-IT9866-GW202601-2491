    ...
    uint8_t data)
{
    ...
    *pdbuf              = (uint8_t)(data);
    ...
    {
        (void)printf("RN67_WriteI2c_Byte Write Error, reg=%02x val=%02x\n", RegAddr, data);
    }
    return result;
}

uint32_t
RN67_WriteI2c_ByteMask (
    uint8_t RegAddr,
    uint8_t data,
    uint8_t mask)
{
    ...
    void)
{
    ...
    switch (RN67_CH_SET[current_vin])
    {
        case RN67_HD720P_25FPS:
            (void)printf("RN6752 VIN(%d) = HD720P_25FPS \n", current_vin);
        ...
        case RN67_D1PAL_50FPS:
            (void)printf("RN6752 VIN(%d) =  PAL_50I \n", current_vin);
            RN675x_D1initial(RN675x_PAL, sizeof(RN675x_PAL) / sizeof(REGPAIR), current_vin);
            RN675x_Enable(current_vin, false);
            break;

        default:
            (void)printf("RN6752 TIMING Error \n");
            break;
    }
}

RN67_TIMING_ID
RN675x_FormatDectect (
    uint8_t reg00)
{
    if ((reg00 & 0x20) == 0x20)
    {
...
{
    unsigned char i = 0;
    (void)printf("RN6752V1 Initialize \n");
    RN67_I2c_init();
    RN675xM_Pre_initial();
}
    ...
    uint16_t Mode)
{
    ...
    switch (video_state)
    {
        ...
        case RN67_VIDEO_DECTECT:
            ...
            // (void)printf("[DECTECT]format = %d format_count = %d\n", dectect_format, format_count);
    ...
    switch (Property)
    {
        case VideoInCH:
            if ((Value & 0xff) != current_vin)
            {
                ...
                video_state = RN67_VIDEO_UNKNOWN;
                (void)printf("[RN6752SetProperty]CH CHANGE = %d\n", current_vin);
                RN675xM_Pre_initial();
            }
            break;
