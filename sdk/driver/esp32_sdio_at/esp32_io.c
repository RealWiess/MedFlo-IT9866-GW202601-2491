...
static void
esp32_irq (struct sdio_func * func);
    ...
    const struct sdio_device_id * id)
{
    ...
    if (rc)
    {
        (void)printf("esp32: enable function fail! rc = 0x%08X \n", rc);
    }
    ...
    struct sdio_func * func)
{
    ...
    if (rc)
    {
        (void)printf("esp32: release irq fail! \n");
    }
    sdio_release_host(func);
...
static int
esp32_get_rx_data_size (
    struct esp32_sdio_at    * esp32,
    uint32_t                * rx_size)
{
    int         rc;
    uint32_t    len;
    ...
    uint32_t                len)
{
    ...
    if (cmd->flags & ESP32_DATA_READ)
    {
        ...
        if ((data->actual_len + len) > data->length)
        {
            ...
            (void)printf("esp32: user buffer full!! \n");
        }
        recvbuf[len] = '\0';
        // (void)printf("%s \n", recvbuf);
        // (void)printf("len: %d \n", len);
    ...
    struct sdio_func * func)
{
    ...
    if (rc)
    {
        (void)printf("esp32_get_intr fail 0x%X \n", rc);
        return;
    }
    rc = esp32_clear_intr(func, intr_raw);
    if (rc)
    {
        (void)printf("esp32_clear_intr fail 0x%X \n", rc);
        return;
    }

// (void)printf("intr_raw: 0x%X �Aintr_st: 0x%X\n", intr_raw, intr_st);
    ...
    if (intr_raw & HOST_SLC0_RX_NEW_PACKET_INT_ST)
    {
        ...
        for (;;)
        {
            ...
            {
                (void)printf("esp32: get rx data size timeout \n");
                goto end;
            }
        }
        // (void)printf("esp32: rx data size %d \n", len);

        if (len > READ_BUFFER_LEN)
        {
            len = READ_BUFFER_LEN;
            (void)printf("rx size(%d) > buffer size(%d) ==> TODO!! \n", len, READ_BUFFER_LEN);
        }
        len_align   = (len + 3) & (~3);
    ...
    if (rc)
    {
        (void)printf("esp32_irq() rc:%d (0x%08X) \n", rc, rc);
    }
    return;
}

static int
esp32_get_tx_buffer_num (
    struct esp32_sdio_at    * esp32,
    uint32_t                * tx_num)
{
    uint32_t    len;
    ...
    int     length)
{
    ...
    for (;;)
    {
        ...
        if (!rc && (num * buffer_size >= (uint32_t)length))
        {
            break;
        }
        if (rc)
        ...
        {
            (void)printf(" buffer is not enough: %d, %d required. \n", num, buffer_used);
            rc = ERR_ESP32_WAIT_TX_BUF_TIMEOUT;
            goto end;
        }
        else
        {
            (void)printf(" buffer is not enough: %d, %d required. Retry....\n", num, buffer_used);
        }
    }
    ...
    {
        (void)printf("[ERR] esp32 length:0x%X > 0x1F800 \n", len);
    }
    ...
    if (rc)
    {
        (void)printf("esp32_sdio_at_transfer() rc = %d (0x%08X)", rc, rc);
    }
    return rc;
}
    ...
    void)
{
    ...
    if (rc)
    {
        (void)printf("esp32: claim irq fail! rc = 0x%08X \n", rc);
    }
    sdio_release_host(func);
    ...
    struct esp32_cmd * cmd)
{
    ...
    if (esp32.cmd)
    {
        (void)printf("esp32: in used!!! err cmd: %s \n", cmd->cmd_buf);
    ...
    if (cmd->flags & (ESP32_WAIT_ACK | ESP32_WAIT_FOR_SEND))
    {
        ...
        {
            (void)printf("esp32: cmd length = %d > ESP32_CMD_LEN(%d) \n", cmd->cmd_len, ESP32_CMD_LEN);
            while (1)
            {
            }
        }
        // (void)printf("cmd_len:%d \n", cmd->cmd_len);
        ...
        // (void)printf("esp32_at_cmd: cmd->flags = %d \n", cmd->flags);

        res = itpSemWaitTimeout(&esp32.sem, (cmd->timeout ? cmd->timeout : ESP32_TIMEOUT));
        if (res)
        {
            (void)printf("esp32: %s wait ack timeout\n", cmd->cmd_buf);
        ...
        if (cmd->flags & ESP32_DATA_WRITE)
        {
            ...
            if (res)
            {
                (void)printf("esp32: %s wait data ack timeout\n", cmd->cmd_buf);
                res = ERR_ESP32_WAIT_ACK_TIMEOUT;
                goto out;
            }
        }
...
out:
    ...
    if (res)
    {
        (void)printf("esp32_at_cmd() res = %d (0x%08X) \n", res, res);
    }
    return res;
}
