...
static struct ble_gatt_svc_def **ble_gatts_svc_defs_last = NULL;
...
ble_gatts_add_svcs(const struct ble_gatt_svc_def *svcs)
{
	...
	if (ble_gatts_svc_defs_last)
	{
		free(ble_gatts_svc_defs_last);
		ble_gatts_svc_defs_last = NULL;
	}
	ble_gatts_svc_defs_last = malloc(ble_gatts_num_svc_defs * sizeof *ble_gatts_svc_defs);
	memcpy(ble_gatts_svc_defs_last, ble_gatts_svc_defs, (ble_gatts_num_svc_defs * sizeof *ble_gatts_svc_defs));
...
int
ble_gatts_resume(void)
{
	int rc;
	if (!ble_gatts_svc_defs_last)
		return 0;
	rc = ble_gatts_count_cfg(*ble_gatts_svc_defs_last);
	if (rc != 0) {
		return rc;
	}

	rc = ble_gatts_add_svcs(*ble_gatts_svc_defs_last);
	if (rc != 0) {
		return rc;
	}
