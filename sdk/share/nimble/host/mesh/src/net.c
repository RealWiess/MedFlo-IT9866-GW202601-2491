...
#define SRC(pdu)           (endian_get_be16(&(pdu)[5]))
#define DST(pdu)           (endian_get_be16(&(pdu)[7]))
...
static bool check_dup(struct os_mbuf *data)
{
	...
	val = endian_get_be32(tail - 4) ^ endian_get_be32(tail - 8);
		      ...
		      enum bt_mesh_net_if net_if)
{
	...
	if (!bt_mesh_is_provisioned()) {
		while (1)
		{
			printf("Not provisioned; dropping packet\n");
			usleep(100);
		}
