...
float   fixed2float64 (Word64 p, Word32 q);
...
extern void    btr_fly_r2 (
       ComplexInt16 * __restrict a,
       ComplexInt16 * __restrict b); // extern auto register\static
extern ComplexInt32 complex32_mul (ComplexInt32 * __restrict a,
                                   ComplexInt32 * __restrict b);
...
#if (defined(__arm__) && !defined(_MSC_VER)) ^                                 \
    (defined(__or32__) || defined(__or1k__))
typedef union {
    Word64 a;
    #ifdef __or32__
    struct
...
static __inline Word16 sat_32_16 (Word32 a)
{ // gcc pass arguments wide, return values narrow
    #ifdef __or32__
    Word32 temp;

    asm volatile("l.addhs %0, %1, r0"
                 : "=r"(temp)
                 : "r"(a)
                 : "cc"); // l.addhs temp, a, #0
    ...
    m = 0x00007fff;
        ...
        #ifdef __GNUC__
    // asm volatile("mov %0, %1, asr #15" :"=&r"(b) :"r"(a)); // mov b, a, asr
    // #15 asm volatile("teq %0, %1, asr #31" : :"r"(b), "r"(a) :"cc");// teq b,
    // a, asr #31 asm volatile("eorne %0, %1, %0, asr #31" :"+r"(a) :"r"(m));

    __asm__ __volatile__("mov %0, %1, asr #15\n\t"
                         "teq %0, %1, asr #31\n\t"
                         "eorne %1, %2, %1, asr #31\n\t"
                         : "+r"(b), "+r"(a)
                         : "r"(m)
...
static __inline Word32 L_mult (Word16 var1, Word16 var2)
{
    ...
    // asm volatile("l.mul %0, %1, %2" :"=&r"(L_var_out) :"r"((Word32)var1),
    // "r"((Word32)var2) :"cc"); asm volatile("l.slls %0, %0, %1"
    // :"+r"(L_var_out) :"r"((Word32)1));

    __asm__ __volatile__("l.mul %0, %1, %2\n\t"
                         "l.adds %0, %0, %0\n\t"
                         : "+r"(L_var_out)
    ...
    Word32 L_var_out;
        ...
        #ifdef __GNUC__
    // asm volatile("smulbb %0, %1, %2" :"=r"(L_var_out) :"r"((Word32)var1),
    // "r"((Word32)var2)); asm volatile("qadd %0, %0, %0" :"+r"(L_var_out)); //
...
static __inline Word32 L_mac (Word32 a, Word16 x, Word16 y)
{
    ...
    Word32 temp;
        ...
        #ifdef __GNUC__
    // asm volatile("smulbb %0, %1, %2" :"=r"(temp) :"r"((Word32)x),
    // "r"((Word32)y)); asm volatile("qdadd %0, %0, %1" :"+r"(a) :"r"(temp));
...
static __inline Word32 L_msu (Word32 a, Word16 x, Word16 y)
{
    ...
    Word32 temp;
        ...
        #ifdef __GNUC__
    // asm volatile("smulbb %0, %1, %2" :"=r"(temp) :"r"((Word32)x),
    // "r"((Word32)y)); asm volatile("qdsub %0, %0, %1" :"+r"(a) :"r"(temp));
...
static __inline void MLAI (Word64 b)
{ // 16-bit immediate
    #ifdef __or32__
  // UWord32 a, c;
    // a = b >> 1;
...
#if !defined(__arm__) || defined(_MSC_VER)
static __inline Complex16_t complex_mul (Complex16_t a, Complex16_t b)
{                  // assume b exploits Q15 format
...
static __inline Complex16_t complex_swap (Complex16_t a)
{ // * -j
    Complex16_t c;
    c.real = a.imag;
    c.imag = -a.real;
...
{
    #if 1
    a.real = (b > 0) ? sat_32_16(a.real + (1 << (b - 1))) >> b
                     : sat_32_16(lsl(a.real, -b));
    a.imag = (b > 0) ? sat_32_16(a.imag + (1 << (b - 1))) >> b
...
#if !defined(__arm__) || defined(_MSC_VER)
static __inline Word16 exp_adj (Word16 a, Word16 b)
