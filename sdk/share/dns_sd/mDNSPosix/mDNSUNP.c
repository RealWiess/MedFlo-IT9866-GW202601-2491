...
/* include recvfrom_flags2 */
    ...
    *flagsp = msg.msg_flags;        /* pass back results */
    ...
    //Modified by Steven for more than one packet the last tail will point to itself to cause deadlock.
    for (cmptr = CMSG_FIRSTHDR(&msg); cmptr != NULL && cmptr != preCmptr;
         preCmptr = cmptr, cmptr = CMSG_NXTHDR(&msg, cmptr)) {

        //printf("%s(%d), controllen: %d, 0x%X\n", __FILE__, __LINE__, msg.msg_controllen, cmptr);
