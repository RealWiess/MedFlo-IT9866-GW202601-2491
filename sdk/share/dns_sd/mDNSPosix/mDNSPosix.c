...
// Fix close(socket#) cannot be called when mDNS close.
#define assert(x)   do { if (!(x)) printf("mDNS error %s %u\n", __FUNCTION__, __LINE__); } while (0)
	...
	if (packetLen >= 0)
		...
		if (!intf)
			...
			if (reject)
				...
				{
                                if (mDNS_GetAnnouncing()==0) {
                                    printf("mDNSPosix set Announcing \n ");
                                    mDNS_SetAnnouncing(1);
                                }
...
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, const char *intfName, int intfIndex)
	...
	intf = (PosixNetworkInterface*)calloc(sizeof(*intf), 1);
	...
	if (err == 0)
        ...
        //
        //intf->coreIntf.ip.ip.v4.b[2]=111;
        //intf->coreIntf.ip.ip.v4.b[3]=2;
