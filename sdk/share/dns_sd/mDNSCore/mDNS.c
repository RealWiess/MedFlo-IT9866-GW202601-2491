...
mDNSlocal void SendResponses(mDNS *const m)
	...
	for (rr = m->ResourceRecords; rr; rr=rr->next)
		{
		// set announce to mDNS address
              if (mDNS_GetAnnouncing()){
		...
		if (rr->ImmedAnswer == mDNSInterfaceMark)		// Sending this record on all appropriate interfaces
			...
			if (TimeToAnnounceThisRecord(rr, m->timenow + rr->ThisAPInterval/2) || mDNS_GetAnnouncing())
				{
				rr->AnnounceCount--;
                            if (gSetAnnouneNum>0)
                                gSetAnnouneNum--;
				...
				debugf("Announcing %##s (%s) %d", rr->resrec.name->c, DNSTypeName(rr->resrec.rrtype), rr->AnnounceCount);
                          if (mDNS_GetAnnouncing() && (rr->AnnounceCount == 0 || gSetAnnouneNum==0)){
                              mDNS_SetAnnouncing(0);                              
				}
			}
...
// get/set Announcing 
static int gResetAnnoune =0;

// get/set multi-room
static int gMultiroom =0;

int  mDNS_GetAnnouncing()
{
    return gResetAnnoune;
}
 
void mDNS_SetAnnouncing(int nState)
{
    gResetAnnoune = nState;
    if (nState==0) {
        gSetAnnouneNum =0;
    } else if (mDNS_GetMultiroom()==1){
        // if multi room playing,not announce
        gSetAnnouneNum =0;    
    } else {
        gSetAnnouneNum = 12;        
    }
    printf("mDNS_SetAnnouncing %d multi-room %d %d \n",nState,mDNS_GetMultiroom(),gSetAnnouneNum);
}

int  mDNS_GetMultiroom()
{
    return gMultiroom;
}

void mDNS_SetMultiroom(int nState)
{
    gMultiroom = nState;
    if (nState==0)
        gMultiroom =0;
