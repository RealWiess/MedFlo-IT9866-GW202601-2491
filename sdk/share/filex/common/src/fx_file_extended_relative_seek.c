...
FX_MEDIA *media_ptr;
    ...
    if (seek_from == FX_SEEK_BEGIN)
    {
        ...
        /* Yes, use the base file seek routine to seek from the beginning of
           the file.  */
        ret = (_fx_file_extended_seek(file_ptr, byte_offset));
    }
    ...
    else if (seek_from == FX_SEEK_END)
    {
        ...
        if (byte_offset >= file_ptr -> fx_file_current_file_size)
        {
            ...
            ret = (_fx_file_extended_seek(file_ptr, ((ULONG64) 0)));
        }
        else
        {

            /* Logically seek from the end of the file.  */
            ret = (_fx_file_extended_seek(file_ptr, file_ptr -> fx_file_current_file_size - byte_offset));
        }
    }
    ...
    else if (seek_from == FX_SEEK_FORWARD)
    {
        ...
        ret = (_fx_file_extended_seek(file_ptr, file_ptr -> fx_file_current_file_offset + byte_offset));
    }
