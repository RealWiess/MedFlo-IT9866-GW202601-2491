...
ULONG                    trace_timestamp;
    ...
    /* coverity[LOCK] pthread/ThreadX have recursive lock, could ignore.*/
    FX_PROTECT
    ...
    *errors_detected =  0;
    ...
    if (scratch_memory_size < ((media_ptr -> fx_media_total_clusters >> 3) + 1))
    {
        ...
        /* Release media protection.  */
        FX_UNPROTECT
        printf("scratch_memory_size2 %d %d %d\r\n", scratch_memory_size, media_ptr -> fx_media_total_clusters, ((media_ptr -> fx_media_total_clusters >> 3) + 1));
            ...
            *errors_detected =  *errors_detected | current_errors;
            ...
            if (dir_entry_ptr -> fx_dir_entry_attributes & FX_DIRECTORY)
            {
                ...
                if (current_directory_index >= FX_MAX_DIRECTORY_NESTING)
                {
                    ...
                    /* Release media protection.  */
                    FX_UNPROTECT
                    printf("Check for current directory stack overflow\r\n");
