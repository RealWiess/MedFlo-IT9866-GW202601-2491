...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _fx_directory_path_set                              PORTABLE C      */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*  DESCRIPTION                                                           */
/*                                                                        */
/*    This function sets the input directory to the path specified by the */
/*    caller.  If this path is not found, an error code is returned.      */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
...
/*                                                                        */
/**************************************************************************/
UINT  _fx_directory_path_set(FX_MEDIA *media_ptr, FX_PATH *search_path, CHAR *new_path_name)
{
...
CHAR         abs_str[FX_MAXIMUM_PATH]; /* TODO: possible will be better to use dynamic memory for exFAT only.  */
    ...
    /* Setup path directory buffer. */
    search_path->fx_path_directory.fx_dir_entry_name = search_path->fx_path_name_buffer;
    search_path->fx_path_directory.fx_dir_entry_short_name[0] =  0;
    search_path->fx_path_directory.fx_dir_entry_name[0] =        0;
    search_path->fx_path_string[0] =                      (CHAR)0;
    search_path->fx_path_string[FX_MAXIMUM_PATH - 1] =      (CHAR)0;
    ...
    path_string_ptr =  &(search_path->fx_path_string[0]);
    ...
    if ((!new_path_name) || (((new_path_name[0] == '\\') || (new_path_name[0] == '/')) && (new_path_name[1] == (CHAR)0)))
    {
        ...
        search_path->fx_path_directory.fx_dir_entry_name[0] =  (CHAR) 0;
        search_path->fx_path_string[0] =                       (CHAR) 0;
        search_path->fx_path_string[FX_MAXIMUM_PATH - 2] =     (CHAR) 0;
    }
    else
    {
        ...
        if ((new_path_name[0] != '\\') && (new_path_name[0] != '/'))
        {
            ...
            /* Setup the path to the global media path.  */
            path_ptr =  search_path;
            ...
            if (path_ptr -> fx_path_string[FX_MAXIMUM_PATH - 2] == '*')
            {
                ...
                /* directory set is complete, return status.  */
                return(FX_SUCCESS);
            }
        ...
        else
        {
            ...
            if (*new_path_name)
            {
                ...
                /* No, we couldn't fit the entire path.  Place a "*" character
                   ...
                   the path is kept just the the directory get call, so
