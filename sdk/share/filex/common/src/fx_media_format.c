...
UCHAR *byte_ptr;
UINT   i, j, root_sectors, total_clusters, bytes_needed;
UINT   sectors_per_fat, f, s;
    ...
    /* Jack : reserved size is custom design to avoid MBR erased when FAT updating.  */
    //reserved_sectors =  1;
#endif
    ...
    sectors_per_fat = ((sectors_per_fat / sectors_per_cluster) + 1) * sectors_per_cluster;  // Jack for alignment FAT addr
