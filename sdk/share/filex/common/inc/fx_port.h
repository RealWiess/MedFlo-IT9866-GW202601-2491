...
#define FX_PROTECT                      if (media_ptr -> fx_media_id != FX_MEDIA_ID) return(FX_MEDIA_NOT_OPEN); \
                                        else if (pthread_mutex_lock(&(media_ptr -> fx_media_protect)) != 0) return(FX_MEDIA_NOT_OPEN);
#define FX_UNPROTECT                    pthread_mutex_unlock(&(media_ptr -> fx_media_protect));
#endif
...
#define FX_CALLER_CHECKING_EXTERNS      extern  pthread_t      *_tx_thread_current_ptr; \
                                        extern  pthread_t       _tx_timer_thread; \
