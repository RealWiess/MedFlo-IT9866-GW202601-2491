...
static int lfs_bd_read0(lfs_t *lfs,
...
static int lfs_bd_read(lfs_t *lfs,
    ...
    n=lfs_bd_read0(lfs, pcache, rcache, hint, block, off, buffer, size);
#if 0
    ithPrintf("[%c] %s(%d, %d:%08X, %d) ", n?'X':'i', __FUNCTION__, block, off, off, size);
    size=(size>8)?8:size;
    if (!n) {
        for (i=0; i<size; i++) {
            ithPrintf("%02X ", ((unsigned char*)buffer)[i]);
        }
    }
    ithPrintf("\n");
#endif
...
#ifndef LFS_READONLY
static int lfs_bd_prog0(lfs_t *lfs,
        lfs_cache_t *pcache, lfs_cache_t *rcache, bool validate,
        lfs_block_t block, lfs_off_t off,
        const void *buffer, lfs_size_t size) {
    const uint8_t *data = buffer;
...
static int lfs_bd_prog(lfs_t *lfs,
    ...
    n=lfs_bd_prog0(lfs, pcache, rcache, validate, block, off, buffer, size);
#if 0
    ithPrintf("[%c] %s(%d, %d:%08X, %d) ", n?'X':'i', __FUNCTION__, block, off, off, size);
    size=(size>8)?8:size;
    if (!n) {
