...
ULONG   *mapping_buf_ptr;
    ...
    _lx_nor_flash_logical_sector_find(nor_flash, logical_sector, LX_FALSE, &old_mapping_address, &old_sector_address, &old_phy_block, &old_phy_sec);
       
    /* Allocate a new physical sector for this write.  */
    _lx_nor_flash_physical_sector_allocate(nor_flash, logical_sector, &new_mapping_address, &new_sector_address, &phy_block, &phy_sec);
    ...
    if (new_mapping_address)
    {
        ...
        if (old_mapping_address)
        {
            ...
            mapping_buf_ptr = nor_flash->lx_nor_flash_mapping_map + old_phy_block * nor_flash -> lx_nor_flash_physical_sectors_per_block + old_phy_sec;
            *mapping_buf_ptr = old_mapping_entry;
        }
        ...
        mapping_buf_ptr = nor_flash->lx_nor_flash_mapping_map + phy_block * nor_flash -> lx_nor_flash_physical_sectors_per_block + phy_sec;
        *mapping_buf_ptr = new_mapping_entry;        
