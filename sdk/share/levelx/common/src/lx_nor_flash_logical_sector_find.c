...
/*                                                                        */
/**************************************************************************/
UINT  _lx_nor_flash_logical_sector_find(LX_NOR_FLASH *nor_flash, ULONG logical_sector, ULONG superceded_check, ULONG **physical_sector_map_entry, ULONG **physical_sector_address, ULONG *phy_block, ULONG *phy_sec/*, UINT load_from_nor*/)
{
    ...
    *physical_sector_address =    (ULONG *) 0;
    ...
    if (nor_flash -> lx_nor_flash_sector_mapping_cache_enabled)
    {
        ...
        if ((sector_mapping_cache_entry_ptr -> lx_nor_sector_mapping_cache_logical_sector) == (logical_sector | LX_NOR_SECTOR_MAPPING_CACHE_ENTRY_VALID))
        {
            ...
            *phy_block = (ULONG)(*physical_sector_map_entry - nor_flash -> lx_nor_flash_base_address) / nor_flash -> lx_nor_flash_words_per_block;
            *phy_sec = ((ULONG)(*physical_sector_map_entry-nor_flash->lx_nor_flash_block_physical_sector_mapping_offset) /sizeof(ULONG)) % (nor_flash -> lx_nor_flash_words_per_block / LX_NOR_SECTOR_SIZE);
    ...
    while (total_blocks--) 
    {
        ...
        while (total_sectors--)
        {
            ...
            ULONG   *mapping_buf_ptr;
            ULONG   list_word_temp;
            list_word_ptr =  block_word_ptr + nor_flash -> lx_nor_flash_block_physical_sector_mapping_offset + j;
            mapping_buf_ptr = nor_flash->lx_nor_flash_mapping_map + i * nor_flash -> lx_nor_flash_physical_sectors_per_block + j;
