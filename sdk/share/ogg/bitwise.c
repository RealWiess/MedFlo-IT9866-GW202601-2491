...
void oggpack_write(oggpack_buffer *b,unsigned long value,int bits){
  ...
  b->ptr[0]|=(unsigned long long)value<<b->endbit;
...
void oggpackB_write(oggpack_buffer *b,unsigned long value,int bits){
  ...
  b->ptr[0]|=(unsigned long long)value>>(24+b->endbit);
...
int main(void){
  ...
  for(i=0;i<test2size;i++){
    ...
    if(oggpack_look(&r,32)!=large[i]){
      fprintf(stderr,"%ld != %ld (%lx!=%lx):",oggpack_look(&r,32),(long)large[i],
  ...
  for(i=0;i<test2size;i++){
    ...
    if(oggpackB_look(&r,32)!=large[i]){
      fprintf(stderr,"%ld != %ld (%lx!=%lx):",oggpackB_look(&r,32),(long)large[i],
