...
#define USERIAL_ASSERT(error_string, assertion)                                                                                                                                    \
    do                                                                                                                                                                             \
    {                                                                                                                                                                              \
        if (!(assertion))                                                                                                                                                          \
        {                                                                                                              \
    ...
    } while (false) // do { (void)(assertion); } while(0)
    ...
    void)
{
    ...
    if (bus_initialised == USERIAL_FALSE)
    {
        ...
        ithGpioCtrlEnable(bt_bus.info.rxPin, ITH_GPIO_PULL_ENABLE); /* Rx pull up */
        ithGpioCtrlEnable(bt_bus.info.rxPin, ITH_GPIO_PULL_UP);
    ...
    tUSERIAL_IOCTL_DATA *p_data)
{
    ...
    case USERIAL_OP_BAUD_WR:
        ...
        /*    (void)printf("change Baud rate event CB (%d)\n", p_data->baud);
...
static void
bt_hci_transport_driver_bus_drop_data(
    uint32_t size)
{
...
userial_result_t
bt_hci_transport_driver_bus_read_handler(
    BT_HDR **pp_packet)
{
    ...
    // (void)printf("HCI EVT 0x%04x", packet_type & 0x0ffff);
    ...
    case HCI_ACL_DATA_PACKET:
    {
        ...
        {
            (void)printf("bt_bus error invalid acl len %i, handle %04x", header.content_length, header.hci_handle);
            result = USERIAL_ERROR;
