...
uint32_t           DA_waitratio = 8;
uint32_t           AD_waitratio = 8;
...
{
    int SleepTime = 0;
    if ((sample <= i2s_buffer_size / 4))
    {
        SleepTime = 0;
    }
    else if ((sample >= i2s_buffer_size / 4) && (sample < i2s_buffer_size / 2))
    {
        SleepTime = waitRatio * size;
    }
    else if ((sample >= i2s_buffer_size / 2) && (sample < 3 * i2s_buffer_size / 4))
    {
        SleepTime = waitRatio * size * 2;
    }
    else
    {
        SleepTime = waitRatio * size * 3;
    }
    // (void)printf("sample=%d waitRatio=%d size=%d SleepTime=%d\n",sample,waitRatio,size,SleepTime);
    usleep(SleepTime);
}
...
int audio_get_bsize_from_bps (STRC_I2S_SPEC * i2s_spec)
{
    ...
    else if ((bps >= 1024) && (bps < 1200))
    {
        _size = 1280;
    }
    else if ((bps >= 512) && (bps < 1024))
    {
        _size = 640;
    }
    else if ((bps >= 256) && (bps < 512))
    {
        _size = 320;
    }
    else
...
int audio_TX_Data_Send (STRC_I2S_SPEC * i2s_spec, uint8_t * ptr, int size)
{
    ...
    {
#if 0
        (void)printf("i2s buffer full set DA_w(%d) = DA_r(%d) (miss some data play)\n",DA_w,DA_r);
#endif
...
int audio_RX_Data_Recv (STRC_I2S_SPEC * i2s_spec, uint8_t * ptr, int size)
{
...
int audio_HDMIRX_Data_Recv (STRC_I2S_SPEC * i2s_spec, uint8_t * ptr, int size, int number)
{
