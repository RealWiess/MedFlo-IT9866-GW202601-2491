...
#define YAFFS_SHORT_NAME_LENGTH		255//15
...
#define	YAFFS_CHECK_LEVEL_NONE (0)
#define	YAFFS_CHECK_LEVEL_WRITE (1)
#define	YAFFS_CHECK_LEVEL_ALL (2)
...
struct yaffs_block_info {
	...
	s32 soft_del_pages;	/* number of soft deleted pages */
	s32 pages_in_use;	/* number of pages in use */
	u32 block_state;	/* One of the above block states. */
				/* NB use unsigned because enum is sometimes
				 * an int */
	u32 needs_retiring;	/* Data has failed on this block, */
				/*need to get valid data off and retire*/
	u32 skip_erased_check;/* Skip the erased check on this block */
	u32 gc_prioritise;	/* An ECC check or blank check has failed.
				   Block should be prioritised for GC */
	u32 chunk_error_strikes;	/* How many times we've had ecc etc
				failures on this block and tried to reuse it */
	u32 has_summary;	/* The block has a summary */

	u32 has_shrink_hdr;	/* This block has at least one shrink header */
	u32 seq_number;		/* block sequence number for yaffs2 */
	u32 erase_cnt;		/* Block erase count */
...
struct yaffs_obj {
	u8 deleted;		/* This should only apply to unlinked files. */
	u8 soft_del;		/* it has also been soft deleted */
	u8 unlinked;		/* An unlinked file.*/
	u8 fake;		/* A fake object has no presence on NAND. */
	u8 rename_allowed;	/* Some objects cannot be renamed. */
	u8 unlink_allowed;
	u8 dirty;		/* the object needs to be written to flash */
	u8 valid;		/* When the file system is being loaded up, this
				 * object might be created before the data
				 * is available
				 * ie. file data chunks encountered before
				* the header.
				 */
	u8 lazy_loaded;	/* This object has been lazy loaded and
				 * is missing some detail */

	u8 defered_free;	/* Object is removed from NAND, but is
				 * still in the inode cache.
				 * Free of object is defered.
				 * until the inode is released.
				 */
	u8 being_created;	/* This object is still being created
				 * so skip some verification checks. */
	u8 is_shadowed;	/* This object is shadowed on the way
				 * to being renamed. */

	u8 xattr_known;	/* We know if this has object has xattribs
				 * or not. */
	u8 has_xattr;		/* This object has xattribs.
...
struct yaffs_param {
	...
	int lazy_gc;				/* Lazy garbage collection mode */
	int eccThr;
};
...
struct yaffs_dev {
	...
	u8 block_info_alt;	/* allocated using alternative alloc */
	u8 chunk_bits_alt;	/* allocated using alternative alloc */
...
void yaffs_handle_chunk_error(struct yaffs_dev *dev,
			      struct yaffs_block_info *bi);
void yaffs_handle_chunk_error_ecc(struct yaffs_dev *dev,
			      struct yaffs_block_info *bi, enum yaffs_ecc_result ecc_result);
