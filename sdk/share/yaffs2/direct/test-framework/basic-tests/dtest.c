...
int cache_bypass_bug_test(const char *mountpt)
{
	...
	yaffs_mount(mountpt);
	sprintf(name,"%s/a",mountpt);
	// Create a file of 2000 bytes.
	make_a_file(name,'X',2000);

	a = yaffs_open(name,O_RDWR, S_IREAD | S_IWRITE);
...
void small_overwrite_test(const char *mountpt,int nmounts)
{
	...
	for(i = 0; i < nmounts; i++){
		...
		for(j = 0; j < 100000; j+=100){
			...
			yaffs_write(h0,xx,700);
			yaffs_write(h1,xx,700);
...
void seek_overwrite_test(const char *mountpt,int nmounts)
{
	...
	printf("seek_overwrite_test finish\n");
}
...
void test_flash_traffic(const char *mountpt)
{
	...
	create_file_of_size(name0,2 * 1024 * 1024);
	dump_dev_stats(dev,"2MB written");
	for(i = 0; i < 10; i++)
		create_file_of_size(name1,1024 * 1024);
	dump_dev_stats(dev,"10x 1MB files written");
...
void format_test(const char *mountpt)
{
	...
	ret = yaffs_format(mountpt, 1, 1, 0);
	printf("yaffs_format(...,1, 1, 0) of unmounted returned %d."
		" Should return 0\n\n\n", ret);
...
int yaffs_main()
{
	...
	while(1)
	{
		...
		scan_pattern_test("C:/",10000,10);

		small_overwrite_test("D:/",10);

		seek_overwrite_test("C:/",10);

		format_test("A:/");

		checkpoint_fill_test("A:/",20);

		format_test("B:/");

		simple_rw_test("B:/");

		cache_bypass_bug_test("A:/");

		format_test("C:/");

		test_flash_traffic("C:/");
