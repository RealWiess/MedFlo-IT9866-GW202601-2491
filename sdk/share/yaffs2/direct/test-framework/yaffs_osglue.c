...
static int bc_gc_init = 0;
static int bg_gc_stop = 0;
...
void yaffsfs_LockInit(void)
{
	...
	pthread_create(&bc_gc_thread, &attr, bg_gc_func, NULL);  //YC mark
...
void yaffsfs_bg_gc_exit()
{
	...
	{
		bg_gc_stop = 1;
		pthread_join(bc_gc_thread, NULL);
		bc_gc_init = 0;
		printf("Yaffs: close bg_gc task.\n");
	}
}
