...
struct yaffsfs_FileDes {
	u8 isDir; 		/* This s a directory */
	u8 reading;
	u8 writing;
	u8 append;
	u8 shareRead;
	u8 shareWrite;
	s32 inodeId;		/* Index to corresponding yaffsfs_Inode */
	s32 handleCount;	/* Number of handles for this fd */
	union {
			...
			int oflag, int mode, int sharing)
{
	...
	if (handle < 0) {
		...
		if (obj && !openDenied) {
			...
			yaffs_strcpy(obj->file_path, path);
...
Y_LOFF_T yaffs_ftell(int handle)
{
	struct yaffsfs_FileDes *fd = NULL;
	Y_LOFF_T pos = -1;
...
int yaffs_close(int handle)
{
	...
	if (!h || !obj || !f)
		...
		/* clean up */
		int filePath_lenth = strlen(obj->file_path);
		if(!f->isDir)
		...
		if(filePath_lenth > 9)
		{
			if(!yaffs_strncmp(obj->file_path + filePath_lenth - 9, ".yaffstmp", 9))
			{
				char filePath_new[NAME_MAX + 1];
				yaffs_strncpy(filePath_new, obj->file_path, filePath_lenth - 9);
				filePath_new[filePath_lenth - 9] = '\0';
				yaffs_rename_reldir(NULL, obj->file_path, filePath_new, 1);
				strlcpy(obj->file_path, filePath_new, sizeof(obj->file_path));
			}
		}
	}
...
int yaffs_rename_reldir(struct yaffs_obj *reldir,
			const YCHAR *oldPath, const YCHAR *newPath, int noLock)
{
...
int yaffs_rename_reldev(struct yaffs_dev *dev, const YCHAR *oldPath,
			const YCHAR *newPath)
{
	return yaffs_rename_reldir(ROOT_DIR(dev), oldPath, newPath, 0);
}
int yaffs_rename(const YCHAR *oldPath, const YCHAR *newPath)
{
	return yaffs_rename_reldir(NULL, oldPath, newPath, 0);
}
...
int yaffs_unmount2_common(struct yaffs_dev *dev, const YCHAR *path, int force)
{
	...
	if (dev) {
		if (dev->is_mounted) {
			...
			yaffs_remove_device(dev);  //YC add for unmount hang issue, 20240802
...
struct yaffs_dirent *yaffsfs_readdir_no_lock(yaffs_DIR * dirp)
{
	...
	if (dsc && dsc->inUse) {
		...
		if (dsc->nextReturn) {
			...
			dsc->de.d_namlen = strlen(dsc->de.d_name);
