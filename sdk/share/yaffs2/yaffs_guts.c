...
static int yaffs_find_alloc_block(struct yaffs_dev *dev)
{
	...
	for (i = dev->internal_start_block; i <= dev->internal_end_block; i++) {
		...
		if (bi->block_state == YAFFS_BLOCK_STATE_EMPTY) {
			...
			u32 j;
			struct yaffs_ext_tags checkTag;
			int erased_ok = 1;
			if(dev->param.lazy_gc)
			{
				if(bi->erase_cnt == 0)
				{
					erased_ok = yaffs_erase_block(dev, dev->alloc_block_finder);
				}				
			}
			else
			{
				for(j = 1 ; j < dev->param.chunks_per_block ; j ++)
				{
					int ret = dev->tagger.read_chunk_tags_fn(dev,
							dev->alloc_block_finder * dev->param.chunks_per_block + j,
							NULL, &checkTag);
					if(!ret || checkTag.ecc_result)
					{
						erased_ok = yaffs_erase_block(dev, dev->alloc_block_finder);
						yaffs_trace(YAFFS_TRACE_ERROR,
							"Error on erased block %d, page %d, ret %d, ecc %d", dev->alloc_block_finder, j, ret, checkTag.ecc_result);
						break;
					}
				}
			}
			if(!erased_ok)
			{
				...
				yaffs_trace(YAFFS_TRACE_ERROR | YAFFS_TRACE_BAD_BLOCKS,
					"re-erase failed %d", dev->alloc_block_finder);
				yaffs_retire_block(dev, dev->alloc_block_finder);
				continue;
			}
				 ...
				 struct yaffs_ext_tags *tags, int use_reserver)
{
	...
	do {
		...
		if (dev->param.always_check_erased == YAFFS_CHECK_LEVEL_ALL)
		...
		if (!bi->skip_erased_check || dev->param.always_check_erased >= YAFFS_CHECK_LEVEL_WRITE)
		{
			write_ok =
			    yaffs_verify_chunk_written(dev, chunk, data, tags);
			if(write_ok == YAFFS_OK && tags->ecc_result)	// ECC bits occur when wrote, skip this block 
			{
				erased_ok = 0;
				write_ok = YAFFS_FAIL;
				yaffs_trace(YAFFS_TRACE_ERROR,
				  "**>> yaffs chunk %d write error, ret %d, ecc %d",
				  chunk, write_ok, tags->ecc_result);
