...
/******************************************************************************
 * the read flow for reading the GT911's register by using iic repead start
 ******************************************************************************/
...
static int gtp_i2c_write(uint8_t* cmd, uint32_t cmdlen, uint8_t *buf, uint32_t len)
{
	...
	if(cmdBuf != NULL)
	{
		...
		gt911_evt = alloca(sizeof(*gt911_evt));
		gt911_evt->slaveAddress   = TP_I2C_DEVICE_ID;
		gt911_evt->cmdBuffer      = cmdBuf;
		gt911_evt->cmdBufferSize  = cmdlen + len;
		gt911_evt->dataBufferSize = 0;		
