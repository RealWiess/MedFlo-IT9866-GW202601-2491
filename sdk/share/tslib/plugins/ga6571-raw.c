...
//#define   ENABLE_GA6571_FW_UPGRADE
/****************************************************************************
 * touch cofig setting
 ****************************************************************************/
...
#define GTP_REG_READ_POINT    0x814F	//0x8150
...
extern uint8_t tpReseting;

//#define   ENABLE_GA6571_PATCH_TOUCH_UP_ISSUE
 ...
 ******************************************************************************/
int gtp_i2c_read(uint8_t* cmd, uint8_t *buf, uint32_t len)
{
    int i2cret= (int)-1;
    ITPI2cInfo *ga6571_evt;

    ga6571_evt = alloca(sizeof(*ga6571_evt));
    ga6571_evt->slaveAddress   = TP_I2C_DEVICE_ID;
    ga6571_evt->cmdBuffer      = cmd;
    ga6571_evt->cmdBufferSize  = 2;
    ga6571_evt->dataBuffer     = buf;
    ga6571_evt->dataBufferSize = len;
    i2cret = read(gTpInfo.tpDevFd, ga6571_evt, 1);
    return i2cret;
}

int gtp_i2c_write(uint8_t* cmd, uint32_t cmdlen, uint8_t *buf, uint32_t len)
{
    int i2cret = (int)-1;
    ITPI2cInfo *ga6571_evt;
	uint8_t	*cmdBuf = NULL;

	cmdBuf = (uint8_t*)malloc(cmdlen + len);
	if(cmdBuf != NULL)
	{
		memcpy(cmdBuf, cmd, cmdlen);
		memcpy(&cmdBuf[cmdlen], buf, len);
		
		ga6571_evt = alloca(sizeof(*ga6571_evt));
		ga6571_evt->slaveAddress   = TP_I2C_DEVICE_ID;
		ga6571_evt->cmdBuffer      = cmdBuf;
		ga6571_evt->cmdBufferSize  = cmdlen + len;
		ga6571_evt->dataBufferSize = 0;		
		i2cret = write(gTpInfo.tpDevFd, ga6571_evt, 1);	
...
void _tpDoPowerOnSeq_vendor(void)
{
    //for ga6571 Linux  power on methods.

    printf("DO POWER-ON sequence, reset pin:%d\n",gTpSpec.tpResetPin);
    if(gTpSpec.tpResetPin == (char)-1) return;

    //1.set "Reset pin" are output-low for 1ms
    ...
    //set INT pin" are input-low for 1ms
    ...
    //2.set "Reset pin" output HIGH for 100ms
    ithGpioSet(gTpSpec.tpResetPin);
    usleep(100*1000);
    //3.set "Reset pin" output LOW for 10ms
    ithGpioClear(gTpSpec.tpResetPin);
    usleep(10*1000);
    //4.set "Reset pin" output HIGH for 60ms
    ithGpioSet(gTpSpec.tpResetPin);
    usleep(60*1000);

    //TT 5 ms ~ 20ms
    //5.set "INT pin" input HIGH for 5 ms
    ithGpioSet(gTpSpec.tpIntPin);
    usleep(5*1000);
    //6.set "INT pin" input LOW for 5 ms
    ithGpioClear(gTpSpec.tpIntPin);
    usleep(5*1000);
    //7.set "INT pin" input HIGH for 50 ms
    ithGpioSet(gTpSpec.tpIntPin);
    usleep(50*1000);
...
static int _tpDoInitProgram_vendor(void)
{
    ...
    #ifdef ENABLE_GA6571_FW_UPGRADE
    ...
    //for workaround the ga6571 version A F/W issue
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
    ...
    //printf("tick1:%d\n",tick1);
    gtp_tpResetLogic(tmpBuf[0]);
    //uint32_t tick2 = SDL_GetTicks();
    //printf("tick2:%d\n",tick2);
#endif
    if((tmpBuf[0] & 0x80) == 0)
    {
    ...
    {
        #ifdef  ENABLE_GA6571_PATCH_TOUCH_UP_ISSUE
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
    ...
    if(loopNum)
    {
        ...
        for(i = 0; i < loopNum; i++)
        {
            ...
            smp->id = (unsigned int)(buf[8*i + 1]&0xF); //touch id

            smp->finger = loopNum;

            smp->x = (int)((((unsigned int)buf[3+8*i]<<8)&0x0F00) | ((unsigned int)buf[2+8*i]));
            smp->y = (int)((((unsigned int)buf[5+8*i]<<8)&0x0F00) | ((unsigned int)buf[4+8*i]));        	    
    ...
    while(real_nr++<nr)
    {
        ...
        switch(res)
        {
            ...
            default:    //error, send touch-up
                /*
                  To skip some ga6571's meaningless INT signal when TP event is touch-up(TOUCH_NO_CONTACT)
                */
 ...
 */
static int ga6571_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops ga6571_ops =
{
    ga6571_read,
};

TSAPI struct tslib_module_info *ga6571_mod_init(struct tsdev *dev, const char *params)
{
    ...
    TSLIB_MODULE_INIT(ga6571_mod_init);
#endif
