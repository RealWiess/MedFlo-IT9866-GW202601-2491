...
#ifdef  TP_MULTI_FINGER_ENABLE
    gTpSpec.tpReadChipRegCnt    = (int)(MAX_FINGER_NUM * 4);
#else
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	char loopNum = (char)MAX_FINGER_NUM;
	
	if( gTpSpec.tpSampleNum > 1)
	{	
		for(i = 0; i < MAX_FINGER_NUM; i++)
		{
			if( (buf[4*i] & 0x80) == 0x80 )
			{
				numOfTchPnt++;
				lastFnIndex = i + 1;
			}
	...
	{
		if( (buf[0] & 0x80) == 0x80 )	numOfTchPnt = 1;
	}
	
	if(!numOfTchPnt)	return (-1);
	...
	{		
		...
		for(i = 0; i < loopNum; i++)
		{
			if( (buf[4*i] & 0x80) == 0x80 )
			{
				smp->x = (int)((((unsigned int)buf[4*i]<<4)&0x0700) | ((unsigned int)buf[4*i + 1]));
				smp->y = (int)((((unsigned int)buf[4*i]<<8)&0x0700) | ((unsigned int)buf[4*i + 2]));
				smp->pressure = 1;
				smp->id = i;
 ...
 */ 
static int st1912_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops st1912_ops =
{
	st1912_read,
};

TSAPI struct tslib_module_info *st1912_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(st1912_mod_init);
#endif
