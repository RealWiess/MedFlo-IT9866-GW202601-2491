...
extern volatile int gTpPowerOnSeqDone;
#endif
/*##################################################################################
...
static bool _raChkIntActive(void)
{
	...
	if(g_raUsbIntCnt)
	{
		//printf("	[RA] got UIC(c=%d) \n",g_raUsbIntCnt);			
		return true;
	}
	#endif
	
	if(RA_INT_PIN == RA_GPIO_PIN_NO_DEF)	return false;
		
	regValue = ithGpioGet(RA_INT_PIN);
...
{
	if(RA_INT_PIN == RA_GPIO_PIN_NO_DEF)	return;
...
static unsigned int _raSendKeypadValue(void)
{
	uint32_t var=0;
    pthread_mutex_lock(gpRaSpec->raMutex);
...
static bool _raNeedToGetSample(void)
{
    ...
    if(_raGetIntr()==true)
    {
    	...
    	else
    	{
        	...
        	if(g_raUsbIntCnt)
        	{
        		//force to set "gpRaInfo->tpNeedToGetSample = 1"
        		//printf("	needGetSpl: %d, %d\n",g_raUsbIntCnt, gpRaInfo->tpNeedToGetSample);
        		if(!gpRaInfo->tpNeedToGetSample) gpRaInfo->tpNeedToGetSample = 1;
        	}
        	#endif
		...
		else
    	{
    	 	struct ts_sample *s1 = (struct ts_sample *)(&tmpNxt[i-1]);
    	  	struct ts_sample *s2 = (struct ts_sample *)(&ts[i]);
    	   	...
    	   	if( (s1==NULL) || (s2==NULL) )
    	   	{
    	   		printf("	[TP ERROR]: incorrect pointer:s1=%p, s2=%p\n",s1,s2);
    	   		continue;
    	   	}
...
int _raDoInitial(void)
{
    ...
    gpRaSpec->tpMoveDetectUnit = 1;	//default value
    
    //initialize the TP SPEC first.
    gpRaSpec->rawApi.raInitSpec();
    	
#ifdef CFG_ITP_TP_INIT_ENABLE 
    while(gTpPowerOnSeqDone == 0)
    {
    	usleep(2000);
    }
#else
    ...
    if(raQueue == NULL)
    {
    	printf( "[TouchPanel]%s() L#%ld: ERROR, create xQueueCreate() fail! res=%p\n", __FUNCTION__, __LINE__, raQueue );
    	return -1;
    }
#endif
