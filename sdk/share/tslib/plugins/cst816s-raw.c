...
2021/12/16: TP NOTE
	[]exist reset pin
	[]single touch
	[]7-bytes data-register
	[]use FT53xx's INT-type
	[]Got this TP's datasheet & sample code, but programming guide.
*/
...
static void _tpDoPowerOnSeq_vendor(void)
{
    ...
    else
    {
    	...
    	//1.set "Reset pin" is output-low for 10ms	
    	ithGpioSetMode( rstPin, ITH_GPIO_MODE0);
    	ithGpioClear(rstPin);
    	ithGpioSetOut(rstPin);
    	ithGpioEnable(rstPin);	
    	
    	//pull low for >20ms(NOT sure)
    	usleep(20*1000);
    
        //2.set "Reset pin" output HIGH for 200ms(NOT sure)
    	ithGpioSet(rstPin);
    	usleep(200*1000);
    }
}
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	{		
		...
		for(i = 0; i < loopNum; i++)
		{
			if( buf[2] & 0x01 )	smp->pressure = 1;
			else	smp->pressure = 0;
				
			smp->id = (unsigned int)(0); //touch id
			smp->finger = loopNum;
			smp->x = (int)((((unsigned int)buf[3]<<8)&0x0F00) | ((unsigned int)buf[4]));
			smp->y = (int)((((unsigned int)buf[5]<<8)&0x0F00) | ((unsigned int)buf[6]));
 ...
 */ 
static int cst816s_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops cst816s_ops =
{
	cst816s_read,
};

TSAPI struct tslib_module_info *cst816s_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(cst816s_mod_init);
#endif
