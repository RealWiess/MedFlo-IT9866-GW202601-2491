...
//#define	ENABLE_GT9271_FW_UPGRADE
//#define	ENABLE_GT9271_PATCH_TOUCH_UP_ISSUE
/****************************************************************************
 * touch cofig setting
 ****************************************************************************/
...
#ifdef	ENABLE_GT9271_PATCH_TOUCH_UP_ISSUE
...
/******************************************************************************
 * the read flow for reading the GT9271's register by using iic repead start
 ******************************************************************************/
static int gtp_i2c_read(uint8_t* cmd, uint8_t *buf, uint32_t len)
{
	int i2cret= (int)-1;
	ITPI2cInfo *gt9271_evt;

	gt9271_evt = alloca(sizeof(*gt9271_evt));
	gt9271_evt->slaveAddress   = TP_I2C_DEVICE_ID;
	gt9271_evt->cmdBuffer      = cmd;
	gt9271_evt->cmdBufferSize  = 2;
	gt9271_evt->dataBuffer     = buf;
	gt9271_evt->dataBufferSize = len;	
	i2cret = read(gTpInfo.tpDevFd, gt9271_evt, 1);	
	return i2cret;
}

static int gtp_i2c_write(uint8_t* cmd, uint32_t cmdlen, uint8_t *buf, uint32_t len)
{
	int i2cret = (int)-1;
	ITPI2cInfo *gt9271_evt;
	uint8_t	*cmdBuf = NULL;
	...
	if(cmdBuf != NULL)
	{
		...
		gt9271_evt = alloca(sizeof(*gt9271_evt));
		gt9271_evt->slaveAddress   = TP_I2C_DEVICE_ID;
		gt9271_evt->cmdBuffer      = cmdBuf;
		gt9271_evt->cmdBufferSize  = cmdlen + len;
		gt9271_evt->dataBufferSize = 0;		
		i2cret = write(gTpInfo.tpDevFd, gt9271_evt, 1);	
...
static void _tpDoPowerOnSeq_vendor(void)
{
	//for gt9271 Linux  power on methods.	
	char rstPin = (char)TP_GPIO_RESET_PIN;
	char intPin = (char)TP_INT_PIN;
	...
	ithGpioSetMode( intPin, ITH_GPIO_MODE0);
	ithGpioClear(intPin);
	ithGpioSetOut(intPin);
	ithGpioEnable(intPin);
...
static int _tpDoInitProgram_vendor(void)
{
	...
	#ifdef ENABLE_GT9271_FW_UPGRADE
	...
	//for workaround the GT9271 version A F/W issue
 ...
 */ 
static int gt9271_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops gt9271_ops =
{
	gt9271_read,
};

TSAPI struct tslib_module_info *gt9271_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(gt9271_mod_init);
#endif
