...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpReadChipRegCnt    = (int)9;
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
	...
	//it must use 2 I2C cmd to get the X/Y data
	//It can not be merged into one-write-read flow
	//and it's point buffer address also be chaned into 0x00(ori is 0x12)
	//and again, the structure of point buffer is also changed.
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	if( (buf[2] & 0xF) <= 2 )
	{
		...
		{
			s->x = (int)((((unsigned int)buf[3]<<8)&0x0100) | ((unsigned int)buf[4]));
			s->y = (int)((((unsigned int)buf[5]<<8)&0x0100) | ((unsigned int)buf[6]));
			s->pressure = 1;
		}		
 ...
 */ 
static int chsc6540_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops chsc6540_ops =
{
	chsc6540_read,
};

TSAPI struct tslib_module_info *chsc6540_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(chsc6540_mod_init);
#endif
