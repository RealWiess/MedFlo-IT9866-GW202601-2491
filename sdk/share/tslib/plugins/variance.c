			    ...
			    int max_slots, int nr)
{
	...
	if (var->cur_mt == NULL || max_slots > var->slots || nr > var->nr) {
		printf("tslib: WARNING: no multitouch using VARIANCE filter: %p (%x,%x)(%x,%x)\n",var->cur_mt,max_slots,var->slots,nr ,var->nr);
		if (var->cur_mt) {
			for (i = 0; i < var->nr; i++) {
				if (var->cur_mt[i])
					free(var->cur_mt[i]);
		...
		for (i = 0; i < nr; i++) {
			...
			
			{
			    ...
			    if(my_flag != max_slots) printf("variance max_slots = %d, %p\n",max_slots, samp_mt[0][0].next);
