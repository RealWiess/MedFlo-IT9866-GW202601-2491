    ...
    while(real_nr++<nr)
    {
        ...
        switch(res)
        {
            ...
            default:    //error, send touch-up
                /*
                  To skip some rm5t66a's meaningless INT signal when TP event is touch-up(TOUCH_NO_CONTACT)
                */
...
static int _tpProbeSample(struct ts_sample *samp, int nr)
{
    ...
    {
        printf("Warning: need to swap sample0 with other sample, sp=%p, fn=%x, p=%d\n", s, s->finger, s->pressure);
    }
#endif
