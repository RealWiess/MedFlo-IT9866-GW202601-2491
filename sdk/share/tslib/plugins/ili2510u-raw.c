...
[2021.11.10] porting NOTE:
	[]this is a USB interface Touch panel(without reset & INT pins)
	[]Need to enable USBHCC & USBHCC_HOST & USBH_CD_HID
	[]Need the itp_tp.c for ts_open() in "SDL_castor3touch.c"
	[]currently supported 5 fingers multi-touch
	[]Muximun of X is 0x3FFF, Muximun of Y is 0x257F (default resolution is 1280x480)
*/
...
#ifdef CFG_TOUCH_USB
extern unsigned int g_raUsbIntCnt = 0;
...
/*##################################################################################
 *                        the private function implementation of itp's USB HID driver
 ###################################################################################*/
void itp_usbh_hid_connect_cb(uint8_t uid)
{
    ithPrintf("tp:uid(%d) %s(%d)\n", uid, __FUNCTION__, __LINE__);
}
...
void itp_usbh_hid_receive_report_cb(uint8_t uid, uint8_t report_id, uint8_t *report_data, uint32_t report_size)
{
    ...
    //ithPrintVram8((uint32_t)report_data, report_size);
   	...
   	memcpy(tmpBuf , report_data, 32);

   	if (xQueueSend(usbQueue, tmpBuf, 0) != pdTRUE)	sendQueErr = 1;
   	...
   	if (sendQueErr)
    {
    	printf("	sendQueErr: QC=%d, %d\n",g_raUsbIntCnt, sendQueErr);
    }	
    //printf("	*** SQ: %d ***\n",g_raUsbIntCnt);
}
...
static void _tpInitSpec_vendor(void)
{
    gTpSpec.tpIntPin          	= (char)TP_GPIO_PIN_NO_DEF;           //from Kconfig setting
    ...
    gTpSpec.tpInterface         = (char)TP_INTERFACE_USB;	  //from Kconfig setting
    ...
    printf("	To create ILI2510u's usb queue for TP's data\n"); 
    usbQueue = xQueueCreate(TP_QUEUE_LEN, (unsigned portBASE_TYPE)32);
    if(usbQueue == NULL)
    {
    	printf( "[TouchPanel]%s() L#%ld: ERROR, create usbQueue() fail! res=%ld\n", usbQueue );
    }
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
    ...
    if ( xQueueReceive(usbQueue, buf, 0)!=pdTRUE )
    {        	
      	printf("	xQue-NoQue:usbQue=0\n");
    }
...
{	
	...
	{
		for(i=0; i<gTpSpec.tpSampleNum; i++)	if(buf[(i * 5) + 0] & 0x40)	numOfTchPnt++;
	}
	...
	{		
		...
		{
			if(buf[(i * 5) + 0] & 0x40)	smp->pressure = 1;
			else	smp->pressure = 0;
				
			smp->id = (unsigned int)(buf[(i * 5) + 0] & 0xF)	; //touch id
			smp->finger = numOfTchPnt;
			smp->x = (int)(((unsigned int)(buf[i*5 + 2]&0x3F)<<8) + buf[i*5 + 1]);
			smp->y = (int)(((unsigned int)(buf[i*5 + 4]&0x3F)<<8) + buf[i*5 + 3]);
 ...
 */ 
static int ili2510u_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops ili2510u_ops =
{
	ili2510u_read,
};

TSAPI struct tslib_module_info *ili2510u_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(ili2510u_mod_init);
#endif
