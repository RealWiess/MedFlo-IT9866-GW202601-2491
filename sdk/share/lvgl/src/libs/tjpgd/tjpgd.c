...
)
{
    ...
    while(ndata) {  /* Process all tables in the segment */
		...
		if (i < 3) {
			uint8_t* pd = alloc_pool(jd, 64 * sizeof (int8_t));/* Allocate a memory block for the table */
			if (!pd) return JDR_MEM1;				/* Err: not enough memory */
			jd->qtdata[i] = pd;						/* Register the table */
			memcpy(pd, data, 64);
		}
#endif
...
#ifdef CONFIG_ITE_ITJPEG
                        jd->samp[0][0] = b >> 4; jd->samp[0][1] = b & 15;
#endif
                    }
                    else {          /* Cb/Cr component */
                        if(b != 0x11 && b != 0x12) return JDR_FMT3;          /* Err: Sampling factor of Cb/Cr must be 1 */
#ifdef CONFIG_ITE_ITJPEG
					    jd->samp[i][0] = b >> 4; jd->samp[i][1] = b & 15;
#endif
                    }
