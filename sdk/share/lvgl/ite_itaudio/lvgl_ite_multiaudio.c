...
int lv_ite_audio_start (int id)
{
    ...
    {
        case 0:
            flowstream_start_play(f, p->playpath, p->mode, p->eof_cb);
            break;
        case 1:
            flowstream_mix_sound(f, p->playpath, p->mode, p->eof_cb);
            break;
    }
...
void lv_ite_audio_pause (int id)
{
    lv_iteaudio_t * p = &play[id];
    IteFlower *     f = p->flower;
    flowstream_status_set(f, FILTER_PAUSE);
    if (flowstream_check_unmix())
    {
        i2s_pause_DAC(true);
    }
...
void lv_ite_audio_resume (int id)
{
    lv_iteaudio_t * p = &play[id];
    IteFlower *     f = p->flower;
    flowstream_status_set(f, FILTER_RESUME);
