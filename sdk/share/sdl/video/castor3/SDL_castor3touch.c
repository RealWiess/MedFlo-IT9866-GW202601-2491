...
#define	DO_CHECK_CURRENT_FINGER	TSLIB_CHECK_MAGIC_NUM
...
static uint8_t gTpJustCheckCurrFingers = 0;
...
static bool check_last_sample(int tId, struct ts_sample* smp)
{
	...
	if( lstSmp[tId].id != tmpID )	tRst = false;
...
void Castor3_PumpTouchEvent(void)
{
	...
	if(c3samp[0].chkCurrFn == DO_CHECK_CURRENT_FINGER)
	{
		gTpJustCheckCurrFingers = 1;
	}
	...
	if(gLastFingerNum!=curFgr)
	{
		...
		if(gLastFingerNum>curFgr)
		{
			//printf("	txy0(%d,%d)(%d,%d,%d)\n", 0, c3samp[0].id, c3samp[0].pressure, c3samp[0].x, c3samp[0].y);
			//printf("	txy0(%d,%d)(%d,%d,%d)\n", 1, c3samp[1].id, c3samp[1].pressure, c3samp[1].x, c3samp[1].y);
			
			if((gLastFingerNum == 2) && (curFgr == 1))
			{
				if(c3samp[0].id == c3samp[1].id)
				{
					//printf("	GOT TchEvId Changed~~~\n");
					c3samp[1].id = lstSmp[0].id;
				}
			}
	...
	if(gTpJustCheckCurrFingers == 0)
	{
		if(gLastFingerNum>curFgr)
		{
			curFgr = gLastFingerNum;
		}
	}
