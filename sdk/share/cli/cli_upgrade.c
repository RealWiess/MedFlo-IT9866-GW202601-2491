...
static portBASE_TYPE prvTaskUploadCommand (char *       pcWriteBuffer,
                                           ...
                                           const char * pcCommandString)
{
    ...
    if (xReady == pdFALSE)
    {
        ...
        if (strncmp(pcParameter1, "0x", 2) == 0 ||
            strncmp(pcParameter1, "0X", 2) == 0)
        {
        ...
        CLI_LOG_DBG("%s: size=%d\n", pcParameter1, packageSize);
    ...
    else
    {
        ...
        do
        {
            ...
            readsize =
                read(ITP_DEVICE_USBDACM, &packageBuffer[bufpos], remainsize);

            CLI_LOG_DBG("readsize=%u remainsize=%u\n", readsize, remainsize);
...
static portBASE_TYPE prvTaskUpgradeCommand (char *       pcWriteBuffer,
                                            size_t       xWriteBufferLen,
                                            const char * pcCommandString)
{
    ...
    if (packageBuffer)
    {
        ...
        CLI_LOG_INFO("Flushing NOR cache...\n");
        ioctl(ITP_DEVICE_NOR, ITP_IOCTL_FLUSH, NULL);
#endif

#if (CFG_LFS_CACHE_SIZE > 0)
        CLI_LOG_INFO("Flushing LFS cache...\n");
...
static portBASE_TYPE prvTaskUpgradeHandshakeCommand (
    char * pcWriteBuffer, size_t xWriteBufferLen, const char * pcCommandString)
{
    ...
    if (xReady == pdFALSE)
    {
        ...
        CLI_LOG_INFO("Start CLI handshake upgrade...\n");
        CLI_LOG_DBG("%s: size=%d\n", pcParameter1, packageSize);

        ITCCliStream cliStream;
        itcCliStreamOpen(&cliStream, ITP_DEVICE_USBDACM, packageSize);

        if (ugCheckCrc(&cliStream.stream, NULL))
        {
            snprintf((char *)pcWriteBuffer, xWriteBufferLen, "%s\r\n",
                     "ERROR: Check CRC fail");
            goto error;
        ...
        ret = ugUpgradePackage(&cliStream.stream);
...
static const xCommandLineInput xTaskUploadCommand = {
    ...
    (const char * const)"upload <size>: Upload package file with specified "
                        "size. USB ACM only.\r\n",
...
static const xCommandLineInput xTaskUpgradeHandshakeCommand = {
    (const char * const)"ughs",
    (const char * const)"upgrade: Upgrade package file using handshake "
                        "mode\r\n",
    prvTaskUpgradeHandshakeCommand,
    1
};
// clang-format on
...
void cliUpgradeInit (void)
{
    ...
    FreeRTOS_CLIRegisterCommand(&xTaskUpgradeHandshakeCommand);
}
