...
static void * CliTask (void * arg)
{
    ...
    char cmdbuf[128];
        ...
        defined(CFG_MT7921S_USE_UART0_CLI)
    ...
    while (!cliQuit)
    {
        ...
        if (uart_ret)
        {
            for (i = 0; i < uart_ret; i++)
            {
                ...
                readPos++;
    #if defined(CFG_MT7921S_USE_UART0_CLI)
                if (cInputString[i] == '\0' || cInputString[i] == '\n')
                {
                    cmdbuf[readPos - 1] = '\0';
    #else
                if (cInputString[i] == '\0')
    ...
    procCmd:
        do
        {
            ...
            xReturned = FreeRTOS_CLIProcessCommand(
                cmdbuf, pcOutputString, configCOMMAND_INT_MAX_OUTPUT_SIZE);
    ...
    while (!cliQuit)
    {
        ...
        if (ret == 0)
        {
            CLI_LOG_DBG("read null\n");
            continue;
        }
        CLI_LOG_DBG("read(%d) %s\n", ret, cInputString);
        ...
        do
        {
            ...
            if (len > 0)
            {
                ...
                CLI_LOG_DBG("write(%d) %s", ret, pcOutputString);
            }
...
void cliInit (void)
{
    ...
    (defined(CFG_SD0_STATIC) || defined(CFG_SD1_STATIC) ||                     \
     defined(CFG_NAND_ENABLE) || defined(CFG_NOR_ENABLE))
    cliRawDataInit();
#endif

#if defined(CFG_OPENRTOS_USE_TRACE_FACILITY) &&                                \
    defined(CFG_OPENRTOS_GENERATE_RUN_TIME_STATS)
    cliStatsInit();
#endif
