...
static portBASE_TYPE prvTaskWriteRawDataCommand (char *       pcWriteBuffer,
                                                 size_t       xWriteBufferLen,
                                                 const char * pcCommandString)
{
    static portBASE_TYPE xReady = pdFALSE;
    static int           fd     = -1;
    static uint32_t position, filesize, blocksize, gapsize, alignsize, bufsize,
        pos, pos2;
    ...
    if (xReady == pdFALSE)
    {
        ...
        pcParameter2 = (char *)FreeRTOS_CLIGetParameter(
            pcCommandString, 2, &xParameter2StringLength);
        pcParameter3 = (char *)FreeRTOS_CLIGetParameter(
            pcCommandString, 3, &xParameter3StringLength);

        if (xParameter1StringLength == 0 || xParameter2StringLength == 0 ||
            xParameter3StringLength == 0)
        {
        ...
        CLI_LOG_DBG("blocksize=%d gapsize=%d\n", blocksize, gapsize);

        if (strncmp(pcParameter2, "0x", 2) == 0 ||
            strncmp(pcParameter2, "0X", 2) == 0)
        {
        ...
        if (strncmp(pcParameter3, "0x", 2) == 0 ||
            strncmp(pcParameter3, "0X", 2) == 0)
        {
        ...
        CLI_LOG_DBG("%s: position=0x%X size=%u\n", pcParameter1, position,
                    filesize);
        ...
        if (alignsize > 0)
        {
            CLI_LOG_WARN("Position 0x%X and block+gap size 0x%X are not "
                         "aligned; align to 0x%X\n",
                         position, (blocksize + gapsize),
                         (pos * (blocksize + gapsize)));
        }
    ...
    else
    {
        ...
        do
        {
            ...
            do
            {
                ...
                size2 +=
                    read(ITP_DEVICE_USBDACM, buf + size2, readsize - size2);

                CLI_LOG_DBG("readsize=%u size2=%u\n", readsize, size2);
...
static portBASE_TYPE prvReadRawDataCommand (char *       pcWriteBuffer,
                                            size_t       xWriteBufferLen,
                                            const char * pcCommandString)
{
    static portBASE_TYPE xReady = pdFALSE;
    ...
    else
    {
        ...
        do
        {
            ...
            if (size2 != alignsize)
            {
                CLI_LOG_ERR("ERROR: Cannot read data; %u != %u\n", size2,
                            alignsize);
                pcWriteBuffer[0] = '\0';
                goto error;
            }

            CLI_LOG_DBG("write: size=%u\n", readsize);
            ...
            do
            {
                if (ioctl(ITP_DEVICE_USBDACM, ITP_IOCTL_IS_CONNECTED, NULL) ==
                    0)
                {
                    CLI_LOG_ERR("ERROR: USB disconnected\n");
                    pcWriteBuffer[0] = '\0';
                    goto error;
                }
        ...
        if (filesize % 512 == 0) // workaround linux-cdc-acm.inf cannot receive
                                 // multiple of 512 bytes bug
        {
            size2 = write(ITP_DEVICE_USBDACM, pcWriteBuffer, 1);
            CLI_LOG_DBG("workaround written size: %u\n", size2);
        }
...
static const xCommandLineInput xWriteRawDataCommand = {
    ...
    (const char * const)"write-rawdata <nand|nor|sd0|sd1> <position> <size>: "
                        "Writes data to specified storage with position and "
                        "size. position must align to block size. USB ACM "
                        "only.\r\n",
...
static const xCommandLineInput xReadRawDataCommand = {
    ...
    (const char * const)"read-rawdata <nand|nor|sd0|sd1> <position> <size>: "
                        "Reads data from specified storage with position and "
                        "size. position must align to block size. USB ACM "
                        "only.\r\n",
