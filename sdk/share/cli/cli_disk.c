 ...
 */
static portBASE_TYPE     prvDirCommand (char *       pcWriteBuffer,
                                        ...
                                        const char * pcCommandString)
{
    ...
    do
    {
        ...
        {
            /* This is the first time the function has been called this run of
            the dir command.  Ensure the directory is open. */
            char * current_path = getcwd(cRAMBuffer, sizeof(cRAMBuffer));
            if (current_path == NULL)
            {
                break;
            }

            xDirectory = opendir(current_path);
            if (xDirectory != NULL)
            {
        ...
        if (xDirectoryOpen == pdTRUE)
        {
            ...
            if (pxFileInfo != NULL)
            {
                ...
                current_path = getcwd(cRAMBuffer, sizeof(cRAMBuffer));
                if (current_path == NULL)
                {
                    break;
                }

                strlcat(cRAMBuffer, pxFileInfo->d_name, sizeof(cRAMBuffer));
                if (stat(cRAMBuffer, &sbuf) != 0)
                {
                    break;
                }
 ...
 * more than one task at a time.
 */
static portBASE_TYPE prvDelCommand (char *       pcWriteBuffer,
                                    size_t       xWriteBufferLen,
                                    const char * pcCommandString)
{
    char *                       pcParameter;
    ...
    do
    {
        ...
        pcParameter = (char *)FreeRTOS_CLIGetParameter(
            pcCommandString, uxFirstParameter, &xParameterStringLength);
                                     ...
                                     const char * pcCommandString)
{
    ...
    do
    {
        ...
        pcParameter1 = (char *)FreeRTOS_CLIGetParameter(
            pcCommandString, uxFirstParameter, &xParameter1StringLength);

        /* Obtain the name of the destination file, and the length of its name.
         */
        pcParameter2 = (char *)FreeRTOS_CLIGetParameter(
            pcCommandString, uxSecondParameter, &xParameter2StringLength);
        ...
        pxCommandLineFile1 = fopen((const char *)pcParameter1, "rb");
        if (pxCommandLineFile1 != NULL)
        {
            /* Open the destination file. */
            pxCommandLineFile2 = fopen((const char *)pcParameter2, "wb");
            if (pxCommandLineFile2 != NULL)
            {
                while (xFinished == pdFALSE)
                {
                    /* About to use the RAM buffer, ensure this task has
                    exclusive access to it while it is in use. */
                    if (xSemaphoreTake(xRamBufferMutex, xMaxDelay) == pdPASS)
                    {
                        if ((xBytesRead =
                                 fread(cRAMBuffer, 1, sizeof(cRAMBuffer),
                                       pxCommandLineFile1)) != EOF)
                        {
                            if (xBytesRead != 0U)
                            {
                                if ((xBytesWritten = fwrite(
                                         cRAMBuffer, 1, xBytesRead,
                                         pxCommandLineFile2)) == xBytesRead)
                                {
                                    if (xBytesWritten < xBytesRead)
                                    {
                                        snprintf((char *)pcWriteBuffer,
                                                 xWriteBufferLen,
                                                 "Error writing to %s, disk "
                                                 "full?\r\n\r\n",
                                                 pcParameter2);
                                        xFinished = pdTRUE;
                                    }
...
static const xCommandLineInput xDirCommand = {
    ...
    (const char * const)"dir: Displays the name and size of each file in the "
                        "root directory\r\n",
    prvDirCommand,
    0
};
...
static const xCommandLineInput xCopyCommand = {
    ...
    (const char * const)"copy <filename1> <filename2>: Copies <filename1> to "
                        "<filename2>, creating or overwriting <filename2>\r\n",
    prvCopyCommand,
    2
};
// clang-format on
