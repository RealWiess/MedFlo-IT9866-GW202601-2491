...
#if 1    
static void _WifiMgr_Sta_Scan_Result_Print(wifi_ap_list_t* scan_ap_list, int num )
{
    int k;
    char ssid[WIFI_SSID_MAXLEN+1];
    wifi_ap_list_t *record;    
    ...
    for ( k = 0; k < num; k++ )
    {
		...
		memcpy(ssid, record->ap_info[k].ssid, 32);
        ...
        printf("       BSSID         : %02X:%02X:%02X:%02X:%02X:%02X\n",
                                                  (uint8_t)record->ap_info[k].bssid[0], (uint8_t)record->ap_info[k].bssid[1], (uint8_t)record->ap_info[k].bssid[2],
                                                  (uint8_t)record->ap_info[k].bssid[3], (uint8_t)record->ap_info[k].bssid[4], (uint8_t)record->ap_info[k].bssid[5] );
        printf("       RSSI          : %ddBm\n", (int)record->ap_info[k].rssi );
        printf("       Channel       : %d\n", (int)record->ap_info[k].channel);
    }
}
#endif    
...
_WifiMgr_Sta_Connect_Process(void)
{
    ...
    int nChannel;
	uint32_t nIp, gateway, netmask;
    ip_addr_t ipaddr_t, netmask_t, gw_t;
    char ip[16] = {0};
...
//    security_type   = gWifiMgrVar.SecurityMode;
    ...
    //wifi_test_connect();
    sleep(1);
    nRet = wpa_sta_connect(ssid, password, -1);
    
    if (nRet) {
        printf("wlan_sta_connect fail, ret=%d\n", nRet);
        goto end;
    }
...
//    printf("[WIFIMGR] security_type = %s \r\n",_WifiMgr_Sta_Get_Security_String_By_Index(security_type));
    ...
    if (setting.dhcp) {
        ...
        ip_addr_set_zero(&gw_t);
        ip_addr_set_zero(&ipaddr_t);
        ip_addr_set_zero(&netmask_t);

        printf("dhcp_start\n");
        netif_set_addr(fhost_to_net_if(0), &ipaddr_t, &netmask_t, &gw_t);
		dhcp_start(fhost_to_net_if(0));
        pAicNetif = fhost_to_net_if(0);
        ...
        while (connect_cnt)
        {
            ...
            if (pAicNetif->ip_addr.addr != 0)
            {
                printf("\r\n[WIFIMGR] DHCP setting OK\r\n");
                
                ipaddr_ntoa_r(&pAicNetif->ip_addr,ip,sizeof(ip)); 
                printf("ip = %s\n",ip);
                dhcp_available = 1;
            ...
            putchar('.');
            //printf("ipaddr_t.addr 0x%x 0x%x\n",ipaddr_t.addr,pAicNetif->ip_addr.addr);
            fflush(stdout);
            connect_cnt--;
            if (connect_cnt == 0)
            {
...
_WifiMgr_Sta_Scan_Process(struct net_device_info *apInfo)
{
    ...
    int i = 0;
    int nHideSsid = 0;
    int num = 0 ; // sizeof(results)/sizeof(mhd_ap_info_t);
    ...
    if (scan_ap_list == NULL) {
        printf("%s: scan_ap_list allocation failed\n", __func__);
        return NULL;
    }
    memset(scan_ap_list, 0, sizeof(wifi_ap_list_t));
    wlan_if_getscan(scan_ap_list);
    printf("sacn %d ap \n",scan_ap_list->ap_count);
    num = scan_ap_list->ap_count;
    _WifiMgr_Sta_Scan_Result_Print(scan_ap_list,num);
...
//#define AIC_MAX_PASSWD_LEN  64    
    memcpy(user_ap_cfg.aic_ap_ssid.array,gWifiMgrVar.ApSsid,32);
    user_ap_cfg.aic_ap_ssid.length = strlen(gWifiMgrVar.ApSsid);
    memcpy(user_ap_cfg.aic_ap_passwd.array,gWifiMgrVar.ApPassword,64);
    user_ap_cfg.aic_ap_passwd.length = strlen(gWifiMgrVar.ApPassword);

    

    wlan_start_ap(&user_ap_cfg);

   // printf("_WifiMgr_HostAP_Init %s %s , %d  \n",gWifiMgrVar.ApSsid,gWifiMgrVar.ApPassword,strlen(gWifiMgrVar.ApPassword));
...
//    itpRegisterDevice(ITP_DEVICE_SOCKET, &itpDeviceSocket);
    ...
    /* create STA mode tasklet thread */
    {
	    ...
	    struct sched_param sta_param;

        //MGR_INFO("==== Create [STA mode tasklet] thread ==== \n");
        pthread_attr_init(&sta_attr);
    	sta_param.sched_priority = 1;
	    pthread_attr_setschedparam(&sta_attr, &sta_param);
    ...
    /* create WifiMgr main process tasklet thread */
    {
	    ...
	    pthread_attr_setschedparam(&main_proc_attr, &main_param);        
    ...
    if (gWifiMgrVar.IS_WifiMgr_Switch_Mode_Thread == false) {/* Create task only first time */
    	...
    	sw_param.sched_priority = 1;
	    pthread_attr_setschedparam(&sw_attr, &sw_param);                
...
uint32_t WifiMgr_P2P_GO_Start(void)
{
    ...
    // ref_mirror_project 
    if (gMirrorP2PEnable) {
        printf("WifiMgr_P2P_GO_Start gMirrorP2PEnable \n");
        ioctl(ITP_DEVICE_WIFI, ITP_IOCTL_WIFIAP_ENABLE, NULL);	
        netif_set_default(fhost_to_net_if(0));
        ioctl(ITP_DEVICE_WIFI, ITP_IOCTL_WIFI_ADD_NETIF, fhost_to_net_if(0));
    }

    dhcps_init();
    printf("WifiMgr_P2P_GO_Start user_p2p_start \n");
    //user_p2p_start(1);//user_p2p_start(0);
    struct aic_sta_cfg cfg = {0};
    AIC_WIFI_MODE mode = WIFI_MODE_P2P;
