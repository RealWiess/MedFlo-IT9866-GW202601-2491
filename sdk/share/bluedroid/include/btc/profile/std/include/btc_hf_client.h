...
#include "api/bd_hf_client_api.h"
...
typedef union {
    ...
    struct volume_update_args {
        bd_hf_volume_control_target_t type;
        int volume;
    } volume_update;

    // BTC_HF_CLIENT_DIAL_EVT
    struct dial_args {
        char number[BD_BT_HF_CLIENT_NUMBER_LEN + 1];
    } dial;
    ...
    struct send_chld_cmd_args {
        bd_hf_chld_type_t type;
        int idx;
    } chld;

    // BTC_HF_CLIENT_SEND_BTRH_CMD_EVT
    struct send_btrh_cmd_args {
        bd_hf_btrh_cmd_t cmd;
    } btrh;
    ...
    struct reg_data_callback {
        bd_hf_client_incoming_data_cb_t recv;
