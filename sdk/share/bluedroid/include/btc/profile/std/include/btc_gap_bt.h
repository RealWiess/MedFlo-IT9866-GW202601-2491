...
typedef union {
    ...
    struct set_bt_scan_mode_args {
        bd_bt_scan_mode_t mode;
    } set_scan_mode;

    // BTC_GAP_BT_ACT_START_DISCOVERY
    struct start_disc_args {
        bd_bt_inq_mode_t mode;
        uint8_t inq_len;
        uint8_t num_rsps;
    } start_disc;
    ...
    struct set_pin_type_args {
        bd_bt_pin_type_t pin_type;
        uint8_t pin_code_len;
        bd_bt_pin_code_t pin_code;
    } set_pin_type;
...
bd_err_t btc_gap_bt_get_cod(bd_bt_cod_t *cod);
