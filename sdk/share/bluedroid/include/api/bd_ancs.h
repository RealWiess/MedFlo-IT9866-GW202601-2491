...
typedef enum {
   ...
   //Reserved EventID values = 3¡V255
} bd_EventID;
...
typedef enum {
   ...
   //Reserved CategoryID values   = 12¡V255
} bd_CategoryID;
...
typedef enum {
   ...
   //Reserved CommandID values = 3¡V255
} bd_CommandID;
...
typedef enum {
   ...
   //Reserved NotificationAttributeID values = 8¡V255
} bd_NotificationAttributeID;
...
typedef enum {
   ActionIDPositive = 0,
   ActionIDNegative = 1,
   //Reserved ActionID values = 2¡V255
} bd_ActionID;

//AppAttributeID Values
typedef enum {
   AppAttributeIDDisplayName = 0,
   //Reserved AppAttributeID values = 1¡V255
} bd_AppAttributeID;
...
#define BD_NOTIFICATIONUID_LEN   4

char *EventID_to_String(bd_EventID EventID);
char *CategoryID_to_String(bd_CategoryID CategoryID);
void bd_receive_apple_notification_source(uint8_t *message, uint16_t message_len);
void bd_receive_apple_data_source(uint8_t *message, uint16_t message_len);
char *Errcode_to_String(bd_error_code status);
