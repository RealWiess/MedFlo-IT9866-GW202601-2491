...
#include "hci/vendor/include/bt_vendor_rtk.h"
//#include "userial.h"
#include "hci/vendor/include/userial_vendor.h"
#include "hci/vendor/include/upio.h"
#include <unistd.h>
...
//#include "hci/vendor/src/rtl8723d_fw.c"
//#include "hci/vendor/src/rtl8723d_config.c"

#include "rtlbt_common.h"

#include "rtl8821cs_config.c"
#include "rtl8821cs_fw.c"
...
#if (BTHW_DBG == TRUE)
    #define BTHWDBG(param, ...) HCI_TRACE_DEBUG(param, ## __VA_ARGS__);
#else
...
//signature: realtech
const uint8_t RTK_EPATCH_SIGNATURE[8] = {0xFB, 0x63, 0x09, 0x62, 0x08, 0xD1, 0x07, 0xD0};
const uint8_t EXTENSION_SECTION_SIGNATURE[4] = {0x0A, 0x71, 0xB0, 0x71};
...
uint8_t line_speed_to_userial_baud(uint32_t line_speed)
{
    ...
    if (line_speed == 4000000) {
        ...
        HCI_TRACE_ERROR("userial vendor: unsupported baud speed %d", line_speed);
        baud = USERIAL_BAUD_115200;
    }
...
void rtk_update_altsettings(patch_info *patch_entry, unsigned char *config_buf_ptr, size_t *config_len_ptr)
{
    ...
    unsigned int count = 0, temp = 0, i = 0, j;
    ...
    if (data_len != config_len - sizeof(struct rtk_bt_vendor_config)) {
        HCI_TRACE_ERROR("rtk_update_altsettings: config len(%x) is not right(%x)", data_len, config_len - sizeof(struct rtk_bt_vendor_config));
        return;
    }
...
uint32_t rtk_parse_config_file(unsigned char **config_buf, size_t *filelen, uint8_t bt_addr[6], uint16_t mac_offset)
{
    ...
    if (le32_to_cpu(config->signature) != RTK_VENDOR_CONFIG_MAGIC) {
        HCI_TRACE_ERROR("config signature magic number(0x%x) is not set to RTK_VENDOR_CONFIG_MAGIC", config->signature);
        return 0;
    }

    if (config_len != *filelen - sizeof(struct rtk_bt_vendor_config)) {
        HCI_TRACE_ERROR("config len(0x%x) is not right(0x%x)", config_len, *filelen - sizeof(struct rtk_bt_vendor_config));
        return 0;
    }
                           ...
                           uint32_t *config_baud_rate, char *config_file_short_name, uint16_t mac_offset)
{
    ...
    if ((*config_buf = malloc(MAX_ORG_CONFIG_SIZE + MAX_ALT_CONFIG_SIZE)) == NULL) {
        HCI_TRACE_ERROR("malloc buffer for config file fail(0x%x)\n", filelen);
        //close(fd);
        return -1;
    }

    memcpy(*config_buf, rtlbt_config, rtlbt_config_len);

    //*config_buf = rtl8723d_config;
    filelen = rtlbt_config_len;
...
int rtk_get_bt_firmware(uint8_t **fw_buf, char *fw_short_name)
{
    ...
    buf_size = rtlbt_fw_len;

    if (!(*fw_buf = malloc(buf_size))) {
        HCI_TRACE_ERROR("Can't alloc memory for fw&config, errno:%d", errno);
        return -1;
    }

    memcpy(*fw_buf, rtlbt_fw, buf_size);
...
struct rtk_epatch_entry *rtk_get_patch_entry(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    if (!entry) {
        HCI_TRACE_ERROR("rtk_get_patch_entry: failed to allocate mem for patch entry");
        return NULL;
    }
    ...
    if (i == patch->number_of_patch) {
        HCI_TRACE_ERROR("rtk_get_patch_entry: failed to get etnry");
        free(entry);
        entry = NULL;
    }
...
void rtk_get_bt_final_patch(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    if (cfg_cb->lmp_subversion == LMPSUBVERSION_8723a) {
        if (memcmp(cfg_cb->fw_buf, RTK_EPATCH_SIGNATURE, 8) == 0) {
            HCI_TRACE_ERROR("8723as check signature error!");
            cfg_cb->dl_fw_flag = 0;
            goto free_buf;
        } else {
    ...
    if (memcmp(cfg_cb->fw_buf, RTK_EPATCH_SIGNATURE, 8)) {
        HCI_TRACE_ERROR("check signature error");
        cfg_cb->dl_fw_flag = 0;
        goto free_buf;
    }
    ...
    if (memcmp(cfg_cb->fw_buf + cfg_cb->fw_len - 4, EXTENSION_SECTION_SIGNATURE, 4)) {
        HCI_TRACE_ERROR("check extension section signature error");
        cfg_cb->dl_fw_flag = 0;
        goto free_buf;
    }
    ...
    if ((hw_cfg_cb.project_id_mask != PROJECT_ID_MASK_ALL) && ((hw_cfg_cb.project_id_mask & (1 << proj_id)) == 0)) {
        HCI_TRACE_ERROR("hw_cfg_cb.project_id_mask is 0x%08x, fw project_id is %d, does not match!!!",
...
#if 1
    {       //LOAD new driver in sdk/driver/bluetooth/rtl/src/fwpatches.RTL8723
            uint32_t configSize = rtlbt_config_len;
            uint32_t patchSize = rtlbt_fw_len;
            cfg_cb->total_len = configSize + patchSize;
            printf("%s(%d), configSize: %d, patchSize: %u, total: %d\n", __FUNCTION__, __LINE__, configSize, patchSize, configSize + patchSize);
            cfg_cb->total_buf = malloc(cfg_cb->total_len);
            if (!cfg_cb->total_buf)
            {
                printf("%s(%d), alloc bt patch + config buffer(%d bytes)is failded\n", __FUNCTION__, __LINE__,cfg_cb->total_len);
                exit(-1);
            }
            memcpy(cfg_cb->total_buf, rtlbt_fw, patchSize);
            memcpy(&cfg_cb->total_buf[patchSize], rtlbt_config, configSize);
    }
#else
        ...
        switch (hw_cfg_cb.state) {
            ...
            case HW_CFG_READ_CHIP_TYPE: {
                ...
                if (check_match_state(&hw_cfg_cb, PATCH_OPTIONAL_MATCH_FLAG_CHIPTYPE) > 1) { // check if have multiple matched patch_entry by lmp_subversion,hci_version, hci_revision and chiptype
                    HCI_TRACE_ERROR("check_match_state(lmp_subversion:0x%04x, hci_version:%d, hci_revision:%d chip_type:%d): Multi Matched Patch\n", hw_cfg_cb.lmp_subversion, hw_cfg_cb.hci_version, hw_cfg_cb.hci_revision, hw_cfg_cb.chip_type);
                    is_proceeding = FALSE;
                    break;
                }
...
CFG_START:
            ...
            case HW_CFG_START: {
#ifdef BT_CHIP_PROBE_SIMULATION
                {
                    ...
                    {
                        HCI_TRACE_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d $$$ BEGIN $$$\n", i);
                        ...
                        if (check_match_state(&hw_cfg_test, 0) > 1) {
                            HCI_TRACE_ERROR("check_match_state hw_cfg_test(lmp_subversion:0x%04x, hci_version:%d, hci_revision:%d chip_type:%d): Multi Matched Patch\n", hw_cfg_test.lmp_subversion, hw_cfg_test.hci_version, hw_cfg_test.hci_revision, hw_cfg_test.chip_type);

                            if (bt_chip_chars[i].chip_type != CHIPTYPE_NONE) {
                                HCI_TRACE_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d *** Include ChipType ***\n", i);
                        ...
                        HCI_TRACE_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d $$$ END $$$\n", i);
                    }
                }
#endif
                ...
                if ((prtk_patch_file_info == NULL) || (prtk_patch_file_info->lmp_subversion == 0)) {
                    HCI_TRACE_ERROR("get patch entry error");
                    is_proceeding = FALSE;
                    break;
                }
                ...
                if (hw_cfg_cb.config_len < 0) {
                    HCI_TRACE_ERROR("Get Config file fail, just use efuse settings");
                ...
                if (hw_cfg_cb.fw_len < 0) {
                    HCI_TRACE_ERROR("Get BT firmware fail");
                    hw_cfg_cb.fw_len = 0;
                } else {
                ...
                if ((uint32_t)hw_cfg_cb.total_len > hw_cfg_cb.max_patch_size) {
                    HCI_TRACE_ERROR("total length of fw&config(0x%08x) larger than max_patch_size(0x%08x)", hw_cfg_cb.total_len, hw_cfg_cb.max_patch_size);
                    is_proceeding = FALSE;
                    break;
                }
            ...
            case HW_CFG_SET_UART_BAUD_HOST:
                ...
                userial_vendor_set_baud(host_baudrate);
...
DOWNLOAD_FW:
    ...
    if (is_proceeding == FALSE) {
        HCI_TRACE_ERROR("vendor lib fwcfg aborted!!!");
