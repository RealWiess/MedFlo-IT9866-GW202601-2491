...
#include "rtl8723ds_config.c"
#include "rtl8723ds_fw.c"
...
//signature: realtech
const uint8_t RTK_EPATCH_SIGNATURE[8] = {0xF9, 0x63, 0x0D, 0x62, 0x0C, 0xD1, 0x0B, 0xD0};
const uint8_t EXTENSION_SECTION_SIGNATURE[4] = {0x6F, 0x42, 0xDE, 0x12};
...
void rtk_get_bt_final_patch(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    {       //LOAD new driver in sdk/driver/bluetooth/rtl/src/fwpatches.RTL8723
            ...
            if (!cfg_cb->total_buf)
            {
                printf("%s(%d), alloc bt patch + config buffer(%b bytes)is failded\n", __FUNCTION__, __LINE__,cfg_cb->total_buf);
                exit(-1);
            }
