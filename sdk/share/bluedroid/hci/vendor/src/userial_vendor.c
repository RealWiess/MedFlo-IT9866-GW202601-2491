...
static UART_OBJ *pUartInfo = NULL;
...
int userial_vendor_open(tUSERIAL_CFG *p_cfg, void *arg)
{
    ...
    {
        printf("%s %d\n", __FILE__, __LINE__);
        pUartInfo = (UART_OBJ*)malloc(sizeof(UART_OBJ));
        pUartInfo->port = ITH_UART0;
        pUartInfo->parity = ITH_UART_EVEN;
        pUartInfo->txPin = CFG_GPIO_UART0_TX;
        pUartInfo->rxPin = CFG_GPIO_UART0_RX;
        pUartInfo->baud = CFG_UART0_BAUDRATE;
        pUartInfo->timeout = 0;
#if defined(CFG_UART0_DMA)
        pUartInfo->mode = UART_DMA_MODE;
#elif defined(CFG_UART0_INTR)
        pUartInfo->mode = UART_INTR_MODE;
#endif
        pUartInfo->forDbg = true;

	    itpRegisterDevice(ITP_DEVICE_UART0, &itpDeviceUart0);
	    (void)ioctl(ITP_DEVICE_UART0, ITP_IOCTL_RESET, (void*)pUartInfo);
        //free(pUartInfo);
    }
    else
    {
        printf("\n\n pUartInfo has CREATED!! \n\n");
    }
...
void userial_vendor_set_baud(uint32_t userial_baud)
{
    ...
    {
        //iteUartTerminate(ITH_UART0);
        //iteUartReset(ITH_UART0, pUartInfo);
        ...
        itpRegisterDevice(ITP_DEVICE_UART0, &itpDeviceUart0);
        ioctl(ITP_DEVICE_UART0, ITP_IOCTL_RESET, (void*)pUartInfo);
    }
    //free(pUartInfo);
}
