...
#include "hci/include/hci/hci_internals.h"
#include "hci/include/hci/hci_hal.h"
#include "hci/include/hci/hci_layer.h"
#include "osi/allocator.h"
#include "hci/include/hci/packet_fragmenter.h"
...
static pthread_t  hcihost_task_handler;
static int task_run = 0;
static QueueHandle_t hcihost_queue;
...
int hci_start_up(void)
{
	...
	hcihost_queue = xQueueCreate(HCI_HOST_QUEUE_LEN, sizeof(BtTaskEvt_t));
    ...
    //pthread_t btc_task_handler;
	pthread_attr_t attr;
	pthread_attr_init(&attr);
    //pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	pthread_create(&hcihost_task_handler , &attr, hci_host_thread_handler , NULL);
...
{	
    ...
    task_run = 0;
    hci_host_task_post(SIG_HCI_HOST_NUM, TASK_POST_BLOCKING);
    hci_host_startup_flag  = false;
    pthread_join(hcihost_task_handler, NULL);
    vQueueDelete(hcihost_queue);
}
...
task_post_status_t hci_host_task_post(SIG_HCI_HOST_t type, task_post_t timeout)
{
	...
	ret = xQueueSend(hcihost_queue, &evt, timeout);
...
static void* hci_host_thread_handler(void *arg)
{
    ...
    while (task_run) {
        if (TRUE == xQueueReceive(hcihost_queue , &e, AOS_WAIT_FOREVER)) {
...
static void hal_says_packet_ready(BT_HDR *packet)
{
    if (packet->event != MSG_HC_TO_STACK_HCI_EVT) {
                    ...
                    printf("hal_says_packet_ready packet->len:%u packet->data:", packet->len);
