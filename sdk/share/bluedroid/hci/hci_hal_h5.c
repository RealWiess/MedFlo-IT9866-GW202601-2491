...
static pthread_t hcih5_task_handler;
static int task_run;
static QueueHandle_t hcih5_queue;
...
task_post_status_t hci_hal_h5_task_post(SIG_HCI_HAL_t type)
{
    BtTaskEvt_t evt;
    ...
    if (xQueueSend(hcih5_queue, &evt, sizeof(BtTaskEvt_t)) == TRUE) {
...
static void hci_hal_h5_rx_handler(void)
{
    //BtTaskEvt_t e;
    //unsigned int len;
    ...
    while (task_run) {
#if 0
        if (TRUE == xQueueReceive(hcih5_queue, &e, AOS_WAIT_FOREVER)) {
...
static bool h5_hal_open(const hci_hal_callbacks_t *upper_callbacks)
{
	...
	hcih5_queue = xQueueCreate(HCI_H5_QUEUE_LEN, sizeof(BtTaskEvt_t));
	...
	pthread_create(&hcih5_task_handler , &attr, (void *)hci_hal_h5_rx_handler , NULL);
...
static void h5_hal_close()
{
    ...
    task_run = 0;
    pthread_join(hcih5_task_handler, NULL);
    hcih5_task_handler = (pthread_t)NULL;
    vQueueDelete(hcih5_queue);
    hcih5_queue = NULL;
}
