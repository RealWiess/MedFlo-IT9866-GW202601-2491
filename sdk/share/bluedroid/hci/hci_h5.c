...
#include "hci/include/hci/hci_h5_int.h"
#include "hci/include/bt_skbuff.h"
#include "hci/include/bt_list.h"
...
{
    return (uint8_t)BT_CONTEXT(skb)->PacketType;
}
...
static void rtk_notify_hw_h5_init_result()
{
...
static uint16_t h5_wake_up()
{
    ...
    while (NULL != (skb = h5_dequeue())) {
        ...
        write(ITP_DEVICE_UART0, data, data_len);
...
uint8_t internal_event_intercept_h5(void)
{
    ...
    if (event_code == HCI_COMMAND_COMPLETE_EVT) {
        ...
        HCI_TRACE_DEBUG("event_code(0x%x)  opcode (0x%x) p_cb->int_cmd_rsp_pending %d", event_code, opcode, (int)p_cb->int_cmd_rsp_pending);
...
static uint8_t hci_recv_frame(sk_buff *skb, uint8_t pkt_type)
{
    ...
    (void)data_len;
    HCI_TRACE_DEBUG("UART H5 RX: length = %lu", data_len);
...
static void data_ready_cb_thread(void *arg)
{
    ...
    while ((skb = skb_dequeue_head(rtk_h5.recv_data)) != NULL)
    {
        skb_free(&skb);
    }
}
            ...
            if (rtk_h5.data_retrans_count < DATA_RETRANS_COUNT) {
                while ((skb = skb_dequeue_tail(rtk_h5.unack)) != NULL) {
                    ...
                    for (int i = 0 ; i < data_len; i++) {
                        HCI_TRACE_DEBUG("0x%02X", pdata[i]);
                    }
...
void h5_stop_data_ready_cb_thread(void)
{
    h5_data_ready_running = 0;
    osi_sem_give(&rtk_h5.data_cond);
    pthread_join(rtk_h5.thread_data_ready_cb, NULL);
    osi_sem_free(&rtk_h5.data_cond);
}
...
static int create_data_retransfer_thread()
{
	...
	pthread_create(&rtk_h5.thread_data_retrans , &attr, (void *)data_retransfer_thread , NULL);
...
static int create_data_ready_cb_thread()
{
	...
	pthread_attr_init(&attr);
    //pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	pthread_create(&rtk_h5.thread_data_ready_cb , &attr, (void *)data_ready_cb_thread , NULL);
#if 0
...
void hci_h5_int_init(hci_h5_hal_callbacks_t *h5_callbacks)
{
    ...
    h5_vendor->send_command(VENDOR_OPEN_USERIAL, &uart_dev);
...
{
    uint32_t read_len = read(ITP_DEVICE_UART0, data_buffer, sizeof(data_buffer));
    if (!read_len)
    {
        usleep(5*1000);
        return 0;
...
size_t hci_h5_int_read_data(uint8_t *data_buffer_t, size_t max_size)
{
