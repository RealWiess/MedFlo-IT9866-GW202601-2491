...
#include "hci/include/hci/hci_packet_parser.h"
    ...
    size_t feature_pages_count)
{
    ...
    if (stream != NULL) {
        ...
        configASSERT(*page_number_ptr < feature_pages_count);
    ...
    size_t minimum_bytes_after)
{
    ...
    configASSERT(event_code == HCI_COMMAND_COMPLETE_EVT);
//    assert(parameter_length >= (parameter_bytes_we_read_here + minimum_bytes_after));
    ...
    if (expected_opcode != NO_OPCODE_CHECKING) {
        configASSERT(opcode == expected_opcode);
    }
