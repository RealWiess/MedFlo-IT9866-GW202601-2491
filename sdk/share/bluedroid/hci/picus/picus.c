...
#define DEFAULT_PICUS_UART_PORT 1
...
void DBGPRINT(int level, const char *format, ...)
{
    ...
    if (n < 0 || n > 300)
        printf("[picus] vsnprintf fail\n");
    va_end(Arg);
    printf("[picus] %s\n", buf);

    ret = fflush(stdout);
    if (ret)
        printf("[picus] fflush error\n");
...
#else   //LOG TO UART
    if (dest == LOG_TO_UART) {
        //DBGPRINT(SHOW, "%s act:%d len:%d", __func__, act, len);
        if (act == LOG_START) {
...
static void picus_config_fw_debug_info(int b_on)
{
    ...
    unsigned char cmd[] = {0x01, 0x5d, 0xfc, 0x04, 0x00, 0x00, 0x02, 0x00};//picus log via acl, level off
...
int picus_cmd_handler(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "t:d:c:p:n:s:b:l:v:b:x:u:mfo")) != -1) {
        ...
        case 'd':
            if (strcmp(optarg, "kill") == 0) {
                ...
                //bt_driver_trigger_controller_codedump(); jing unmark
                btmtk_send_assert_cmd();
                goto done;
    ...
    if (g_log_dest == LOG_TO_UART)
        bt_uart_init(g_log_out_port, uart1_baudrate); //not implement
