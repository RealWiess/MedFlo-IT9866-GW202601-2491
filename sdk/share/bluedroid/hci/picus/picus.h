...
#define BLUETOOTH_TASK_PRIO 88  //jing test
#define HAL_UART_0 0  //defult used for print debug info
#define HAL_UART_MAX  8  //jing test
#define PICUS_TASK_PRI 87 //jing test
...
extern int btmtk_send_assert_cmd();
extern int bt_uart_deinit(int fd);
extern void btmtk_enable_bperf(bool enable);
extern void bt_driver_register_state_change_cb(bt_state_change_cb cb, int *curr_state);
extern void btmtk_register_fwlog_recv_cb(btmtk_fwlog_recv_cb cb);
extern void* bt_uart_init(int port, int speed);
extern int btmtk_debug_tx(unsigned char *cmd, unsigned int length);
extern uint32_t bt_uart_write(int fd, unsigned char *buf, int len);
#endif
