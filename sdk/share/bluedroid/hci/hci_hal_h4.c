...
//static QueueHandle_t hcih4_task_handler;
static int task_run;
//static QueueHandle_t hcih4_queue;
//static char queue_buf[HCI_H4_QUEUE_LEN * sizeof(BtTaskEvt_t)];

//static void host_send_pkt_available_cb(void);
static int host_recv_pkt_cb(uint8_t *data, uint16_t len);

//static void hci_hal_h4_rx_handler(void);
...
static uint32_t read_byte(uint8_t *data)
{
    int32_t read_len;
...
static uint32_t write_byte(uint8_t *data, uint32_t len)
{
    ...
    HCI_TRACE_DEBUG("Send %p (len %lu)", data, len);
...
static inline void read_payload(void)
{
    ...
    read = read_byte(h4_dev.rx.buf + h4_dev.rx.cur_len);
...
static bool hal_open(const hci_hal_callbacks_t *upper_callbacks)
{
    ...
    hci_hal_env_init(HCI_HAL_SERIAL_BUFFER_SIZE);
...
task_post_status_t hci_hal_h4_task_post(void)
{
    //BtTaskEvt_t evt;
