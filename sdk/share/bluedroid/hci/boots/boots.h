...
#include "boots_cfg.h"
#include "syslog.h"
#include "utils.h" //fix build error 
...
#define BPRINT_I(fmt, ...) \
    do { \
        if (boots_get_dbg_level() >= BOOTS_MSG_LVL_INFO) { \
            printf("[INFO] %s:%d: " fmt "\n", __FILE__, __LINE__, ##__VA_ARGS__); \
        } \
    } while (0)
...
#define BPRINT_W(fmt, ...) \
    do { \
        if (boots_get_dbg_level() >= BOOTS_MSG_LVL_WARN) { \
            printf("[WARN] %s:%d: " fmt "\n", __FILE__, __LINE__, ##__VA_ARGS__); \
        } \
    } while (0)
...
#define BPRINT_E(fmt, ...) \
    do { \
        if (boots_get_dbg_level() >= BOOTS_MSG_LVL_ERR) { \
...
#define SHOW_RAW(len, buf) \
    do { \
        if (boots_get_dbg_level() >= BOOTS_MSG_LVL_DBG) { \
            BPRINT("Raw data: "); \
            print_hex_dump("Hex Dump:", buf, len); \
        } \
    } while (0)
...
extern int btmtk_debug_rx(unsigned char *buf, unsigned int buf_len);
extern int btmtk_debug_tx(unsigned char *cmd, unsigned int length);
typedef void (*bt_rx_data_ready_cb)(void);
extern int bt_driver_register_event_cb(bt_rx_data_ready_cb cb, int restore_cb);
//---------------------------------------------------------------------------
#endif // __BOOTS_H__
