...
#define BTMTK_PLATFORM_MT7933

#if defined(BTMTK_PLATFORM_MT7933)
//#include "btif_mt7933.h" jing mark
//#include "memory_attribute.h"  jing mark
#endif
...
int boots_uart_init_freertos(int port, int speed)
{
    ...
    // status_t = hal_uart_register_callback(port, uart_read_from_input, NULL);  jing mark
...
int boots_uart_read_freertos(int fd, unsigned char *buf, int len)
{
    if (g_inited_uart_port < 0 ||
        ...
        printf("fd = %d doesn't init or wrong. (exp: %d)", fd, g_inited_uart_port);
        return 0;
    }
...
int boots_uart_read_freertos_with_retry(int fd, unsigned char *buf, int len)
{
    ...
    do {
        ...
        if (total_read < (uint32_t)len) {
            printf("boots_uart_read: wait 10ms, total = %d/%d, retry (%d)", (int)total_read, len, retry);
