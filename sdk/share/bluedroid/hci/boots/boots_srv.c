...
#define HAL_UART_2 2 //test, no longer used
//---------------------------------------------------------------------------
...
void boots_srv_main(void * parm)
{
    ...
    printf("%s: type = 0x%x", __func__, type);
    ...
    while (1){
        if (type == BOOTS_SRV_TX) {
            ...
            BPRINT_D("boots tx server wakeup");
            boots_srv_mutex_lock(boots_srv_buf_mutex);
            if (cmd_buffer[0] != 0 || cmd_buffer[1] != 0) {
                len = cmd_buffer[0] + (cmd_buffer[1] << 8);
                //bt_driver_tx(&cmd_buffer[2], len); jing mark
                btmtk_debug_tx(&cmd_buffer[2], len);
                memset(cmd_buffer, 0, CMD_BUF_LEN);
            }
            boots_srv_mutex_unlock(boots_srv_buf_mutex);
            ...
            ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
            BPRINT_D("boots rx server wakeup");

            /*check read*/
            boots_srv_mutex_lock(boots_srv_buf_mutex);
            BPRINT_D("lock boots_srv_mutex_lock");
            ...
            //ret = bt_driver_rx(p_buf, evt_buf_left); jing mark
            BPRINT_D("evt_buf_left= %d", evt_buf_left);
            ret = btmtk_debug_rx(p_buf, evt_buf_left);
            if (ret > 0) {
                if (boots_client_semaphore) {
                    g_evt_buf_op.write_offset += ret;
                    printf("DRV RX: write_offset = %d", (int)g_evt_buf_op.write_offset);
                ...
                BPRINT_D("btmtk_debug_rx %d fail", ret);
            }
            boots_srv_mutex_unlock(boots_srv_buf_mutex);

        } else if (type == BOOTS_SRV_UART) {
            if (g_evt_buf_op.wait_buf) {
                printf("UART SRV: wait 10ms for buf rls");
            ...
            if (g_relay_mode) {
                ...
                if (cmd_buffer[0] == 00) {
                    printf("%s: SW workaround for HAPS, skip first 00 byte", __func__);
                    //bt_driver_tx(cmd_buffer + 1, len - 1); jing mark
                } else {
                    //bt_driver_tx(cmd_buffer, len); jing mark
                }
                boots_srv_mutex_unlock(boots_srv_buf_mutex);
                ...
                printf("UART RX: event len %d", len);
                if (len < 0) {
...
bool create_boots_srv(void)
{
    ...
    if (xReturned == pdPASS) {
        ...
        printf("%s boots server main tx thread create success", __func__);
    } else {
    ...
    if (xReturned == pdPASS) {
        ...
        printf("%s boots server main rx thread create success", __func__);
    } else {
    ...
    if (xReturned == pdPASS) {
        /* The task was created.  Use the task's handle to delete the task. */
        printf("%s boots server main uart thread create success", __func__);
    } else {
        printf("%s boots server main uart thread create fail", __func__);
        return false;
    }
