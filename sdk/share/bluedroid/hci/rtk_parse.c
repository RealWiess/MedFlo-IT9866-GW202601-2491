...
#define ALOGD(...) ALOGD(__VA_ARGS__)
#define ALOGI(...) ALOGI(__VA_ARGS__)
#define ALOGE(...) ALOGE(__VA_ARGS__)
#define ALOGW(...) ALOGW(__VA_ARGS__)
...
uint8_t list_allocate_add(uint16_t handle, uint16_t psm, int8_t profile_index, uint16_t dcid, uint16_t scid)
{
    ...
    pprof_info->profile_index = (uint8_t)profile_index;
...
void rtk_vendor_cmd_to_fw(uint16_t opcode, uint8_t parameter_len, uint8_t* parameter)
{
    ...
    p_buf->offset = 0;
    p_buf->len = (uint16_t)(HCI_CMD_PREAMBLE_SIZE + parameter_len);
...
void rtk_notify_profileinfo_to_fw()
{
    ...
    buffer_size = (uint32_t)(1 + handle_number*3 + 1);
    ...
    *p++ = rtk_prof.profile_status;
    ...
    rtk_vendor_cmd_to_fw(HCI_VENDOR_SET_PROFILE_REPORT_COMMAND, (uint8_t)buffer_size, p_buf);
...
void update_profile_state(uint8_t profile_index, uint8_t is_busy)
{
    ...
    if(is_busy)
    {
        ...
        {
            need_update = TRUE;
            rtk_prof.profile_status |= (uint8_t)(BIT(profile_index));
        }
    }
    else
    {
...
{
    uint16_t handle, pdu_len, channel_ID, psm, scid, dcid, result; //total_len , command_len,  //status;
