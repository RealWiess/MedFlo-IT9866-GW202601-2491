...
#define MIN_FRAMES                   (100)
    ...
    #if defined(CFG_FFMPEG_AUDIO_DECODE)
...
void video_refresh_timer (PlayerInstance * is, double pts)
{
	...
	#if 0
    (void)printf("YC: video timestamp = %f\n", pts);
	#endif
#ifdef AV_SYNC_STC
    ...
    else
    {
        ...
        diff           = pts - ref_clock;
		#if 0
        (void)printf("YC: diff = %f, pts = %f, ref_clock = %f\n", diff, pts, ref_clock);
		#endif
        ...
        (void)usleep(actual_delay * 1000000);
    }
    ...
    #if 0
    (void)printf("YC: current pts=%f, stc_time = %f, delay_1 = %f\n", pts, ref_clock, delay);
        #endif
    diff           = pts - ref_clock - is->video_start_pos;
	#if 0
    (void)printf("YC: diff = %f\n", diff);
    #endif
    sync_threshold = (delay > AV_SYNC_THRESHOLD) ? delay : AV_SYNC_THRESHOLD;
...
void render_frame (AVCodecContext * avctx, AVFrame * picture)
{
    ...
    while (1)
    {
        ...
        if (dbuf != NULL)
    #else
        if ( (dbuf != NULL) || (global_player_prop->inst->abort_request != 0) )
    #endif
        {
...
void render_frame_from_streaming (AVCodecContext * avctx, AVFrame * picture)
{
    bool              bprint = true;
    ITV_DBUF_PROPERTY prop   = {0};
...
#if defined(CFG_AUDIO_ENABLE) && !defined(CFG_FFMPEG_AUDIO_DECODE)
static void flush_audio_hw (PlayerInstance * is)
{
    ...
    if (nAudioEngine == ITE_WAV_DECODE)
    {
        iteAudioStopQuick();
    #ifdef CFG_BUILD_I2S
        i2s_enable_fading(true); // fading in
    #endif
        (void)usleep(1000);
...
static void * video_thread (void * arg)
{
    ...
    {
        AVPacket pkt;
        while ( (is->paused != 0) && (is->videoq.abort_request == 0) )
        {
            (void)usleep(10000);
        }
        if (is->videoq.abort_request != 0)
        {
            goto the_end;
        }
        ...
        else if (ret > 0)
        {
            ...
            if ( (pkt.dts == AV_NOPTS_VALUE) && (frame->opaque1 != NULL) && (*(int64_t *)frame->opaque1 != AV_NOPTS_VALUE) )
            {
...
static void * flower_thread (void * arg)
{
    flow_start_AVsound_play(gf);
}
#endif

#if defined(CFG_FFMPEG_AUDIO_DECODE) && defined(CFG_RESERVE_FILTER)
    #define FFMPEG_AUDIO_MAX_SIZE 16 * 1024
static void * audio_thread (void * arg)
{
    int              ret;
    bool             _init = false;
    PlayerInstance * is    = (PlayerInstance *)arg;
    rbuf_ite *       Rbuf  = ite_rbuf_init(FFMPEG_AUDIO_MAX_SIZE); // tmp buf

    for (;;)
    {
        AVPacket pkt;
        if ( (streamQCount() > 32) && (is->audioq.abort_request == 0) )
        {
            (void)usleep(100000);
            continue;
        }

        while ( (is->paused != 0) && (is->audioq.abort_request == 0) )
        {
            usleep(10000);
        }
        if (is->audioq.abort_request != 0)
        {
            goto the_end;
        }
        ...
        else if (ret > 0)
        {
            ...
            if (is->audioctx->codec_id == CODEC_ID_AAC)
            {
                if (!_init)
                {
                ...
                if (1)
                {
                    ...
                    if ( (pkt.data[0] != 0xff) || (pkt.data[1] & 0xf0 != 0xf0) )
                    {
                        ...
                        om = allocb_ite(pkt.size + 7);
                        aacADTSHeader(adtsHeader, is->audioctx, pkt.size);
                        (void)memcpy(om->b_wptr, adtsHeader, 7);
                        om->b_wptr += 7;
                    }
                    else
                    {
                        om = allocb_ite(pkt.size);
                    }
                    (void)memcpy(om->b_wptr, pkt.data, pkt.size);
                    om->b_wptr += pkt.size;
                    streamQmblkPut(om);
                }
            }
            else
            {
                ...
                ite_rbuf_put(Rbuf, pkt.data, pkt.size);

                do
                {
                    packet.data = Rbuf->b_rptr;
                    packet.size = ite_rbuf_get_avail_size(Rbuf);
                    got_picture = 0;
                    rev         = avcodec_decode_audio4(is->audioctx, frame, &got_picture, &packet);
                    if (got_picture != 0)
                    {
                        if (!_init)
                        {
                            gf->dInfo.sr        = is->audioctx->sample_rate;
                            gf->dInfo.nch       = is->audioctx->channels;
                            gf->dInfo.bytes20ms = 20 * gf->dInfo.sr * gf->dInfo.nch * 2 / 1000;
                            gf->dInfo.bitsize   = 16;
                            gf->dInfo.codectype = -1;
                            gf->dInfo.init      = true;
                            _init               = true;
                        }
                        streamQPutShape(frame->data[0], frame->linesize[0],
                                        gf->dInfo.bytes20ms); // every frame chop to 20ms bytes
                        // need to implement
                        #if 0
                        (void)printf("frame->linesize[0] = %d, sample rate = %d\n", frame->linesize[0],
                        is->audioctx->sample_rate); addr: frame->data[0] size: frame->linesize[0]
						#endif
                    }
                    if ( (rev > 0) && (rev <= packet.size) )
                    {
                        Rbuf->b_rptr += rev;
                        if ( (is->audioctx->codec_id == CODEC_ID_FLAC) && (ite_rbuf_get_avail_size(Rbuf) < 512) )
                        {
                            break;
                        }
                    }
                }while( (rev > 0) && (rev < packet.size) )
                ite_rbuf_rearrange(Rbuf); // rearrange residual undecoder data
            }
            av_free_packet(&pkt);
            av_free(frame);
        }
        (void)usleep(1000 * (streamQCount() + 1));
    }
the_end:
    ite_rbuf_free(Rbuf);
    pthread_exit(NULL);
...
static void * audio_thread (void * arg)
{
    ...
    CreateDetachedThread(flower_thread, NULL);
    #endif
    for (;;)
    {
        AVPacket pkt;
...
static void * subtitle_thread (void * arg)
{
    ...
    {
        AVPacket pkt;
        while ( (is->paused != 0) && (is->subtitleq.abort_request == 0) )
        {
            (void)usleep(10000);
        }
        if (is->subtitleq.abort_request != 0)
        {
            goto the_end;
        }
        ret = packet_queue_get(&is->subtitleq, &pkt, 0);
        if (ret < 0)
        {
            goto the_end;
        }
        ...
        {
            double       ref_clock, diff;
            int64_t      stc_time = 0;
            SubPicture * subtitle = av_malloc(sizeof(SubPicture));
            subtitle->pts         = (double)pkt.dts * av_q2d(is->subtitle_st->time_base);
            strncpy(subtitle->sub, pkt.data, sizeof(subtitle->sub));
#ifdef CFG_BUILD_ITV
            fc_sync_gettime(&is->stc, &stc_time);
#endif
            ref_clock = (double)stc_time / STC_TIME_BASE;
            diff      = subtitle->pts - ref_clock;
			#if 0
            (void)printf("diff = %f, pts = %f, ref_clock = %f\n", diff, subtitle->pts, ref_clock);
			#endif
            if (diff > 0)
            {
                (void)usleep(diff * 1000000);
...
static int stream_component_open_for_videoloop (PlayerInstance * is, int stream_index, void * opaque)
{
    ...
    switch (avctx->codec_type)
    {
        case AVMEDIA_TYPE_AUDIO:
            ...
            pthread_create(&is->audio_tid, &attr, audio_thread, (void *)is);
            break;
        ...
        if (avctx->width > 1920 || avctx->height > 1080)
        {
            ...
            is->videoctx     = avctx;
#ifdef CFG_BUILD_ITV
            itv_update_video_window(avctx->width, avctx->height);
#endif
            packet_queue_init(&is->videoq);

            pthread_attr_init(&attr);
            pthread_attr_setstacksize(&attr, 16 * 1024);
            pthread_create(&is->video_tid, &attr, video_thread_for_videoloop, (void *)is);
            break;
...
static int stream_component_open (PlayerInstance * is, int stream_index, void * opaque)
{
    ...
    if (avctx->codec_id != CODEC_ID_TEXT)
    {
        codec = avcodec_find_decoder(avctx->codec_id);
    ...
    switch (avctx->codec_type)
    {
        ...
        if (avctx->width > 1080 || avctx->height > 1920)
        {
            ...
            pthread_attr_init(&attr);
            pthread_attr_setstacksize(&attr, 16 * 1024);
            pthread_create(&is->video_tid, &attr, video_thread, (void *)is);
            break;
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            pthread_attr_init(&attr);
            pthread_attr_setstacksize(&attr, 16 * 1024);
            pthread_create(&is->subtitle_tid, &attr, subtitle_thread, (void *)is);
            break;
...
void * read_thread_for_videoloop (void * arg)
{
    ...
    (void)memset(st_index, -1, sizeof(st_index));
    ...
    for (;;)
    {
        ...
        if ( (is->videoq.size + is->audioq.size > MAX_QUEUE_SIZE) ||
            ( (is->videoq.nb_packets > MIN_FRAMES) || (is->audioq.nb_packets > MIN_FRAMES) ))
        {
            /* wait 1 ms */
            (void)usleep(1000);
            continue;
    ...
    seekto:
        if (is->seek_req != 0)
        {
            ...
            else
            {
                if (is->audio_stream >= 0)
                {
					...
					#endif
#if defined(CFG_AUDIO_ENABLE) && !defined(CFG_FFMPEG_AUDIO_DECODE) && defined(__OPENRTOS__)
                    flush_audio_hw(is);
#endif
                }
        ...
        if ( (pkt->stream_index == is->audio_stream) && (pkt_in_play_range != 0) )
        {
            packet_queue_put(&is->audioq, pkt);
        }
        else if ( (pkt->stream_index == is->video_stream) && (pkt_in_play_range != 0) )
        {
            packet_queue_put(&is->videoq, pkt);
        }
        else if ( (pkt->stream_index == is->subtitle_stream) && (pkt_in_play_range != 0) )
        {
    ...
    /* wait until the end */
    while (pprop->inst->abort_request == 0)
    {
        if (is->seek_req != 0)
        {
            goto seekto;
...
void * read_thread (void * arg)
{
    ...
    if (is->audio_disable == 0)
    {
        st_index[AVMEDIA_TYPE_AUDIO] =
            av_find_best_stream(ic, AVMEDIA_TYPE_AUDIO, -1, st_index[AVMEDIA_TYPE_VIDEO], NULL, 0);
    }

    if (is->video_disable == 0)
    {
        st_index[AVMEDIA_TYPE_SUBTITLE] =
            av_find_best_stream(ic, AVMEDIA_TYPE_SUBTITLE, -1, st_index[AVMEDIA_TYPE_VIDEO], NULL, 0);
    }
...
#if defined(CFG_FFMPEG_AUDIO_DECODE) && defined(CFG_RESERVE_FILTER)
        (void)usleep(1000 * (is->audioq.nb_packets + 1)); // only audio,dynamic ctrl release time ,prevent read_thread occupy CPU
#else
        (void)usleep(1000);
#endif
    }
    ...
    for (;;)
    {
        ...
        else if (ret > 0)
        {
            ...
            if (got_picture != 0)
            {
                ...
                render_frame_from_streaming(is->videoctx, frame);
            }
...
static void * audio_thread_for_rtsp_client (void * arg)
{
    ...
    for (;;)
    {
        ...
        else if (ret > 0)
        {
            ...
            int64_t stc_time;
			#if 0
            (void)printf("YC: audio timestamp = %f\n", pkt.timestamp);
			#endif
            if (!is->get_first_I_frame && !is->first_pkt_is_audio)
            {
                is->audio_is_sync = true;
...
void ithMediaPlayer_InitAVDecodeEnv ()
{
    ...
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 16 * 1024);
    pthread_create(&inst->video_tid, &attr, video_thread_for_rtsp_client, (void *)pprop);
    pthread_create(&inst->audio_tid, &attr, audio_thread_for_rtsp_client, (void *)pprop);
...
int ithMediaPlayer_h264_decode_from_rtsp (unsigned char * inputbuf, int inputbuf_size, double timestamp)
{
    ...
    {
        /* wait 1 ms */
        (void)printf("YC: %s, %d, is->videoq.nb_packets = %d\n", __FUNCTION__, __LINE__, is->videoq.nb_packets);
        usleep(1000);
    }
...
void ithMediaPlayer_setVideoDisplayResolution (int width, int height)
{
    PlayerProps *    pprop = global_player_prop;
    PlayerInstance * is;
    ...
    (void)printf("Set video display width: %d, height: %d\n", width, height);
    is->videoctx->width  = width;
    is->videoctx->height = height;
...
static int ithMediaPlayer_stop (void)
{
    ...
    is = pprop->inst;
    if ( (is != NULL) && (is->paused != 0) ) // press stop when video is paused
    {
        is->paused = false;
#ifdef CFG_BUILD_ITV
        fc_sync_pause(&is->stc);
...
#endif
#ifdef CFG_BUILD_I2S
    i2s_enable_fading(false); // close fading in
#endif
    pthread_mutex_unlock(&player_mutex);
    return 0;
}
...
static int ithMediaPlayer_play_videoloop (void)
{
    ...
    if (!pprop->isFilePlayer_thread_created)
    {
        ...
        pthread_attr_init(&attr);
        pthread_attr_setstacksize(&attr, 16 * 1024);
        rc = pthread_create(&pprop->read_tid, &attr, read_thread_for_videoloop, (void *)pprop);
        if (rc != 0)
        {
            av_log(NULL, AV_LOG_ERROR, "create thread failed %d\n", rc);
            ithMediaPlayer_stop();
...
static int ithMediaPlayer_deinit ()
{
    ...
    if ( (pprop != NULL) && (pprop->show_status != 0) )
    {
        if (pprop->inst != NULL)
        {
            free(pprop->inst);
...
static int ithMediaPlayer_get_video_resolution_ext (int * width, int * height, char * filepath)
{
    int               err;
    AVFormatContext * ic = NULL;
    av_register_all();
    ...
    else
    {
        ...
        for (i = 0; i < ic->nb_streams; i++)
        {
            ...
            {
                (void)printf("avctx->width = %d, avctx->height = %d\n", avctx->width, avctx->height);
                *width  = avctx->width;
                *height = avctx->height;
                break;
            }
...
ithMediaPlayer fileplayer = {
    ...
    ithMediaPlayer_get_video_resolution_ext,
    ...
    ithMediaPlayer_setVideoDisplayResolution
};
...
ithMediaPlayer fileplayer = {
    ...
    .getVideoResolution_ext      = ithMediaPlayer_get_video_resolution_ext,
    ...
    .setVideoDisplayResolution 	 = ithMediaPlayer_setVideoDisplayResolution
};
#endif
// clang-format on
