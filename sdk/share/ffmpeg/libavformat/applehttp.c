...
static int get_char(AVIOContext *s)
{
...
static int get_line(AVIOContext *s, char *buf, int maxlen)
{
    int i = 0;
    char c;
...
static int read_chomp_line_ex(AVIOContext *s, char *buf, int maxlen)
{
...
static int parse_playlist_ex(AppleHTTPContext *c, const char *url,
                          struct variant *var, AVIOContext *in)
{
    int ret = 0, duration = 0, is_segment = 0, is_variant = 0, bandwidth = 0;
    enum KeyType key_type = KEY_NONE;
    ...
    read_chomp_line_ex(in, line, sizeof(line));
    if (strcmp(line, "#EXTM3U")) {
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
    ...
    while (!in->eof_reached/*!url_feof(in)*/) {
        read_chomp_line_ex(in, line, sizeof(line));
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            struct variant_info info = {{0}};
            is_variant = 1;
...
static int open_input(struct variant *var)
{
    ...
    if (seg->key_type == KEY_NONE) {
		printf("segment url = %s, cur_seq_no = %d, start_seq_no = %d\n", seg->url, var->cur_seq_no, var->start_seq_no);
...
reload:
        ...
        if (v->cur_seq_no < v->start_seq_no) {
            av_log(NULL, AV_LOG_WARNING,
                   "skipping %d segments ahead, expired from playlists, cur_seq_no = %d, start_seq_no = %d\n",
                   v->start_seq_no - v->cur_seq_no, v->cur_seq_no, v->start_seq_no);
        ...
        if (ret < 0)
        {
        	printf("open input segment url fail, retry count = %d\n", retry1);
			retry1++;
			ffurl_close(v->input);
		    v->input = NULL;
			#ifdef CFG_NET_ENABLE
			if(http_return_503)
			{
				printf("http receive 503 error code, skip current segment\n");
				http_return_503 = 0;
			    v->cur_seq_no++;
...
static int applehttp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    if ((ret = parse_playlist_ex(c, s->filename, NULL, s->pb)) < 0)
        goto fail;
    if (c->n_variants == 0) {
