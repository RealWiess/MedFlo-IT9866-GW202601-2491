...
{"chunked_post", "use chunked transfer-encoding for posts", OFFSET(chunked_post), AV_OPT_TYPE_INT, 1.0, 0, 1, E },
{"headers", "custom HTTP headers, can override built in default headers", OFFSET(headers), AV_OPT_TYPE_STRING, 0.0, 0, 0, D|E },
{"user-agent", "override User-Agent header", OFFSET(user_agent), AV_OPT_TYPE_STRING, 0, 0, 0, DEC},
{NULL}
};
    ...
    if (s->chunksize != UINT64_MAX) {
        if (!s->chunksize) {
            ...
            for(;;) {
                ...
                av_dlog(NULL, "Chunked encoding data size: %"PRIu64"'\n", s->chunksize);
