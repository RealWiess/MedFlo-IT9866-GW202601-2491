...
#define MAX_SAMPLE_COUNT 60000   //60000*24bytes = 1440000 bytes;
#endif
...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        avpriv_new_chapter(c->fc, i, time_base, start, AV_NOPTS_VALUE, str);
        }
#else
...
#if defined(WIN32)
    {
    int chan_tab[] = {2,1,2,3,3,4,4,5};
    st->codec->channels = chan_tab[acmod] + lfeon;
    }
#else
...
#if defined (CFG_CHIP_PKG_IT9852)    
    if(sc->sample_count > MAX_SAMPLE_COUNT)
        return -1;
#endif    
    if (sample_size)
        return 0;
...
static int mov_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    if(mov->duration && mov->time_scale)
    {
        s->duration = (mov->duration/mov->time_scale)*1000000;
    }
    return 0;
}
...
#if defined(WIN32)
        MOVAtom fake_atom;
        fake_atom.type = AV_RL32("root");
        fake_atom.size = INT64_MAX;
        mov->found_mdat = 0;
        if (s->pb->seekable||
