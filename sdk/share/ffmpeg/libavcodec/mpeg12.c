        ...
        //MPEG-2 fps
            ...
            if (s->aspect_ratio_info > 1) {
#if defined(WIN32)
                AVRational a1 = {s1->pan_scan.width, s1->pan_scan.height};
                AVRational a2 = {s->width, s->height};
                AVRational dar =
                    av_mul_q(av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info], a1), a2);
#else
                AVRational dar =
                ...
                if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
                   (av_cmp_q(dar, a3) && av_cmp_q(dar, a4))) {
                    AVRational a5 = {s->width, s->height};
                    s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info], a5);
                }
#else
                if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
...
#if defined(WIN32)
                    AVRational a6 = {s1->pan_scan.width, s1->pan_scan.height};
                    s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info], a6);
#else
                    s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                                 (AVRational) {s1->pan_scan.width, s1->pan_scan.height});
