...
//                              Constant Definition
//=============================================================================
#if(CFG_RAM_SIZE >= 0x4000000)
#define ENABLE_H264_3_CMD_DATA_BUF
#ifndef FAST_DISPLAY_ENABLE
#define MULTI_SLICE
#endif
#else
...
static int decode_nal_units(MMP_UINT8 *buf, int buf_size);
int AVC_Video_Decoder_DecodeExtradata(MMP_UINT8 *buf, int size);
...
AVC_Video_Decoder_Reload()
{
    ...
    while (return_state == 0 && gptAVCVideoDecoder->remainByte > 0)
    {
        ...
        else
        {
            ...
            {
                av_log(NULL, AV_LOG_ERROR, "Error Start 1 %p %p\n", pNalStream, pBufferEndAddr);
        ...
        if (pNalStream >= pBufferEndAddr)
        {
            av_log(NULL, AV_LOG_ERROR, "Error Start 2 %p %p\n", pNalStream, pBufferEndAddr);
            pNalStream = pBufferStartAddr + (pNalStream - pBufferEndAddr);
        }
        ...
        case NAL_UNIT_TYPE_SEQ_PARAM_SET:
            ...
            if (nalSize <= MAX_NAL_SIZE)
                ...
                av_log(NULL, AV_LOG_ERROR, "! Nal size (%d) > %d\n", (int)nalSize, MAX_NAL_SIZE);

            if (ret != AVC_ERROR_SUCCESS)
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse SPS fail (0x%x)!!", (unsigned int)ret);
            }
            break;
        ...
        case NAL_UNIT_TYPE_PIC_PARAM_SET:
            ...
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse PPS fail (0x%x)!!", (unsigned int)ret);
            }
            break;
        ...
        case NAL_UNIT_TYPE_SEI:
            ...
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse SEI fail (0x%x)!!", (unsigned int)ret);
            }
            break;
        ...
        case NAL_UNIT_TYPE_NON_IDR_PICTURE:
            ...
            if (ret == ERROR_RESOLUTION)
            {
                av_log(NULL, AV_LOG_ERROR, "resolution not support (width = %d, height = %d), frame_rate_code: %d\n",
                       (int)gptAVCDecoder->frameWidth,
                       (int)gptAVCDecoder->frameHeight,
                       (int)gptAVCVideoDecoder->frame_rate_code);
        ...
        gptAVCVideoDecoder->remainByte < 0)
    {
        gptAVCVideoDecoder->pSegEndAddr =
            (MMP_UINT8 *)((MMP_UINT32)gptAVCVideoDecoder->pPacketEndAddr);
    }
    else
    {
        gptAVCVideoDecoder->pSegEndAddr =
            (MMP_UINT8 *)((MMP_UINT32)gptAVCVideoDecoder->pPacketStartAddr);
    }
    ...
    int            *data_size)
{
...
#ifdef FAST_DISPLAY_ENABLE //|| defined(ENABLE_7_FRAME_BUFFER)
    ...
    *data_size = 0;
...
#endif
    {
        ...
        pict->data[0]                           = (uint8_t *)ithVideoQuery(VIDEO_ADDRESS_FRAME_BUF_Y, rdIndex);
        pict->data[1]                           = (uint8_t *)ithVideoQuery(VIDEO_ADDRESS_FRAME_BUF_U, rdIndex);
        pict->data[2]                           = (uint8_t *)ithVideoQuery(VIDEO_ADDRESS_FRAME_BUF_V, rdIndex);
        pict->data[3]                           = (uint8_t *)rdIndex;
...
_VIDEO_DECODER_SetFrameBufferReference()
{
...
#if defined(MULTI_SLICE) || defined(FAST_DISPLAY_ENABLE)
    ...
    av_log(NULL, AV_LOG_DEBUG, "T %d %d: idx %d poc %d cur_poc %d D %d Update %d %08x\n", img->type, img->nal_reference_idc,
           (int)curMinFramePocIndex, (int)curMinFramePoc, (int)curFramePoc, (int)decodeBufSelect, (int)curUpdateBufIndex, (unsigned int)displayOrderQueue);
}
...
int iTE_h264_Dtv_decode_frame(AVCodecContext *avctx, void *data, int *data_size, AVPacket *avpkt)
{
	...
	if (gptAVCDecoder == NULL)
	{
		av_log(NULL, AV_LOG_ERROR, "gptAVCDecoder is NULL.\n");
		return -1;
	}
...
not_extra:
    ...
    while (!(return_state & VIDEO_RETURN_END_OF_PACKET) )
    {
            ...
            (return_state & VIDEO_RETURN_OUT_REF_FRAME_RANGE))
        {
            ...
            av_log(NULL, AV_LOG_ERROR, "error, return_state = %lu\n", return_state);
            break;
        }
...
#define TEST_FIREHW_BITRATE 0
        ...
        {
            AVC_Video_Decoder_Fire();
#if defined(ENABLE_2_VIDEO_FRAME_BUF) || defined(FAST_DISPLAY_ENABLE)
			AVC_Video_Decoder_Wait();
#endif
...
int iTE_h264_Dtv_display_frame(AVCodecContext *avctx, void *data, int *data_size, int index)
{
	MMP_UINT32        displayOrderQueue        = gtAVCFrameBufControl.displayOrderQueue;
	MMP_UINT32        curUpdateBufIndex        = 0xFF;
	...
	curUpdateBufIndex = (img->decoding_buf_idx+index+1)%gptAVCDecoder->frameBufCount;
    displayOrderQueue = (displayOrderQueue << 8) | curUpdateBufIndex;
    gtAVCFrameBufControl.displayOrderQueue = displayOrderQueue;
...
AVCodec ff_iTE_h264_Dtv_decoder = {
    ...
    .ext_func		= iTE_h264_Dtv_display_frame,
