...
#ifdef CFG_FFMPEG_AUDIO_DECODE
static int16_t outSample[1024 * 4 *2];
static uint8_t outputPCM[1024 * 256];
static int outputPCM_size = 0;
static bool b_firstAACPacket = false;
#endif

void updateMessageQueue();

#if defined(__OPENRTOS__) && defined(CFG_AUDIO_ENABLE) && !defined(CFG_FFMPEG_AUDIO_DECODE)
    ...
    if ((avctx->codec->capabilities & CODEC_CAP_DELAY) || avpkt->size)
    {
        ...
        else
        {
            ...
            if (nTemp)
            {
				/*if(nTemp==1){
					nTemp=2;
					iteAudioSetAttrib(ITE_AUDIO_FFMPEG_PAUSE_AUDIO, &nTemp);
            ...
            else
            {
                ...
                do
                {
                    ...
                    {   
                        for(int i=0;i<10;i++){
                            av_log(avctx, AV_LOG_ERROR, "timeout %d bufSize(%d)(%d)\n",timeout,bufSize,avpkt->size);  
                        }
                        return ret;
                    }
                } while (bufSize < avpkt->size);                
