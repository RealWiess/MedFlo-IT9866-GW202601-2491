    ...
    if (!MP3decoder(f))
    {
        ...
        else
        {
            ...
            else
            {
                size_t ret;
                printf("play repeat\n");
                ret         = fseek(d->fd, d->offset, SEEK_SET);
                d->fileRead = true;
            }
        }
    }
...
static void mp3grab_open (IteFilter * f, void * arg)
{
    ...
    parsingMp3IsID3v2(d->fd, (void *)NULL, 0, &d->offset); // parsing ID3V2tag
    ...
    tmp     = parsing_mp3(d->fd, (char *)file, (void *)NULL, 0, &flower->dInfo);
...
/*static void mp3grab_get_rate(IteFilter *f, void *arg){
    Mp3grabData *d=(Mp3grabData*)f->data;
    *(int*)arg = d->rate;
}

static void mp3grab_get_channels(IteFilter *f, void *arg){
    Mp3grabData *d=(Mp3grabData*)f->data;
    *(int*)arg = d->nchannels;
}

static void mp3grab_pause(IteFilter *f, void *arg){
    Mp3grabData *d=(Mp3grabData*)f->data;
    bool pause=*((bool*)arg);
    pthread_mutex_lock(&d->mutex);
    if(pause)
