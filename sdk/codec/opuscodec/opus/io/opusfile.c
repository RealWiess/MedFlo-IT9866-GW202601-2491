...
/*void op_update_memory(OggOpusFile *_of,const unsigned char *_data,size_t _size){
    int ret;
    //ret=op_mem_stream_update(_data,_size);
    if(!ret) printf("ERROR %s %d\n",__FUNCTION__,__LINE__);
}
...
int op_raw_seek(OggOpusFile *_of,opus_int64 _pos){
  ...
  //op_decode_clear(_of);//the data will be dropped.
  _of->bytes_tracked=0;
  _of->samples_tracked=0;
...
/*ITE add*/
void op_raw_mem_seek(OggOpusFile *_of,const unsigned char *_data,size_t _size,opus_int64 _pos){
    op_mem_stream_reset(_of->stream,_data,_size);
    op_raw_seek(_of,_pos);
}
