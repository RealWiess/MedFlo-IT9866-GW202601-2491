...
#endif
{
    ...
    for (;;)
    {  // Forever Loop
        WaveInfoMode WaveInfo;
        mblkbuf blk,*wavblk=&blk;
        unsigned char  buf[READBUF_SIZE];
        /*wavblk init*/
        memset(buf, 0, READBUF_SIZE);
        mblk_init(wavblk, buf, READBUF_SIZE);

        /*WavInfo init*/
        memset(&WaveInfo, 0, sizeof(WaveInfoMode));

        int DACinit=0;
        while(1){
            pcmData gFrame;
            int readbytes=wavDecoder(wavblk,&WaveInfo,&gFrame);

            if(readbytes){
                int pcmlen = gFrame.len;
                ...
                OutPutPcmData(gFrame.data_p, pcmlen);//put pcm to buf
                wavblk->b_rptr+=readbytes;

            }else{
                int ReadSize,empty;
                mblk_buf_rearrange(wavblk);
                empty = READBUF_SIZE-mblk_get_avail_size(wavblk);//get wav buf empty size
                ReadSize= DecFillReadBuffer(wavblk, empty);//fill wav buf from streambuf
