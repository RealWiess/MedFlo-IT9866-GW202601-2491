...
#define ITU_VERSION "ITU-VERSION-20250828-1138"
...
#define ITU_LANGUAGE_TABLE_WORDLEN  512     ///< Maximum length of each language table item.
...
#define ITU_FREETYPE_MAX_FONTS      20      ///< Maximum count of loaded fonts.
#define ITU_GIF_MAX_FRAME           1000    ///< The maximum frame count for GIF support (memory limit issue)
#define ITU_ARB_MAX_POINTS          300     ///< The maximum points used with path of arbitrary fill widget.


#define ITU_USE_SPRITE_COMMON_BUF 0 //sprite use common decompress buffer or not. (1: use) / (0: not[default])
#define ITU_DCPS_BUFFER_COUNT 2   ///< The count of buffer pool what used in dcps in place mode.

#ifdef CFG_ITU_DECOMPRESS_SURFACE_INPLACE
#define ITU_DOUBLE_DBUFF ITU_DCPS_BUFFER_COUNT
#undef ITU_USE_SPRITE_COMMON_BUF
#define ITU_USE_SPRITE_COMMON_BUF 0 //dcps in place mode not support sprite common buffer
#else
#define ITU_DOUBLE_DBUFF 1
#endif

#define ITU_LITE_MODE    0x2     ///< Use ITU LITE mode
#define ITU_FONT_SURFACE_CACHE_STRING_COUNT  1000

#ifndef _WIN32
#if (((CFG_CHIP_FAMILY == 970) || (CFG_CHIP_FAMILY == 9860)|| (CFG_CHIP_FAMILY == 9830) || (CFG_CHIP_FAMILY == 9870) || (CFG_CHIP_FAMILY == 9880)) && defined(CFG_ITU_FT_CACHE_ENABLE))
...
/**
 * ITU error code definition.
 */
typedef enum
{
    ITU_ERROR_CODE_FOPEN         =   0,      ///< error from fopen
    ITU_ERROR_CODE_DEFAULT       = 100       ///< default error code
} ITUErrorCode;
...
#define ITU_DRAWDCPS    0x80    ///< This surface required decompress from widget draw.
#define ITU_DRAWREQ     0x100   ///< This surface required to draw for in place mode.
#define ITU_M2DVMEM     0x200   ///< This surface used M2D VMEM.
#define ITU_SPRITE_BUF  0x400   ///< This surface used local common buffer
#define ITU_MASKED      0x800   ///< This surface is masked.
#define ITU_JPEGALPHAED 0x1000  ///< This surface had jpeg alpha.
//#define ITU_SURFACE_DEBUG_JPEGBUFF ///< Temporary debug jpeg under dcps in place mode.

/**
 * Corner mask scale.
 */
typedef struct ITUCornerMaskScale
{
    float topLeft;
    float topRight;
    float bottomLeft;
...
typedef struct ITUSurface
{
    ...
    struct ITUCornerMaskScale cornerMaskScale;  ///< scale of corner mask.
    ...
    struct ITUSurface    *parent;   ///< Compressed parent surface
} ITUSurface;
...
/**
 * Draw time infomation to video surface.
 */
...
* @return the STN LCD display surface. null if failed
*/
extern void(*ituReplaceSurface)(ITUSurface* surf, int w, int h, int pitch, ITUPixelFormat format);
...
* @param dx the x coordinate of destination surface, in pixels.
* @param dy the y coordinate of destination surface, in pixels.
* @param coordinate the (x,y) coordinate of point 0-3 on source rectangle, in pixels.
* @param src the source surface to draw.
*/
extern void(*ituShearing)(ITUSurface* dest, int dx, int dy, ITU_RECTANGLE coordinate, ITUSurface* src, ITUColor* color);

/**
* Mirror source surface to destination surface.
*
* @param dest the destination surface to draw to.
* @param dx the x coordinate of destination surface, in pixels.
* @param dy the y coordinate of destination surface, in pixels.
...
* @param sy the y coordinate of source surface, in pixels.
*/
extern void(*ituMirror)(ITUSurface* dest, int dx, int dy, ITUSurface* src, int sx, int sy);


/**
* cut up surface to do shearing.
*
* @param dest the destination surface to draw to.
* @param dx the x coordinate of destination surface, in pixels.
* @param dy the y coordinate of destination surface, in pixels.
* @param src the source surface to draw.
* @param srcCoods the coordinates (x,y) of each piece and size after source surface cutting
* @param destCoods the relative coordinates (x,y) of each piece and size after destination surface cutting.
*/
extern void(*ituShearingGDC)(ITUSurface *dest, int dx, int dy, ITUSurface *src, ITU_COODS srcCoods, ITU_COODS destCoods);

/**
* Alpha blending source surface to use alpha of destination surface.
*
* @param dest the destination surface to draw to.
* @param dx the x coordinate of destination surface, in pixels.
* @param dy the y coordinate of destination surface, in pixels.
* @param src the source surface to draw.
*/
extern void(*ituAlphaBlendSelDstAlpha)(ITUSurface* dest, int dx, int dy, ITUSurface* src);

/**
* Fills specified color to specified surface and shearing.
*
...
* @param color the color value to draw.
*/
extern void(*ituColorFillWithShearing)(ITUSurface * dest, int dx, int dy, int w, int h, ITU_RECTANGLE coordinate, ITUColor * color);
/**
...
ITUSurface* ituJpegLoad_QR(float percentage, uint8_t* data, int size, unsigned int flags, int *header_width, int *header_height);
...
/**
* Loads JPEG with ARGB888 data to surface, but alpha value is zero.
*
* @param width surface width.
* @param height surface height.
* @param data the JPEG data.
...
* @return the loaded surface.
*/
ITUSurface *ituJpegLoadToARGB8888(int width, int height, uint8_t *data, int size, unsigned int flags);
...
ITUSurface *ituJpegLoadFile_QR(float percentage, char* filepath, unsigned int flags, int *header_width, int *header_height);
void ituJpegSaveFile_QR(ITUSurface *surf, char *filepath, int header_width, int header_height);
...
/**
 * Harfbuzz Style definition
 */
typedef struct
{
    int lang;     ///< (0 default)(1 Arabic)(2 Hebrew)(3 Khmer)(4 Thai)
    int dir;      ///< (0 default)(1 from right to left)
} ITUFtHarfbuzzStyle;

typedef struct
{
    int count[ITU_FREETYPE_MAX_FONTS];
    wchar_t* cidArr[ITU_FREETYPE_MAX_FONTS][1000];
    uint32_t* gidArr[ITU_FREETYPE_MAX_FONTS][1000];
} THarfbuzzGID;
...
/**
* Set the font working with Harfbuzz.
*
* @param index The index of loaded font to set.
* @param hbstyle The font special style. (see define for ITUFtHarfbuzzStyle)
* @param enable Working for Harfbuzz now(true) or not(false).
*/
void ituFtSetHarfbuzzMode(int index, ITUFtHarfbuzzStyle hbstyle, bool enable);

/**
* Get the working Harfbuzz style for this font.
*
* @param index The index of loaded font to get style.
* @return the harfbuzz style for this font. (see define for ITUFtHarfbuzzStyle)
*/
 ...
 * @param widget the text widget (or null).
 * @param boldsize the bold font size (normal font use 0).
 * @return the y direction modification.
 */
...
#define ITU_FT_STYLE_HARFBUZZ   0x8       ///< using Harfbuzz
...
/**
* Get the current font style for this font.
*
* @param fontindex the fontindex of this font.
* @return the style of this font.
*/
unsigned int ituFtGetFontExtraStyle(int fontindex);

/**
* Set the current font style for this font.
*
* @param fontindex the fontindex of this font.
* @param style the font style.
*/
...
/**
* Sets the font search priority of current text draw when found no valid glyph in current fontIndex.
*
* @param priority the priority int array.
...
typedef enum
{
    ...
    ITU_EVENT_SKIPPRESS             = 38,   ///< Notify the skip press event.
    ITU_EVENT_MOUSEMOVEUP           = 39,   ///< Notify the mouse-moveup event to all visible widgets
...
#define ITU_EXTERNAL_IN_PLACE   0x2000000   ///< This widget surface loaded under external and in place mode.
#define ITU_MOUSE_EV_TOPCHECK   0x4000000   ///< This widget drop mouse related event when other cover it.
#define ITU_ABANDON_FLAG        0x8000000   ///< This flag used for abandonment usage.

#ifndef _WIN32
    #if (CFG_CHIP_FAMILY == 9830) || (CFG_CHIP_FAMILY == 9870) || (CFG_CHIP_FAMILY == 9870)
        #undef ITU_FIT_TO_RECT
        #undef ITU_CUT_BY_RECT
        #define ITU_FIT_TO_RECT  ITU_ABANDON_FLAG
        #define ITU_CUT_BY_RECT  ITU_ABANDON_FLAG
    #endif
#endif
...
typedef enum
{
	...
	ITU_SPINNER,                ///< Spinner
    ITU_POLYGON,                ///< Polygon
    ITU_ARBITRARYFILL,          ///< ArbitraryFill
 ...
 * Widget definition
 */
#define ITUWIDGET_TREE_USE_ALLOCA
 ...
 * @param widget Pointer referring to the widget.
 */
#define ituWidgetEnable(widget)                             (((ITUWidget*)(widget))->flags |= ((unsigned int)ITU_ENABLED))
 ...
 * @param widget Pointer referring to the widget.
 */
#define ituWidgetDisable(widget)                             (((ITUWidget*)(widget))->flags &= ~((unsigned int)ITU_ENABLED))
 ...
 * @param widget Pointer referring to the widget.
 */
#define ituWidgetIsEnabled(widget)                           ((((ITUWidget*)(widget))->flags) & ((unsigned int)ITU_ENABLED))
...
typedef struct ITULayerTag
{
    ...
    int xmlVersion;                                     ///< The version of this *.xml/*.itu
...
/**
* set external layer file path. (ex: B:/standard.Layer2)
*
* @param filename the full path_file_name which you want to load external layer data. (null string to use default itu path)
*/
void ituLayerExternalSetPath(char* filename);
 ...
 */
#define ITUICON_CORNER_MASK_ACTIVE  0x1
#define ITUICON_CORNER_MASK_LINKING 0x2
typedef struct ITUIconTag
{
	...
	unsigned int iconflags;     ///< The flags of icon.
	ITUSurface* staticMaskTL; ///< The static TopLeftMask surface.
	ITUSurface* staticMaskTR; ///< The static TopRightMask surface.
	ITUSurface* staticMaskBL; ///< The static BottomLeftMask surface.
	ITUSurface* staticMaskBR; ///< The static BottomRightMask surface.
	ITUSurface* maskTL;       ///< The TopLeftMask surface.
	ITUSurface* maskTR;       ///< The TopRightMask surface.
	ITUSurface* maskBL;       ///< The BottomLeftMask surface.
	ITUSurface* maskBR;       ///< The BottomRightMask surface.
	ITUSurface* lmaskTL;      ///< The loaded TopLeftMask surface.
	ITUSurface* lmaskTR;      ///< The loaded TopRightMask surface.
	ITUSurface* lmaskBL;      ///< The loaded BottomLeftMask surface.
	ITUSurface* lmaskBR;      ///< The loaded BottomRightMask surface.
} ITUIcon;
...
/**
 * Loads JPEG data to the icon widget.(RGB565)
 *
 * @param icon The icon widget to load.
 * @param data The JPEG data.
 * @param size The JPEG data size.
...
/**
 * Loads JPEG data to the icon widget.(ARGB8888)
 *
 * @param icon The icon widget to load.
 * @param data The JPEG data.
 * @param size The JPEG data size.
...
/**
* Load external surface data for Icon.
*
* @param icon The icon widget to load external.
*/
...
#define ITU_TEXT_BRIEF_LONGWORD  0x8    ///< flag to use brief(...) to replace when string length too long.

#define ITU_TEXT_LATS_BIT 3      ///< Shift bit defined for char multibyte.
...
typedef struct
{
    ...
    char* tableString;              ///< String from language table.
    ...
    int useLangTableFile;           ///< Use lang_str_tab or not.
    ITUStringSet* stringSet;        ///< The string set definition.
	int fontPriority[ITU_FREETYPE_MAX_FONTS];  ///< The font index priority.
} ITUText;
...
/**
* Set the text to use brief long word.
*
* @param text The text widget to set.
* @param OnOff The Brief long word turn On(true) or Off(false).
*/
void ituTextSetBriefLongWord(ITUText* text, bool OnOff);

/**
* Set text language table string to string.
*
* @param text The text widget with the language table index set.
* @return The process is successful or not (string result will store text->tableString).
*/
bool ituTextProcLangTableToString(ITUText* text);

/**
* Set wide string to the text.
*
* @param text The text widget to set.
* @param wcString The wide string.
*/
...
#define ITU_TEXTBOX_EN_SPACEWRAP_LONGF    0x40    ///< English Word wrap on a multi-line text box when length > widget width.
#define ITU_TEXTBOX_EN_SPACEWRAP          0x80    ///< English Word wrap on a multi-line text box.
...
typedef struct
{
    ...
    int rowsCount;                          ///< Current rows count.
    int maxrowsCount;                       ///< Maximum rows count.
    ITUAction actions[ITU_ACTIONS_SIZE];    ///< Actions for events to trigger
} ITUTextBox;
...
/**
* Set the text box to use English wordwrap.
*
* @param textbox The text box widget to set.
* @param OnOff The English wordwrap turn On(true) or Off(false).
*/
void ituTextBoxSetEngWordWrap(ITUTextBox* textbox, bool OnOff);

/**
 * Gets current rows total count of the text box widget.
 *
 * @param textbox Pointer referring to the text box widget.
...
/**
 * Gets maximum rows total count of the text box widget.
 *
 * @param textbox Pointer referring to the text box widget.
 * @return The rows count.
 */
...
/**
 * Sets maximum rows total count of the text box widget.
 *
 * @param textbox Pointer referring to the text box widget.
 * @param The new rows count.
 */
 ...
 */
#define ITU_BUTTON_FLAG_SLIDE_MOUSEUP           0x1
#define ITU_BUTTON_FLAG_OUTSIDE_FORCE_MOUSEUP   0x2
...
typedef struct
{
	...
	int ImgTargetIndex;                     ///< The index of all IconListBox Target.
	char dImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the DefaultImage.
	ITUWidget* dImgILB;                     ///< The IconListBox widget to show the DefaultImage.
	char fImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the FocusImage.
	ITUWidget* fImgILB;                     ///< The IconListBox widget to show the FocusImage.
	char pImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the PressImage.
	ITUWidget* pImgILB;                     ///< The IconListBox widget to show the PressImage.
...
/**
* Loads external data to the button widget.
*
* @param btn The button widget to load.
* @param layer The layer data to load.
*/
...
*
* @param btn The button to set.
* @param onoff The switch to turn on or off.
*/
void ituButtonSetSlideMouseUP(ITUButton* btn, bool onoff);

/**
* Set Style index of the button to use the Default/Focus/Press image index with iconlistbox target.
*
* @param btn The button to set.
* @param index The Style index for each image iconlistbox target.
*/
void ituButtonSetStyleIndex(ITUButton* btn, int index);

/**
* set the button drop down/up event when touch other button over it.
*
* @param widget The button widget to do this setting.
* @param onoff The switch to enable or not.
*/
void ituButtonSetTopCheck(ITUWidget* widget, bool onoff);

/**
* Sets the button to receive mouseup action when pressed and move to outside.
*
* @param btn The button to set.
* @param onoff The switch to turn on or off.
*/
...
typedef struct
{
	...
	char pImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the PressImage.
	ITUWidget* pImgILB;                     ///< The IconListBox widget to show the PressImage.
	char cImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the CheckedImage.
	ITUWidget* cImgILB;                     ///< The IconListBox widget to show the CheckedImage.
	char fcImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the FocusCheckedImage.
	ITUWidget* fcImgILB;                     ///< The IconListBox widget to show the FocusCheckedImage.
} ITUCheckBox;
...
/**
* Load external surface data for CheckBox.
*
* @param checkbox The checkbox widget to load external.
*/
void ituCheckBoxLoadExternalDataAPI(ITUCheckBox* checkbox);

/**
* Set Style index of the checkbox to use the Default/Focus/Press/Checked/FocusChecked image index with iconlistbox target.
*
* @param chk The checkbox to set.
* @param index The Style index for each image iconlistbox target.
*/
...
/**
* Set Style index of the radiobox to use the Default/Focus/Press/Checked/FocusChecked image index with iconlistbox target.
*
* @param chk The radiobox to set.
* @param index The Style index for each image iconlistbox target.
*/
...
/**
* Clones the progress bar widget.
*
* @param widget The progress bar widget to clone.
* @param cloned Retrieved cloned progress bar widget.
* @return true if clone is success, false otherwise.
*/
bool ituProgressBarClone(ITUWidget* widget, ITUWidget** cloned);
...
/**
* Exits the trackbar widget.
*
* @param widget The trackbar widget to exit.
*/
void ituTrackBarExit(ITUWidget* widget);

/**
* Clones the trackbar widget.
*
* @param widget The trackbar widget to clone.
* @param cloned Retrieved cloned trackbar widget.
* @return true if clone is success, false otherwise.
*/
bool ituTrackBarClone(ITUWidget* widget, ITUWidget** cloned);
...
/**
* Sets the angle range to the circle progress bar widget.
*
* @param bar The circle progress bar widget to set.
* @param start Angle to start.
* @param end Angle to end.
*/
void ituCircleProgressBarSetAngle(ITUCircleProgressBar* bar, int start, int end);

/**
* Sets the range progress icon target for circle progress bar.
*
* @param bar The circle progress bar widget to set.
* @param icon The progress icon target for the special range value.
* @param value The progress icon target will change when current value bigger than this value.
*/
...
#define ITU_ANIM_LASTFRAME_STOP     0x800   ///< Stop at last frame and will not go to the next keyframe.
...
/**
* Set the animation widget playing will stop at last frame and not go to the next keyframe.
*
* @param animation The animation widget to set.
* @param enable The setting on or not.
*/
...
#define ITU_WHEEL_OUTSIDE_BOUNCE_DIST_FACTOR 0 // (0:disable 2:default) dragging outside max dist = item_size / this_factor (under non-cycle-mode)
...
/**
* Set the wheel to do animation under goto.
* @param wheel The wheel widget.
* @param enable Turn On of Off.
*/
...
typedef struct ITUStepWheelTag
{
	...
	ITUColor focusColor;        ///< Focused text color
	ITUColor normalColor;       ///< Normal text color
    int count;                  ///< the item count for internal check and usage.
    int touchPos;               ///< x or y coordinate of touch point.(x for horizontal mode, y for vertical mode)
    int columnWidth;            ///< column width for internal usage.
    int fontHeight;             ///< Font height.
    int focusFontHeight;        ///< Font height of focused text.
    ...
    int totalframe;             ///< Total frame count of animation.
    int org_totalframe;         ///< use to backup the original totalframe
    int forceFactor;            ///< the force factor for sliding speed (1~5 unit)
    int nonCycleBoundTor;       ///< the bound tolerance(drag/slide) for non-Cycle mode (0~30 pixel)
    ITUWidget* childArr[ITU_WIDGET_CHILD_MAX];    ///< the child array cache for internal performance improve
    int temp[6];                ///< temporary usage
    bool cycleMode;             ///< cycle mode or not
    bool verticalMode;          ///< vertical mode or not
    bool fontsquareMode;        ///< the font square mode.
    bool slidable;              ///< slide able or not
    bool sliding;               ///< sliding status
    bool idleNow;               ///< wheel idle state
    bool byteAlign[2];          ///< use for byte alignment.
...
*
* @param spw The stepwheel widget to goto.
*/
...
*
* @param spw The stepwheel widget.
* @return the focus item. NULL if not focused item.
*/
...
/**
* Set the specified force factor for sliding speed of the stepwheel widget.
*
* @param spw The stepwheel widget.
* @param factor The factor default is 1 and could be 1~5. (bigger number is faster)
*/
...
/**
* Get the index of item that most center nearby.
*
* @param spw The stepwheel widget.
...
/**
* Set the stepwheel outside draggable able or not.
*
* @param spw The stepwheel widget.
* @param draggable The draggable enable or not.(true for enable, false for disable)
*/
...
typedef struct ITUCoverFlowTag
{
	...
	int dragFlag;                   ///< for drag status flag
	int bounceFlag;                 ///< for bounce status flag
	int orgItemSize;                ///< for original item size memo
...
/**
* reset the coverflow widget.
*
* @param coverflow The coverflow widget to reset.
*/
...
/**
* Exits the meter widget.
*
* @param widget The meter widget to exit.
*/
void ituMeterExit(ITUWidget* widget);

/**
* Clones the meter widget.
*
* @param widget The meter widget to clone.
* @param cloned Retrieved cloned meter widget.
* @return true if clone is success, false otherwise.
*/
bool ituMeterClone(ITUWidget* widget, ITUWidget** cloned);
...
typedef struct ITUVideoTag
{
    ...
    int volume;                     ///< Volume.
    int angle;                      ///< angle
    ITUSurface* surfTmp;            ///< temp surface for internal usage.
 ...
 * @param sec The specified sec of video widget to start playing. -1 indicates playing from 0 sec.
 */
void ituVideoPlay(ITUVideo* video, int sec);
...
/**
 * Set video widget Rotate.
 *
 * @param video The video widget to rotate.
 * @param angle. Set the rotate angle.
 */
void ituVideoSetRotateAngle(ITUVideo* video, int angle);
...
/**
* set the background button drop down/up event when touch other button over it.
*
* @param widget The background button widget to do this setting.
* @param onoff The switch to enable or not.
*/
...
#define ITU_DRAWPEN_ONESHOT_MODE   0x80   ///< This drawpen is under oneshot mode.
...
/**
* Set the oneshot mode of the DrawPen widget to enable or not.
*
* @param drawpen The DrawPen widget.
* @param oneshot The switch for touch. True is Enable oneshot mode and false for not.
*/
...
#define ITU_COMBOTABLE_MAX_PAGEITEM    1000
...
typedef struct ITUComboTableTag
{
	...
	int dragPIPE[ITU_COMBOTABLE_MAX_PAGEITEM]; ///< The drag pipe system usage.
...
/** @defgroup itu_widget_spinner Spinner
*  @{
*/

/**
* Spinner widget definition.
*/
typedef struct ITUSpinnerTag
{
	ITUIcon icon;                   ///< Base icon widget definition.
	unsigned int spinnerFlags;      ///< The flags to indicate the status of the spinner.
	ITUSurface* staticSpASurf;      ///< The static surface for spin A part of the spinner.
	ITUSurface* surfSpA;            ///< The work surface for spin A part of the spinner.
	ITUSurface* staticSpBSurf;      ///< The static surface for spin B part of the spinner.
	ITUSurface* surfSpB;            ///< The work surface for spin B part of the spinner.
	ITUSurface* staticSpCSurf;      ///< The static surface for spin C part of the spinner.
	ITUSurface* surfSpC;            ///< The work surface for spin C part of the spinner.
	ITUSurface* staticMkSurf;       ///< The static surface for the mask of the spinner.
	ITUSurface* surfMk;             ///< The work surface for the mask of the spinner.
    ITUSurface* destTmp;            ///< The work surface for internal usage
    ITUSurface* srcTmp;             ///< The work surface for internal usage.
	int bgAngle;                    ///< The angle of background.
	int spAngle;                    ///< The angle of spin.
	int mkAngle;                    ///< The angle of mask.
	int ImgTargetIndex;                     ///< The index of all IconListBox Target that linked to spinner.
	char bImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the background.
	ITUWidget* bImgILB;                     ///< The IconListBox widget to show the background.
	char saImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the spinA.
	ITUWidget* saImgILB;                     ///< The IconListBox widget to show the spinA.
	char sbImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the spinB.
	ITUWidget* sbImgILB;                     ///< The IconListBox widget to show the spinB.
	char scImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the spinC.
	ITUWidget* scImgILB;                     ///< The IconListBox widget to show the spinC.
	char mImgILBName[ITU_WIDGET_NAME_SIZE]; ///< The name of IconListBox widget to show the mask.
	ITUWidget* mImgILB;                     ///< The IconListBox widget to show the mask.
	int orgWidth;                           ///< Original width for scale animation
	int orgHeight;                          ///< Original height for scale animation
	int spIndex;                            ///< The index of spin that using now.(0A1B2C)
    int maskMode;                           ///< use spin as mask or not.
} ITUSpinner;

/**
* Initializes the spinner widget.
*
* @param spin The spinner widget to initialize.
*/
void ituSpinnerInit(ITUSpinner* spin);

/**
* Loads the spinner widget. This is called by scene manager.
*
* @param spin The spinner widget to load.
* @param base The address in the scene file buffer.
*/
void ituSpinnerLoad(ITUSpinner* spin, uint32_t base);

/**
* Exits the spinner widget.
*
* @param widget The spinner widget to exit.
*/
void ituSpinnerExit(ITUWidget* widget);

/**
* Clones the spinner widget.
*
* @param widget The spinner widget to clone.
* @param cloned Retrieved cloned spinner widget.
* @return true if clone is success, false otherwise.
*/
bool ituSpinnerClone(ITUWidget* widget, ITUWidget** cloned);

/**
* Draws the spinner widget to the specified surface.
*
* @param widget The spinner widget to draw.
* @param dest The surface to draw to.
* @param x The x coordinate of destination surface, in pixels.
* @param y The y coordinate of destination surface, in pixels.
* @param alpha the alpha value to do the constant alphablending to the surface.
...
/**
* Set the angle to background image of spinner.
*
* @param widget The spinner widget to set.
* @param angle The angle.
*/
void ituSpinnerSetBgAngle(ITUWidget* widget, int angle);

/**
* Set the angle to mask image of spinner.
*
* @param widget The spinner widget to set.
* @param angle The angle.
*/
...
/**
* Set the angle to spin image of spinner.
*
* @param widget The spinner widget to set.
* @param angle The angle.
* @param spIndex The spin index of spinner.(0 use spinA, 1 use spinB, 2 use spinC)
*/
void ituSpinnerSetAngle(ITUWidget* widget, int angle, int spIndex);

/** @} */ // end of itu_widget_spinner

/** @defgroup itu_widget_polygon Polygon
 *  @{
 */

#define ITU_POLYGON_MAX_POINT_COUNT   16  ///< Maximum point count.

 /**
  * Polygon widget definition.
  */
typedef struct
{
    ITUBackground bg;                                  ///< Base background widget definition
    unsigned int polygonFlags;                         ///< The flags to indicate the status of the polygon.
    int pointCount;                                    ///< Total count of points
    int xPoints[ITU_POLYGON_MAX_POINT_COUNT];          ///< The x coordinate of points
    int yPoints[ITU_POLYGON_MAX_POINT_COUNT];          ///< The y coordinate of points
    ITUSurface* cacheSurf;                             ///< The cache surface for internal usage.
    ITUColor fillColor;                                ///< The fill color
    ITUPoint pointArray[ITU_POLYGON_MAX_POINT_COUNT];  ///< The points array.
} ITUPolygon;

/**
 * Initializes the polygon widget.
 *
 * @param pg The polygon widget to initialize.
 */
void ituPolygonInit(ITUPolygon* pg);

/**
 * Loads the polygon widget. This is called by scene manager.
 *
 * @param pg The polygon widget to load.
 * @param base The address in the scene file buffer.
 */
void ituPolygonLoad(ITUPolygon* pg, uint32_t base);

/**
 * Exits the polygon widget.
 *
 * @param widget The polygon widget to exit.
 */
void ituPolygonExit(ITUWidget* widget);

/**
 * Updates the polygon widget by specified event.
 *
 * @param widget The polygon widget to update.
 * @param ev The event to notify.
 * @param arg1 The event related argument #1.
 * @param arg2 The event related argument #2.
 * @param arg3 The event related argument #3.
 * @return true if the polygon widget is modified and need to be redraw, false if no need to be redraw.
 */
bool ituPolygonUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3);

/**
 * Draws the polygon widget to the specified surface.
 *
 * @param widget The polygon widget to draw.
 * @param dest The surface to draw to.
 * @param x The x coordinate of destination surface, in pixels.
 * @param y The y coordinate of destination surface, in pixels.
 * @param alpha the alpha value to do the constant alphablending to the surface.
...
/**
* Set the new point count of the polygon widget.
*
* @param pg The polygon widget to set.
* @param count The new point count.
*/
void ituPolygonSetPointCount(ITUPolygon* pg, int count);

/**
* Set the polygon point array of the polygon widget.
*
* @param pg The polygon widget to set.
* @param pointX The polygon x point array.
* @param pointY The polygon y point array.
* @param count The point count.
*/
void ituPolygonSetPoint(ITUPolygon* pg, int* pointX, int* pointY, int count);

/** @} */ // end of itu_widget_polygon

/** @defgroup itu_widget_arbitraryfill ArbitraryFill
*  @{
*/

/**
* ArbitraryFill widget definition.
*/
typedef struct ITUArbitraryFillTag
{
    ITUIcon icon;                     ///< Base icon widget definition.
    unsigned int arbitraryfillFlags;  ///< The flags to indicate the status of the spinner.
    ITUSurface* staticPathSurf;       ///< The work surface for internal usage
    ITUSurface* surfPath;             ///< The work surface for internal usage.
    ITUSurface* destTmp;              ///< Surface for internal usage.
    ITUSurface* srcTmp;               ///< Surface for internal usage.
    int value;                        ///< target value.
    int current;                      ///< current value.
    int count;                        ///< count of arb array.
    int fillstep;                     ///< the step distance when fill.
    int pathAX[ITU_ARB_MAX_POINTS];   ///< X points of pathA.
    int pathAY[ITU_ARB_MAX_POINTS];   ///< Y points of pathA.
    int pathBX[ITU_ARB_MAX_POINTS];   ///< X points of pathB.
    int pathBY[ITU_ARB_MAX_POINTS];   ///< Y points of pathB.
} ITUArbitraryFill;

/**
* Initializes the ArbitraryFill widget.
*
* @param arbf The ArbitraryFill widget to initialize.
*/
void ituArbitraryFillInit(ITUArbitraryFill* arbf);

/**
* Loads the ArbitraryFill widget. This is called by scene manager.
*
* @param arbf The ArbitraryFill widget to load.
* @param base The address in the scene file buffer.
*/
void ituArbitraryFillLoad(ITUArbitraryFill* arbf, uint32_t base);

/**
* Exits the ArbitraryFill widget.
*
* @param widget The ArbitraryFill widget to exit.
*/
void ituArbitraryFillExit(ITUWidget* widget);

/**
* Clones the ArbitraryFill widget.
*
* @param widget The ArbitraryFill widget to clone.
* @param cloned Retrieved cloned ArbitraryFill widget.
* @return true if clone is success, false otherwise.
*/
bool ituArbitraryFillClone(ITUWidget* widget, ITUWidget** cloned);

/**
* Draws the ArbitraryFill widget to the specified surface.
*
* @param widget The ArbitraryFill widget to draw.
* @param dest The surface to draw to.
* @param x The x coordinate of destination surface, in pixels.
* @param y The y coordinate of destination surface, in pixels.
* @param alpha the alpha value to do the constant alphablending to the surface.
...
/** @} */ // end of itu_widget_arbitraryfill
...
/**
* Font surface cache hash array definition.
*/
typedef struct
{
	uint32_t hash;
...
/**
* Font surface cache spec(SCS) definition.
*/
typedef struct
{
	unsigned char* surf;
	...
	int fp;
	int bottomShift;
	int lb;  //glyph left bearing
	int wrb; //glyph width + right bearing
	wchar_t* wStr; //one word wchar
} ITUFontSCS;
...
typedef struct
{
	...
	char pathExternal[PATH_MAX];                                ///< assign external layer file path
	...
	uint32_t vAlphaBuf[ITU_DCPS_BUFFER_COUNT];                  ///< The share alpha buffer used for surfpl.
	uint32_t vPoolBuf[ITU_DCPS_BUFFER_COUNT];                   ///< The share pool buffer used for surfpl.
	ITUSurface* surfpl[ITU_DCPS_BUFFER_COUNT];                  ///< The memory pool to handle surface buffer for all display widget.
	char dbuffIDX;                                              ///< The index for double buff under dcps in place mode.
	int poolSize;                                               ///< internal usage.
	int prefer_lang;                                            ///< internal usage for default lang check.
	ITUFontArrayHash fontArrHash[ITU_FONT_SURFACE_CACHE_STRING_COUNT]; ///< internal usage for font surface cache.
	ITUFontSCS fontSurf[ITU_FONT_SURFACE_CACHE_STRING_COUNT]; ///< internal usage for font surface cache.
	void (*ErrorFunc)(int errorCode);                           ///< The error function callback.
} ITUScene;
...
/**
 * Get the XML/ITU version of current scene.
 *
 * @param scene The scene to get version.
 * @return The version number of this scene.
 */
...
/**
* Execute font surface array hash check and add.
*
* @param scene The scene to execute the command.
* @param fontIndex Font index of this string.
* @param bold Font bold of this string.
* @param str Font string.
* @param height Font height of this string.
* @param hId Font arr index of this string.
* @param color Font color of this string.
* @return The return value. (true is add new font surface hash, false is hash exist).
*/
bool ituSceneFontArrHashAdd(ITUScene* scene, int fontIndex, int bold, wchar_t* str, int height, int* hId, const ITUColor* color);

/**
* Check font surface array hash only for size.
*
* @param scene The scene to execute the command.
* @param fontIndex Font index of this string.
* @param bold Font bold of this string.
...
/**
* Preset font surface array hash when use getSize first.
*
* @param scene The scene to execute the command.
* @param bold Font bold of this string.
* @param str Font string.
...
/**
* Check and set required flag under decompress in place mode.
* @param scene The scene to check.
* @param drawReq mark dcps or not.
*/
void ituSceneCheckD2D(ITUScene* scene, bool drawReq);

/**
* Set draw required flag under decompress in place mode.
* @param scene The scene to set.
* @param widget The widget what call setD2D.
*/
void ituSceneSetD2D(ITUScene* scene, const ITUWidget* widget);

/**
* Clear draw required flag under decompress in place mode.
* @param scene The scene to clear.
* @return The clear executed or not.
*/
bool ituSceneClearD2D(ITUScene* scene);

/**
* Set scene prefer lang.
*
* @param scene The scene to set.
* @param lang The prefer lang index. (-1: disable this function, 0: default index, 1: prefer use index 1 language,..etc)
*/
void ituSceneSetPreferLang(ITUScene* scene, int lang);

/**
* Error callback for the scene.
*
* @param errorCode The error code to process.
*/
void ErrorFuncEmpty(int errorCode);

#define ituError(scene, errorfunc) (((ITUScene*)(scene))->ErrorFunc = (errorfunc))
...
/**
* Check the touch coordinate inside the widget or not.
*
* @param widget the widget to check for.
* @param x the touch/mouse X coordinate
* @param y the touch/mouse Y coordinate
* @return the check result. True is coordinate inside the widget, False for not.
*/
bool ituTouchIsInsideWidget(ITUWidget* widget, int x, int y);

/**
* Open and Get the image size of Bitmap.
*
* @param filepath the path of bitmap file.
* @param width the point to store the width.
* @param height the point to store the height.
*/
void ituBitmapFileGetSize(char* filepath, int* width, int* height);

/**
* Load the Bitmap image to surface.(only support 24bits bitmap)
*
* @param filepath the path of bitmap file.
* @param surf the surface to store image.(only support ARGB8888 surface)
*/
...
* Copy the surface.
*
* @param surfSrc the source surface.
* @param surfDst the destination surface.
* @param sx the source copy start x
* @param sy the source copy start y
* @param sw the source copy width
* @param sh the source copy height
* @param dx the dest x
* @param dy the dest y
* @return true if copy is success, false otherwise. (surfSrc/surfDst must same format and work for 565/8888 only)
*/
...
* Stretch the surface.
*
* @param surf the surface to stretch.
...
* wchar to char.
 *
 * @param pwchar The wide char pointer.
 * @param size The size_t pointer to get size.
 * @return The result.
*/
char* wchar_to_char(const wchar_t* pwchar, size_t* size);

/**
* Set corner mask link for the widget.
*
* @param widget The widget to set corner mask link.
* @param maskScale The scale related information using for corner mask.
* @param iconSrc The source icon widget that is corner image embedded.
* @param enable Set the corner mask link enable or not.
*/
void ituCornerMaskLinkSetup(ITUWidget* widget, ITUCornerMaskScale* maskScale, ITUIcon* iconSrc, bool enable);

/**
* Surface corner mask render.
*
* @param widget The target widget. (could be null)(use widget as target when widget is not null)
* @param surfDest The surface to set corner mask.(could be null)(use widget as target when surfDest is null)
* @param maskTL The source top-left mask image.(set null will use mask image of widget)
* @param maskTR The source top-right mask image.(set null will use mask image of widget)
* @param maskBL The source bottom-left mask image.(set null will use mask image of widget)
* @param maskBR The source bottom-right mask image.(set null will use mask image of widget)
*/
...
/**
* Is the HUD turned off.
*
* @return true if HUD disable, false otherwise.
*/
bool ituHudIsOff(void);

/**
* Settting the HUD turned off.
*/
void ituHudOff(void);

/**
* Settting the HUD turned on.
*/
void ituHudOn(void);

/**
* Arbitrary color fill.(software)
*
* @param dest The surface to fill.
* @param p1 The point 1.
* @param p2 The point 2.
* @param p3 The point 3.
* @param p4 The point 4.
