                                      ...
                                      unsigned int flags)
{
    ...
    if (format == ITU_A8)
    {
        if (flags == ITU_STATIC)
        {
            gfxSurface = gfxCreateSurfaceByPointer(w, h, surface->surf.pitch, GFX_COLOR_A_8, surface->surf.addr,
                                                   h * surface->surf.pitch);
        }
        else
        {
            gfxSurface = gfxCreateSurfaceByBuffer(w, h, surface->surf.pitch, GFX_COLOR_A_8, surface->surf.addr,
                                                  h * surface->surf.pitch);
        }
    ...
    {
        gfxSurface = gfxCreateSurfaceByPointer(w, h, surface->surf.pitch, gfxformat, surface->surf.addr,
                                               h * surface->surf.pitch);
        ...
        case ITU_ARGB4444:
            {
                ...
                if (surf)
                {
                    ...
                    RotateImage16((unsigned short *)destPtr, (unsigned short *)srcPtr, dstwidth, dstheight, dstwidth,
                                  src->width, src->height, src->width, cx, cy, angle, scaleX, scaleY);
...
static void M2dHWFlip (ITUSurface * surf)
{
    ...
    #endif
    {
        ...
        M2dRotate(m2dtempSurf, dX, dY, srcSurf, cX, cY, angle, 1.0f, 1.0f);
