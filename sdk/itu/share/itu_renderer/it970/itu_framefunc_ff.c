...
void ituFrameFuncExit (void)
{
    ...
    if (g_ui_buff_addr[0])
    {
        ...
        (void)memset(g_ui_buff_addr, 0, sizeof(g_ui_buff_addr));
    }
#endif
    g_inited = false;
}
...
void ituFrameFuncColorFill (int rect_x, int rect_y, int rect_w, int rect_h, int alpha, int red, int green, int blue)
{
    ...
    {
        (void)printf("gITUSurface addr fail ...\n");
        return;
    }
...
void ituFrameFuncDrawJpeg (int gui_x, int gui_y, int gui_w, int gui_h, char * jpg_data, int jpg_size)
{
    ...
    (void)printf("  disp(%ux%u), dispMode=%d, real(%ux%u), img(%ux%u), slice=%u, pitch(%u, %u)\r\n", jpgUserInfo.jpgRect.w,
           jpgUserInfo.jpgRect.h, initParam.dispMode, jpgUserInfo.real_width, jpgUserInfo.real_height,
           jpgUserInfo.imgWidth, jpgUserInfo.imgHeight, jpgUserInfo.slice_num, jpgUserInfo.comp1Pitch,
           jpgUserInfo.comp23Pitch);
    ...
    outStreamInfo.jstream.mem[1].pAddr =
        (uint8_t *)(outStreamInfo.jstream.mem[0].pAddr + outStreamInfo.jstream.mem[0].length);
    ...
    outStreamInfo.jstream.mem[2].pAddr =
        (uint8_t *)(outStreamInfo.jstream.mem[1].pAddr + outStreamInfo.jstream.mem[1].length);
...

    {
        ...
        (void)printf("jpgUserInfo.colorFormate = %d\n", jpgUserInfo.colorFormate);
        ...
        else
        {
            ...
            while ((!(VP_field_status & 1 << 16) || !(VP_field_status & 1 << 17) || !(VP_field_status & 1 << 18) ||
                    !(VP_field_status & 1 << 19) || !(VP_field_status & 1 << 21)) &&
                   timeout--)
            {
                (void)usleep(100);
            }
            ...
            if (!timeout)
            {
                (void)printf("JPEG scaling timeout (VP timeout)\n");
            }
        }
