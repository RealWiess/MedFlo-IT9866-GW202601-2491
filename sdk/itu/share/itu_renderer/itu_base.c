...
static void TransformBltDefault(ITUSurface* dest, int dx, int dy, ITUSurface* src, int sx, int sy, int sw, int sh, int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, bool bInverse, ITUPageFlowType type, ITUTransformType transformType)
{
    // DO NOTHING
}
...
static void ReplaceSurfaceDefault(ITUSurface* surf, int w, int h, int pitch, ITUPixelFormat format)
{
    // DO NOTHING
}
void(*ituReplaceSurface)(ITUSurface* surf, int w, int h, int pitch, ITUPixelFormat format) = ReplaceSurfaceDefault;

static void ShearingDefault(ITUSurface* dest, int dx, int dy, ITU_RECTANGLE coordinate, ITUSurface* src, ITUColor* color)
{
    // DO NOTHING
}
void(*ituShearing)(ITUSurface* dest, int dx, int dy, ITU_RECTANGLE coordinate, ITUSurface* src, ITUColor* color) = ShearingDefault;

static void MirrorDefault(ITUSurface* dest, int dx, int dy, ITUSurface* src, int sx, int sy)
{
    // DO NOTHING
}
void(*ituMirror)(ITUSurface* dest, int dx, int dy, ITUSurface* src, int sx, int sy) = MirrorDefault;

static void ShearingGDCDefault(ITUSurface *dest, int dx, int dy, ITUSurface *src, ITU_COODS srcCoods, ITU_COODS destCoods)
{
    // DO NOTHING
}
void(*ituShearingGDC)(ITUSurface *dest, int dx, int dy, ITUSurface *src, ITU_COODS srcCoods, ITU_COODS destCoods) = ShearingGDCDefault;

static void AlphaBlendSelDstAlphaDefault(ITUSurface* dest, int dx, int dy, ITUSurface* src)
{
    // DO NOTHING
}
void(*ituAlphaBlendSelDstAlpha)(ITUSurface* dest, int dx, int dy, ITUSurface* src) = AlphaBlendSelDstAlphaDefault;

static void ColorFillWithShearingDefault(ITUSurface * dest, int dx, int dy, int w, int h, ITU_RECTANGLE coordinate, ITUColor * color)
{
    // DO NOTHING
}
void(*ituColorFillWithShearing)(ITUSurface * dest, int dx, int dy, int w, int h, ITU_RECTANGLE coordinate, ITUColor * color) = ColorFillWithShearingDefault;
