...
static void blitIdIntrCallback (void)
{
    ...
    #if (CFG_CHIP_FAMILY == 9830) || (CFG_CHIP_FAMILY == 9870)
...
static void M2dDestroySurface (ITUSurface * surf)
{
    ...
    if ((surf->flags & ITU_M2DVMEM))
    {
        if (surf && !(surf->flags & ITU_STATIC))
        {
            (void)printf("gfxVmemFree in m2d. (%d,%d)\n", surf->width, surf->height);
            gfxVmemFree((uint8_t *)surf->addr);
        }
    }
    else
    {
...
static void M2dBitBlt (ITUSurface * dest, int dx, int dy, int w, int h, ITUSurface * src, int sx, int sy)
{
    ...
    if (((dest->flags & 0x80000000) == 0x80000000) || ((src->flags & ITU_JPEGALPHAED) == ITU_JPEGALPHAED))
    {
                           ...
                           uint8_t alpha)
{
    ...
    (void)memcpy(&destSurf->m2dSurf->fgColor, &destSurf->surf.fgColor, sizeof(GFXColor));
    if((src->flags & ITU_JPEGALPHAED) == ITU_JPEGALPHAED)
	{
        gfxSurfaceBitblt(
            destSurf->m2dSurf,
            dx,
...
static void M2dAlphaBlendSelDstAlpha (ITUSurface * dest, int dx, int dy, ITUSurface * src)
{
    M2dSurface *  destSurf   = (M2dSurface *)dest;
    M2dSurface *  srcSurf    = (M2dSurface *)src;
    GFXSurfaceSrc srcSurface = {0};
    ...
    gfxSurfaceAlphaBlendSelDstAlpha(destSurf->m2dSurf, dx, dy, &srcSurface);
    ...
    ITU_COODS  destCoods)
{
    ...
    sCoods.coods = (GFX_RECTANGLE*)srcCoods.coods;
    sCoods.cloums = srcCoods.cloums;
    sCoods.row = srcCoods.row;
    dCoods.coods = (GFX_RECTANGLE*)destCoods.coods;
    dCoods.cloums = destCoods.cloums;
    dCoods.row = destCoods.row;
    ...
    gfxSurfaceShearingGDC(&dstSurface, &srcSurface, sCoods, dCoods);
...
void ituM2dInit (void)
{
    ...
    ituMirror                = M2dMirror;
    ituShearingGDC           = M2dShearingGDC;

    ituAlphaBlendSelDstAlpha = M2dAlphaBlendSelDstAlpha;
...
#ifdef ITU_ENABLE_DEFER_DESTROY
void ituM2dDestroyDefferBuffer(void)
{
    DeferDestroyEntry tEntry = {0};
    gfxwaitEngineIdle();
    ithEnterCritical();
