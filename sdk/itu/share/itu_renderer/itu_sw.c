...
const static unsigned char DITHER_TABLE[16] = {1, 13, 2, 14, 9, 5, 10, 6, 3, 15, 0, 12, 11, 7, 8, 4};

#define RGB565_DITHER(a, r, g, b, x, y)                                                                                \
    if (a != 0)                                                                                                        \
    {                                                                                                                  \
        int d = ((y & 0x3) << 2) | (x & 0x3);                                                                          \
        if (((r & 7) > (uint32_t)(DITHER_TABLE[d] >> 1)) && ((r & 0xf8) != 0xf8))                                      \
        {                                                                                                              \
            r += 0x08;                                                                                                 \
        }                                                                                                              \
        if (((g & 3) > (uint32_t)(DITHER_TABLE[d] >> 2)) && ((g & 0xfc) != 0xfc))                                      \
        {                                                                                                              \
            g += 0x04;                                                                                                 \
        }                                                                                                              \
        if (((b & 7) > (uint32_t)(DITHER_TABLE[d] >> 1)) && ((b & 0xf8) != 0xf8))                                      \
        {                                                                                                              \
            b += 0x08;                                                                                                 \
        }                                                                                                              \
    }
...
static ITUSurface * SWCreateSurface (int w, int h, int pitch, ITUPixelFormat format, const uint8_t * bitmap,
                                     unsigned int flags)
{
...
#ifdef SPEED_UP
    surf->lockAddr = (uint8_t *)ithMapVram(vram_addr, surf->lockSize, ITH_VRAM_WRITE);
#else
        ...
        if (format == ITU_GLYPH_1BPP)
        {
            for (i = 0; i < h; i++)
            {
                ...
                for (j = 0; j < w; j++)
                {
                    ...
                    {
                        base[surf->width * i + j] =
                            ITH_RGB565(surf->fgColor.red, surf->fgColor.green, surf->fgColor.blue);
                    }
                }
            }
        }
        else if (format == ITU_GLYPH_4BPP)
        {
            for (i = 0; i < h; i++)
            {
                ...
                for (j = 0; j < (int)w; j++)
                {
                    ...
                    r  = ((((bc & 0xf800U) >> 11U) << 3U) * (255U - c) + surf->fgColor.red * c) / 255U;
                    g  = ((((bc & 0x07e0U) >> 5U) << 2U) * (255U - c) + surf->fgColor.green * c) / 255U;
                    b  = (((bc & 0x001fU) << 3U) * (255U - c) + surf->fgColor.blue * c) / 255U;
    ...
    else if (surf->format == ITU_ARGB8888)
    {
        ...
        for (i = 0; i < h; i++)
        {
            ...
            for (j = 0; j < (int)w; j++)
            {
                ...
                c                         = bitmap[w * i + j];
                bc                        = base[surf->width * i + j];
                a                         = (((bc & 0xFF000000U) >> 24U) * (255U - c) + surf->fgColor.alpha * c) / 255U;
                r                         = (((bc & 0x00FF0000U) >> 16U) * (255U - c) + surf->fgColor.red * c) / 255U;
                g                         = (((bc & 0x0000FF00U) >> 8U) * (255U - c) + surf->fgColor.green * c) / 255U;
                b                         = ((bc & 0x000000FFU) * (255U - c) + surf->fgColor.blue * c) / 255U;
        ...
        else if (src->format == ITU_ARGB1555)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    a  = (sc & 0x8000U) ? 255U : 0U;
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - a) + (((sc & 0x7C00U) >> 10U) << 3U) * a) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - a) + (((sc & 0x03E0U) >> 5U) << 3U) * a) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - a) + ((sc & 0x001FU) << 3U) * a) / 255U;
        ...
        else if (src->format == ITU_ARGB4444)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    a  = (sc & 0xF000U) >> 12U;
                    a |= a << 4U;
                    r = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - a) + (((sc & 0x0F00U) >> 8U) << 4U) * a) / 255U;
                    g = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - a) + (((sc & 0x00F0U) >> 4U) << 4U) * a) / 255U;
                    b = (((dc & 0x001fU) << 3U) * (255U - a) + ((sc & 0x000FU) << 4U) * a) / 255U;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
        }
        else if (src->format == ITU_ARGB8888)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    dc = destPtr[dest->width * y + x];
                    a  = sc >> 24;
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - a) + ((sc & 0x00FF0000U) >> 16U) * a) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - a) + ((sc & 0x0000FF00U) >> 8U) * a) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - a) + (sc & 0x000000FFU) * a) / 255U;
        ...
        else if (src->format == ITU_MONO)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    if (value)
                    {
                        destPtr[dest->width * y + x] =
                            ITH_RGB565(src->fgColor.red, src->fgColor.green, src->fgColor.blue);
                    }
                }
            }
        }
        else if (src->format == ITU_RGB565A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - a) + (((sc & 0xf800U) >> 11U) << 3U) * a) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - a) + (((sc & 0x07e0U) >> 5U) << 2U) * a) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - a) + ((sc & 0x001FU) << 3U) * a) / 255U;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
        ...
        else if (src->format == ITU_A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    sc = srcPtr[src->width * y + x];
                    dc = destPtr[dest->width * y + x];
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - sc) + dest->fgColor.red * sc) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - sc) + dest->fgColor.green * sc) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - sc) + dest->fgColor.blue * sc) / 255U;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
        }
        ...
        if (src->format == ITU_ARGB8888)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    dc = destPtr[dest->width * y + x];
                    ac = sc >> 24;
                    a  = (((dc & 0xFF000000U) >> 24U) * (255U - ac) + 255U * ac) / 255U;
                    r  = (((dc & 0x00FF0000U) >> 16U) * (255U - ac) + ((sc & 0x00FF0000U) >> 16U) * ac) / 255U;
                    g  = (((dc & 0x0000FF00U) >> 8U) * (255U - ac) + ((sc & 0x0000FF00U) >> 8U) * ac) / 255U;
                    b  = ((dc & 0x000000FFU) * (255U - ac) + (sc & 0x000000FFU) * ac) / 255U;
        ...
        else if (src->format == ITU_RGB565A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    dc = destPtr[dest->width * y + x];
                    a  = *((uint8_t *)srcPtr + src->pitch * src->height + src->width * y + x);
                    r  = (((dc & 0x00FF0000U) >> 16U) * (255U - a) + (((sc & 0xf800U) >> 11U) << 3U) * a) / 255U;
                    g  = (((dc & 0x0000FF00U) >> 8U) * (255U - a) + (((sc & 0x07e0U) >> 5U) << 2U) * a) / 255U;
                    b  = ((dc & 0x000000FFU) * (255U - a) + ((sc & 0x001fU) << 3U) * a) / 255U;
                    destPtr[dest->width * y + x] = ITH_ARGB8888(a, r, g, b);
                }
            }
            ituUnlockSurface(src);
        ...
        else if (src->format == ITU_ARGB4444)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    ac |= ac << 4U;
                    a = (((dc & 0xFF000000U) >> 24U) * (255U - ac) + 255U * ac) / 255U;
                    r = (((dc & 0x00FF0000U) >> 16U) * (255U - ac) + (((sc & 0x0F00U) >> 8U) << 4U) * ac) / 255U;
                    g = (((dc & 0x0000FF00U) >> 8U) * (255U - ac) + (((sc & 0x00F0U) >> 4U) << 4U) * ac) / 255U;
                    b = ((dc & 0x000000FFU) * (255U - ac) + ((sc & 0x000FU) << 4U) * ac) / 255U;
                    destPtr[dest->width * y + x] = ITH_ARGB8888(a, r, g, b);
                }
            }
        }
...
static void SWStretchBlt (ITUSurface * dest, int dx, int dy, int dw, int dh, ITUSurface * src, int sx, int sy, int sw,
                          int sh)
{
    int w = dw;
    int h = dh;
...
static void SWAlphaBlend (ITUSurface * dest, int dx, int dy, int w, int h, ITUSurface * src, int sx, int sy,
                          uint8_t alpha)
{
    int x, y;
    int dw = dest->width - dx;
        ...
        if (src->format == ITU_RGB565)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    sc = srcPtr[src->width * y + x];
                    dc = destPtr[dest->width * y + x];
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - alpha) + (((sc & 0xf800U) >> 11U) << 3U) * alpha) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - alpha) + (((sc & 0x07e0U) >> 5U) << 2U) * alpha) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - alpha) + ((sc & 0x001fU) << 3U) * alpha) / 255U;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
            ituUnlockSurface(src);
        ...
        else if (src->format == ITU_A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    a  = sc;
                    a  = a * alpha / 255;
                    r  = ((((dc & 0xf800U) >> 11U) << 3U) * (255U - a) + dest->fgColor.red * a) / 255U;
                    g  = ((((dc & 0x07e0U) >> 5U) << 2U) * (255U - a) + dest->fgColor.green * a) / 255U;
                    b  = (((dc & 0x001fU) << 3U) * (255U - a) + dest->fgColor.blue * a) / 255U;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
            ituUnlockSurface(src);
        ...
        if (src->format == ITU_ARGB1555)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    sc = srcPtr[src->width * y + x];
                    dc = destPtr[dest->width * y + x];
                    a  = ((((dc & 0x8000U) >> 15U) << 7U) * (255U - alpha) + (((sc & 0x8000U) >> 15U) << 7U) * alpha) / 255U;
                    r  = ((((dc & 0x7C00U) >> 10U) << 3U) * (255U - alpha) + (((sc & 0x7C00U) >> 10U) << 3U) * alpha) / 255U;
                    g  = ((((dc & 0x03E0U) >> 5U) << 3U) * (255U - alpha) + (((sc & 0x03E0U) >> 5U) << 3U) * alpha) / 255U;
                    b  = (((dc & 0x001FU) << 3U) * (255U - alpha) + ((sc & 0x001FU) << 3U) * alpha) / 255U;
        ...
        if (src->format == ITU_ARGB4444)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    sc = srcPtr[src->width * y + x];
                    dc = destPtr[dest->width * y + x];
                    a  = ((((dc & 0xF000U) >> 12U) << 4U) * (255U - alpha) + (((sc & 0xF000U) >> 12U) << 4U) * alpha) / 255U;
                    r  = ((((dc & 0x0F00U) >> 8U) << 4U) * (255U - alpha) + (((sc & 0x0F00U) >> 8U) << 4U) * alpha) / 255U;
                    g  = ((((dc & 0x00F0U) >> 4U) << 4U) * (255U - alpha) + (((sc & 0x00F0U) >> 4U) << 4U) * alpha) / 255U;
                    b  = (((dc & 0x000FU) << 4U) * (255U - alpha) + ((sc & 0x000FU) << 4U) * alpha) / 255U;
        ...
        if (src->format == ITU_ARGB8888)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    ac = sc >> 24;
                    ac = (ac * alpha) / 255;
                    a  = (((dc & 0xFF000000U) >> 24U) * (255U - ac) + ((sc & 0xFF000000U) >> 24U) * ac) / 255U;
                    r  = (((dc & 0x00FF0000U) >> 16U) * (255U - ac) + ((sc & 0x00FF0000U) >> 16U) * ac) / 255U;
                    g  = (((dc & 0x0000FF00U) >> 8U) * (255U - ac) + ((sc & 0x0000FF00U) >> 8U) * ac) / 255U;
                    b  = ((dc & 0x000000FFU) * (255U - ac) + (sc & 0x000000FFU) * ac) / 255U;
                    destPtr[dest->width * y + x] = ITH_ARGB8888(a, r, g, b);
        ...
        else if (src->format == ITU_RGB565A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    dc = destPtr[dest->width * y + x];
                    a  = *((uint8_t *)srcPtr + src->pitch * src->height + src->width * y + x);
                    a  = (((dc & 0xFF000000U) >> 24U) * (255U - alpha) + a * alpha) / 255U;
                    r  = (((dc & 0x00FF0000U) >> 16U) * (255U - alpha) + (((sc & 0xf800U) >> 11U) << 3U) * alpha) / 255U;
                    g  = (((dc & 0x0000FF00U) >> 8U) * (255U - alpha) + (((sc & 0x07e0U) >> 5U) << 2U) * alpha) / 255U;
                    b  = ((dc & 0x000000FFU) * (255U - alpha) + ((sc & 0x001fU) << 3U) * alpha) / 255U;
                    destPtr[dest->width * y + x] = ITH_ARGB8888(a, r, g, b);
                }
            }
            ituUnlockSurface(src);
    ...
    if (surf->format == ITU_RGB565)
    {
        ...
        for (i = 0; i < h; i++)
        {
            ...
            for (j = 0; j < w; j++)
            {
                ...
                else //fix for color alpha no effect with software colorfill when target surface not ARGB8888
                {
                    uint32_t a, r, g, b;
                    uint16_t dc = ITH_RGB565(255, 255, 255);
                    uint32_t sc = ITH_ARGB8888(color->alpha, color->red, color->green, color->blue);
...
static void SWGradientFill (ITUSurface * surf, int x, int y, int w, int h, ITUColor * startColor, ITUColor * endColor,
        ...
        if (surf->format == ITU_RGB565)
        {
            ...
            for (i = 0; i < h; i++)
            {
                ...
                for (j = 0; j < w; j++)
                {
                    ...
                    g = (endColor->green * (i + j) / (w + h)) +
                        (startColor->green - startColor->green * (i + j) / (w + h));
                    b = (endColor->blue * (i + j) / (w + h)) +
                        (startColor->blue - startColor->blue * (i + j) / (w + h));
                    RGB565_DITHER(255, r, g, b, j, i);
                    ptr[surf->width * i + j] = ITH_RGB565(r, g, b);
                }
            }
        ...
        else if (surf->format == ITU_ARGB1555)
        {
            ...
            for (i = 0; i < h; i++)
            {
                ...
                for (j = 0; j < w; j++)
                {
                    ...
                    a = (endColor->alpha * (i + j) / (w + h)) +
                        (startColor->alpha - startColor->alpha * (i + j) / (w + h));
                    r = (endColor->red * (i + j) / (w + h)) + (startColor->red - startColor->red * (i + j) / (w + h));
                    g = (endColor->green * (i + j) / (w + h)) +
                        (startColor->green - startColor->green * (i + j) / (w + h));
                    b = (endColor->blue * (i + j) / (w + h)) +
...
static void RotateImage16 (unsigned short * dst, unsigned short * src, int dstWidth, int dstHeight, int dstPitch,
                           int srcWidth, int srcHeight, int srcPitch, int centerX, int centerY, float angle,
                           float scale)
{
    ...
    for (v = 0; v < dstHeight; v++)
    {
        ...
        for (h = 0; h < dstWidth; h++)
        {
            ...
            else
            {
                ...
                leftDist  = (dstX & 0x0FFFFU) >> 8;
                rightDist = 256 - leftDist;
                upDist    = (dstY & 0x0FFFFU) >> 8;
                downDist  = 256 - upDist;
                ...
                dlValue   = dlPixel >> 12;
                drValue   = drPixel >> 12;
                newA      = ((ulValue * downDist + dlValue * upDist) * rightDist +
                        (urValue * downDist + drValue * upDist) * leftDist + 32768) >>
                       16;
                ...
                newR    = ((ulValue * downDist + dlValue * upDist) * rightDist +
                        (urValue * downDist + drValue * upDist) * leftDist + 32768) >>
                       16;
                ...
                newG    = ((ulValue * downDist + dlValue * upDist) * rightDist +
                        (urValue * downDist + drValue * upDist) * leftDist + 32768) >>
                       16;
                ...
                newB    = ((ulValue * downDist + dlValue * upDist) * rightDist +
                        (urValue * downDist + drValue * upDist) * leftDist + 32768) >>
                       16;
...
static void RotateImage32 (unsigned int * dst, unsigned int * src, int dstWidth, int dstHeight, int dstPitch,
                           int srcWidth, int srcHeight, int srcPitch, int centerX, int centerY, float angle,
                           float scale)
{
    float        dst0X_o, dst0Y_o, dst1X_o, dst1Y_o, dst2X_o, dst2Y_o, width_cosT, width_sinT, height_cosT, height_sinT;
...
static void SWRotate (ITUSurface * dest, int dx, int dy, ITUSurface * src, int cx, int cy, float angle, float scaleX,
                      float scaleY)
{
    ...
    dstwidth = dstheight = (int)(rmax * 2.0f);
    ...
    if (src->format == ITU_RGB565)
    {
        ...
        if (surf)
        {
            ...
            RotateImage16((unsigned short *)destPtr, (unsigned short *)srcPtr, dstwidth, dstheight, dstwidth,
                          src->width, src->height, src->width, cx, cy, angle, scaleX);
    ...
    else if (src->format == ITU_ARGB8888)
    {
        ...
        if (surf)
        {
            ...
            RotateImage32((unsigned int *)destPtr, (unsigned int *)srcPtr, dstwidth, dstheight, dstwidth, src->width,
                          src->height, src->width, cx, cy, angle, scaleX);
    ...
    else if (src->format == ITU_RGB565A8)
    {
        ...
        if (srcSurf)
        {
            ...
            if (surf)
            {
                ...
                RotateImage32((unsigned int *)destPtr, (unsigned int *)srcPtr, dstwidth, dstheight, dstwidth,
                              src->width, src->height, src->width, cx, cy, angle, scaleX);
...
static void SWTransform (ITUSurface * dest, int dx, int dy, int dw, int dh, ITUSurface * src, int sx, int sy, int sw,
                         int sh, int cx, int cy, float scaleWidth, float scaleHeight, float angle, ITUTileMode tilemode,
                         bool enableAlpha, bool enableConstantAlpha, uint8_t constantAlphaValue)
{
    ...
    else
    {
        if (angle != 0.0f)
        {
            SWRotate(dest, dx + dw / 2, dy + dh / 2, src, cx, cy, angle, scaleWidth, scaleHeight);
        }
        else if ((scaleWidth != 1.0f) || (scaleHeight != 1.0f))
        {
            int dW = (int)((float)src->width * scaleWidth);
            int dH = (int)((float)src->height * scaleHeight);
            SWStretchBlt(dest, dx, dy, dW, dH, src, sx, sy, sw, sh);
        }
        else
        {
            SWBitBlt(dest, dx, dy, sw, sh, src, sx, sy);
        }
    }
}
...
static void SWFlip (ITUSurface * surf)
{
    ...
    if (ituEmuWin)
    {
        ...
        (void)memcpy(bits, ptr, ithLcdGetPitch() * ithLcdGetHeight());
        ithUnmapVram(ptr, ithLcdGetPitch() * ithLcdGetHeight());

        StretchBlt(dc, 0, 0, ithLcdGetWidth() * ituEmuScale / 100, ithLcdGetHeight() * ituEmuScale / 100, bitmapDc, 0,
                   0, ithLcdGetWidth(), ithLcdGetHeight(), SRCCOPY);
...
static void SWTransformBlt (ITUSurface * dest, int dx, int dy, ITUSurface * src, int sx, int sy, int sw, int sh, int x0,
                            int y0, int x1, int y1, int x2, int y2, int x3, int y3, bool bInverse, ITUPageFlowType type,
...
static void SWReflected (ITUSurface * dest, int dx, int dy, ITUSurface * src, int sx, int sy, int reflectedWidth,
...
static void SWDrawLine (ITUSurface * dest, int32_t x0, int32_t y0, int32_t x1, int32_t y1, ITUColor * color,
                        int32_t lineWidth)
{
#ifdef CFG_WIN32_SIMULATOR
...
static void SWReplaceSurface (ITUSurface * surf, int w, int h, int pitch, ITUPixelFormat format)
{
    surf->width  = w;
    surf->height = h;
    surf->pitch  = (pitch) ? pitch : (w * ituFormat2Bpp(format) / 8);
    surf->format = format;
}

Matrix3x3 computeHomography(const PointF src[4], const PointF dst[4]) {
    float A[8][8] = {0};
    float B[8] = {0};
...
static void SWShearing(ITUSurface * dest, int dx, int dy, ITU_RECTANGLE coordinate, ITUSurface * src, ITUColor* color)
{
    int width = src->width;
    ...
    PointF dstPoints[4] = {
        ...
        { 2*width - coordinate.x3, -coordinate.y3 },
        { 2*width - coordinate.x2, 2*height - coordinate.y2 },
        { -coordinate.x1, 2*height - coordinate.y1 }};

    PointF srcTri[4] = { { 0, 0 }, { width, 0 }, { width, height }, { 0, height } };

    int maxX = MAX(dstPoints[0].x, dstPoints[3].x);
    int maxY = MAX(dstPoints[0].y, dstPoints[1].y);

    printf("src(%d %d) dest(%d %d)(%d %d)\n", width, height, dx, dy, dest->width, dest->height);
    printf("format(%d %d)\n", dest->format, src->format);
    printf("dstPoints(%f %f) (%f %f) (%f %f) (%f %f)\n", dstPoints[0].x, dstPoints[0].y, dstPoints[3].x, dstPoints[3].y, dstPoints[2].x, dstPoints[2].y, dstPoints[1].x, dstPoints[1].y);

    if (dstPoints[0].x > 0 || dstPoints[3].x > 0) {

        int offsetX = maxX;
        for (int i = 0; i < 4; ++i)
            dstPoints[i].x -= offsetX;
    }

    if (dstPoints[0].y > 0 || dstPoints[1].y > 0) {

        int offsetY = maxY;
        for (int i = 0; i < 4; ++i)
            dstPoints[i].y -= offsetY;
    }
    printf("dstPoints(%f %f) (%f %f) (%f %f) (%f %f)\n", dstPoints[0].x, dstPoints[0].y, dstPoints[3].x, dstPoints[3].y, dstPoints[2].x, dstPoints[2].y, dstPoints[1].x, dstPoints[1].y);

    ituSetColor(&dest->fgColor, color->alpha, color->red, color->green, color->blue);
    ...
    if (src->format == ITU_RGB565)
    {
        ...
        Matrix3x3 H = computeHomography(srcTri, dstPoints);

        for (int y = 0; y < height * 2; ++y) {

            for (int x = 0; x < width * 2; ++x) {
                float fx = (float)x, fy = (float)y;
                float denom = H.m[2][0] * fx + H.m[2][1] * fy + H.m[2][2];
                if (denom == 0) continue;
                float srcX = (H.m[0][0] * fx + H.m[0][1] * fy + H.m[0][2]) / denom;
                float srcY = (H.m[1][0] * fx + H.m[1][1] * fy + H.m[1][2]) / denom;
                int ix = (int)(srcX + 0.5f);
                int iy = (int)(srcY + 0.5f);

                if (ix >= 0 && ix < width && iy >= 0 && iy < height) {
                    destPtr[dest->width * (dy - maxY + y) + (dx - maxX + x)] = srcPtr[iy * width + ix];
                }
            }
        }
    }
        ...
        for (int y = 0; y < height * 2; ++y) {
            ...
            for (int x = 0; x < width * 2; ++x) {
                ...
                if (ix >= 0 && ix < width && iy >= 0 && iy < height) {
                        ...
                        destPtr[dest->width * (dy - maxY + y) + (dx - maxX + x)] = ITH_RGB565(r, g, b);
                }
            }
        }
    }  
...
static void SWMirror(ITUSurface* dest, int dx, int dy, ITUSurface* src, int sx, int sy)
{
    for (int i = 0; i < src->width; i++)
    {
        SWBitBlt(dest, dx + i, dy, 1, dest->height, src, src->width - i - 1, sy);
    }

    //SW flip function
    //for (int i = 0; i < src->height; i++)
    //{
    //    SWBitBlt(dest, dx, dy + i, dest->width, 1, src, sx, src->height - i - 1);
    //}
}
...
void ituSWInit (void)
{
    ...
    ituReplaceSurface = SWReplaceSurface;
    ituShearing       = SWShearing;
    ituMirror         = SWMirror;
