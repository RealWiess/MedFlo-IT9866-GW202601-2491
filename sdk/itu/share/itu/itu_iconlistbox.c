...
static void IconListBoxLoadExternalData (ITUIconListBox * ilistbox, ITULayer * layer)
{
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ...
        if (surf)
        {
            ...
            {
                ilistbox->surfArray[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ...
        if (surf)
        {
            ...
            {
                ilistbox->focusSurfArray[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
                                                               (const uint8_t *)surf->addr, surf->flags);
            }
        }
    }
...
void ituIconListBoxInit (ITUIconListBox * ilistbox, int width)
{
    ...
    (void)memset(ilistbox, 0, sizeof(ITUIconListBox));
...
{
    const ITUWidget * widget = (ITUWidget *)ilistbox;
    assert(ilistbox);
