...
#define ITUSTOPANYWHERE_DEBUG 0
#if ITUSTOPANYWHERE_DEBUG
    #define stopanywhereLog(fmt, ...) (void)printf(fmt, ##__VA_ARGS__)
#else
    #define stopanywhereLog(fmt, ...)
#endif
...
bool ituStopAnywhereClone (ITUWidget * widget, ITUWidget ** cloned)
{
    ...
    if (*cloned == NULL)
    {
         ...
         * ITUStopAnywhere object. Since ITUStopAnywhere is a struct that extends
         * ITUWidget (i.e., ITUWidget is the first member), it is safe to cast
         * or copy the memory using sizeof(ITUStopAnywhere).
         *
         * Some static analysis tools may issue a warning here, suggesting that
         * copying sizeof(ITUStopAnywhere) from a base type pointer (ITUWidget*) is
         * suspicious. However, in our design, we guarantee that 'widget' always
         * points to a valid ITUStopAnywhere instance, so this warning is considered
         * a false positive and can be ignored.
         */
        /* coverity[SIZEOF_MISMATCH] */
        (void)memcpy(newWidget, widget, sizeof(ITUStopAnywhere));
...
{
    const ITUWidget * widget = (ITUWidget *)saw;
    int               i      = 0;
    for (i = 0; i < 10; i++)
    {
    ...
    if (saw)
    {
        ...
        if (child)
        {
            ...
            if (saw->bounce == 1)
            {
                ...
                int alignavgshiftsize = 0;
                if (baseunit > 0)
                {
                    alignavgshiftsize = (widget->rect.width % baseunit) / 2;
                }
                ...
                int right_side_boundary =
                    ((widget->rect.width * (100 - STOPANYWHERE_BOUNCE_RATIO)) / 100) - alignavgshiftsize;
            ...
            else
            {
                if (targetX > 0)
                {
                    ...
                    {
                        (void)printf("[DEBUG] Func: %s [Fix X from %d to %d]\n", __func__, targetX, 0);
                    }
        ...
        if (child)
        {
            ...
            else
            {
                if (targetY > 0)
                {
                    if (STOPANYWHERE_DEBUG_FIXXY)
                    {
                        (void)printf("[DEBUG] Func: %s [Fix Y from %d to %d]\n", __func__, targetY, 0);
                    }
...
void ituStopAnywhereSetXY (ITUStopAnywhere * saw, int x, int y, int line)
{
    if (saw)
    {
        const ITUWidget * sawwidget = (ITUWidget *)saw;
        ITUWidget *       child     = StopAnywhereGetVisibleChild(saw);
        ...
        if (child)
        {
            ...
            {
                (void)printf("[Debug][StopAnywhere %s][X %d][Y %d] [%d]\n", sawwidget->name, fixX, fixY, line);
            }
        }
    }
}

bool ituStopAnywhereFindLoop (const ITCTree * tree)
{
    ITCTree * child = tree->child;

    // ITUWidget* checkwidget = (ITUWidget*)child;
    // if (checkwidget)
    //  (void)printf("widget name %s\n", checkwidget->name);
    ...
    for (; child; child = child->sibling)
    {
        ...
        if (ww)
        {
            if ((ww->type == ITU_BUTTON) || (ww->type == ITU_CHECKBOX) || (ww->type == ITU_POPUPBUTTON))
            {
...
bool ituStopAnywhereCheckDraggableChildXY (const ITCTree * tree, int x, int y)
{
    ITCTree * child = tree->child;
    ...
    for (; child; child = child->sibling)
    {
        ...
        if (ww)
        {
            ...
            if (ww->type == ITU_TRACKBAR)
            {
                ...
                // (void)printf("checking %d, %d for %s\n", xx, yy, ww->name);
...
int ituStopAnywhereAlignCal (ITUWidget * widget, ITUWidget * sawchild, bool horizontal)
{
    ...
    ITCTree *               child;
    const ITCTree *         tree       = (ITCTree *)sawchild;
    ITUWidget *             finalChild = NULL;
    const ITUStopAnywhere * saw        = (ITUStopAnywhere *)widget;
    ...
    if (tree)
    {
        for (child = tree->child; child; child = child->sibling)
        {
            ...
            finalchildCenterPos = (horizontal) ? (finalChild->rect.x + finalChild->rect.width / 2 + sawchild->rect.x)
                                               : (finalChild->rect.y + finalChild->rect.height / 2 + sawchild->rect.y);
        }
    }
    ...
    // if (saw->frame == 0)
    //  (void)printf("[Align Target Cal] [%d]\n", result);
...
void ituStopAnywhereAlignAni (ITUStopAnywhere * saw, ITUWidget * sawchild, int target, bool horizontal)
{
    if (target != 0)
    {
        ...
        if (horizontal)
        {
            stopanywhereLog("[StopAnywhere doing Align from %d to %d] [frame %d]\n", saw->childX, saw->childX + ani_pos,
                            saw->frame);
...
void ituStopAnywhereSetDrag (ITUStopAnywhere * current_drag_saw, ITUStopAnywhere * target_drag_saw, int argX, int argY)
{
    ...
    {
        const ITCTree * sawTree = (ITCTree *)target_drag_saw;
...
bool ituStopAnywhereUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if ((ev == ITU_EVENT_MOUSELONGPRESS) && (widget->flags & ITU_HAS_LONG_PRESS))
    {
        ...
        const ITCTree * pMainContainerTree = (ITCTree *)child_widget;
        ...
        if (ituWidgetIsInside(widget, x, y))
        {
            ...
            for (pTreeChild = pMainContainerTree->child; pTreeChild; pTreeChild = pTreeChild->sibling)
            {
                ...
                int         vRectPos2 =
                    (horizontal) ? (vRectPos1 + pChildWidget->rect.width) : (vRectPos1 + pChildWidget->rect.height);
                if (pChildWidget)
                {
                    stopanywhereLog("[SAW] checking item index %d - %s\n", vDebugCount, pChildWidget->name);
                    ...
                    if ((vRectPos1 <= saw->logTP) && (vRectPos2 > saw->logTP) && (saw->logDrag < 0))
                    {
                        ...
                        stopanywhereLog("[SAW] Found dragging item %s\n", pChildWidget->name); //
                    }
                    if (saw->logDrag >=
                        0) // when found dragging item, start to push it to the end of tree to display above each item
                    {
                        if (pTreeChild->sibling)
                        {
                            ITCTree *         pSwapTmp       = pTreeChild->sibling;
                            const ITUWidget * pSwapTmpWidget = (ITUWidget *)pSwapTmp;
                            // should change the vSawActiveConfigTree when perform itcTreeSwap to make sure our array
                            // data sync
                            ...
                            stopanywhereLog("[SAW] switch item %s to next %s\n", pChildWidget->name,
                                            pSwapTmpWidget->name);

                            // due to perform itcTreeSwap(pTreeChild, pTreeChild->sibling) ex: pTreeChild(2) -> 12345 ->
                            // 13245 for loop next step pTreeChild(2) = pTreeChild->sibling(4) ex: pTreeChild will be 4
    ...
    else if (ev == ITU_EVENT_LOAD)
    {
        ...
        ? ("Horizontal") : ("Vectical")));
                        ...
                        (void)printf("[%s] item (%d)(%s) at (%d, %d) [next should be at (%d, %d)]\n", widget->name, count,
        item->name, item->rect.x, item->rect.y, item->rect.x + ((horizontal) ? (item->rect.width) : (0)), item->rect.y +
        ((horizontal) ? (0) : (item->rect.height)));
                    }
                }
            }
        }*/
        ...
        if ((widget->flags & ITU_DRAGGING) && (saw->logTP <= 0))
        {
            ...
            if (STOPANYWHERE_DRAGGING_PIPE)
            {
                if (saw->logXs < saw->logYs)
                {
                    ...
                    // (void)printf("drag arr logXs[%d] to child[%d] distX[%d]\n", saw->logXs, saw->childX, distX);
            ...
            if (saw->stopAnywhereFlags & ITU_STOPANYWHERE_BOUNCING)
            {
                ...
                if (saw->bounce == 1) // Horizontal bounce
                {
                    ...
                    if (saw->frame >= saw->totalframe)
                    {
                        ...
                        stopanywhereLog("[StopAnywhere][Undragging][X to %d][Frame %d]\n", sawchild->rect.x,
                                        saw->frame);
                    ...
                    else
                    {
                        ...
                        {
                            ani_pos =
                                ((saw->childX - alignavgshiftsize) * (saw->totalframe - saw->frame) / saw->totalframe) +
                                alignavgshiftsize;
                        }
                        else if (saw->stopAnywhereFlags & ITU_STOPANYWHERE_BOUNCING_R)
                        {
                            ani_pos = saw->childX +
                                      ((widget->rect.width - alignavgshiftsize - (saw->childX + sawchild->rect.width)) *
                ...
                else if (saw->bounce == 2) // Vertical bounce
                {
                    ...
                    {
                        alignavgshiftsize = (widget->rect.height % baseunit) / 2;
                    }
                    else
                    {
                        /* do nothing */
                    ...
                    if (saw->frame >= saw->totalframe)
                    {
                        ...
                        stopanywhereLog("[StopAnywhere][Undragging][Y to %d][Frame %d]\n", sawchild->rect.y,
                                        saw->frame);
                    ...
                    else
                    {
                        ...
                        {
                            ani_pos =
                                ((saw->childY - alignavgshiftsize) * (saw->totalframe - saw->frame) / saw->totalframe) +
                                alignavgshiftsize;
                        }
                        else if (saw->stopAnywhereFlags & ITU_STOPANYWHERE_BOUNCING_B)
                        {
                            ani_pos =
                                saw->childY +
                                ((widget->rect.height - alignavgshiftsize - (saw->childY + sawchild->rect.height)) *
                                 saw->frame / saw->totalframe);
                        }
                        else
                        {
        ...
        if ((widget->flags & ITU_DRAGGING) || (saw->stopAnywhereFlags & ITU_STOPANYWHERE_BOUNCING))
        {
            ...
            if ((timetick - saw->clock) <= STOPANYWHERE_SLIDE_CHECK_TIMING)
            {
                ...
                if (lastDist > STOPANYWHERE_SLIDE_REQUIRE_DIST)
                {
                    ...
                    (void)printf("Sliding factorX %d, factorY %d\n", saw->factorX, saw->factorY);
                }
                else
                {
                    (void)printf("[1800][lastDist, lastDistX, lastDistY] [%d, %d, %d]\n", lastDist, lastDistX, lastDistY);
                }
            }
            else
            {
                (void)printf("[%d][Clock][Tick, saw_last_clock][%d, %d]\n", STOPANYWHERE_SLIDE_CHECK_TIMING, (int)timetick,
                       (int)saw->clock);
            }
    ...
    else if (ev == ITU_EVENT_MOUSEMOVE)
    {
        if ((widget->flags & ITU_DRAGGING) && (saw->logTP >= 0))
        {
            ...
            const ITCTree * pMainContainerTree   = (ITCTree *)pMainContainerWidget;
            ITCTree *       pTreeChild           = NULL;
            // touch position should be compared with the position of the main container
            ...
            if (pItemCurr)
            {
                ...
                for (pTreeChild = pMainContainerTree->child; pTreeChild; pTreeChild = pTreeChild->sibling)
                {
                    ...
                    int         vRectPos1     = (horizontal) ? (pChildWidget->rect.x) : (pChildWidget->rect.y);
                    int         vRectPos2     = (horizontal) ? (vRectPos1 + pChildWidget->rect.width / 2)
                                                             : (vRectPos1 + pChildWidget->rect.height / 2);
                    // boundary of the pItemCurr
                    int         vRectPosCurr1 = (horizontal) ? (pItemCurr->rect.x) : (pItemCurr->rect.y);
                    int         vRectPosCurr2 = (horizontal) ? (vRectPosCurr1 + pItemCurr->rect.width)
                                                             : (vRectPosCurr1 + pItemCurr->rect.height);

                    if ((!pTreeChild) || (pChildWidget == pItemCurr))
                    {
                        continue;
                    }
        ...
        else if (widget->flags & ITU_DRAGGING)
        {
            ...
            if (((!horizontal) && (X_dir)) || ((horizontal) && (Y_dir)))
            {
                const ITCTree * tree   = (ITCTree *)widget;
                ITCTree *       parent = tree->parent;
                while (parent)
                {
                    ITUWidget * pWidget = (ITUWidget *)parent;
                    if (pWidget->type == ITU_COVERFLOW)
                    {
                        ituWidgetUpdate(widget, ITU_EVENT_MOUSEUP, 0, 0, 0);
                        ituWidgetUpdate(pWidget, ITU_EVENT_MOUSEDOWN, -1, arg2, arg3);
                        return true;
                    }
                    parent = parent->parent;
                }
            ...
            if (saw->bounce == 0) //
            {
                if (((sawchild->rect.width == widget->rect.width) && X_dir &&
                     (abs(Xtd) > STOPANYWHERE_MIN_OL_DRAGACT_DIST)) ||
                    ((sawchild->rect.height == widget->rect.height) && Y_dir &&
                     (abs(Ytd) > STOPANYWHERE_MIN_OL_DRAGACT_DIST)))
                {
                    ITUWidget * child_widget  = StopAnywhereGetVisibleChild(saw);
                    ...
                    while (child_widget)
                    {
                        if (child_widget->type == ITU_STOPANYWHERE)
                        {
                            const ITCTree * point  = (ITCTree *)child_widget;
                        ...
                        {
                            child_widget = StopAnywhereGetVisibleChild((ITUStopAnywhere *)child_widget);
                        }
                    }

                    if (finding_check)
                    {
                        const ITCTree * sawTree = (ITCTree *)saw;
                        ITCTree *       parent  = sawTree->parent;
            ...
            if ((abs(Xtd) >= ITU_DRAG_DISTANCE) || (abs(Ytd) >= ITU_DRAG_DISTANCE))
            {
                ...
                ituUnPressWidget(sawchild);
                // (void)printf("[Unpress] x,y[%d,%d] init[%d,%d]\n", x, y, saw->initX, saw->initY);
            }
            ...
            if (ituWidgetIsInside(widget, x, y))
            {
                ...
                if (STOPANYWHERE_DRAGGING_PIPE)
                {
                    ...
                    saw->logDrag = target;
                    // (void)printf("drag arr item %d arranged.\n", count);
                }
                else
                {
                    ituStopAnywhereSetXY(saw, (saw->childX + distX), (saw->childY + distY), __LINE__);
    ...
    if (result)
    {
        (void)printf("[StopAnywhere][%s][Update True].\n", widget->name);
    }
    #endif
...
void ituStopAnywhereDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
            ...
            if (child->visible && ituWidgetIsOverlapClipping(child, dest, destx, desty))
            {
                ...
                if ((widget->flags & ITU_DRAGGING) && (STOPANYWHERE_DRAGGING_EYEMOTION) && movecheck)
                {
                    ...
                    if (STOPANYWHERE_DRAGGING_EYEMOTION == 2)
                    {
                        ...
                        if (motiondebug)
                        {
                            (void)printf("[StopAnywhere]eye_motion[1][%d,%d]\n\n", destx + (x_step / 2),
                                   desty + (y_step / 2));
                        }
                        ...
                        if (motiondebug)
                        {
                            (void)printf("[StopAnywhere]eye_motion[2][%d,%d]\n", destx + x_step, desty + y_step);
                        }
                    }
    ...
    else
    {
        ...
        if (surf)
        {
            ...
            ituRotate(dest, destx + rect->width / 2, desty + rect->height / 2, surf, surf->width / 2, surf->height / 2,
...
void ituStopAnywhereInit (ITUStopAnywhere * saw)
{
    ...
    (void)memset(saw, 0, sizeof(ITUStopAnywhere));
...
void ituStopAnywhereLoad (ITUStopAnywhere * saw, uint32_t base)
{
    ...
    if (STOPANYWHERE_DRAGGING_PIPE)
    {
        ...
        if (((sawchild->rect.width - ituWidgetGetWidth(saw)) > 100) &&
            (sawchild->rect.height <= ituWidgetGetHeight(saw)))
        {
            saw->hv_mode = 1; // Horizontal
        }
        else if (((sawchild->rect.height - ituWidgetGetHeight(saw)) > 100) &&
                 (sawchild->rect.width <= ituWidgetGetWidth(saw)))
        {
...
void ituStopAnywhereSyncTreeData (ITUStopAnywhere * saw, char * pConfigWord)
{
    ...
    if (pMainContainerTree == NULL)
    {
        return;
    }

    vTreeCount = itcTreeGetChildCount(pMainContainerTree);
    if (vTreeCount > STOPANYWHERE_MAX_TREE_ITEM)
    {
        return;
    }

    pChar = (char *)malloc(sizeof(char) * 256);
    if (NULL == pChar)
    {
        return;
    }
    (void)memset(pChar, '\0', sizeof(char) * 256);

    vSawActiveId = ituStopAnywhereFindActiveArrayIdx(saw);
    ...
    for (i = 0; i < vTreeCount; i++)
    {
        ...
        {
            char vTmp[10] = {'\0'};
            snprintf(vTmp, sizeof(vTmp), "%d%s", vSawActiveConfigTree[vSawActiveId][i],
                     ((i + 1) < vTreeCount) ? (",") : (""));
            strlcat(pChar, vTmp, sizeof(char) * 256);
        }
    }
    ...
    if (len > 0)
    {
        (void)memset(pConfigWord, '\0', sizeof(char) * 256);
        (void)memcpy(pConfigWord, pChar, sizeof(char) * len);
    }
...
bool ituStopAnywhereLoadTreeSeq (ITUStopAnywhere * saw, char * pConfigWord)
{
    ...
    (void)memset(vConfigArr, -1, sizeof(int) * STOPANYWHERE_MAX_TREE_ITEM);
    ...
    if (vPrintDebug)
    {
        i = 0;
        (void)printf("=====================================================\n");
        (void)printf("[StopAnywhere] Loaded Tree Seq for %s\n", saw->widget.name);
        while (vConfigArr[i] >= 0)
        {
            (void)printf("%d", vConfigArr[i]);
            if ((i + 1) < STOPANYWHERE_MAX_TREE_ITEM)
            {
                if (vConfigArr[i + 1] >= 0)
                {
                    (void)printf(", ");
            ...
            i++;
            if (i >= STOPANYWHERE_MAX_TREE_ITEM)
            {
                break;
            }
        }
    ...
    for (pTreeChild = pMainContainerTree->child; pTreeChild; pTreeChild = pTreeChild->sibling)
    {
        ...
        if (vPrintDebug)
        {
            const ITUWidget * pTmp = (ITUWidget *)pArrayOrg[i];
            (void)printf("[StopAnywhere] pArrayOrg[%d] is %s\n", i, pTmp->name);
        }
    ...
    for (i = 0; i < (vTreeCount - 1); i++)
    {
        ...
        else
        {
            if (vConfigArr[i + 1] >= 0)
            {
                ...
                if (vPrintDebug)
                {
                    const ITUWidget * pTmp1 = (ITUWidget *)pArrayOrg[vConfigArr[i]];
                    const ITUWidget * pTmp2 = (ITUWidget *)pArrayOrg[vConfigArr[i + 1]];
                    (void)printf("[StopAnywhere] set %s->sibling to %s\n", pTmp1->name, pTmp2->name);
                }
            }
    ...
    for (pTreeChild = pMainContainerTree->child; pTreeChild; pTreeChild = pTreeChild->sibling)
    {
        ...
        if (vPrintDebug)
        {
            (void)printf("[StopAnywhere] set %s %s to %d\n", pChild->name, (vHorizontal) ? ("x") : ("y"), vPos);
        }
