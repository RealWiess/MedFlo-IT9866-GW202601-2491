        ...
        if (imagecoverflow->itemCount > 0)
        {
            ...
            while ((pent = readdir(pdir)) != NULL)
            {
                ...
                if (ptr)
                {
                    ...
                    strlcpy(buf, IMAGE_EXTENSIONS, sizeof(buf));
                ...
                if (found)
                {
                    ...
                    strlcpy(imagecoverflow->filePath, imagecoverflow->path, sizeof(imagecoverflow->filePath));
                    strlcat(imagecoverflow->filePath, pent->d_name, sizeof(imagecoverflow->filePath));
                    ...
                    if (f)
                    {
                        ...
                        if (fstat(fileno(f), &sb) != -1)
                        {
                            ...
                            {
                                imagecoverflow->imageSize =
                                    fread(imagecoverflow->imageData, 1, imagecoverflow->imageSize, f);
    ...
    if ((coverflow->coverFlowFlags & ITU_IMAGECOVERFLOW_DESTROYING) == 0)
    {
        if (ev == ITU_EVENT_TIMER)
        {
            if (coverflow->coverFlowFlags & ITU_IMAGECOVERFLOW_CREATED)
            {
                ...
                icon->widget.flags &= ~ITU_FIT_TO_RECT; // 9830 no support
    #endif
#endif
                if (ext)
                {
...
void ituImageCoverFlowInit (ITUImageCoverFlow * imageCoverFlow, ITULayout layout, char * path)
{
    ...
    (void)memset(imageCoverFlow, 0, sizeof(ITUImageCoverFlow));
...
void ituImageCoverFlowReload (ITUImageCoverFlow * imagecoverflow)
{
    ...
    const ITUCoverFlow * coverflow       = (ITUCoverFlow *)imagecoverflow;
    assert(imagecoverflow);
    ITU_ASSERT_THREAD();
    ...
    if (strlen(imagecoverflow->path) > 0)
    {
        ITUCoverFlow * coverflowl = (ITUCoverFlow *)imagecoverflow;
        pthread_t      task;
        pthread_attr_t attr;

        if (coverflowl->coverFlowFlags & ITU_IMAGECOVERFLOW_CREATED)
        {
            free(imagecoverflow->imageData);
            imagecoverflow->imageData = NULL;
            imagecoverflow->imageSize = 0;
            coverflowl->coverFlowFlags &= ~ITU_IMAGECOVERFLOW_CREATED;
        }
        ...
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
        pthread_create(&task, &attr, ImageCoverFlowCreateTask, imagecoverflow);
        coverflowl->coverFlowFlags |= ITU_IMAGECOVERFLOW_BUSYING;
        coverflowl->coverFlowFlags &= ~ITU_IMAGECOVERFLOW_RELEASING;
    }
}
