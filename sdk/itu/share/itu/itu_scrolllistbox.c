...
bool              ituScrollListBoxUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        {
            char buf[8]; // Bless
            (void)printf("[pageIndex is %d, pageCount is %d]\n", listbox->pageIndex, listbox->pageCount);
    ...
    else if (ev == ITU_EVENT_MOUSEMOVE)
    {
        if (ituWidgetIsEnabled(widget) && (widget->flags & ITU_DRAGGING))
        {
            ...
            // (void)printf("0: offset=%d y=%d\n", offset, y);

            if ((absoffset <= widget->rect.height / 2) || (listbox->pageIndex > 1 && offset > 0) ||
            ...
            if ((widget->flags & ITU_DRAGGABLE) && (widget->flags & ITU_DRAGGING))
            {
                    ...
                    ((listbox->pageIndex > 1 && offset > 0) || (listbox->pageIndex < listbox->pageCount && offset < 0)))
                {
                    ...
                    if (offset > 0)
                    {
                        ...
                        // (void)printf("1: frame=%d offset=%d inc=%d pageIndex=%d\n", scrolllistbox->frame, offset,
                        // scrolllistbox->inc, listbox->pageIndex);
                    }
                    else
                    {
                        scrolllistbox->inc = -widget->rect.height / scrolllistbox->totalframe;
                ...
                {
                    scrolllistbox->frame =
                        scrolllistbox->totalframe - absoffset / (widget->rect.height / scrolllistbox->totalframe);
...
{
    const ITUListBox * listbox = (ITUListBox *)scrolllistbox;
...
void ituScrollListBoxInit (ITUScrollListBox * scrolllistbox, int width)
{
    ...
    (void)memset(scrolllistbox, 0, sizeof(ITUScrollListBox));
