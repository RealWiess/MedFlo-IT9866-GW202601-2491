...
void              ituMeterExit (ITUWidget * widget)
{
    ...
    for (; i < 3; i++)
    {
        if (meter->acw_surf[i])
        {
            ituSurfaceRelease(meter->acw_surf[i]);
            meter->acw_surf[i] = NULL;
        }
        if (meter->acw_shadow[i])
        {
            ituSurfaceRelease(meter->acw_shadow[i]);
            meter->acw_shadow[i] = NULL;
        }
        if (meter->cw_surf[i])
        {
            ituSurfaceRelease(meter->cw_surf[i]);
            meter->cw_surf[i] = NULL;
        }
        if (meter->cw_shadow[i])
        {
            ituSurfaceRelease(meter->cw_shadow[i]);
            meter->cw_shadow[i] = NULL;
        }
    }
...
bool ituMeterClone (ITUWidget * widget, ITUWidget ** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ITUWidget * newWidget = malloc(sizeof(ITUMeter));
        if (newWidget == NULL)
        {
            return false;
        }
         ...
         * ITUMeter object. Since ITUMeter is a struct that extends
         * ITUWidget (i.e., ITUWidget is the first member), it is safe to cast
         * or copy the memory using sizeof(ITUMeter).
         *
         * Some static analysis tools may issue a warning here, suggesting that
         * copying sizeof(ITUMeter) from a base type pointer (ITUWidget*) is
         * suspicious. However, in our design, we guarantee that 'widget' always
         * points to a valid ITUMeter instance, so this warning is considered
         * a false positive and can be ignored.
         */
        /* coverity[SIZEOF_MISMATCH] */
        (void)memcpy(newWidget, widget, sizeof(ITUMeter));
            ...
            if (ituWidgetIsEnabled(widget))
            {
                ...
                if (ituWidgetIsInside(widget, x, y))
                {
                    ...
                    if (vx2 * vx2 + vy2 * vy2 >= meter->minRadius * meter->minRadius)
                    {
                        ...
                        if ((angle < meter->startAngle) && ((angle + 360.0f) >= meter->startAngle) &&
                            ((angle + 360.0f) <= meter->endAngle))
                        {
                            angle += 360.0f;
                        }
                        else if ((angle > meter->endAngle) && ((angle - 360.0f) <= meter->endAngle) &&
                                 ((angle - 360.0f) >= meter->startAngle))
                        {
                            angle -= 360.0f;
                        }
                        ...
                        if (meter->startAngle < meter->endAngle)
                        {
                            if (meter->startAngle <= angle && angle <= meter->endAngle)
                            {
                                ...
                                int value = (int)roundf((angle - meter->startAngle) * meter->maxValue / range);
                        ...
                        else if (meter->startAngle > meter->endAngle)
                        {
                            ...
                            if (meter->startAngle >= angle && angle >= meter->endAngle)
                            {
                                ...
                                int value = (int)roundf((meter->startAngle - angle) * meter->maxValue / range);
...
void ituMeterDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    {
        (void)printf("[SurfacePool] MeterDraw but SurfacePool not ready yet!\n");
        return;
    }
    ...
    if (widget->visible && (widget->alpha > 0) && (alpha > 0))
    {
        if (meter->pointerIcon)
        {
            ...
            {
                ituIconLoadExternalDataAPI(meter->pointerIcon);
            }
            else
            {
                ituIconLoadStaticData(meter->pointerIcon);
            }
        }
    }
#endif
    ...
    if (meter->pointerIcon && meter->pointerIcon->surf)
    {
        ...
        if (meter->endAngle > meter->startAngle)
        {
            ...
            last_angle =
                meter->startAngle + (meter->endAngle - meter->startAngle) * meter->lastvalue / (float)meter->maxValue;
        }
        else if (meter->endAngle < meter->startAngle)
        {
            ...
            last_angle =
                meter->startAngle - (meter->startAngle - meter->endAngle) * meter->lastvalue / (float)meter->maxValue;
        }
        ...
        if (meter->shadowmode)
        {
            ...
            if (meter->diffvalue != 0)
            {
                ...
                {
                    local_clock_wise =
                        (meter->endAngle - meter->startAngle) * meter->diffvalue / (float)meter->maxValue;
                }
                else if (meter->endAngle < meter->startAngle)
                {
                    local_clock_wise =
                        (meter->startAngle - meter->endAngle) * meter->diffvalue / (float)meter->maxValue;
                }
            }
            ...
            if (meter->lastvalue != meter->value) // do not fit when finger moving
            {
                ...
                // target pointer
                float        target = last_angle + (step / meter->shadow_loop_max) * (meter->shadow_loop_count + 1);

                if (angle_shift_level >= ITU_METER_BLUR_LEVEL_FAST) // big
                {
                    int i = 2;
                    for (; i >= 0; i--) // check valid surf [2]->[1]->[0]
                    {
                ...
                {
                    int i = 1;
                    for (; i >= 0; i--) // check valid surf [1]->[0]
                    {
                        if (local_clock_wise > 0.0)
                        {
                            surfLocal = meter->cw_surf[i];
                ...
                if (surfLocal)
                {
                    ...
                    ituRotate(dest, x + meter->pointerIcon->widget.rect.x + meter->pointerX,
                              y + meter->pointerIcon->widget.rect.y + meter->pointerY, surfLocal, meter->pointerX,
                              meter->pointerY, target, 1.0f, 1.0f);
#else
                    ituRotate(dest, x + meter->pointerIcon->widget.rect.x, y + meter->pointerIcon->widget.rect.y,
                              surfLocal, meter->pointerX, meter->pointerY, target, 1.0f, 1.0f);
#endif
                }
            }
            else
        ...
        if ((meter->shadowmode == 0) || first_load_draw)
        {
            ...
            else if (meter->pointerIcon->widget.alpha == 255)
            {
                ...
                ituRotate(dest, x + meter->pointerIcon->widget.rect.x, y + meter->pointerIcon->widget.rect.y,
                          meter->pointerIcon->surf, meter->pointerX, meter->pointerY, angle, 1.0f, 1.0f);
#else
                ituRotate(dest, x + meter->pointerIcon->widget.rect.x + meter->pointerX,
                          y + meter->pointerIcon->widget.rect.y + meter->pointerY, meter->pointerIcon->surf,
                          meter->pointerX, meter->pointerY, angle, 1.0f, 1.0f);
#endif
            }
            else
            {
                ...
                if (surf != NULL)
                {
                    ITUIcon* icon = &meter->bg.icon;
                    if (icon != NULL)
                    {
                        if ((widget->color.alpha == 0) || (icon->surf != NULL))
                        {
                            ITUColor ec = { 0,0,0,0 };
...
#ifndef CFG_M2D_ENABLE
                    ituRotate(surf, meter->pointerIcon->widget.rect.x + meter->pointerX,
                              meter->pointerIcon->widget.rect.y + meter->pointerY, meter->pointerIcon->surf,
                              meter->pointerX, meter->pointerY, angle, 1.0f, 1.0f);
#else
                    ituRotate(surf, meter->pointerIcon->widget.rect.x, meter->pointerIcon->widget.rect.y,
                              meter->pointerIcon->surf, meter->pointerX, meter->pointerY, angle, 1.0f, 1.0f);
#endif
                    ituAlphaBlend(dest, x, y, widget->rect.width, widget->rect.height, surf, 0, 0,
                                  meter->pointerIcon->widget.alpha);
...
void ituMeterInit (ITUMeter * meter)
{
    ...
    (void)memset(meter, 0, sizeof(ITUMeter));
    ...
    ituWidgetSetClone(meter, ituMeterClone);
...
{
    const ITUWidget * widget = (ITUWidget *)meter;
    assert(meter);
    ...
    if (!(widget->flags & ITU_EXTERNAL))
    {
        ...
        for (i = 0; i < 3; i++)
        {
            if (meter->cw_shadow[i])
            {
                ...
                {
                    meter->cw_surf[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
            ...
            if (meter->acw_shadow[i])
            {
                ...
                {
                    meter->acw_surf[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
                                                          (const uint8_t *)surf->addr, surf->flags);
                }
