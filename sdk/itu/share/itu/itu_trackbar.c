...
bool ituTrackBarClone (ITUWidget * widget, ITUWidget ** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ITUWidget * newWidget = malloc(sizeof(ITUTrackBar));
        if (newWidget == NULL)
        {
            return false;
        }
         ...
         * ITUTrackBar object. Since ITUTrackBar is a struct that extends
         * ITUWidget (i.e., ITUWidget is the first member), it is safe to cast
         * or copy the memory using sizeof(ITUTrackBar).
         *
         * Some static analysis tools may issue a warning here, suggesting that
         * copying sizeof(ITUTrackBar) from a base type pointer (ITUWidget*) is
         * suspicious. However, in our design, we guarantee that 'widget' always
         * points to a valid ITUTrackBar instance, so this warning is considered
         * a false positive and can be ignored.
         */
        /* coverity[SIZEOF_MISMATCH] */
        (void)memcpy(newWidget, widget, sizeof(ITUTrackBar));
...
bool ituTrackBarUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_MOUSEMOVE)
    {
        if (--tbar->delayCount <= 0)
        {
            if (ituWidgetIsEnabled(widget) && tbar->tracker && ituButtonIsPressed(tbar->tracker))
            {
                if (arg1 < 0)
                {
                    tbar->tracker->bg.icon.widget.alpha = abs(arg1);
                }
                        ...
                        else
                        {
                            ...
                            for (i = tbar->min; i <= tbar->max; i += tbar->step)
                            {
                                ...
                                if (v0 <= x && x <= v1)
                                {
                                    ...
                                    // (void)printf("x=%d v0=%d v1=%d l=%d r=%d val=%d\n", x, v0, v1, diffLeft, diffRight,
                                    // value);
                                    break;
                                }
                            }
                ...
                else // if (tbar->layout == ITU_LAYOUT_VERTICAL)
                {
                    ...
                    if (tbar->max > tbar->min && tbar->step > 0)
                    {
                        ...
                        if (y > (int)(sh * (tbar->max - tbar->min)) + tbar->gap)
                        {
                            value = tbar->min;
                        }
                        else if (y < tbar->gap)
                        ...
                        else
                        {
                            ...
                            for (i = tbar->min; i <= tbar->max; i += tbar->step)
                            {
                                ...
                                int v1 =
                                    ituWidgetGetHeight(tbar) - (int)(sh * (i + tbar->step - tbar->min)) - tbar->gap;
                                ...
                                if (y <= v0 && y >= v1)
                                {
                                    ...
                                    // (void)printf("y=%d v0=%d v1=%d b=%d t=%d val=%d\n", y, v0, v1, diffBottom, diffTop,
                                    // value);
                                    break;
                                }
                            }
    ...
    else if (ev == ITU_EVENT_MOUSEUP)
    {
        if (ituWidgetIsEnabled(widget))
        {
            ...
            if (tbar->tracker && ituScene->dragged == (ITUWidget *)tbar->tracker)
            {
                ...
                tbar->tracker->bg.icon.widget.alpha = 255;
    ...
    if (arg1 < 0)
    {
        result |= ituIconUpdate(widget, ev, 1, arg2, arg3);
    }
    else
    {
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        if (tbar->tracker)
        {
            ...
            else // if (tbar->layout == ITU_LAYOUT_VERTICAL)
            {
                ...
                ituWidgetSetY(tbar->tracker,
                              ituWidgetGetHeight(tbar) - (int)(sh * (tbar->value - tbar->min)) - h - tbar->gap);
            }
            ...
            {
                char buf[8];
                snprintf(buf, sizeof(buf), "%i", tbar->value);
        ...
        switch (ev)
        {
            case ITU_EVENT_KEYDOWN:
                if (tbar->layout == ITU_LAYOUT_HORIZONTAL)
                {
                    ...
                    if (tbar->max > tbar->min && tbar->step > 0)
                    {
                        sw = (ituWidgetGetWidth(tbar) - tbar->gap * 2 - ituWidgetGetWidth(tbar->tracker)) /
                             (tbar->max - tbar->min);
                    }
                    else
                    {
                ...
                else // if (tbar->layout == ITU_LAYOUT_VERTICAL)
                {
                    ...
                    if (tbar->max > tbar->min && tbar->step > 0)
                    {
                        sh = (ituWidgetGetHeight(tbar) - tbar->gap * 2 - ituWidgetGetHeight(tbar->tracker)) /
                             (tbar->max - tbar->min);
                    }
                    else
                    {
...
void ituTrackBarDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    {
        (void)printf("[SurfacePool] TrackBarDraw but SurfacePool not ready yet!\n");
        return;
    }
#endif
    ...
    if (!bg->icon.surf || (bg->icon.surf->width < widget->rect.width || bg->icon.surf->height < widget->rect.height) ||
        (bg->icon.surf->format == ITU_ARGB1555 || bg->icon.surf->format == ITU_ARGB4444 ||
         bg->icon.surf->format == ITU_ARGB8888))
    {
        if (desta == 255)
        {
        ...
        else if (desta > 0)
        {
            ...
            if (surf)
            {
                ...
                {
                    ituGradientFill(surf, 0, 0, rect->width, rect->height, &widget->color, &bg->graidentColor,
                                    bg->graidentMode);
                }
        ...
        if (desta > 0)
        {
            if (widget->flags & ITU_STRETCH)
            {
                ...
                if (desta == 255)
                {
                    if (widget->angle == 0)
                    {
                        ituStretchBlt(dest, destx, desty, rect->width, rect->height, bg->icon.surf, 0, 0,
                                      bg->icon.surf->width, bg->icon.surf->height);
                    }
                    else
                    {
                        ...
                        ituRotate(dest, destx + rect->width / 2, desty + rect->height / 2, bg->icon.surf,
                                  bg->icon.surf->width / 2, bg->icon.surf->height / 2, (float)widget->angle, scaleX,
                                  scaleY);
                    }
                }
                else
                {
                    ...
                    if (surf)
                    {
                        ...
                        if (widget->angle == 0)
                        {
                            ituStretchBlt(surf, 0, 0, rect->width, rect->height, bg->icon.surf, 0, 0,
                                          bg->icon.surf->width, bg->icon.surf->height);
                        }
                        else
                        {
                            ...
                            ituRotate(surf, rect->width / 2, rect->height / 2, bg->icon.surf, bg->icon.surf->width / 2,
                                      bg->icon.surf->height / 2, (float)widget->angle, scaleX, scaleY);
                        }
                        ituAlphaBlend(dest, destx, desty, rect->width, rect->height, surf, 0, 0, desta);
                        ituDestroySurface(surf);
                    }
                ...
                ituTransform(dest, destx, desty, rect->width, rect->height, bg->icon.surf, 0, 0, bg->icon.surf->width,
                             bg->icon.surf->height, bg->icon.surf->width / 2, bg->icon.surf->height / 2, scaleX, scaleY,
            ...
            else
            {
                ...
                {
                    ituAlphaBlend(dest, destx, desty, bg->icon.surf->width, bg->icon.surf->height, bg->icon.surf, 0, 0,
                                  desta);
                }
            }
        }
...
void ituTrackBarInit (ITUTrackBar * tbar, ITULayout layout)
{
    ...
    (void)memset(tbar, 0, sizeof(ITUTrackBar));

    ituBackgroundInit(&tbar->bg);

    ituWidgetSetType(tbar, ITU_TRACKBAR);
    ituWidgetSetName(tbar, tbarName);
    ituWidgetSetExit(tbar, ituTrackBarExit);
    ituWidgetSetClone(tbar, ituTrackBarClone);
