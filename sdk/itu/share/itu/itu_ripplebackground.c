...
bool ituRippleBackgroundUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    {
        if (ituWidgetIsEnabled(widget) && (rb->rippleBackgroundFlags & ITU_RIPPLEBACKGROUND_DRAGGABLE) &&
            (rb->rippleBackgroundFlags & ITU_RIPPLEBACKGROUND_DRAGGING))
        {
            int dx = arg2 - widget->rect.x;
            int dy = arg3 - widget->rect.y;
...
void ituRippleBackgroundDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    if (rb->rippleBackgroundFlags & ITU_RIPPLEBACKGROUND_DROPPING)
    {
        ...
        {
            const uint16_t * srcPtr = (uint16_t *)ituLockSurface(icon->surf, 0, 0, width, height);
            if (srcPtr)
            {
                ...
                if (surf)
                {
                    ...
                    if (destPtr)
                    {
                        int ly, lx, a, b, i, mapIndex, newIndex, range;
                        ...
                        for (ly = 0; ly < height; ly++)
                        {
                            ...
                            {
                                ripple_t data = (ripple_t)((rippleMap[mapIndex - width] + rippleMap[mapIndex + width] +
                                                            rippleMap[mapIndex - 1] + rippleMap[mapIndex + 1]) >>
                                                           1);
        ...
        {
            const uint32_t * srcPtr = (uint32_t *)ituLockSurface(icon->surf, 0, 0, width, height);
            if (srcPtr)
            {
                surf = ituCreateSurface(width, height, 0, icon->surf->format, NULL, 0);
                if (surf)
                {
                    ...
                    if (destPtr)
                    {
                        int lx, ly, a, b, i, mapIndex, newIndex;
        ...
        {
            ITU_LOG_WARN("Unsupport pixel format: %d\n", dest->format);
        }
...
void ituRippleBackgroundInit (ITURippleBackground * rb)
{
    ...
    (void)memset(rb, 0, sizeof(ITURippleBackground));
