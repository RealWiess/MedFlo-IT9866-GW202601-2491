...
bool ituFlowWindowUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (((widget->type == ITU_WHEEL) || (widget->type == ITU_STEPWHEEL)) && (ev == ITU_EVENT_LAYOUT))
    {
        ...
        {
            const ITUWheel* wheel = (ITUWheel*)widget;
            ...
            if (cycle_tor > 0)
            {
                const ITUWidget* fc = (ITUWidget*)itcTreeGetChildAt(widget, focus_c);
        ...
        else //for ITU_STEPWHEEL
        {
            const ITUStepWheel* spw = (ITUStepWheel*)widget;
            size = fwin->borderSize;
            for (node = widget->tree.child; node; node = node->sibling)
            {
                child = (ITUWidget*)node;
...
void ituFlowWindowInit(ITUFlowWindow* fwin, ITULayout layout)
{
    ...
    (void)memset(fwin, 0, sizeof (ITUFlowWindow));
