...
{
    const ITUWipeEffect * we = (ITUWipeEffect *)effect;
    int                   value;
    assert(effect);
    assert(widget);
    ...
    switch (we->type)
    {
        case ITU_WIPE_IN_LEFT:
            ...
            ituWidgetSetBound(widget, widget->rect.x + value, widget->rect.y, widget->rect.width - value,
                              widget->rect.height);
            break;

        case ITU_WIPE_IN_UP:
            value = widget->rect.height - widget->rect.height * effect->currStep / effect->totalStep;
            ituWidgetSetBound(widget, widget->rect.x, widget->rect.y + value, widget->rect.width,
                              widget->rect.height - value);
            break;
...
void ituWipeEffectInit (ITUWipeEffect * we, ITUWipeType type)
{
    ...
    (void)memset(we, 0, sizeof(ITUWipeEffect));
