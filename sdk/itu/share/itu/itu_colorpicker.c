...
void ituColorPickerDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    if (cp->colorPickerFlags & ITU_COLORPICKER_PICKING)
    {
        ...
        {
            const uint16_t* ptr = (uint16_t*)ituLockSurface(dest, destx + cp->touchX, desty + cp->touchY, 1, 1);
            if (ptr)
            {
                //(void)printf("*ptr=0x%04X\n", *ptr);
        ...
        {
            const uint32_t* ptr = (uint32_t*)ituLockSurface(dest, destx + cp->touchX, desty + cp->touchY, 1, 1);
            if (ptr)
            {
                cp->pickedColor.alpha = (*ptr & 0xFF000000U) >> 24U;
                cp->pickedColor.red   = (*ptr & 0x00FF0000U) >> 16U;
                cp->pickedColor.green = (*ptr & 0x0000FF00U) >> 8U;
                cp->pickedColor.blue  = *ptr & 0x000000FFU;
                ituUnlockSurface(dest);
            }
        }
        else
...
void ituColorPickerInit(ITUColorPicker* cp)
{
    ...
    (void)memset(cp, 0, sizeof (ITUColorPicker));
