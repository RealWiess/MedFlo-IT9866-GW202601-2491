...
bool              ituScrollIconListBoxUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        {
            char buf[8];
            snprintf(buf, sizeof(buf), "%i", listbox->pageIndex);
        ...
        {
            char buf[8];
            snprintf(buf, sizeof(buf), "%i", listbox->pageCount);
    ...
    else if (ev == ITU_EVENT_MOUSEMOVE)
    {
        if (ituWidgetIsEnabled(widget) && (widget->flags & ITU_DRAGGING))
        {
            ...
            if ((absoffset <= widget->rect.height / 2) ||
                ((listbox->pageIndex > 1 && offset > 0) || (listbox->pageIndex < listbox->pageCount && offset < 0)))
            {
            ...
            if ((widget->flags & ITU_DRAGGABLE) && (widget->flags & ITU_DRAGGING))
            {
                    ...
                    ((listbox->pageIndex > 1 && offset > 0) || (listbox->pageIndex < listbox->pageCount && offset < 0)))
                {
                    ...
                    if (offset > 0)
                    {
                        ...
                        // (void)printf("1: frame=%d offset=%d inc=%d pageIndex=%d\n", scrolliconlistbox->frame, offset,
                        // scrolliconlistbox->inc, listbox->pageIndex);
                    }
                    else
                    {
                        scrolliconlistbox->inc = -widget->rect.height / scrolliconlistbox->totalframe;
                        // (void)printf("2: frame=%d offset=%d inc=%d pageIndex=%d\n", scrolliconlistbox->frame, offset,
                        // scrolliconlistbox->inc, listbox->pageIndex);
                    }
                ...
                {
                    scrolliconlistbox->frame = scrolliconlistbox->totalframe -
                                               absoffset / (widget->rect.height / scrolliconlistbox->totalframe);
                    ...
                    {
                        scrolliconlistbox->inc = -widget->rect.height / scrolliconlistbox->totalframe;
                        // (void)printf("3: frame=%d offset=%d inc=%d pageIndex=%d\n", scrolliconlistbox->frame, offset,
                        // scrolliconlistbox->inc, listbox->pageIndex);
                    }
                    else
                    {
                        scrolliconlistbox->inc = widget->rect.height / scrolliconlistbox->totalframe;
                        // (void)printf("4: frame=%d offset=%d inc=%d pageIndex=%d\n", scrolliconlistbox->frame, offset,
                        // scrolliconlistbox->inc, listbox->pageIndex);
                    }
                }
...
{
    const ITUListBox * listbox = (ITUListBox *)scrolliconlistbox;
...
static void ScrollIconListBoxBind (ITUScrollIconListBox * scrolliconlistbox, char * param)
{
    ...
    if (target)
    {
        ...
        {
            const ITUScrollListBox * targetslistbox = (ITUScrollListBox *)target;
        ...
        {
            const ITUScrollMediaFileListBox * targetsmflistbox = (ITUScrollMediaFileListBox *)target;
        ...
        {
            const ITUScrollIconListBox * targetsilistbox = (ITUScrollIconListBox *)target;
...
void ituScrollIconListBoxInit (ITUScrollIconListBox * scrolliconlistbox, int width)
{
    ...
    (void)memset(scrolliconlistbox, 0, sizeof(ITUScrollIconListBox));
...
void ituScrollIconListBoxSetSelectable (ITUScrollIconListBox * scrolliconlistbox, bool selectable)
{
    ...
    if (selectable)
    {
        ...
        if ((count / 3) > 0)
        {
            (void)printf("[ScrollIconListBox]Set selectable successfaul! (%d)\n", (count / 3));
        }
        else
        {
            (void)printf("[ScrollIconListBox]Set selectable fail!\n");
        }
    }
    else
    {
        scrolliconlistbox->listbox.listbox.itemCount = 0;
        (void)printf("[ScrollIconListBox]Set non-selectable!\n");
    }
}
