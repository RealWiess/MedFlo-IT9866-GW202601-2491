    ...
    if (ev == ITU_EVENT_MOUSEDOWN)
    {
        if (ituWidgetIsEnabled(widget))
        {
            ...
            if (ituWidgetIsInside(widget, x, y))
            {
                ...
                {
                    int xx  = arg2 - widget->rect.x;
                    int pos = bar->length * xx / widget->rect.width + 1;
                ...
                {
                    int yy  = arg3 - widget->rect.y;
                    int pos = bar->length * yy / widget->rect.height + 1;
    ...
    if (desta == 255)
    {
        ...
        if (bar->pos > 0 && bar->length > 0)
        {
            if (bar->layout == ITU_LAYOUT_HORIZONTAL)
            {
                ...
                if (((xx + width) > widget->rect.width) || (bar->pos == bar->length))
                {
                    ituColorFill(dest, destx + xx, desty, (widget->rect.width - xx), rect->height, &bar->fgColor);
                }
                else
                {
                    ituColorFill(dest, destx + xx, desty, width, rect->height, &bar->fgColor);
                }

                // (void)printf("[x %d] [width %d] [widget width %d]--[pos %d][length %d]\n", x, width, widget->rect.width,
                // bar->pos, bar->length);
            }
            else // if (bar->layout == ITU_LAYOUT_VERTICAL)
            {
                ...
                if (((yy + height) > widget->rect.height) || (bar->pos == bar->length))
                {
                    ituColorFill(dest, destx, desty + yy, rect->width, (widget->rect.height - yy), &bar->fgColor);
                }
                else
                {
                    ituColorFill(dest, destx, desty + yy, rect->width, height, &bar->fgColor);
                }

                // (void)printf("[y %d] [height %d] [widget height %d]--[pos %d][length %d]\n", yy, height,
                // widget->rect.height, bar->pos, bar->length);
            }
        }
    }
    else if (desta > 0)
        ...
        if (surf)
        {
            ...
            if (bar->pos > 0 && bar->length > 0)
            {
                if (bar->layout == ITU_LAYOUT_HORIZONTAL)
                {
                    ...
                    int xx    = (bar->pos - 1) * width;

                    ituColorFill(surf, xx, 0, width, rect->height, &bar->fgColor);
                }
                else // if (bar->layout == ITU_LAYOUT_VERTICAL)
                {
                    int height = widget->rect.height / bar->length;
                    int yy     = (bar->pos - 1) * height;

                    ituColorFill(surf, 0, yy, rect->width, rect->height, &bar->fgColor);
                }
            }
...
void ituScrollBarInit (ITUScrollBar * bar)
{
    ...
    (void)memset(bar, 0, sizeof(ITUScrollBar));
...
void ituScrollBarSetLength (ITUScrollBar * bar, int length)
{
    ...
    if (length < 0)
    {
        ITU_LOG_WARN("incorrect length: %d\n", length);
        return;
    }
    bar->length = length;
