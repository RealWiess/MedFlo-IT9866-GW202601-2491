...
bool ituContainerClone(ITUWidget* widget, ITUWidget** cloned)
{
    ...
    if (*cloned == NULL)
    {
        void *       newObj    = malloc(sizeof(ITUContainer));
        ...
        (void)memcpy(newObj, srcObj, sizeof(ITUContainer));
...
{
    const ITUWidget* a = *(ITUWidget**)arg1;
    const ITUWidget* b = *(ITUWidget**)arg2;
...
bool ituContainerUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        if (container->containerFlags & ITU_CONTAINER_GRID)
        {
            ITCTree* node;
            ITUWidget* child;
            ITUWidget* children[ITU_WIDGET_CHILD_MAX] = { NULL };
            ITURectangle* rect;
    ...
    else if (ev == ITU_EVENT_MOUSEUP)
    {
        ...
        {
            const ITUWidget* nearestChild = ContainerGetNearestChild(container, ituScene->dragged);
            if (nearestChild)
            {
                ITCTree* node;
                ITUWidget* children[ITU_WIDGET_CHILD_MAX] = { NULL };
...
void ituContainerDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    else
    {
        ...
        #else
            ituTransform(
                dest, destx, desty, surf->width, surf->height,
                surf, 0, 0, surf->width, surf->height,
                surf->width / 2, surf->height / 2,
                1.0f, 1.0f, (float)widget->angle,
    ...
    {
        ITCTree* node_t = widget->tree.child;

        for (; node_t; node_t = node_t->sibling)
        {
            ITUWidget* child = (ITUWidget*)node_t;
...
void ituContainerInit(ITUContainer* container)
{
    ...
    (void)memset(container, 0, sizeof (ITUContainer));
