        ...
        if (desta == 255)
        {
            ...
            if (widget->tree.child)
            {
                for (node = widget->tree.child->child; node; node = node->sibling)
                {
                    ...
                    ituColorFill(dest, destx, desty + childchild->rect.y + childchild->rect.height - grid->lineSize,
                                 rect->width, grid->lineSize, &grid->lineColor);
                }
            }

            ituColorFill(dest, destx, desty, rect->width, grid->lineSize, &grid->lineColor);
            ituColorFill(dest, destx, desty + rect->height - grid->lineSize, rect->width, grid->lineSize,
                         &grid->lineColor);
            ituColorFill(dest, destx, desty + grid->lineSize, grid->lineSize, rect->height - grid->lineSize * 2,
                         &grid->lineColor);
            ituColorFill(dest, destx + rect->width - grid->lineSize, desty + grid->lineSize, grid->lineSize,
                         rect->height - grid->lineSize * 2, &grid->lineColor);
        }
        else if (desta > 0)
        {
            ...
            if (surf)
            {
                ...
                ituColorFill(surf, 0, grid->lineSize, grid->lineSize, rect->height - grid->lineSize * 2,
                             &grid->lineColor);
                ituColorFill(surf, rect->width - grid->lineSize, grid->lineSize, grid->lineSize,
                             rect->height - grid->lineSize * 2, &grid->lineColor);
...
ITUWidget * ituTableGridAddItem (ITUTableGrid * grid, ITUWidget * templateItem, bool clone)
{
    ...
    if (child)
    {
        ...
        if (childchild)
        {
            ...
            if (clone)
            {
                ...
                ituTreeClone(templateItem, &cloned);
            }
            else
            {
                cloned = templateItem;
    ...
    ITU_LOG_WARN("ituTableGridAddItem() failed");
    return NULL;
}
...
void ituTableGridInit (ITUTableGrid * tableGrid)
{
    ...
    (void)memset(tableGrid, 0, sizeof(ITUTableGrid));
