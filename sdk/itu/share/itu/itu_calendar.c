...
#define USE_LUNAR_EVAL 0  //Lunar test related
...
static char* GetDayOf(int wCurYear, int wCurMonth, int wCurDay)
{
    ...
    strlcpy(szShuXiang, sizeof(szShuXiang), "%s", cShuXiang[((wCurYear - 4) % 60) % 12]);
    snprintf(szNongli, sizeof(szNongli), "%s (%s%s) years", szShuXiang, cTianGan[((wCurYear - 4) % 60) % 10], cDiZhi[((wCurYear - 4) % 60) % 12]);
    /* - Generated lunar months, day ==> wNongliDay, - */
    if (wCurMonth < 1)
    {
        snprintf(szNongliDay, sizeof(szNongliDay), "leap% s", cMonName[-wCurMonth]);
    }
    else
    {
        strlcpy(szNongliDay, cMonName[wCurMonth], sizeof(szNongliDay));
    }
    strlcat(szNongliDay, "month", sizeof(szNongliDay));
    strlcat(szNongliDay, cDayName[wCurDay], sizeof(szNongliDay));
    return strlcat(szNongli, szNongliDay, sizeof(szNongli));
#else
    wCurDay = nTheDate;
...
static void CalendarUpdateWeekDays(ITUCalendar* cal)
{
    ...
    (void)memset(cal->weekDays, 0, sizeof (cal->weekDays));
...
static void CalendarListBoxOnLoadPage(ITUListBox* listbox, int pageIndex)
{
    ITCTree* node;
...
static void CalendarUpdate(ITUCalendar* cal)
{
    ...
    {
        char buf[8];
        snprintf(buf, sizeof(buf), "%i", cal->year);
    ...
    if (cal->monthTarget)
    {
        if (cal->monthTarget->type == ITU_TEXT)
        {
            ...
            {
                char buf[8];
                snprintf(buf, sizeof(buf), "%i", cal->month);
    ...
    {
        char buf[8];
        snprintf(buf, sizeof(buf), "%i", cal->day);
...
bool ituCalendarUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    else if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        if ((cal->dayText == NULL) && (cal->dayName[0] != '\0'))
        {
...
void ituCalendarInit(ITUCalendar* cal)
{
    ...
    (void)memset(cal, 0, sizeof (ITUCalendar));
