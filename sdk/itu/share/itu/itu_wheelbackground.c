...
bool              ituWheelBackgroundUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if ((ev == ITU_EVENT_MOUSEDOWN) || (ev == ITU_EVENT_MOUSEMOVE && wbg->pressed))
    {
        if (ituWidgetIsEnabled(widget))
        {
            ...
            {
                int   orgX, orgY, vx1, vy1, vx2, vy2, dot, det;
                float angle;
                ...
                dot   = vx1 * vx2 + vy1 * vy2;
                det   = vx1 * vy2 - vy1 * vx2;
                angle = atan2f((float)det, (float)dot) * (float)(180.0f / M_PI);
                ...
                {
                    int value = wbg->orgValue + (int)roundf(angle) - wbg->pressedAngle;
...
void ituWheelBackgroundInit (ITUWheelBackground * wbg)
{
    ...
    (void)memset(wbg, 0, sizeof(ITUWheelBackground));
