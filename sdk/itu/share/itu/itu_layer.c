...
bool ituLayerClone (ITUWidget * widget, ITUWidget ** cloned)
{
    ...
    if (*cloned == NULL)
    {
         ...
         * ITULayer object. Since ITULayer is a struct that extends
         * ITUWidget (i.e., ITUWidget is the first member), it is safe to cast
         * or copy the memory using sizeof(ITULayer).
         *
         * Some static analysis tools may issue a warning here, suggesting that
         * copying sizeof(ITULayer) from a base type pointer (ITUWidget*) is
         * suspicious. However, in our design, we guarantee that 'widget' always
         * points to a valid ITULayer instance, so this warning is considered
         * a false positive and can be ignored.
         */
        /* coverity[SIZEOF_MISMATCH] */
        (void)memcpy(newWidget, widget, sizeof(ITULayer));
...
bool ituLayerUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        {
            const ITULayer * ll = (ITULayer *)node;
...
void ituLayerInit (ITULayer * layer)
{
    ...
    (void)memset(layer, 0, sizeof(ITULayer));
                ...
                for (i = 0; i < ITU_ACTIONS_SIZE; i++)
                {
                    ...
                    if (action->ev == ITU_EVENT_LEAVE && action->action == ITU_ACTION_HIDE)
                    {
                        if (action->param[0] != '\0')
                        {
                            ...
                            strlcpy(buf, action->param, sizeof(buf));
...
void ituLayerExternalSetPath (char * filename)
{
    ...
    if (filename)
    {
        strlcpy(ituScene->pathExternal, filename, sizeof(ituScene->pathExternal));
    }
    else
    {
        strlcpy(ituScene->pathExternal, "", sizeof(ituScene->pathExternal));
    }
}
