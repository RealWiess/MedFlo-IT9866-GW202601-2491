...
{
    const ITUOscilloscope * osc = (ITUOscilloscope *)widget;
    ...
    if (*cloned == NULL)
    {
        ...
        (void)memcpy(newWidget, widget, sizeof(ITUOscilloscope));
    ...
    if (values)
    {
        (void)memcpy(values, osc->values, sizeof(int) * (osc->resolutionX + 1));
...
void ituOscilloscopeDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    {
        float gw = (float)rect->width / osc->gridResolutionX;
        float gh = (float)rect->height / osc->gridResolutionY;
        ...
        if (gw > 1.0f)
        {
            for (i = 1; i <= osc->gridResolutionX; ++i)
            {
                ...
                dx1 = dx0;
                dy1 = rect->height;
                ituDrawLine(dest, destx + dx0, desty + dy0, destx + dx1, desty + dy1, &osc->gridColor,
                            osc->gridLineWidth);
            }
        }
...
void ituOscilloscopeInit (ITUOscilloscope * osc)
{
    ...
    (void)memset(osc, 0, sizeof(ITUOscilloscope));
