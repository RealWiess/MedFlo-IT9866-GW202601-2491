...
bool ituProgressBarClone (ITUWidget * widget, ITUWidget ** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ITUWidget * newWidget = malloc(sizeof(ITUProgressBar));
        if (newWidget == NULL)
        {
            return false;
        }

        (void)memcpy(newWidget, widget, sizeof(ITUProgressBar));
        newWidget->tree.child = newWidget->tree.parent = newWidget->tree.sibling = NULL;
        *cloned                                                                  = newWidget;
    }

    if (!(widget->flags & ITU_EXTERNAL) && bar->staticBarSurf)
    {
        ITUProgressBar * newBar = (ITUProgressBar *)*cloned;
        ITUSurface *     surf   = bar->staticBarSurf;
        ...
        {
            newBar->barSurf = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
                                               (const uint8_t *)surf->addr, surf->flags);
        }

        ituWidgetUpdate(newBar, ITU_EVENT_LOAD, 0, 0, 0);
    }
...
static void ProgressBarLoadExternalData (ITUProgressBar * bar, ITULayer * layer)
{
    ...
    if (!bar->staticBarSurf || !(widget->flags & ITU_EXTERNAL))
    {
        return;
    }
#else
    if (!bar->staticBarSurf || bar->barSurf || !(widget->flags & ITU_EXTERNAL))
    {
        return;
    }
#endif
    ...
    if (layer)
    {
        ...
        if (surf)
        {
            ...
            {
                bar->barSurf = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
                                                (const uint8_t *)surf->addr, surf->flags);
            }
        }
...
void ituProgressBarDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    {
        (void)printf("[SurfacePool] ProgressBarDraw but SurfacePool not ready yet!\n");
        return;
    }
    ...
    if (desta == 255)
    {
        if (bar->layout == ITU_LAYOUT_HORIZONTAL)
        {
            ...
            else
            {
                ...
                else if (widget->color.alpha > 0 && rect->width > width)
                {
                    ...
                    if (surf)
                    {
                        ...
                        ituAlphaBlend(dest, destx + width, desty, rect->width - width, rect->height, surf, 0, 0,
                                      widget->color.alpha);
            ...
            if (width > 0)
            {
                ...
                if (widget->visible && (widget->alpha > 0) && (alpha > 0))
                {
                    ...
                    if (widget->flags & ITU_EXTERNAL)
                    {
                        ProgressBarLoadExternalData(bar, NULL);
                    }
                    else
                    {
                        ituProgressBarLoadStaticData(bar);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // ituGradientFill(dest, destx, desty, width, rect->height, &bar->fgColor,
                            // &bg->graidentColor, bg->graidentMode);
                            ituGradientFill(dest, destx, desty, rect->width, rect->height, &bar->fgColor,
                                            &bg->graidentColor, bg->graidentMode);
                        }
                    }
                }
        ...
        else // if (bar->layout == ITU_LAYOUT_VERTICAL)
        {
            ...
            else
            {
                ...
                else if (widget->color.alpha > 0 && rect->height > height)
                {
                    ...
                    if (surf)
                    {
                        ...
                        ituAlphaBlend(dest, destx, desty, rect->width, rect->height - height, surf, 0, 0,
                                      widget->color.alpha);
                        ituDestroySurface(surf);
                    }
                }
            ...
            if (height > 0)
            {
                ...
                if (bar->barSurf)
                {
                    ituBitBlt(dest, destx, desty + rect->height - height, rect->width, height, bar->barSurf, 0,
                              rect->height - height);
                }
                else
                {
                    ...
                    {
                        // ituGradientFill(dest, destx, desty + rect->height - height, rect->width, height,
                        // &bar->fgColor, &bg->graidentColor, bg->graidentMode);
                        ituGradientFill(dest, destx, desty, rect->width, rect->height, &bar->fgColor,
                                        &bg->graidentColor, bg->graidentMode);
                    }
                }
                ...
                if (surf)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                ituGradientFill(surf, 0, 0, rect->width, rect->height, &bar->fgColor,
                                                &bg->graidentColor, bg->graidentMode);
                            }
                        }
                    }
        ...
        else // if (bar->layout == ITU_LAYOUT_VERTICAL)
        {
            ...
            if (height > 0)
            {
                ...
                if (surf)
                {
                    ...
                    if (bar->barSurf)
                    {
                        ituBitBlt(surf, 0, rect->height - height, rect->width, height, bar->barSurf, 0,
                                  rect->height - height);
                    }
                    else
                    {
                        ...
                        {
                            ituGradientFill(surf, 0, 0, rect->width, rect->height, &bar->fgColor, &bg->graidentColor,
                                            bg->graidentMode);
                        }
                    }
                }
...
void ituProgressBarInit (ITUProgressBar * bar, ITULayout layout)
{
    ...
    (void)memset(bar, 0, sizeof(ITUProgressBar));
    ...
    ituWidgetSetClone(bar, ituProgressBarClone);
...
{
    const ITUWidget * widget = (ITUWidget *)bar;
    assert(bar);
