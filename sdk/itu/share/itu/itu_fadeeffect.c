...
bool FadeEffectFindLoopAlphaFix(const ITCTree* tree, int alpha)
{
    ITCTree *child = tree->child;

    for (; child; child = child->sibling)
    {
        const ITUWidget* ww = (ITUWidget*)child;
        if (ww != NULL)
        {
            if (ww->type == ITU_METER)
            {
                ...
                //(void)printf("[FadeEffectFindLoopAlphaFix] found %s\n", ww->name);
            }
            else
            {
...
{
    const ITUFadeEffect* fe = (ITUFadeEffect*) effect;
    assert(effect);
    assert(widget);
...
void ituFadeEffectInit(ITUFadeEffect* fe, bool fadeIn)
{
    ...
    (void)memset(fe, 0, sizeof (ITUFadeEffect));
