...
bool ituScrollTextClone(ITUWidget* widget, ITUWidget** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ...
        (void)memcpy(newWidget, widget, sizeof(ITUScrollText));
...
bool ituScrollTextUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    else if (ev == ITU_EVENT_LAYOUT && text->state != STOP)
    {
        ...
        if (s)
        {
            ...
            ituFtGetTextDimension(s, &text->width, NULL, widget, boldSize);
        }
...
{
    const ITUScrollText* stext = (ITUScrollText*) widget;
    ITUText* text = (ITUText*) widget;
    ITURectangle* rect = (ITURectangle*) &widget->rect;
    int destx, desty, boldSize = 0;
    ...
    if (desta == 255)
    {
        ...
        {
            ituFtGetTextDimension(string, &w, &h, widget, boldSize);
...
void ituScrollTextInit(ITUScrollText* text, int width)
{
    ...
    (void)memset(text, 0, sizeof (ITUScrollText));
...
void ituScrollTextSetString(ITUScrollText* text, char* string)
{
    ...
    if (string)
    {
        ...
        ituFtGetTextDimension(string, &text->width, NULL, text, boldSize);
    }
}
