...
bool ituSpriteClone(ITUWidget* widget, ITUWidget** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ...
        (void)memcpy(newWidget, widget, sizeof(ITUSprite));
...
bool ituSpriteUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_TIMER)
    {
        if (sprite->playing)
        {
            if (--sprite->delayCount <= 0)
            {
                ...
                if (sprite->child && !(widget->flags & ITU_PROGRESS))
                {
                    ituWidgetUpdate(sprite->child, ITU_EVENT_LOAD, ITU_USE_SPRITE_COMMON_BUF, 0, 0);
                    ituWidgetUpdate(sprite->child, ITU_EVENT_LOAD_EXTERNAL, ITU_USE_SPRITE_COMMON_BUF, 0, 0);
                }
    ...
    else if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        {
            result |= ituWidgetUpdateImpl(widget, ITU_EVENT_LOAD, ITU_USE_SPRITE_COMMON_BUF, 0, 0);
            sprite->child = child;
        }
        else
        {
            if (child && sprite->child != child)
            {
                ...
                if (sprite->child)
                {
                    result |= ituWidgetUpdate(sprite->child, ITU_EVENT_LOAD, ITU_USE_SPRITE_COMMON_BUF, 0, 0);
					result |= ituWidgetUpdate(sprite->child, ITU_EVENT_LOAD_EXTERNAL, ITU_USE_SPRITE_COMMON_BUF, 0, 0);
                }
            }
        }
        widget->dirty = true;
...
void ituSpriteInit(ITUSprite* sprite)
{
    ...
    (void)memset(sprite, 0, sizeof (ITUSprite));
