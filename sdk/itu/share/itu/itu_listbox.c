...
void ituListBoxInit (ITUListBox * listbox, int width)
{
    ...
    (void)memset(listbox, 0, sizeof(ITUListBox));
...
{
    const ITUWidget * widget = (ITUWidget *)listbox;
    ITUText *         item;
    int               count;
    assert(listbox);
    ITU_ASSERT_THREAD();

    if (widget->type == ITU_SCROLLLISTBOX || widget->type == ITU_SCROLLMEDIAFILELISTBOX ||
        widget->type == ITU_SCROLLICONLISTBOX)
    {
    ...
    if (listbox->focusIndex >= 0)
    {
        ...
        if (item)
        {
            ...
            {
                ituWidgetSetColor(item, listbox->readFontColor.alpha, listbox->readFontColor.red,
                                  listbox->readFontColor.green, listbox->readFontColor.blue);
            }
            else
            {
                ituWidgetSetColor(item, listbox->orgFontColor.alpha, listbox->orgFontColor.red,
                                  listbox->orgFontColor.green, listbox->orgFontColor.blue);
            }
    ...
    if (index >= 0)
    {
        ...
        if (item)
        {
            ituTextSetBackColor(item, listbox->focusColor.alpha, listbox->focusColor.red, listbox->focusColor.green,
                                listbox->focusColor.blue);
            if (listbox->focusFontColor.alpha)
            {
                ituWidgetSetColor(item, listbox->focusFontColor.alpha, listbox->focusFontColor.red,
                                  listbox->focusFontColor.green, listbox->focusFontColor.blue);
            }
