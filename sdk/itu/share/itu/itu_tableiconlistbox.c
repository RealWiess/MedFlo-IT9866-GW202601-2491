    ...
    if ((NewCount < count) && (NewCount > 0))
    {
        ...
        for (; i < (count - NewCount); i++)
        {
            ...
            else
            {
                ...
                for (; prevSibling->sibling != thisNode; prevSibling = prevSibling->sibling)
                    ;
...
static void TableIconListBoxLoadExternalData (ITUTableIconListBox * tableiconlistbox, ITULayer * layer)
{
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ...
        if (surf)
        {
            ...
            {
                tableiconlistbox->surfArray[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ...
        if (surf)
        {
            ...
            {
                tableiconlistbox->focusSurfArray[i] = ituCreateSurface(
                    surf->width, surf->height, surf->pitch, surf->format, (const uint8_t *)surf->addr, surf->flags);
            }
        }
    }
}
...
void ituTableIconListBoxLoadStaticData (ITUTableIconListBox * tableiconlistbox)
{
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ...
        {
            tableiconlistbox->focusSurfArray[i] = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format,
                                                                   (const uint8_t *)surf->addr, surf->flags);
        }
    }
}
...
bool ituTableIconListBoxUpdate (ITUWidget * widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    else if (ev == ITU_EVENT_MOUSEMOVE)
    {
        if (ituWidgetIsEnabled(widget) && (widget->flags & ITU_DRAGGING))
        {
            ...
            // (void)printf("0: touchOffset=%d offset=%d y=%d\n", tableiconlistbox->touchOffset, offset, y);
            ...
            if (topY <= BOUNDARY_TOR && bottomY >= (widget->rect.height / 2 - BOUNDARY_TOR))
            {
                ...
                for (; i < count; ++i)
                {
                    ITUWidget * childs = (ITUWidget *)itcTreeGetChildAt(widget, i);
                    int         fy     = i * childs->rect.height;
                    fy += offset;
                    ituWidgetSetY(childs, fy);
                }
            }
        }
    }
    ...
    else if (ev == ITU_EVENT_MOUSEUP || ev == ITU_EVENT_MOUSEDOUBLECLICK)
    {
        if (ituWidgetIsEnabled(widget))
        {
            ...
            if ((widget->flags & ITU_DRAGGABLE) && (widget->flags & ITU_DRAGGING))
            {
                ...
                if (topY > 0)
                {
                    int check = (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) -
                                topY *
                                    (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) /
                    ...
                    tableiconlistbox->inc =
                        (dist != 0) ? -(widget->rect.height / 2) / dist : -(widget->rect.height / 2);
                    // (void)printf("1: frame=%d topY=%d inc=%d\n", tableiconlistbox->frame, topY, tableiconlistbox->inc);
                ...
                else if (bottomY < widget->rect.height)
                {
                    ...
                    tableiconlistbox->frame =
                        bottomY * (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) /
                        (widget->rect.height / 2);
                    dist = tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY);
                    tableiconlistbox->inc = (dist != 0) ? (widget->rect.height / 2) / dist : (widget->rect.height / 2);
                    // (void)printf("2: frame=%d bottomY=%d inc=%d\n", tableiconlistbox->frame, bottomY,
                    // tableiconlistbox->inc);
                    widget->flags |= ITU_UNDRAGGING;
                }
                else
    ...
    else if (ev == ITU_EVENT_TIMER)
    {
        ...
        if (widget->flags & ITU_UNDRAGGING)
        {
            ...
            if (tableiconlistbox->inc > 0)
            {
                ...
                for (i = 0; i < count; ++i)
                {
                    ...
                    int         fy, maxc = (int)(round((double)widget->rect.height / child->rect.height));
                    ...
                    // (void)printf("[TIMER]child %d, fy, %d, recth %d, inc %d\n", i, fy, child->rect.height,
                    // tablelistbox->inc);
                }
                ...
                (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)))
            {
                ...
                {
                    maxc = (int)(round((double)widget->rect.height / childfirst->rect.height));
        ...
        else if (tableiconlistbox->inc)
        {
            ...
            if (topY > widget->rect.height / 2)
            {
                tableiconlistbox->frame =
                    (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) -
                    topY * (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) /
                        (widget->rect.height / 2);
                ...
                // (void)printf("3: frame=%d topY=%d inc=%d\n", tableiconlistbox->frame, topY, tableiconlistbox->inc);
                widget->flags |= ITU_UNDRAGGING;
            }
            ...
            else
            {
                ...
                // (void)printf("step=%f %f %f\n", step, tableiconlistbox->inc * expf(-lamda), expf(-lamda));

                for (i = 0; i < count; ++i)
                {
                    ITUWidget * childs = (ITUWidget *)itcTreeGetChildAt(tableiconlistbox, i);
                    int         fy     = tableiconlistbox->touchOffset;
                    fy += i * childs->rect.height;
                    fy += (int)step;
                ...
                if (tableiconlistbox->frame > tableiconlistbox->totalframe)
                {
                    if (topY > 0)
                    {
                        ...
                        i = tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY);
                        tableiconlistbox->inc = (i != 0) ? -(widget->rect.height / 2) / i : -(widget->rect.height / 2);
                        // (void)printf("5: frame=%d topY=%d inc=%d\n", tableiconlistbox->frame, topY, tableiconlistbox->inc);
                        widget->flags |= ITU_UNDRAGGING;
                    }
                    else if ((bottomY < widget->rect.height) && (widget->rect.height / 2 > 0))
                    {
                        tableiconlistbox->frame =
                            bottomY *
                            (tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY)) /
                        ...
                        i = tableiconlistbox->totalframe / (tableiconlistbox->slidePage * UNDRAGGING_DECAY);
                        tableiconlistbox->inc = (i != 0) ? (widget->rect.height / 2) / i : (widget->rect.height / 2);
                        // (void)printf("6: frame=%d bottomY=%d inc=%d\n", tableiconlistbox->frame, bottomY,
                        // tableiconlistbox->inc);
                        widget->flags |= ITU_UNDRAGGING;
                    }
                    else
                    {
                        ...
                        // (void)printf("7: frame=%d bottomY=%d inc=%d\n", tableiconlistbox->frame, bottomY,
                        // tableiconlistbox->inc);
...
void ituTableIconListBoxDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    if (fwin->borderSize > 0)
    {
        if (desta == 255)
        {
            ...
            ituColorFill(dest, destx, desty + rect->height - fwin->borderSize, rect->width, fwin->borderSize,
                         &widget->color);
            ituColorFill(dest, destx, desty + fwin->borderSize, fwin->borderSize, rect->height - fwin->borderSize * 2,
                         &widget->color);
            ituColorFill(dest, destx + rect->width - fwin->borderSize, desty + fwin->borderSize, fwin->borderSize,
                         rect->height - fwin->borderSize * 2, &widget->color);
        }
        else if (desta > 0)
        {
            ITUSurface * surf = ituCreateSurface(rect->width, rect->height, 0, dest->format, NULL, 0);
            if (surf)
            {
                ...
                ituColorFill(surf, 0, fwin->borderSize, fwin->borderSize, rect->height - fwin->borderSize * 2,
                             &widget->color);
                ituColorFill(surf, rect->width - fwin->borderSize, fwin->borderSize, fwin->borderSize,
                             rect->height - fwin->borderSize * 2, &widget->color);
                ituAlphaBlend(dest, destx, desty, rect->width, rect->height, surf, 0, 0, desta);
                ituDestroySurface(surf);
            }
...
void ituTableIconListBoxInit (ITUTableIconListBox * tableiconlistbox, int width)
{
    ...
    (void)memset(tableiconlistbox, 0, sizeof(ITUTableIconListBox));
...
{
    const ITUWidget * widget = (ITUWidget *)tableiconlistbox;
    assert(tableiconlistbox);
    ...
    if (!(widget->flags & ITU_EXTERNAL))
    {
        ...
        for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
        {
            if (tableiconlistbox->staticSurfArray[i])
            {
                ...
                {
                    tableiconlistbox->surfArray[i] = ituCreateSurface(
                        surf->width, surf->height, surf->pitch, surf->format, (const uint8_t *)surf->addr, surf->flags);
                }
...
{
    const ITUListBox * listbox = (ITUListBox *)tableiconlistbox;
    ITUScrollText *    item;
    ITU_ASSERT_THREAD();
    ...
    if (item)
    {
        ...
        snprintf(buf, sizeof(buf), "%d", iconIndex);
