...
{
    bool                result = false;
    const ITUIcon *     icon   = (ITUIcon *)widget;
    ITUWaveBackground * wave   = (ITUWaveBackground *)widget;
    assert(wave);
    ...
    if (ev == ITU_EVENT_TIMER)
    {
        if (ituWidgetIsEnabled(widget) && wave->waveMap && icon->surf && wave->delayCount++ >= wave->delay)
        {
            ...
            {
                int diffy = (int)(wave->ampitude * sinf(2.0f * (float)M_PI *
                                                            ((float)(wave->ycount - y) / (wave->xcount) +
                                                             (float)wave->frame / wave->totalframe) +
                                                        (float)M_PI));
                ...
                for (x = 0; x < wave->xcount; x++)
                {
                    ...
                    int        diffx = (int)(wave->ampitude * sinf(2.0f * (float)M_PI *
                                                                       ((float)(wave->xcount - x) / (wave->xcount) +
                                                                 (float)wave->frame / wave->totalframe) +
                                                                   (float)M_PI));
...
void ituWaveBackgroundDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    if (wave->waveMap && wave->xstep > 0 && wave->ystep > 0 && icon->surf)
    {
        ...
        ituBitBlt(dest, rect->x + x, rect->y + y + wave->offsety, wave->offsetx, rect->height - wave->offsety,
                  icon->surf, 0, wave->offsety);

        if (w > 0 && h > 0)
        {
            int destx = rect->x + x + wave->offsetx;
            int desty = rect->y + y + wave->offsety;
            int i, j;
            ...
            for (i = 0; i < wave->ycount; i++)
            {
                ...
                {
                    const ITUPoint * pt0 = &wave->waveMap[i * (wave->xcount + 1) + j];
                    // const ITUPoint* pt1 = &wave->waveMap[i * (wave->xcount + 1) + (j + 1)];
                    const ITUPoint * pt2 = &wave->waveMap[(i + 1) * (wave->xcount + 1) + (j + 1)];
                    // const ITUPoint* pt3 = &wave->waveMap[(i + 1) * (wave->xcount + 1) + j];
                    int              dx  = j * w;
                    int              dy  = i * h;

                    // (void)printf("(%d,%d) (%d,%d) (%d,%d) (%d,%d) \n", pt0->x, pt0->y, surf->width + pt1->x, pt1->y,
                    // surf->width + pt2->x,  surf->height + pt2->y, pt3->x,  surf->height + pt3->y);
                    ituStretchBlt(dest, destx + dx + pt0->x - 1, desty + dy + pt0->y, w + pt2->x - pt0->x + 1,
                                  h + pt2->y - pt0->y, icon->surf, j * w + wave->offsetx, i * h + wave->offsety, w, h);
                }
            }
        }
...
void ituWaveBackgroundInit (ITUWaveBackground * wave)
{
    ...
    (void)memset(wave, 0, sizeof(ITUWaveBackground));
