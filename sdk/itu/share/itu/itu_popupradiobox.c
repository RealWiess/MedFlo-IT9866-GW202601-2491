...
{
    bool               result        = false;
    const ITUButton *  prb           = (ITUButton *)widget;
    ...
    if (ev == ITU_EVENT_MOUSEDOWN || ev == ITU_EVENT_MOUSEUP)
    {
        ...
        if (ituWidgetIsEnabled(widget) && popupRadioBox->frame == 0)
        {
            ...
            if (ituWidgetIsInside(widget, x, y))
            {
                ...
                {
                    (void)memcpy(&popupRadioBox->orgRect, &widget->rect, sizeof(ITURectangle));
                    popupRadioBox->orgAlpha = widget->alpha;

                    widget->rect.x          = popupRadioBox->orgRect.x - popupRadioBox->orgRect.width *
                                                                    popupRadioBox->incPercent /
                                                                    (popupRadioBox->totalframe * 100);
                    widget->rect.y = popupRadioBox->orgRect.y - popupRadioBox->orgRect.height *
                                                                    popupRadioBox->incPercent /
                                                                    (popupRadioBox->totalframe * 100);
                    widget->rect.width = popupRadioBox->orgRect.width + popupRadioBox->orgRect.width *
                                                                            popupRadioBox->incPercent /
                                                                            (popupRadioBox->totalframe * 100 / 2);
                    widget->rect.height = popupRadioBox->orgRect.height + popupRadioBox->orgRect.height *
                                                                              popupRadioBox->incPercent /
                                                                              (popupRadioBox->totalframe * 100 / 2);
    ...
    else if (ev == ITU_EVENT_TIMER)
    {
        if (popupRadioBox->frame > 0)
        {
            ...
            {
                (void)memcpy(&widget->rect, &popupRadioBox->orgRect, sizeof(ITURectangle));
            ...
            else
            {
                ...
                step           = (popupRadioBox->frame > popupRadioBox->totalframe / 2)
                                     ? (popupRadioBox->totalframe - popupRadioBox->frame)
                                     : popupRadioBox->frame;

                widget->rect.x = popupRadioBox->orgRect.x - popupRadioBox->orgRect.width * step *
                                                                popupRadioBox->incPercent /
                                                                (popupRadioBox->totalframe * 100);
                widget->rect.y = popupRadioBox->orgRect.y - popupRadioBox->orgRect.height * step *
                                                                popupRadioBox->incPercent /
                                                                (popupRadioBox->totalframe * 100);
                widget->rect.width = popupRadioBox->orgRect.width + popupRadioBox->orgRect.width * step *
                                                                        popupRadioBox->incPercent /
                                                                        (popupRadioBox->totalframe * 100 / 2);
                widget->rect.height = popupRadioBox->orgRect.height + popupRadioBox->orgRect.height * step *
                                                                          popupRadioBox->incPercent /
                                                                          (popupRadioBox->totalframe * 100 / 2);
...
void ituPopupRadioBoxDraw (ITUWidget * widget, ITUSurface * dest, int x, int y, uint8_t alpha)
{
    ...
    {
        (void)printf("[SurfacePool] PopupRadioBoxDraw but SurfacePool not ready yet!\n");
        return;
    }
    ...
    if (widget->visible && (widget->alpha > 0) && (alpha > 0))
    {
        if (checkbox->checked)
        {
            ...
            {
                ituCheckBoxLoadExternalDataAPI(checkbox);
            }
            else
            {
    ...
    if ((checkbox->checked && checkbox->checkedSurf) ||
        (widget->active && checkbox->checked && checkbox->focusCheckedSurf))
    {
        ...
        if (!srcSurf || (srcSurf->width < widget->rect.width || srcSurf->height < widget->rect.height) ||
                ...
                else
                {
                    if (widget->flags & ITU_STRETCH)
                    {
                        ...
                        if (surf)
                        {
                            ...
                            ituStretchBlt(surf, 0, 0, rect->width, rect->height, srcSurf, 0, 0, srcSurf->width,
                                          srcSurf->height);
                            ituAlphaBlend(dest, destx, desty, rect->width, rect->height, surf, 0, 0, desta);
                            ituDestroySurface(surf);
                        }
...
void ituPopupRadioBoxInit (ITUPopupRadioBox * popupRadioBox)
{
    ...
    (void)memset(popupRadioBox, 0, sizeof(ITUPopupRadioBox));
