...
bool ituClipperClone(ITUWidget* widget, ITUWidget** cloned)
{
    ...
    if (*cloned == NULL)
    {
        ITUWidget * newWidget = malloc(sizeof(ITUClipper));
        if (newWidget == NULL)
        {
            return false;
        }

        (void)memcpy(newWidget, widget, sizeof(ITUClipper));
        newWidget->tree.child = newWidget->tree.parent = newWidget->tree.sibling = NULL;
        *cloned                                                                  = newWidget;
    }

    newClipper = (ITUClipper*)*cloned;
...
void ituClipperInit(ITUClipper* clipper)
{
    ...
    (void)memset(clipper, 0, sizeof (ITUClipper));
	...
	ituWidgetSetExit(clipper, ituClipperExit);
	ituWidgetSetClone(clipper, ituClipperClone);
}
