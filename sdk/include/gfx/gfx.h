...
//                              Compile Option
//=============================================================================
//#define M2D_DECOMPRESS_SURFACE_INPLACE_DOUBLE_BUFFER

//for HUD project demo
#ifdef CFG_M2D_HUD_ENABLE
#define HUD_INVERSE
//#define HUD_HORIZONTAL
#define HUD_ADVANCE_ROTATE_90 //for 800x480 on 1280x480 panel
...
#define NUMBER_BLOCK 30

#endif

typedef enum _GFX_ROP3
{
    GFX_ROP3_BLACKNESS      = 0x00U,
    GFX_ROP3_NOTSRCERASE    = 0x11U,
    GFX_ROP3_NOTSRCCOPY     = 0x33U,
    GFX_ROP3_SRCERASE       = 0x44U,
    GFX_ROP3_DSTINVERT      = 0x55U,
    GFX_ROP3_SRCINVERT      = 0x66U,
    GFX_ROP3_SRCAND         = 0x88U,
    GFX_ROP3_PAINTINVERT    = 0x5AU,
    GFX_ROP3_MERGEPAINT     = 0xBBU,
    GFX_ROP3_MERGECOPY      = 0xC0U,
    GFX_ROP3_SRCCOPY        = 0xCCU,
    GFX_ROP3_SRCPAINT       = 0xEEU,
    GFX_ROP3_PATCOPY        = 0xF0U,
    GFX_ROP3_PATPAINT       = 0xFBU,
    GFX_ROP3_WHITENESS      = 0xFFU,
} GFX_ROP3;
...
typedef struct _GFXSurface
{
    ...
    int32_t         bitbltID;        ///< the bitblt ID of surface
} GFXSurface;
...
/**
* This routine is used to bitblt source surface to use alpha value of destination surface.
*
* @param dstSurface           The valid pointer of destination GFXSurface.
* @param dstX                 The X coordinates of destination GFXSurface.
* @param dstY                 The Y coordinates of destination GFXSurface.
...
void
gfxRegisterBlitIdCallback(int objId, void* pfCallback);

#ifdef CFG_M2D_HUD_ENABLE
void
gfxHUDInit(uint32_t blockW, uint32_t blockH, GFXHUDRotateType rotType);

void
gfxUpdateSrcAddr(uint32_t srcAddr, uint32_t dstAddr);

void
gfxSelectHudTable(uint32_t srcAddr, uint32_t dstAddr);

void
gfxSkipBgFast(uint16_t* buf);

void
gfxSkipBg(uint16_t* buf);

float
gfxGetHUDAngle(void);

void
gfxSetHUDAngle(float angle);

void
gfxHUDPreProcessing(void);

//void
//gfxHUDPreProcessingFish(GFXSurface* src, GFXSurface* dst);

bool
gfxGetCoordinate(char* srcfilepath, char* dstfilepath, uint16_t* dstBuf, GFXHUDRotateType rotType);

//bool
//gfxGetFishCoordinate(void);

GFXHUDRotateType
gfxGetHudRotateType(void);
