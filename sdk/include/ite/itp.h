...
/**
 * Load Delay-loaded module.
 */
void LoadDLModule(void);
...
#define ITP_MAX_TICK_HANDLER 5       ///< Maximum tick handler count

/**
 * Tick handler.
 */
typedef void (*ITPTickHandler)(void);
 ...
 * @param handler The handler function.
 */
void itpRegisterTickHandler(ITPTickHandler handler);
...
/**
 * Sends message from ISR.
 *
 * @param msgid message queue handle.
...
#define ITP_DEVICE_BIT       8UL                            ///< Device handle offset
#define ITP_DEVICE_MASK      (0xFFUL << ITP_DEVICE_BIT)     ///< Device handle mask
#define ITP_FILE_MASK        0xFFUL                         ///< File handle mask
...
typedef enum
{
    ...
    ITP_DEVICE_USBHRNDIS     = (89 << ITP_DEVICE_BIT),       // < rndis
    ITP_DEVICE_RISCV2        = (90 << ITP_DEVICE_BIT),       // < riscv2 device
    ITP_DEVICE_TP            = (91 << ITP_DEVICE_BIT),       // < Touch Panel
    ITP_DEVICE_MEMBIST       = (92 << ITP_DEVICE_BIT),       ///< MEM BIST
    ITP_DEVICE_USBDRAW       = (93 << ITP_DEVICE_BIT),       ///< USB Device Raw class
    ITP_DEVICE_USBHRAW       = (94 << ITP_DEVICE_BIT),       ///< USB Host Raw class
    ITP_DEVICE_YAFFS2        = (95 << ITP_DEVICE_BIT),       // < Yaffs2
    ITP_DEVICE_USBDECM       = (96 << ITP_DEVICE_BIT),       ///< USB Device CDC-ECM
    ITP_DEVICE_NORAES        = (97 << ITP_DEVICE_BIT),       ///< NOR AES
    ITP_DEVICE_PMIC          = (98 << ITP_DEVICE_BIT),       ///< PMIC PSQ9905
    ITP_DEVICE_LAST
} ITPDeviceType;
...
#define ITP_IOCTL_SET_MODE            100  ///< Set decompress mode(0:SEAL 1:SPEEDY 2:SPEEDY_COMPRESSION)
...
#define ITP_IOCTL_SET_FAST_BOOTING    102  ///< Set decompress fast-booting flag.
#define ITP_IOCTL_CUSTOM_CTL_ID0      103  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID1      104  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID2      105  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID3      106  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID4      107  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID5      108  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID6      109  ///< For Custom device usage.
#define ITP_IOCTL_CUSTOM_CTL_ID7      110  ///< For Custom device usage.
#define ITP_IOCTL_WIFI_SLEEP_STATUS   111  ///< Wifi get current status of doorbell/ctrlboard sleep mode.
#define ITP_IOCTL_WIFI_STOP_DHCP      112  ///< Wifi do the DHCP stop.
#define ITP_IOCTL_WIFI_GET_NET_DEVICE 113  ///< Wifi get net device
...
#define ITP_IOCTL_NETIF_ADD           127  ///< Add and bring netif up
#define ITP_IOCTL_NETIF_REMOVE        128  ///< Down and remove a netif
#define ITP_IOCTL_MOUNT_NOEXBR        129  ///< Mount without extended boot record
#define ITP_IOCTL_FORCE_MOUNT_NOEXBR  130  ///< Force mount without extended boot record
#define ITP_IOCTL_FORMAT_MOUNT        131  ///< Format and mount partition
#define ITP_IOCTL_CHK_DSK             132  ///< Check and repair disk
#define ITP_IOCTL_FIRST_BOOTING       133  ///< First booting build info block
#define ITP_IOCTL_OTP_READ            134  ///< Read OTP data
#define ITP_IOCTL_OTP_WRITE           135  ///< Write OTP data
#if defined(CFG_A_B_DUAL_BOOT_ENABLE) && defined(CFG_DYNAMIC_MOUNT_FS_ENABLE)
#define ITP_IOCTL_MOUNT_DUAL_AB       136  ///< mount system A or B
#endif
#define ITP_IOCTL_PMIC_REBOOT         137  ///< reboot by PMIC
#define ITP_IOCTL_PMIC_GET            138  ///< print ctrl register of PMIC
#define ITP_IOCTL_SKIP_CHK_TMP_FILE   139  ///< Skip auto temp file check on mounting
#define ITP_IOCTL_GET_SPARE_SIZE      140  ///< Gets block spare total size
#define ITP_IOCTL_BG_GC_EXIT          141  ///< Close Yaffs Background GC
...
extern const ITPDevice   itpDeviceNorAES;         ///< NOR AES device
...
extern const ITPFSDevice itpFSDeviceYaffs2;       ///< Yaffs2 device
...
extern const ITPDevice   itpDeviceUsbdEcm;        ///< USB Device CDC-ECM
extern const ITPDevice   itpDeviceUsbEcmex;       ///< USB CDC Ethernet
extern const ITPDevice   itpDeviceUsbhAcm;        ///< USB Host CDCACM/OPTION
extern const ITPDevice   itpDeviceRiscv2;         ///< Software peripheral on riscv cpu2
extern const ITPDevice   itpDeviceTp;             ///< Touch Panel
extern const ITPDevice   itpDeviceMemBist;        ///< MemBist
extern const ITPDevice   itpDeviceUsbdRaw;        ///< USB Device Raw class
extern const ITPDevice   itpDeviceUsbhRaw;        ///< USB Device Raw class
extern const ITPDevice   itpDevicePmicWD;         ///< PMIC PSQ9905

// Device lock definitions
#ifdef CFG_NOR_USE_DPUAES
extern pthread_mutex_t DPU_AES_MUTEX;
#endif
...
typedef struct
{
    ...
    int      writable[ITP_MAX_PARTITION];    ///< Writable flag in MBR.
#endif
} ITPPartition;
...
#define ITP_NAND_FULL_DATA_MODE 0x3 ///< open ITP NAND device with full data mode, which is read/write all data included spare area
...
/**
 * Probes the input of keypad2.
 */
int itpKeypad2Probe(void);
...
typedef struct
{
    ...
    int32_t  x;              ///< X displacement
    int32_t  y;              ///< Y displacement
    int32_t  wheel;          ///< wheel
} ITPMouseEvent;
#else
typedef struct
 ...
 */
/**
 * Calibrate Trigger Type.
 */
typedef enum _TriggerType {
    NO_TRIGGER = 0,     /* Stop */
    SEC_TRIGGER,        /* Second-trigger */
    MIN_TRIGGER,        /* Minute-trigger */
    LAZY_TRIGGER        /* Lazy-trigger */
} TriggerType;
...
/**
 * Set RTC trigger type.
 *
 * @param type The trigger type.
 */
void RtcSetTriggerType(TriggerType type);
 ...
 */
uint32_t itpRtcGetTime(uint32_t *usec);
 ...
 */
void itpRtcSetTime(uint32_t sec, uint32_t usec);

/**
 * Initializes RTC module by software.
 */
void itpRtcInit_SW(void);

/**
 * Gets RTC time by software.
 *
 * @param usec micro second to get if available.
 * @return rtc second value.
 */
...
typedef struct
{
    ...
    #else
    uint32_t address[4];         ///< IPv6 address
    #endif
...
typedef struct
{
    ...
    #else
    uint32_t ip6addr[4];  ///< IPv6 address
    uint8_t  ip6addr_idx; ///< IPv6 address index for prime
    int8_t   ip6addr_state; ////< IPv6 address states
    #endif
} ITPEthernetSetting;
...
/** @addtogroup itp_usb_raw USB Raw
 *  @{
 */
/**
* Function definitions.
*/
#define ITP_USB_SKIP_ZLP           0x1     ///< Skip zero lenght packet
...
typedef enum
{
    ITP_DCPS_SEAL_MODE    = (0),     ///< SEAL decompress mode
    ITP_DCPS_SPEEDY_MODE  = (1),     ///< SPEEDY decompress mode
    ITP_CPS_SPEEDY_MODE   = (2),     ///< SPEEDY compress mode
} ITPDcpsModeEnum;

#define ITP_DCPS_USE_CMDQ       (1)
#define ITP_DCPS_USE_CMDQ_WAIT  (2)
