/*
 * Copyright (c) 2009 ITE Tech. Inc. All Rights Reserved.
 */
/** @file
 * ITE Upgrade Library.
 *
 * @author Jim Tan
 * @version 1.0
 * @date 2013
 * @copyright ITE Tech. Inc. All Rights Reserved.
 */
/** @defgroup ug ITE Upgrade Library
 *  @{
 */
#ifndef ITE_UG_H
#define ITE_UG_H

#include "ite/itc.h"
#include "ite/itp.h"

#ifdef __cplusplus
extern "C"
{
#endif

// Error codes
#define UG_READ_FILE_FAIL      1    ///< Reads file fail error
#define UG_INVALID_FILE_FORMAT 2    ///< Invalid file format error
#define UG_OUT_OF_MEM          3    ///< Out of memory error
#define UG_NO_IMPL             4    ///< No implement error

#ifdef CFG_UPGRADE_BACKUP_FAT_MBR
#define MBR_BACKUP_CRC_ERR         0x1     ///< backup MBR crc error
#define MBR_INUSE_CRC_ERR          0x2     ///< inuse MBR crc error
#define MBR_BACKUP_INUSE_CRC_ERR   0x3     ///< both backup and inuse MBR crc error
#endif

// Custom recovery for upgrade flow
#if defined(CFG_NOR_USE_DPUAES) && (CFG_FW_DPUAES_UPGRADE_MARK_POS > 0)
typedef enum UG_FW_AES_UPGRADE_FLOW_TAG_TAG
{
    UG_FW_AES_UPGRADE_FLOW_ENCRYPT_DO,
    UG_FW_AES_UPGRADE_FLOW_ENCRYPT_DONE,
    UG_FW_AES_UPGRADE_FLOW_UNKNOWN
} UG_FW_AES_UPGRADE_FLOW_TAG;
#endif

#ifdef CFG_UPGRADE_BACKUP_RAW_DATA
typedef enum UG_UPGRADE_FLOW_TAG_TAG
{
    UG_UPGRADE_FLOW_FINISH,
    UG_UPGRADE_FLOW_FW,
    UG_UPGRADE_FLOW_FAT_P0,
    UG_UPGRADE_FLOW_FAT_P1,
    UG_UPGRADE_FLOW_FAT_P2,
    UG_UPGRADE_FLOW_FAT_P3,
    UG_UPGRADE_FLOW_UNKNOWN
} UG_UPGRADE_FLOW_TAG;

#define FLAG_CUSTOM_UPGRADE_FW     (0x1 << 0)
#define FLAG_CUSTOM_UPGRADE_FAT_P0 (0x1 << 1)
#define FLAG_CUSTOM_UPGRADE_FAT_P1 (0x1 << 2)
#define FLAG_CUSTOM_UPGRADE_FAT_P2 (0x1 << 3)
#define FLAG_CUSTOM_UPGRADE_FAT_P3 (0x1 << 4)
#endif

// Upgrade
/** @defgroup ug_upgrade Upgrade
 *  @{
 */

#if defined(CFG_NOR_USE_DPUAES) && (CFG_FW_DPUAES_UPGRADE_MARK_POS > 0)
UG_FW_AES_UPGRADE_FLOW_TAG ugFWAESUpgradeCheck(void);
#endif

#ifndef CFG_UPGRADE_BACKUP_RAW_DATA
/**
 * Checks last upgrade is success or not.
 *
 * @return 0 (success), or otherwise (failed).
 */
int ugUpgradeCheck(void);
#else
UG_UPGRADE_FLOW_TAG ugUpgradeCheck(void);
#endif

/**
 * Do the upgrade procedure from the specified file stream.
 *
 * @param file the file stream.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugUpgradePackage(ITCStream* file);

/**
 * Checks the CRC value of the specified file stream.
 *
 * @param f the file stream.
 * @param buffer Retrieves file content if buffer address is available, null for ignore.
 * @return 0 (OK), or otherwise (ERROR).
 * @par Example:
 * @code
    ITCFileStream fileStream;
    int ret;

    // open package file to do the upgrade
    ret = itcFileStreamOpen(&fileStream, "B:/ITEPKG03.PKG", false);
    if (ret)
        return ret; // open file error
    
    ret = ugCheckCrc(&fileStream, NULL);
    if (ret)
        return ret; // check crc fail
        
    ret = ugUpgradePackage(&fileStream);
    if (ret)
        return ret; // upgrade fail
        
 * @endcode
 */
int ugCheckCrc(ITCStream *f, uint8_t* buffer);

/**
 * Restores files from /backup directory of private partition disk.
 *
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugResetFactory(void);

/** @} */ // end of ug_upgrade

/** @defgroup ug_crc Files CRC check
 *  @{
 */

/**
 * Checks files CRCs by crc file at specified path.
 *
 * @param path the root path to check.
 * @param crcpath the saved crc file path to verify.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugCheckFilesCrc(char* path, char* crcpath);

/**
 * Sets file's CRC to crc file.
 *
 * @param filepath the relative path of file to calculate crc and save.
 * @param path The path to save to crc file.
 * @param crcpath the crc file path to save.
 * @return 0 (OK), or otherwise (ERROR).
 * @par Example:
 * @code
    int ret;
    
    ret = ugCheckFilesCrc("B:", "B:/ite_crc.dat");
    if (ret)
        return ret; // check crc fail

    ret = ugSetFileCrc("/font/DroidSans.ttf", "B:", "B:/ite_crc.dat");
    if (ret)
        return ret; // set crc fail
        
 * @endcode
 */
int ugSetFileCrc(char* filepath, char* path, char* crcpath);

/**
 * Upgrades the files' CRC of crc file.
 *
 * @param path The path to upgrade to crc file.
 * @param crcpath the crc file path to upgrade.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugUpgradeFilesCrc(char* path, char* crcpath);

/**
 * Copies file.
 *
 * @param destPath The path of destination file.
 * @param srcPath The path of source file.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugCopyFile(const char* destPath, const char* srcPath);

/**
 * Restores directory.
 *
 * @param dest The path of destination directory.
 * @param src The path of source directory.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugRestoreDir(const char* dest, const char* src);

/**
 * Copies directory recursively.
 *
 * @param dest The path of destination directory.
 * @param src The path of source directory.
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugCopyDirectory(const char* dest, const char* src);

/**
 * Deletes directory recursively.
 *
 * @param dirname The path of destination directory to delete.
 */
void ugDeleteDirectory(char* dirname);

/**
 * Upgrades on restore mode.
 */
void ugRestoreStart(void);

/** @} */ // end of ug_crc

// Internal functions
typedef struct
{
    uint32_t device_type;
    uint32_t unformatted_size;
    uint32_t partition_count;
    ITPDisk disk;
    uint64_t size[ITP_MAX_PARTITION];
    bool partition;
    bool nopartition;
    bool restore;
} UGDevice;

#if defined(CFG_NOR_USE_DPUAES) && (CFG_FW_DPUAES_UPGRADE_MARK_POS > 0)
int ugFWAESUpgradeTag(UG_FW_AES_UPGRADE_FLOW_TAG flow_tag);
#endif

#ifndef CFG_UPGRADE_BACKUP_RAW_DATA
/**
 * Starts to upgrade. 
 *
 * @param backup true for backuping package mode, or false for upgrading package mode.  
 * @return 0 (OK), or otherwise (ERROR).
 */
int ugUpgradeStart(bool backup);
#else
int ugUpgradeTag(UG_UPGRADE_FLOW_TAG flow_tag);
#endif

/**
 * Upgrades finished.
 * @return 0 (OK), or otherwise (ERROR).
 */
#ifndef CFG_UPGRADE_BACKUP_RAW_DATA
int ugUpgradeFinish(void);
#endif

int ugUpgradeDevice(ITCStream *f, UGDevice* device);
int ugUpgradeDirectory(ITCStream *f, char* drive);
int ugUpgradeEnd(ITCStream *f);
int ugUpgradeFile(ITCStream *f, char* drive);
int ugUpgradeFile_diff(ITCStream *f, char* drive);
int ugUpgradePartition(ITCStream *f, ITPDisk disk, int partition, char* drive, bool format);
int ugUpgradeDoPartition(UGDevice* device);
#ifndef CFG_UPGRADE_BACKUP_RAW_DATA
int ugUpgradeRawData(ITCStream *f, ITPDisk disk, bool restore);
int ugUpgradeRawData_diff(ITCStream *f, ITPDisk disk, bool restore);
#else
int ugCopyRawData(uint32_t dst, uint32_t src, uint32_t len, ITPDisk srcDisk, ITPDisk dstDisk);
int ugUpgradeRawData(ITCStream *f, ITPDisk disk, bool restore, bool upgrade);
int ugUpgradeRawData_diff(ITCStream *f, ITPDisk disk, bool restore, bool upgrade);
#endif
#if defined(CFG_NOR_USE_DPUAES) && (CFG_FW_DPUAES_UPGRADE_MARK_POS > 0)
int ugFWAESUpgradeData(void);
#endif
int ugUpgradeUnformatted(ITCStream *f, ITPDisk disk);
#ifndef CFG_UPGRADE_BACKUP_RAW_DATA
int ugUpgradeBackupFinish(void);
#endif

#ifdef CFG_UPGRADE_BACKUP_FAT_MBR
int ugCheckFatMBR(ITPDisk device_type, uint32_t pos, uint32_t len, bool backup, bool rollback);
#endif

int ugCalcFileCrc(char* filepath, uint32_t* crc);
uint32_t ugCalcBufferCrc(uint8_t* buffer, int size);
int ugGetProrgessPercentage(void);

#ifdef CFG_A_B_DUAL_BOOT_ENABLE
int ugBootargsRead(uint8_t *boot_args);
int ugBootargsWrite(uint8_t boot_args);
void ugUpgradeSetForcePartition(bool isForcePartition);
bool ugUpgradeIsForcePartition(void);
#endif

#ifndef _WIN32
ssize_t ugpatch_getsize(uint8_t *diffbuf);
uint8_t* ugpatch_getbuf(uint8_t *oldbuf, ssize_t oldsize, uint8_t *diffbuf);
uint8_t* hw_ucl_decompress(uint8_t *bi, uint32_t len, uint32_t *size);
#endif

ITPDisk ugGetPartitionDisk(int partition);

#ifdef __cplusplus
}
#endif

#endif // ITE_UG_H
/** @} */ // end of ug
