...
#define SDIO_ATBM_WROOM_32_AT 0x03
...
/**
 * Get the SD/eMMC card's product name and serial information (from the CID).
 *
 * @param index		 the controller index.
 * @param product	 a buffer where the product name will be stored. Ensure the
 * buffer is large enough (e.g., 8 bytes).
 * @param serial	 a pointer to a 32-bit unsigned integer where the SD card's
 * serial number will be stored.
 */
int iteSdGetCidProductAndSerial (int index, char * product, uint32_t product_buflen, uint32_t * serial);

/**
 * Get the eMMC card's EOL information.
 *
 * @param index		 the controller index.
 * @param pre_eol_info	 receive the Pre EOL information.
 * @param lifetime_a	 receive the "Device life time estimation type A".
 * @param lifetime_b	 receive the "Device life time estimation type B".
 */
int iteSdGetEOL (int index, uint8_t * pre_eol_info, uint8_t * lifetime_a,
                 uint8_t * lifetime_b);
