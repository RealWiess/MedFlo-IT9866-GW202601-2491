...
#ifdef CFG_UART0_PARITY
#define UART0_PARITY CFG_UART0_PARITY
#else
...
#ifdef CFG_UART1_PARITY
#define UART1_PARITY CFG_UART1_PARITY
#else
...
#ifdef CFG_UART2_PARITY
#define UART2_PARITY CFG_UART2_PARITY
#else
...
#ifdef CFG_UART3_PARITY
#define UART3_PARITY CFG_UART3_PARITY
#else
...
#ifdef CFG_UART4_PARITY
#define UART4_PARITY CFG_UART4_PARITY
#else
...
#ifdef CFG_UART5_PARITY
#define UART5_PARITY CFG_UART5_PARITY
#else
...
static inline uint32_t uart_judge_port(uint32_t port)
{
	return (port - ITH_UART0) >> 8U;
}

typedef struct UART_OBJ UART_OBJ;

typedef int(*fpUartInit)(UART_OBJ* uart_obj);
typedef void(*fpUartDele)(UART_OBJ** uart_obj);
typedef int(*fpUartSend)(UART_OBJ* uart_obj, char *ptr, int len);
typedef int(*fpUartRead)(UART_OBJ* uart_obj, char *ptr, int len);
...
typedef struct UART_OBJ
{
	...
	uint8_t			mode;
	bool			forDbg;
	fpUartInit		uart_init;
	fpUartDele		uart_dele;
	fpUartSend		uart_send;
	fpUartRead		uart_read;
	void			*pMode;
}UART_OBJ;
