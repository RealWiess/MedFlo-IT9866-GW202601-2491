...
static inline void ithDmaSetRequest(int channel, ITHDmaMode srcMode, ITHDmaRequest srcReq, ITHDmaMode dstMode, ITHDmaRequest dstReq)
{
...
#endif
    ithWriteRegA((ITH_DMA_BASE + ITH_DMA_SRC_CH((uint32_t)ch)), addr);
}
...
#endif
    ithWriteRegA((ITH_DMA_BASE + ITH_DMA_DST_CH((uint32_t)ch)), addr);
}
