...
{
    ithWriteRegMaskH(addr, (uint16_t)(0x1UL << bit), (uint16_t)(0x1UL << bit));
}
...
{
    ithWriteRegMaskH(addr, (uint16_t)0UL, (uint16_t)(0x1UL << bit));
}
...
{
    ithWriteRegMaskA(addr, 0x1UL << bit, 0x1UL << bit);
}
...
{
    ithWriteRegMaskA(addr, 0UL, 0x1UL << bit);
}
...
#define ITH_SET_REG_BIT_A(addr, bit) \
    do { \
        ITH_WRITE_REG_MASK_A((addr), 0x1UL << (bit), 0x1UL << (bit)); \
    } while (false)
...
#define ITH_CLEAR_REG_BIT_A(addr, bit) \
    do { \
        ITH_WRITE_REG_MASK_A((addr), 0UL, 0x1UL << (bit)); \
    } while (false)
