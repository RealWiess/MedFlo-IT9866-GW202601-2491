 ...
 * @param txPin The UART TX GPIO pin.
 ...
 */
static inline uint32_t ithUartGetFifoDepth(ITHUartPort port)
{
    return ((ithReadRegA((uint32_t)port + ITH_UART_FEATURE_REG) & ITH_UART_FIFO_DEPTH_MASK) >> ITH_UART_FIFO_DEPTH_BIT);
}
...
{
    return (ithReadRegA((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_DR) == ITH_UART_LSR_DR;
}
...
{
    return (char)ithReadRegA((uint32_t)port + ITH_UART_RBR_REG);
}
...
{
    return (ithReadRegA((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_THRE) == ITH_UART_LSR_THRE;
}

static inline bool ithUartIsTxWIthFifoEmpty(ITHUartPort port)
{
    return (ithReadRegA((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_TRANS_EMPTY) == ITH_UART_LSR_TRANS_EMPTY;
}


static inline bool ithUartIsRxParityError(ITHUartPort port)
{
    return (ithReadRegA((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_PARITY) == ITH_UART_LSR_PARITY;
}

static inline bool ithUartIsRxDataError(ITHUartPort port)
{
	return (ithReadRegA((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_DATA_ERR) == ITH_UART_LSR_DATA_ERR;
}
...
{
    return (ithReadRegA((uint32_t)port + ITH_UART_IIR_REG) & ITH_UART_IIR_TXFIFOFULL) == ITH_UART_IIR_TXFIFOFULL;
}
...
{
    ithWriteRegA((uint32_t)port + ITH_UART_THR_REG, (uint32_t)c);
}
...
{
    ithSetRegBitA((uint32_t)port + ITH_UART_IER_REG, (uint32_t)intr);
}
...
{
    ithClearRegBitA((uint32_t)port + ITH_UART_IER_REG, (uint32_t)intr);
}
...
{
    return ithReadRegA((uint32_t)port + ITH_UART_IIR_REG);
}
...
{
    ithWriteRegMaskA((uint32_t)port + ITH_UART_MCR_REG, ITH_UART_MCR_DMAMODE2, ITH_UART_MCR_DMAMODE2);
}
...
{
    ithWriteRegMaskA((uint32_t)port + ITH_UART_MCR_REG, 0, ITH_UART_MCR_DMAMODE2);
}
...
{
    ithWriteRegMaskA((uint32_t)port + ITH_UART_LCR_REG, ITH_UART_LCR_SETBREAK, ITH_UART_LCR_SETBREAK);
}
...
{
    ithWriteRegMaskA((uint32_t)port + ITH_UART_LCR_REG, 0, ITH_UART_LCR_SETBREAK);
}
...
{
    ithSetRegBitA((uint32_t)port + ITH_UART_FCR_REG, (uint32_t)ITH_UART_TX_RESET);
}
...
{
    ithSetRegBitA((uint32_t)port + ITH_UART_FCR_REG, (uint32_t)ITH_UART_RX_RESET);
}
...
#define ITH_UART_IS_TX_FULL(port) \
    (ITH_READ_REG_A((uint32_t)port + ITH_UART_IIR_REG) & ITH_UART_IIR_TXFIFOFULL) == ITH_UART_IIR_TXFIFOFULL
...
#define ITH_UART_IS_RX_READY(port) \
    (ITH_READ_REG_A((uint32_t)port + ITH_UART_LSR_REG) & ITH_UART_LSR_DR) == ITH_UART_LSR_DR
