...
{
    ithWriteRegA(module + ITH_MEMDBG_ADDR_TOP_REG, topAddr);
    ithWriteRegA(module + ITH_MEMDBG_ADDR_BOTTOM_REG, bottomAddr);
}
...
{
    uint32_t reg = ithReadRegA(module + ITH_MEMDBG_EN_REG);

    if (reg & ITH_MEMDBG_STATUS_MASK)
        return (reg & 0x1F);
    else
        return -1;
}
...
static inline void ithMemDbgClearIntr(void)
{
    ...
    reg = ithReadRegA(ITH_MEMDBG0 + ITH_MEMDBG_EN_REG);
    if (reg & ITH_MEMDBG_STATUS_MASK)
    {
        ithSetRegBitA(ITH_MEMDBG0 + ITH_MEMDBG_EN_REG, ITH_MEMDBG_RESET_BIT);
        ithClearRegBitA(ITH_MEMDBG0 + ITH_MEMDBG_EN_REG, ITH_MEMDBG_RESET_BIT);
    }

    reg = ithReadRegA(ITH_MEMDBG1 + ITH_MEMDBG_EN_REG);
    if (reg & ITH_MEMDBG_STATUS_MASK)
    {
        ithSetRegBitA(ITH_MEMDBG1 + ITH_MEMDBG_EN_REG, ITH_MEMDBG_RESET_BIT);
        ithClearRegBitA(ITH_MEMDBG1 + ITH_MEMDBG_EN_REG, ITH_MEMDBG_RESET_BIT);
    }
}
