...
echo ========================================
echo   GW202601_LVGL_v2 Build Pipeline
...
if exist "%BUILD%\project\%CFG_PROJECT%\CMakeFiles\%CFG_PROJECT%.dir\build.make" (
    powershell -Command "(Get-Content '%BUILD%\project\%CFG_PROJECT%\CMakeFiles\%CFG_PROJECT%.dir\build.make' -Raw) -replace '--unformatted-device 1 --unformatted-size','--nor-unformatted-size' -replace '--unformatted-device 1 --unformatted-index 0 --unformatted-data','--nor-unformatted-data0' -replace '--unformatted-device 1 --unformatted-index 1 --unformatted-data','--nor-unformatted-data1' -replace '--unformatted-device 1 --unformatted-index 2 --unformatted-data','--nor-unformatted-data2' -replace '--unformatted-pos','--nor-unformatted-data1-pos' -replace '--partition-device 1 ','--' -replace '--partition-index 0 --partition-dir','--nor-partiton0-dir' -replace '--partition-index 1 --partition-dir','--nor-partiton1-dir' -replace '--partition-index 2 --partition-dir','--nor-partiton2-dir' -replace '--partition-index 0 --partition-size','--nor-partiton0-size' -replace '--partition-index 1 --partition-size','--nor-partiton1-size' -replace '--partition-index 2 --partition-size','--nor-partiton2-size' -replace '--partition-index 3 --partition-size','--nor-partiton3-size' -replace ' --unformatted-device 1','' -replace ' --partition ',' ' | Set-Content '%BUILD%\project\%CFG_PROJECT%\CMakeFiles\%CFG_PROJECT%.dir\build.make'"
)
echo [OK] Patched.
echo.
...
if exist "%BUILD_OUT%\ITE_NOR.ROM" (
    ...
    copy /Y "%BUILD_OUT%\ITE_NOR.ROM" "%ROOT%\GW202601_LVGL_v2_!TIMESTAMP!.ROM"
    echo [OK] ROM: ITE_NOR.ROM + GW202601_LVGL_v2_!TIMESTAMP!.ROM
) else (
